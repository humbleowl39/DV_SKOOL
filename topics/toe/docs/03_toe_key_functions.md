...
추적해 보면 원인이 명확합니다. Linux 는 손실을 감지하면 SACK block 을 담아 ACK 를 보내는데, 당신의 TOE 가 이를 그냥 무시합니다. Linux 는 TOE 가 SACK 를 보고 손실된 구간만 선택적으로 재전송할 것을 기대하는데 그게 오지 않으니, 결국 timeout 을 거쳐 connection 을 drop 합니다. 원인은 TOE 의 **RFC 2018 SACK 미구현** 입니다. Linux peer 는 RFC 를 지키는데 당신의 TOE 가 지키지 않은 것입니다.

이것이 TCP interop 의 어려움입니다. SACK (RFC 2018), PAWS (RFC 1323), Congestion Control (RFC 5681) 등 수십 개의 RFC 가 모두 정확히 구현되어야 다양한 peer 와 호환됩니다. 그래서 검증의 90 % 는 packet loss, OOO, duplicate, timeout, zero window 같은 abnormal scenario 에서의 _복구 동작_ 검증에 집중됩니다.
...
TX: "TX path" {
  direction: down
  # unparsed: TX0["App data"]
  # unparsed: TX1["① Segmentation<br/>MSS 단위로 자름"]
  # unparsed: TX2["② Header build<br/>seq, ack, win, flags"]
  # unparsed: TX3["③ Checksum gen<br/>IP + TCP"]
  # unparsed: TX4["④ Retx buffer + RTO timer<br/>사본 보관 + RTO arm"]
  # unparsed: TX5["⑤ Flow / Cong gate<br/>cwnd, snd_wnd 보고<br/>burst 결정"]
  # unparsed: TXOUT["MAC"]
  TX0 -> TX1
  TX1 -> TX2
  TX2 -> TX3
...
RX: "RX path" {
  direction: down
  # unparsed: RX0["Wire → Ethernet Frame"]
  # unparsed: RX1["① Checksum chk<br/>IP + TCP"]
  # unparsed: RX2["② Conn lookup<br/>4-tuple"]
  # unparsed: RX3["③ Seq validate"]
  # unparsed: RX4["④ FSM update<br/>ACK → snd_una<br/>ACK → retx free<br/>Window → snd_wnd<br/>Cong → cwnd"]
  # unparsed: RX5["⑤ Reassembly<br/>OOO 버퍼"]
  # unparsed: RXOUT["Host DMA"]
  RX0 -> RX1
  RX1 -> RX2: "pass"
  RX2 -> RX3
  RX3 -> RX4
...
INITIAL { shape: circle; style.fill: "#333" }
INITIAL -> SlowStart
# unparsed: SlowStart: Slow Start<br/>(cwnd 지수 증가)
# unparsed: CongAvoid: Congestion Avoidance<br/>(cwnd 선형 증가)
# unparsed: FastRecovery: Fast Recovery<br/>(cwnd 반감, SACK 기반 재전송)
...
실무에서는 대부분의 TOE 가 **NewReno 를 기본**으로 지원하고, 고성능 제품은 **Cubic 을 추가** 합니다. 알고리즘 선택을 런타임에 SW 로 설정할 수 있게 하면 워크로드에 따라 유연하게 전환할 수 있어 선호됩니다. 단 Cubic 을 HW 로 구현하려면 3 차 함수를 LUT 또는 근사 연산으로 처리해야 하므로, 면적 예산이 빠듯한 경우 NewReno 만 지원하는 설계도 현실적인 선택입니다.
