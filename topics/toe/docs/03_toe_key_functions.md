...
```
   TX path                                          RX path
   ─────────────────────────────────                ──────────────────────────
   App data                                         Wire ─▶ Ethernet Frame
       │                                                  │
       ▼                                                  ▼
   ┌────────────────┐                              ┌────────────────┐
   │ ① Segmentation │  MSS 단위로 자름             │ ① Checksum chk │  IP+TCP
   └────────────────┘                              └────────────────┘
       │                                                  │ pass
       ▼                                                  ▼
   ┌────────────────┐                              ┌────────────────┐
   │ ② Header build │  seq, ack, win, flags        │ ② Conn lookup  │  4-tuple
   └────────────────┘                              └────────────────┘
       │                                                  │
       ▼                                                  ▼
   ┌────────────────┐                              ┌────────────────┐
   │ ③ Checksum gen │  IP+TCP                      │ ③ Seq validate │
   └────────────────┘                              └────────────────┘
       │                                                  │
       ▼                                                  ▼
   ┌────────────────┐                              ┌────────────────┐
   │ ④ Retx buffer  │  사본 보관 + RTO arm          │ ④ FSM update   │  ACK→snd_una
   │   + RTO timer  │                              │   ACK→retx free│
   └────────────────┘                              │   Window→snd_wnd│
       │                                            │   Cong→cwnd    │
       ▼                                            └────────────────┘
   ┌────────────────┐                                    │
   │ ⑤ Flow/Cong    │  cwnd, snd_wnd 보고             ▼
   │   gate         │  burst 결정                  ┌────────────────┐
   └────────────────┘                              │ ⑤ Reassembly   │  OOO 버퍼
       │                                            └────────────────┘
       ▼                                                  │
...
```
   Time   TX (TOE)                                   Wire / Peer
   ────   ─────────────────────────────────────       ─────────────────────────
   t=0    seg1 (seq=10000, len=1460) → MAC            ─────▶ peer 받음, ack=11460
   t=0+   seg2 (seq=11460, len=1460) → MAC            ─XX─▶  손실!
   t=0+   seg3 (seq=12920, len=1460) → MAC            ─────▶ peer 받음 (OOO)
   t=0+   seg4 (seq=14380, len=1460) → MAC            ─────▶ peer 받음 (OOO)
   t=1ms  ◀───── ACK(ack=11460)  ① 정상 ACK            peer: cumulative ACK
                 → snd_una = 11460
                 → retx buf 의 [10000,11460) 해제
   t=2ms  ◀───── ACK(ack=11460) ② DUP ACK #1          (seg3 받았지만 11460 이 빠짐)
                 → dup_ack_count = 1
   t=3ms  ◀───── ACK(ack=11460) ③ DUP ACK #2
                 → dup_ack_count = 2
   t=4ms  ◀───── ACK(ack=11460) ④ DUP ACK #3
                 → dup_ack_count = 3 ⭐
                 ⑤ Fast Retransmit trigger:
                    - cwnd = cwnd / 2  (혼잡 제어)
                    - ssthresh = cwnd
                    - retx_seq = 11460 (snd_una)
                    - retx buffer 에서 seg2 사본 꺼냄
   t=4ms+ seg2' (seq=11460, len=1460) → MAC          ─────▶ peer 받음
   t=5ms  ◀───── ACK(ack=15840) ⑥ recovery           모두 회복
                 → snd_una = 15840
                 → retx buf 의 [11460,15840) 해제
                 → dup_ack_count = 0
...
```
   ACK 수신
     │
     ├──▶ snd_una 갱신 ────▶ retx buffer 해제 ────▶ TX path: 더 보낼 수 있음 신호
     ├──▶ srtt/rttvar 갱신 ▶ RTO 재계산 ────────▶ RTO timer 갱신
     ├──▶ dup_ack_count   ─▶ ==3? ────────────▶ Fast Retransmit + cwnd halve
     ├──▶ window 갱신     ─▶ snd_wnd ────────▶ Flow gate 갱신
     └──▶ ECN bit         ─▶ cwnd 조정       ▶ Cong control
```
