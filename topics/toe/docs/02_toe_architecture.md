...
```
   ┌─────────── TOE Engine (Server) ───────────────────────────────┐
   │                                                                │
   │  ① DCMAC ─Ethernet Frame─▶ RX Path 의 첫 stage                │
   │                                  │                              │
   │                                  ▼ ② IP/TCP Checksum 검증      │
   │                          (실패 시 drop, 카운터 증가)            │
   │                                  │ pass                         │
   │                                  ▼ ③ 4-tuple 추출 → Connection Table lookup
   │                          ┌──────────────────────┐               │
   │                          │ {srcIP, srcPort,      │  ← O(1) 해시  │
   │                          │  dstIP, dstPort}      │     SRAM      │
   │                          │ → conn_id = 0x4F2     │               │
   │                          │ state=ESTABLISHED      │               │
   │                          │ rcv_nxt=10000          │               │
   ...
   │                                  │                                │
   │                                  ▼ ⑦ DMA TOE buf → Host buf      │
   │                          host RX descriptor ring 에 entry 추가    │
   │                                  │                                │
   │                                  ▼ ⑧ host 알림 (IRQ 또는 polling)│
   └────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
                           App: read() return 256
...
```
       PD-like layer (PCIe BAR + descriptor ring)
        │
        ├── DMA Engine ────────── host memory ↔ TOE buf
        │
        └── Connection Table ─┬── Hot entries (SRAM)
                              ├── Cold entries (DRAM, LRU)
                              └── 4-tuple hash index
                  │
                  └── 양쪽 path 가 read/write
```
