...
!!! objective "학습 목표"
    ...
    - **Diagram** TOE의 TX/RX path와 connection state table 구조를 설명
    - **Apply** Segmentation / Reassembly / Checksum / Flow control이 pipeline에서 어디서 일어나는지 매핑
    - **Distinguish** Stateful vs Stateless offload 구조 차이
    - **Plan** 동시 connection 수 (수천~수백만) 관리를 위한 memory hierarchy
...
!!! tip "💡 이해를 위한 비유"
    **Connection Table** ≈ **은행 창구 번호표 시스템**
    ...
    수백만 명의 고객(TCP 연결)이 각자 자기 순서(Seq Number)와 잔액(Window)을 갖고 있고, 창구(TOE)는 번호표(4-tuple 해시)로 해당 고객 파일을 O(1)에 꺼내 처리한다.

!!! danger "❓ 흔한 오해"
    **오해**: Connection State(TCP 상태)는 세션이 끊겨도 TOE HW에 영구적으로 보존된다.

    **실제**: Connection Table은 고정 크기이며, TIME_WAIT 타임아웃이나 RST로 연결이 종료되면 해당 엔트리는 재사용된다. 비활성 연결은 LRU로 DRAM에 swap-out되어 재접근 시 지연이 발생한다.

    **왜 헷갈리는가**: "stateful offload"라는 용어가 상태가 무한히 유지된다는 인상을 주지만, HW 자원(SRAM/DRAM)은 유한하다.
---
...
- **TX/RX Path 분리**: 독립 pipeline → full-duplex.
- **Connection Table**: 4-tuple (src/dst IP+port) → connection state. Hash 또는 CAM 기반 lookup.
- **Stateful**: TCP 상태 머신을 HW가 직접 관리 (LISTEN→SYN→ESTABLISHED→...). Reordering buffer, retransmission timer.
- **Memory hierarchy**: 활성 connection은 on-chip SRAM, idle은 DRAM. Cache hit rate가 throughput 좌우.
- **AXI host interface**: descriptor ring (TX/RX), interrupt, doorbell.

## 다음 단계

- 📝 [**Module 02 퀴즈**](quiz/02_toe_architecture_quiz.md)
- ➡️ [**Module 03 — Key Functions**](03_toe_key_functions.md)
