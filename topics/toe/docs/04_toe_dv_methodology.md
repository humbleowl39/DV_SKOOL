...
!!! tip "💡 이해를 위한 비유"
    **TOE DV** ≈ **등기 우편 검수원 — 분실/지연/순서꼬임/중복 등 모든 시나리오 검증**

    Connection state 전이, RTO 동작, fast retransmit, congestion control 등 stateful behavior 의 모든 분기 cover.
...
!!! danger "❓ 흔한 오해"
    **오해**: TOE 검증 = 정상 path 만 검증하면 충분

    **실제**: 정상 path 는 일부분. SYN flood, RST 폭주, out-of-order, dup ACK, connection table 고갈 등 abnormal path 가 silent bug 의 source.
