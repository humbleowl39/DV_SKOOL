...
```
                ...
                └────┬─────────────────────────────────┬──┘
                     ...
                     ▼                                  ▼
     ┌─────────────────┐                   ┌────────────────────┐
     │  Host Agent     │                   │ DRAM Behavioral    │
     │  (AXI master)   │                   │ Model (JEDEC)      │
     │  - traffic gen  │                   │ - timing checker   │
     │  - QoS / burst  │                   │ - refresh model    │
     └────────┬────────┘                   └─────────┬──────────┘
              │                                      │
              │   AXI request/response               │ DDR command/data
              │                                      │
   ┌──────────┴────────┐  ┌────────────────────┐  ┌──┴─────────────┐
   │ ① Data Stopwatch  │  │ ② Timing Stopwatch │  │ ③ Perf Stopwatch│
   │ Scoreboard:       │  │ Protocol Checker / │  │ BW / latency    │
   │ AXI W == AXI R    │  │ Timing SVA         │  │ regressors      │
   │ (data integrity)  │  │ (tRCD, tRP, ...)   │  │ vs reference    │
   └───────────────────┘  └────────────────────┘  └─────────────────┘
```
...
```
   AXI master (host agent)        Scoreboard / Checker
        ↓                             ↑       ↑      ↑
    [DUT: MC + PHY]  ────────────►    │       │      │
        ↓                             ① data  ② time  ③ perf
   DRAM Behavioral Model
