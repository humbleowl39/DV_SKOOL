...
```
                       Tesla 가 가진 것 (강함)
   ┌─────────────────────────────────────────────────────┐
   │   Cloud 인증 (VIN bind, OTA signing) ────  L5  ✓★    │
   │   Code-signing, Anti-Rollback ──────────  L5  ✓★    │
   │   SCS Secure Boot (BL 서명 검증) ────────  L1  ✓★    │
   │   Telemetry 모니터링 ───────────────────  L5  ✓     │
   └─────────────────────────────────────────────────────┘
                              ▲
                              │
       ...
       │                                               │
       │   ✗ L2 SecOC (CAN MAC)                        │
       │   ✗ L3 Secure Gateway (Flat CAN)              │
       │   ✗ L1 TEE (GPS 검증을 Normal World 가)        │
       │   △ L4 IDS (Telemetry 만, 차량 내부 IDS 없음)  │
       │                                               │
       └───────────────────────────────────────────────┘
                              │
                              ▼
   [탈옥 동글] ─OBD-II─▶ [CAN Bus] ─위조 GPS/Region─▶ [FSD SoC]
                                          │
                                          ▼
                                   "미국에 있고 FSD 활성화됨"
                                   → 비승인 지역에서 자율주행 동작
...
```
   ...
   │     │                                                 │
   │     ▼ ② 정상 GPS frame 식별                           │
   │   Frame: ID=0x318  Data=[37.5665, 126.9780]  // Seoul │
   │     │                                                 │
   │     ▼ ③ Frame RE — DBC 매핑 추정                      │
   │   "ID 0x318, byte 0..3=lat float, byte 4..7=lon float"│
   │     │                                                 │
   │     ▼ ④ 위조 frame 생성                              │
   │   Frame: ID=0x318  Data=[37.3861, -122.0839] // CA US │
   │     │                                                 │
   │     ▼ ⑤ 정상 GPS ECU 보다 _높은 빈도_ 로 송신          │
   │   (예: 정상 100 ms 주기 → 동글 50 ms 주기)             │
   │     │                                                 │
   └─────┼─────────────────────────────────────────────────┘
         │
         ▼ ⑥ CAN Bus 위에 _두 ID 0x318 frame_ 공존
   ┌── CAN Bus ─────────────────────────────────────────────┐
   │ ... [ID=0x318 Seoul] [ID=0x318 CA] [ID=0x318 CA] ...   │
   └──────────────────────┬─────────────────────────────────┘
                          │
                          ▼ ⑦ FSD SoC 수신
   ┌── FSD SoC (HW3 / HW4) ────────────────────────────────┐
   │                                                        │
   │   CAN Driver — _MAC 검증 없음, FV 검증 없음_           │
   │     │                                                  │
   │     ▼ ⑧ 단순히 "최신 수신값" 저장                      │
   │   GPS state ← 위조 frame (CA 좌표)                     │
   │     │                                                  │
   │     ▼ ⑨ FSD 정책 엔진                                  │
   │   geofence_check(CA) → "승인 지역" → FSD 활성화        │
   │     │                                                  │
   │     ▼ ⑩ 비승인 지역에서 자율주행 동작 시작              │
   └────────────────────────────────────────────────────────┘
```
...
[FSD 가 비승인 지역에서 동작]
     ...
     ├── [GPS 좌표 위조됨]
     │       │
     │       ├── [CAN frame 에 MAC 없음]            ← 근본 원인 ❶ (SecOC 부재)
     │       └── [GPS 가 CAN 경유]                  ← 근본 원인 ❷ (TEE 부재)
     │
     ├── [Region Code 변조됨]
     │       │
     │       └── [설정 frame 에 인증 없음]           ← 근본 원인 ❶ 와 동일
     │
     └── [서버 검증 우회됨]
             │
             ├── [오프라인 시 로컬 캐시 사용]        ← 근본 원인 ❸ (Heartbeat 부재)
             └── [텔레메트리 주기적 → 실시간 아님]
```
...
2020.xx   HW3 MCU 탈옥 (보안 연구자) — voltage glitching 으로 Secure Boot 우회
2023.xx   HW4 (FSD Computer 2) 양산 — 7 nm, 여전히 CAN 인증 미적용
...
교훈: 차량-서버 간 통신에도 TLS 필수
  → Tesla FSD 비교: Tesla 는 서버 통신 보안 강력 (TLS + 서명),
    그러나 차량 내부 CAN 통신은 무방비
```
...
[Configuration Profile]              [로컬 Configuration]
  - 암호학적 서명 (VIN bind)            - Feature Flags
...
Step 3: 위조 frame 주입
  ┌─────────────────────────────────────────┐
  │ 원본 GPS frame (실제 위치: Seoul)         │
  │ ID: 0x318  Data: [37.5665, 126.9780]    │
  │              ↓ 동글이 차단/대체           │
  │ 위조 GPS frame (가짜 위치: California)    │
