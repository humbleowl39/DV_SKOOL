...
!!! tip "💡 이해를 위한 비유"
    **Tesla FSD 탈옥 사례** ≈ **보안 IP 있어도 적용 범위 밖이면 무력 (= 금고는 강한데 문은 열려 있음)**

    Tesla SCS 가 boot/Cloud 인증은 강하게 보호했으나 CAN 통신은 미적용. 외부에서 CAN injection → FSD 위조 신호 통과.
...
!!! danger "❓ 흔한 오해"
    **오해**: Tesla 는 보안에 신경 안 썼다

    **실제**: Tesla 의 서버 보안 / boot 보안은 업계 최고 수준. 문제는 "내부 CAN = 신뢰" 라는 아키텍처 가정의 실패. 의도 부재가 아니라 가정 오류.

    **왜 헷갈리는가**: 결과만 보고 "의도 부재" 로 단순화. 실제는 design assumption 의 잘못.

!!! warning "실무 주의점 — Feature Flag로 보안 기능을 제어하면 안 된다"
    **현상**: FSD 활성화 여부를 차량 로컬 플래그로 판단하면, 공격자가 해당 플래그 값을 변조해 구매하지 않은 기능을 무료로 활성화하거나, 반대로 인가되지 않은 자율 주행 모드를 강제 진입시킬 수 있다.

    **원인**: Feature Flag는 UX 편의 목적으로 설계되었으며, 서명 검증이나 HSM 봉인 없이 플래시 메모리에 평문 저장되는 경우가 많다.
