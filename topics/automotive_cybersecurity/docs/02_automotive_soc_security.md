...
|                  차량 SoC 보안 스택                              |
|                                                                |
|  Layer 5: OTA + Cloud Validation                               |
|    서버 측 VIN 인증, 텔레메트리 모니터링, 원격 비활성화          |
|                                                                |
|  Layer 4: Intrusion Detection System (IDS)                     |
|    CAN 트래픽 이상 탐지, 규칙 / 주기 / ML 기반                  |
|                                                                |
|  Layer 3: Secure Gateway + Domain Isolation                    |
|    도메인 간 방화벽, 메시지 라우팅 / 필터링, OBD-II 격리        |
|                                                                |
|  Layer 2: SecOC (Secure Onboard Communication)                 |
|    CAN 메시지 MAC 인증, Freshness 관리, Replay 방어            |
|                                                                |
|  Layer 1: HSM (Hardware Security Module)                       |
|    키 생성·저장, MAC 연산, Secure Boot, 물리적 변조 방지        |
+================================================================+
       ▲                                          ▲
       │                                          │
   Secure Boot 의 RoT 와                        TEE (TrustZone)
   같은 원리                                     은 L1 ↔ L4 사이에서
   = 변경 불가한 신뢰 기반                       Secure World 격리
```
...
```
   ┌── OEM 공장 (Secure Room) ────────────────────────────────┐
   │                                                           │
   │  [Key Mgmt Server (KMS)]              [ECU on test jig]   │
   │       │                                  │                │
   │       │ ① VIN, ECU serial 수신           │                │
   │       │ ② KDF: K_OEM_root,VIN,ESN→ K_dev│                │
   │       │                                  │                │
   │       │ ③ TLS over JTAG/SWD ─────────────│                │
   │       ▼                                  ▼                │
   │   K_dev (16 B)                     [Provisioning Tool]    │
   │       │                                  │                │
   │       │ ④ Wrap(K_dev, K_kek_factory) ────│                │
   │       │                                  ▼                │
   │       │                            [HSM JTAG/SWD path]    │
   │       │                                  │                │
   │       │                            ⑤ HSM 내부 unwrap      │
...
```
[Secure Boot]                     [Automotive Comms]
     │                                  │
  BootROM + OTP                      HSM + Secure Key Store
  = 부팅 체인의 신뢰 기반            = 통신 체인의 신뢰 기반
     │                                  │
  ROTPK → BL 서명 검증               K_secoc → CAN 메시지 MAC
  비대칭 (RSA / ECDSA)               대칭 (AES-CMAC / GCM)
```
...
```
   App → SecOC encode (L2) → CAN driver
                                  │
                                  ▼
                            Gateway 라우팅 (L3) ─── 도메인 격리 ── Whitelist
                                  │
                                  ▼
                            IDS sniff (L4) ─────── anomaly 탐지
                                  │
                                  ▼
                            Telemetry (L5) ──────── Cloud 검증
                                  │
                                  ▼
   RX SecOC decode (L2) → App 수신
            │
            └─── HSM (L1) ─ K_secoc ─── 모든 layer 의 신뢰 뿌리
```
...
Phase 1 — 제조 시 키 주입 (Factory Provisioning)
  KMS → KDF → wrapped key → HSM → eFuse/OTP burn
...
Phase 3 — 런타임 키 로테이션
  주기: 수천~수만 km, 또는 시간 기반
  K_n → KDF (HSM 내부) 또는 OTA → K_n+1
  - 모든 ECU 동기 전환 명령
...
[SecOC Module]                     [SecOC Module]
    ...
    ▼                                  ▼
[Secured PDU]                      [인증 성공?]
 = Data + Truncated MAC + FV          ├── Yes → App 에 전달
    │                                  └── No  → 폐기 + 에러 보고
    ▼
[CAN-FD 프레임으로 전송]
```
...
ECU 전원 ON → RTOS 부팅 → SecOC 초기화 → Freshness 동기화
                                    ...
                                    이 구간이 취약 (수백 ms ~ 수 초)
                                              │
                                    Freshness Value 를 아직 모름
                                    → MAC 검증 불가 → 메시지 수용? 폐기?

대응 옵션:
  A) Authentication Build-Up — 처음 N 개 메시지 인증 없이 수용
     → 편의 ✓, 보안 ✗
  B) Strict Mode — 인증 전 모든 메시지 폐기
     → 보안 ✓, 기능 ✗ (안전 임계 메시지도 폐기)
  C) FM 우선 부팅 — Freshness Manager ECU 가 최우선 부팅, sync 메시지 배포
     → 현실적 타협, 단 FM 이 SPOF
```
...
[키 전환 명령] ──broadcast──▶ 모든 ECU
  ...
  ECU-C: 아직 K_old 사용 중 ✗ (키 전환 지연)
         │
         └── ECU-A 의 메시지를 K_old 로 검증 → MAC 불일치 → 폐기
             = 정상 메시지가 인증 실패하는 역설
...
|           +-------->-----------+              |
|                                               |
|  ※ GPS 무결성 검증을 Secure World 에서 수행하면|
|    Normal World 의 CAN 메시지로 위치를 속일 수  |
|    없다 — Tesla 탈옥의 핵심 방어               |
+----------------------------------------------+
```
...
= 메시지 인증 요청이 하드웨어 RoT 까지 내려가는 체인
```
