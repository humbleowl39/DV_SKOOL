...
!!! tip "💡 이해를 위한 비유"
    **CAN Bus** ≈ **1980년대 사내 전화 회의 — 모두 동일 버스에 broadcast, 신뢰 가정**

    1983 폐쇄 네트워크 가정으로 설계되어 인증 없음. OBD-II/텔레매틱스로 외부 접근 가능해진 시점부터 안전 가정 무너짐.
...
!!! danger "❓ 흔한 오해"
    **오해**: CAN 에 암호화 추가하면 보안 끝

    **실제**: 암호화 ≠ 인증. CAN 은 인증(누가 보냈는지) 이 핵심 누락 — 암호화는 기밀성, 인증은 무결성. SecOC 는 인증 추가.

    **왜 헷갈리는가**: "crypto = security" 의 단순화. 실제 보안 목표는 다축 (CIA: 기밀성/무결성/가용성).
---
