...
!!! tip "💡 이해를 위한 비유"
    **IOMMU (SMMU)** ≈ **DMA 의 MMU — DMA 도 "가상 주소" 만 보게 하는 보안 레이어**

    Device(DMA) 가 host RAM 직접 접근하면 보안 무효. IOMMU 가 device-side page table 을 두어 device 가 "자기 view 의 가상 주소" 만 보게 한다.
