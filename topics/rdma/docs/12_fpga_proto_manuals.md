...
당신은 RDMA-IP RTL 의 모든 simulation 시나리오를 통과시켰습니다 (`mrun regr` 100%). FPGA 에 bitfile 올리고 lab cabinet 에 실어서 _실제 traffic_ 을 흘립니다. 결과: 첫 5 분 만에 throughput 이 spec 의 _50%_.
...
- **MSI-X vector affinity** — IRQ 가 다른 NUMA node 의 CPU 로 → cache miss 폭증.
- **kernel module load 순서** — `mlx5_core` 가 다른 module 보다 늦게 → init race.

이 모든 의존성은 **spec 에 없고** _운영 manual_ 에만 있습니다. 시뮬에서는 통과한 시나리오가 lab 에서 깨지는 경우가 흔합니다.

이 모듈이 그 _운영 ground truth_ 의 좌표.

!!! question "🤔 잠깐 — 시뮬 vs Lab 의 _가장 큰 차이_"
    Simulation 과 실제 lab 의 _가장 다른_ 한 가지를 떠올려 보세요. 단순히 "RTL vs FPGA" 가 아닙니다.

    ??? success "정답"
        **환경 변수의 수**. Simulation 은 가정한 환경 (cycle-accurate, deterministic, fault-free) 에서 _제한된_ 변수만 통제. Lab 은 _수십 개의 환경 의존성_ (BIOS, kernel, driver version, switch fw, optic, IRQ tuning, NUMA) 가 동시에 작용 — 변수 하나만 잘못해도 throughput cliff.

        이게 운영 manual 이 _spec 분량의 절반_ 정도 차지하는 이유. _재현 가능한_ lab 환경을 만드는 게 검증의 절반.
...
Manual 트리는 세 카테고리로 구성됩니다. **Bring-up** 카테고리는 bitfile 을 FPGA 에 올리는 것부터 시작해, kernel driver 를 로드하고, MB-Shell 콘솔로 RDMA device 를 제어하고, RDMA debug register 로 1차 진단하는 과정을 다룹니다. **디버그** 카테고리는 debug register guide, leaf-spine 토폴로지 setup, adaptive routing 설정을 포함합니다. **Workload** 카테고리는 rdma-test (새 bitfile sanity), fio (block I/O 성능), RCCL (AI collective 시나리오), 그리고 standard DB (회귀 baseline 비교) 로 구성됩니다.

### 4.3 환경 변수와 시나리오의 분리

Adaptive Routing, SR-IOV QoS, leaf-spine 토폴로지 설정은 모두 _환경 변수_ 입니다. 이것들은 RDMA 검증 시나리오와는 독립적으로 관리해야 합니다. 시뮬에서 통과한 시나리오를 lab 에서 돌리기 전에, 환경 변수가 시뮬 가정과 일치하는지 먼저 확인해야 합니다. 예를 들어 시뮬은 single-path 를 가정했는데 lab 에 adaptive routing 이 켜져 있다면, RC 의 strict in-order 가정이 깨지면서 시뮬에서 없던 fail 이 터집니다.
...
세 workload 도구는 검증 단계에 따라 다르게 씁니다. **rdma-test / mango-rdma-test** 는 가볍고 빠르기 때문에 새 bitfile 을 받은 날의 첫 5 분 sanity check 에 씁니다. SEND/WRITE/READ 각각 1000 회를 돌려 기본 동작을 확인합니다. **fio** 는 NVMe-oF over RDMA 경로에서 block I/O 성능을 측정하는 도구로, PR 단위 perf regression 에 씁니다. 4 KB / 64 KB / 1 MB 세 점을 찍어 standard DB 와 비교합니다. **RCCL** 은 GPU collective 통신 워크로드로, AMD MI325X 노드에서 allreduce / all-to-all 을 실제로 돌려보는 AI 대규모 시나리오에 씁니다. GPU 와 driver 의존성이 크므로, 첫 sanity 단계에는 부적합합니다.
...
!!! question "🤔 Q1 — Lab fail 진단 순서 (Bloom: Apply)"
    Bitfile 받자마자 traffic 흘리니 throughput cliff. _첫 3 step_ 으로 무엇을 확인?

    ??? success "정답"
        1. RDMA debug register sticky bit (BAR2 0x38000~) — bring-up 자체가 깨졌나?
        2. PCIe link status — BDF, Gen, Lane width 정상?
        3. IRQ affinity — `cat /proc/interrupts` 에서 IRQ 가 _다른 NUMA_ 로 갔나?

!!! question "🤔 Q2 — Sim ↔ Lab 분리 (Bloom: Evaluate)"
    당신은 RDMA-TB scoreboard 를 짠다. Lab 의 _adaptive routing_ 이 RC packet 을 _OOO_ 로 도착하게 한다. Scoreboard 가 어떻게 _이걸 sim 시나리오와 분리_ 해서 처리해야 하나?

    ??? success "정답"
        Scoreboard 의 _ordering assumption_ 을 config 로 분리. Sim default = strict in-order, Lab mode = OOO 허용. `vrdma_data_env::ordering_mode` 같은 enum 으로 분기.
