...
```
   ① SEND (two-sided)                  ② RDMA WRITE (one-sided)
      sender → recv WQE 소비              sender → 원격 메모리 직접 write
      RECV CQE + SEND CQE 둘 다           SEND CQE 만 (recv 측 CPU 안 깨움)
   ────────                              ────────
   sender ──SEND─▶ recv RQ 의 WR 소비    sender ──WRITE──▶ 원격 메모리 (RETH)
          ◀─ACK──                            ◀──ACK──
                                              (recv CPU 안 깨움)

   ③ RDMA READ (one-sided, 양방향)       ④ ATOMIC (one-sided, 8B 단위)
      sender 가 원격에서 읽어옴              sender 가 원격에서 atomic 연산
   ────────                              ────────
   sender ──READ_REQ──▶                  sender ──CMP_SWAP──▶
                  (responder fetches)             (atomic at responder)
          ◀─READ_RESP─                          ◀─ATOMIC_ACK──
          ◀─READ_RESP─  (multi-packet)
          ◀─READ_RESP─
```
