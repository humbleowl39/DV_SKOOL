...
```
   ...
   ─────────────────────                   ─────────────────────
   ┌─────────────────────┐                ┌─────────────────────┐
   │ VMX root mode       │                │ EL3 (Secure Monitor)│
   │  (Hypervisor)       │                │ EL2 (Hypervisor)    │
   │   Ring 0..3         │                │ EL1 (Guest OS)      │
   │   ↕ VMLAUNCH        │ ↕ HVC          │ EL0 (User App)      │
   │   ↕ VM Exit         │                │                     │
   │ VMX non-root mode   │                │  ↕ exception level  │
   │  (Guest)            │                │    transitions      │
   │   Ring 0..3         │                │  (HVC, ERET, ...)   │
   └─────────────────────┘                └─────────────────────┘
   "두 root 의 분리"                       "처음부터 4 단계 EL"
```
...
```
   ┌─────────── Guest (VMX non-root) ─────────┐    ┌───── Hypervisor (VMX root) ─────┐
   │  Linux init                              │    │                                  │
   │  → APIC base 읽으려 함                    │    │                                  │
   │     RDMSR ECX=0x1B                        │    │                                  │
   │  ① ECX=0x1B 셋업                          │    │                                  │
   │  ② RDMSR ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━▶│ ③ VM Exit                       │
   │     (sensitive)                           │    │   reason=31 (RDMSR)             │
   │     HW 가 자동 trap                        │    │   - Guest RIP, RSP 저장          │
   │                                            │    │   - VMCS guest-state area 갱신  │
   │                                            │    │   ▼ ④ MSR 0x1B 가 emulate 대상? │
   │                                            │    │   ▼ ⑤ Hypervisor 가 값 만들기  │
   │                                            │    │      (예: vAPIC base 0xFEE00000)│
   │                                            │    │   ▼ ⑥ VMCS 의 EAX/EDX 에 결과   │
   │                                            │    │   ▼ ⑦ Guest RIP += 2 (RDMSR len)│
   │     │                                      │    │                                  │
   │     ▼                                      │    │   ⑧ VMRESUME                    │
   │  EAX = 값 low / EDX = 값 high   ◀━━━━━━━━━━━━━━━│                                 │
   │  (RDMSR 다음 명령 실행)                    │    │                                  │
   │  Guest 는 자기가 직접 MSR 읽었다고 믿음     │    │                                  │
   └────────────────────────────────────────────┘    └──────────────────────────────────┘
```
