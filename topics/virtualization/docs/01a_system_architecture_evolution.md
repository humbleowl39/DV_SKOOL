...
```mermaid
flowchart LR
    S1["<b>단계 1</b><br/>HW Only<br/>고정 기능"]
    S2["<b>단계 2</b><br/>Processor (FW)<br/>프로그래머블<br/>저성능"]
    S3["<b>단계 3</b><br/>FW + HW Accel<br/>하이브리드<br/>고성능"]
    S4["<b>단계 4</b><br/>OS(ARM-M) + HW Accel<br/>MPU, DRAM, 시스템콜<br/>애플리케이션 특화"]
    S5["<b>단계 5</b><br/>OS(ARM-A) + MMU<br/>범용, 고성능<br/>IOMMU 없음 (문제)"]
    S6["<b>단계 6</b><br/>+ PEs + IOMMU + LLC + Coherency<br/>확장 가능<br/>현대 SoC"]
    S7["<b>단계 7</b><br/>+ Virtualization<br/>VM 격리<br/>HW 지원"]
    S1 --> S2 --> S3 --> S4 --> S5 --> S6 --> S7
    classDef pivot stroke:#1a73e8,stroke-width:3px
    class S6 pivot
```
...
flowchart TB
    subgraph S5["단계 5 — IOMMU 없음"]
        direction TB
        A1["HW Accel<br/>① 드라이버가 PA 0x1_0000 발급<br/>(예약된 1 GB 영역에서)"]
        A2["② DMA read addr=0x1_0000"]
        A3["System Interconnect"]
        A4["DRAM (PA 0x1_0000 의 1 KB)"]
        A5["HW Accel buffer<br/>③ 1 KB return"]
        A1 --> A2 --> A3 --> A4 --> A5
    end
    subgraph S6["단계 6 — IOMMU 있음"]
        direction TB
        B1["HW Accel<br/>① 드라이버가 IOVA 0x4000 발급<br/>(devicetree/ACPI 가 설정)"]
        B2["② DMA read addr=0x4000 (가상)"]
        B3["IOMMU<br/>③ StreamID 매칭 → STE<br/>④ Stage-1: VA→IPA<br/>⑤ Stage-2: IPA→PA → 0x9_2000"]
        B4["⑥ DMA read addr=0x9_2000"]
        B5["System Interconnect<br/>⑦ permission OK 면 통과"]
        B6["DRAM (PA 0x9_2000)"]
        B7["HW Accel buffer<br/>⑧ 1 KB return (IOVA 그대로 보임)"]
...
```mermaid
flowchart LR
    S6["<b>단계 6</b><br/>현대 SoC"]
    IO["<b>1. IOMMU</b><br/>디바이스 격리<br/>(가상화 전제)"]
    PE["<b>2. PEs</b><br/>GPU / DSP / NPU / DPU<br/>워크로드 다양성"]
    LLC["<b>3. LLC</b><br/>컴포넌트 간<br/>메모리 공유 효율"]
    COH["<b>4. HW Coherency</b><br/>캐시 일관성 자동화<br/>(SW flush 제거)"]
    S6 --> IO
    S6 --> PE
    S6 --> LLC
    S6 --> COH
...
```mermaid
flowchart LR
    FLASH["Flash<br/>(FW 저장)"]
    UI["User inputs"]
    SENS["Sensor"]
    PROC["Processor (FW)"]
    SI["System<br/>Interconnect"]
    ACT["Actuator"]
    OUT["Outputs"]
    RAM["RAM"]
    UI --> SENS --> PROC --> SI --> ACT --> OUT
    FLASH --> PROC
    PROC --- RAM
```
...
flowchart TB
    ...
    RAM["RAM"]
    FLASH --> PROC
    PROC -- "Configuration<br/>Interrupt" --> ACC
    PROC -- "Memory access<br/>(PA direct)" --> SI
    ACC -- "Memory access<br/>(PA direct)" --> SI
    SI --> RAM
    classDef warn stroke:#b8860b,stroke-width:2px
    class ACC warn
```
...
flowchart TB
    FLASH["Flash"]
    subgraph PROC["Processor (OS) — ex) ARM-M"]
...
flowchart TB
    ...
    ACC["HW Accelerator (PA)<br/>물리 주소로<br/>메모리 직접 접근"]
    SI["System Interconnect"]
    DRAM1["DRAM"]
    DRAM2["DRAM"]
    RAM["RAM"]
    FLASH --> PROC
    PROC -- "Configuration<br/>Interrupt ctrlr" --> ACC
    PROC --> SI
    ACC -- "reserved<br/>memory chunk" --> SI
    SI --> DRAM1
    SI --> DRAM2
    SI --> RAM
...
flowchart LR
    ...
    HW --> LLC
    LLC --> DRAM
    classDef shared stroke:#1a73e8,stroke-width:2px
    class LLC shared
```
...
flowchart TB
    VM0["VM0 (4 GB)"]
    VM1["VM1 (2 GB)"]
    HV["Hypervisor<br/>2-stage translation (CPU 메모리)<br/>인터럽트: capture 후 분배<br/>HW Accel: 설정 / 관리"]
    DRAM["DRAM<br/>VM0 STE · VM0 STE · VM1 STE · ..."]
    VM0 --> HV
    VM1 --> HV
    HV --> DRAM
    classDef issue stroke:#c0392b,stroke-width:2px
    class HV issue
```
...
flowchart LR
    R["HW 가상화 지원 3 대 요소"]
    IO["1. I/O 가상화<br/>디바이스를 VM 별 분할 (PF/VF)"]
    INT["2. 인터럽트 가상화<br/>올바른 VM 에 직접 전달"]
    MEM["3. 메모리 가상화<br/>IOMMU 2-stage translation"]
    R --> IO
    R --> INT
    R --> MEM
    classDef root stroke:#1a73e8,stroke-width:3px
...
```mermaid
flowchart TB
    subgraph IP["HW IP (MVIP)"]
        direction TB
        PF["<b>PF</b> (Physical Function)<br/>engine0 (ctrlr)<br/>Hypervisor 가 직접 설정 / 관리<br/>전체 디바이스 초기화 담당"]
        VF0["<b>VF0</b><br/>VM0 용"]
        VF1["<b>VF1</b><br/>VM1 용"]
        VF2["<b>VF2</b><br/>VM2 용"]
    end
    VM0["VM0 Guest OS"]
    VM1["VM1 Guest OS"]
    VM2["VM2 Guest OS"]
    HV["Hypervisor"]
    HV -- "설정 / 관리" --> PF
    VM0 -. "직접 설정 / 사용<br/>(pass-through)" .-> VF0
    VM1 -. "pass-through" .-> VF1
    VM2 -. "pass-through" .-> VF2
    classDef pf stroke:#c0392b,stroke-width:2px
    classDef vf stroke:#137333,stroke-width:2px
    class PF pf
    class VF0,VF1,VF2 vf
```
...
flowchart LR
    ...
    HV["Hypervisor<br/>(인터럽트 라우팅)"]
    VMx["VMx"]
    HW -- "1. IRQ 발생" --> IC
    IC -- "2. 포착" --> HV
    HV -- "3. 어떤 VM 의 인터럽트인지<br/>판단 후 전달" --> VMx
```
