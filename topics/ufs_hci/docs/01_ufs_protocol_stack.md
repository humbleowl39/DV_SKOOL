...
flowchart LR
    subgraph HOST["Host side"]
        direction TB
        H_APP["App<br/>READ 4KB @LBA=0x1000"]
        H_UTP["UTP / UPIU"]
        H_UNI["UniPro<br/>L4 / L3 / L2 / L1.5"]
        H_PHY["M-PHY TX"]
        H_APP -- "SCSI CDB<br/>(16B, opcode=0x28)" --> H_UTP
        H_UTP -- "Command UPIU" --> H_UNI
        H_UNI -- "DL Frame<br/>SOF+Hdr+UPIU+CRC+EOF" --> H_PHY
    end
    subgraph DEV["Device side"]
        ...
        D_UNI["UniPro"]
        D_UTP["UTP / UPIU 디코드"]
        D_MEDIA["Storage media<br/>(NAND read)"]
        D_PHY -- "M-PHY symbol<br/>(8b/10b or PWM)" --> D_UNI
        D_UNI -- "DL Frame 재조립" --> D_UTP
        D_UTP -- "SCSI 명령 실행" --> D_MEDIA
    end
    H_PHY -- "TX lane" --> D_PHY
```
...
```mermaid
flowchart TB
    APP["<b>UFS Application Layer (UTP — UFS Transport Protocol)</b><br/>· SCSI 명령 세트 (READ / WRITE / QUERY)<br/>· UPIU (UFS Protocol Information Unit) 패킷 구성<br/>· Task Management (Abort, LUN Reset 등)"]
    UNI["<b>UniPro (Unified Protocol) — Link Layer</b><br/>· L4: DME (Device Management Entity)<br/>· L3: N-Layer (Network, 보통 point-to-point)<br/>· L2: DL (Data Link — CRC, ACK/NAK, Flow Control)<br/>· L1.5: PHY Adapter (인터페이스 어댑터)"]
    PHY["<b>M-PHY (MIPI Physical Layer)</b><br/>· HS (High Speed) Gear 1~4: 1.46~11.6 Gbps/lane<br/>· PWM (저전력 모드): Gear 1~7<br/>· 1~2 Lane (데이터 레인 수)<br/>· CDR, Calibration, Power Mode 전환"]
    APP -- "UPIU" --> UNI
    UNI -- "시리얼 데이터" --> PHY
```
