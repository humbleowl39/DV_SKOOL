...
```mermaid
flowchart TB
    subgraph HCI["UFS HCI"]
        direction TB
        REG["<b>Host Controller Registers</b> (SW Interface · AHB/AXI)<br/>CAP · HCE · IS · IE<br/>UTRLBA · UTRLDBR · UTMRLBA"]
        XFER["<b>UTP Transfer Request Engine</b><br/>· UTRD 파싱<br/>· UPIU 생성<br/>· DMA (PRDT)<br/>· 완료 처리"]
        TM["<b>UTP Task Mgmt Request Engine</b><br/>· UTMRD 파싱<br/>· Task Mgmt UPIU 생성"]
        UPIU["<b>UPIU Engine</b><br/>· UPIU 패킷 조립 / 분해<br/>· Command / Response / Data 처리"]
        IF["<b>UniPro / M-PHY Interface</b>"]
        REG --> XFER
        REG --> TM
        XFER --> UPIU
        TM --> UPIU
