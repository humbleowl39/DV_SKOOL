...
반도체 하드웨어 인터뷰는 단일 전공 지식만으로 통과하기 어렵다. 같은 "CDC 신호를 어떻게 넘기나요?"라는 질문이 RTL 면접에서는 synchronizer 회로로, DV 면접에서는 검증 전략으로, PD 면접에서는 timing constraint로 변주되어 나오기 때문이다. 한 영역만 깊게 파고 나머지를 비워두면, 면접관이 인접 영역으로 한 발만 넘어가도 답이 막히고 "자기 블록만 아는 사람"이라는 인상을 준다.

이 코스는 Digital RTL, DV, Embedded, Architecture, Analog, Physical Design 6대 영역의 빈출 질문을 가로질러, 각 개념의 정의뿐 아니라 *왜 그렇게 되는지*와 *언제 무엇을 선택하는지*까지 답할 수 있도록 설계되었다. 정의 암기에서 멈추지 않고 trade-off와 인과를 말로 설명하는 훈련이 핵심이며, 그것이 면접에서 "외운 사람"과 "이해한 사람"을 가르는 지점이다.
