...
flowchart TB
    POR["Power-On Reset"]
    MODE["Boot Mode 결정 (BootROM 초기 단계)<br/>① OTP[BOOT_MODE] != UNSET ? YES → OTP 모드 선택 / NO → ②<br/>② Pinstrap GPIO 읽기<br/>  pin == 11 → eMMC primary<br/>  pin == 10 → UFS primary<br/>  pin == 01 → USB DL<br/>  pin == 00 → ③ Default<br/>③ BootROM 하드코딩 default"]
    PRIMARY["Primary Boot Device 시도<br/>UFS / eMMC / SPI NOR / USB DL 중 선택된 path 로<br/>1) Device init (PHY → link → transport)<br/>2) Boot Header / FIP ToC parsing<br/>3) BL2 image + cert 를 SRAM 으로 DMA<br/>4) Module 02-03 의 서명 검증 흐름으로 진입"]
    FALLBACK["Secondary / Tertiary fallback (OTP 사전 설계된 list)<br/>eMMC 실패 → USB DL Mode → 무한 대기 또는 timeout"]

    POR --> MODE --> PRIMARY
    PRIMARY -- "하나라도 실패<br/>→ Secondary 로 fallback" --> FALLBACK
```
...
```mermaid
flowchart LR
    subgraph EMMC["eMMC: 단순 CMD/DATA"]
        direction LR
        EA["App"]
        EB["eMMC Bus"]
        EC["Card"]
    ...
    subgraph UFS["UFS: 3개 레이어 모두 초기화 필요 = 복잡"]
        direction LR
        UA["UTP<br/>(SCSI)"]
        UB["UniPro<br/>(Link)"]
        UC["M-PHY<br/>(Serial)"]
