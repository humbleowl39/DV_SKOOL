...
```
   OTP[ROTPK_HASH]  (제조 시점 anchor — Module 01)
        │
        ▼ ① BL1 (BootROM) 이 cert 의 PK 를 hash 해서 OTP 와 비교
   ┌────────────┐  ② 일치하면 cert.sig 검증 → cert 안의 BL2_hash 신뢰
   │  BL1 (ROM) │     ③ 실제 BL2 image 의 SHA-256 == cert.BL2_hash 검증
   │  Sec EL3   │     ④ 모두 PASS → BL2 entry 로 jump
   └─────┬──────┘
         │ trust transfer (verify-then-execute)
         ▼
   ┌────────────┐     ⑤ BL2 가 BL31/32/33 cert 를 같은 방법으로 검증
   │   BL2      │        (Trusted Key cert → BL3x content cert)
   │  Sec EL1   │     ⑥ 검증 PASS 한 image 만 DRAM 에 풀어서 jump
   └─────┬──────┘
         │
   ┌─────┴─────┬──────────┐
   ▼           ▼          ▼
 ┌───────┐  ┌───────┐  ┌────────┐
 │ BL31  │  │ BL32  │  │ BL33   │ ── (검증된 chain 끝, OS 로 진입)
 │ EL3   │  │ S-EL1 │  │NS-EL1/2│
 └───────┘  └───────┘  └────────┘
```
...
```
   Flash (외부)                        Internal SRAM            BootROM (BL1, EL3)
   ─────────────                       ─────────────────        ────────────────────
   ┌─────────────┐                                              ① POR → reset vector
   │ BL2 image   │                                              ② boot device 결정
   │ (e.g. 2 MB) │                                              ③ DMA: Flash → SRAM
   │             │ ────── DMA copy ───▶  ┌──────────────┐
   │ BL2 cert    │                       │ BL2 image    │      ④ cert parse → cert.PK
   │ (~ 1 KB)    │                       │ BL2 cert     │      ⑤ SHA-256(cert.PK) 계산
   └─────────────┘                       └──────────────┘      ⑥ OTP[ROTPK_HASH] 읽기
                                                               ⑦ ⑤ == ⑥ ?  (PK 인증)
                                                                   ├─ NO  → halt
                                                                   └─ YES → next
                                                               ⑧ Verify(cert.sig, cert.PK)
                                                                   ├─ NO  → halt
                                                                   └─ YES → cert.BL2_hash 신뢰
                                                               ⑨ SHA-256(BL2 image) 계산
                                                               ⑩ ⑨ == cert.BL2_hash ?
                                                                   ├─ NO  → halt
                                                                   └─ YES → ⑪
                                                               ⑪ jump BL2_entry  ★
...
```
          ROTPK (OTP hash)        ← 절대 변경 불가
          /           \
   Trusted Key    Non-Trusted Key   ← cert 안에 포함, 교체 가능
    /      \           |
