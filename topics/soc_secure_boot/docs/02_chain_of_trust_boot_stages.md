...
```mermaid
flowchart TB
    OTP["OTP[ROTPK_HASH]<br/>(제조 시점 anchor — Module 01)"]
    BL1["BL1 (ROM)<br/>Sec EL3<br/>① cert 의 PK 를 hash → OTP 와 비교<br/>② 일치 시 cert.sig 검증 → cert.BL2_hash 신뢰<br/>③ SHA-256(BL2) == cert.BL2_hash<br/>④ 모두 PASS → BL2 entry 로 jump"]
    BL2["BL2<br/>Sec EL1<br/>⑤ BL31/32/33 cert 를 같은 방법으로 검증<br/>(Trusted Key cert → BL3x content cert)<br/>⑥ 검증 PASS 한 image 만 DRAM 에 풀어서 jump"]
    BL31["BL31<br/>EL3"]
    BL32["BL32<br/>S-EL1"]
    BL33["BL33<br/>NS-EL1/2<br/>(검증된 chain 끝, OS 로 진입)"]

    OTP --> BL1
    BL1 -- "trust transfer<br/>(verify-then-execute)" --> BL2
    BL2 --> BL31
    BL2 --> BL32
    BL2 --> BL33
```
...
```mermaid
flowchart TB
    ROTPK["ROTPK (OTP hash)<br/>절대 변경 불가"]
    TK["Trusted Key<br/>(cert 안에 포함, 교체 가능)"]
    NTK["Non-Trusted Key<br/>(cert 안에 포함, 교체 가능)"]
...
```mermaid
flowchart TB
    BL1["BL1 (BootROM)<br/>저장: Mask ROM | 메모리: Internal SRAM<br/>권한: Secure EL3 | 크기: 수십~수백 KB<br/>- CPU 초기화<br/>- 보안 HW 초기화<br/>- Boot Device 접근<br/>- BL2 로드 + 검증 (ROTPK 사용)<br/>- BL2 로 Jump"]
    BL2["BL2 (FSBL)<br/>저장: Flash/UFS | 메모리: SRAM → DRAM<br/>권한: Secure EL1 | 크기: 수 MB 까지<br/>- DRAM 컨트롤러 초기화<br/>- 추가 HW 초기화<br/>- BL31/32/33 로드+검증<br/>- Secure World 설정"]
    BL31["BL31<br/>Secure Monitor (ATF)<br/>EL3 | DRAM 실행"]
    BL32["BL32<br/>Secure OS (TEE) / OP-TEE<br/>S-EL1 | DRAM 실행"]
    BL33["BL33<br/>Non-Sec Bootldr / U-Boot<br/>NS-EL1/2 | DRAM 실행"]
    OS["OS (Linux, Android)<br/>Normal World<br/>NS-EL1"]
...
```mermaid
flowchart LR
    A["BL1<br/>init + auth<br/>(ROM)"]
    B["BL2<br/>DRAM + auth<br/>(SRAM → DRAM)"]
    C["BL31<br/>Secure Monitor<br/>(EL3)"]
    D["BL32<br/>TEE OS<br/>(S-EL1)"]
    E["BL33<br/>Normal Bootloader<br/>(NS-EL1)"]
    A --> B --> C
    B --> D
    B --> E
```
...
```mermaid
flowchart TB
    OTP["OTP (eFuse)<br/>ROTPK Hash"]
    ROOT["Root Certificate<br/>(ROTPK 로 서명됨)"]
    TKC["Trusted Key Certificate<br/>(Root Key 로 서명됨)"]
    BL2C["BL2 Cert<br/>(BL2 해시, 키로 서명됨)"]
    BL3C["BL3 Cert<br/>(BL33 해시, 키로 서명됨)"]

    OTP --> ROOT
    ROOT -- "Root Key 가 검증" --> TKC
    TKC --> BL2C
    TKC --> BL3C
```
...
```mermaid
flowchart TB
    UDS["UDS (Unique Device Secret)<br/>HW 에 고정 (OTP/PUF)"]
    C1["CDI_BL1 = KDF(UDS, Hash(BL1))<br/>BL1 의 ID"]
    C2["CDI_BL2 = KDF(CDI_BL1, Hash(BL2))<br/>BL1+BL2 의 ID"]
    NOTE["각 단계의 CDI 가<br/>이전 단계에 의존<br/>→ 체인 무결성"]
