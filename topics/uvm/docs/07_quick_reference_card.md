...
flowchart TB
    ...
    DRV["uvm_driver<br/><i>DUT 에 자극</i>"]
    MON["uvm_monitor<br/><i>DUT 관찰 → AP</i>"]
    SQR["uvm_sequencer<br/><i>Sequence ↔ Driver 중개</i>"]
    AGP["uvm_agent (Passive)"]
    MONP["uvm_monitor<br/><i>출력 관찰 → AP</i>"]
    SB["uvm_scoreboard<br/><i>기대값 vs 실제값 비교</i>"]
    SUB["uvm_subscriber<br/><i>Coverage 수집</i>"]
    ...
    classDef check stroke:#b8860b,stroke-width:2px
    class AGA,DRV,MON,SQR act
    class AGP,MONP passive
    class SB,SUB check
```
