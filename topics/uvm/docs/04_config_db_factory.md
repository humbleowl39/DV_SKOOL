...
```
   ┌──────── Test (top of TB) ─────────┐
   │                                    │
   │   uvm_config_db#(virtual my_if)::set( null, "uvm_test_top.env.agent.*", "vif", intf );
   │                                                        │
   │                                                        ▼  (게시판에 부착)
   └────────────────────────────────────────────────────────┘
                                   │
   ...
   │                                                ▲ (factory 가 카탈로그 보고 만듦)
   │                                                │
   │                                  type override 등록되어 있으면 → 다른 클래스 인스턴스 반환
   │
   └────────────────────────────────────────────────────────┘
                                   │
                                   ▼
   ...
   │                                                │
   │                                                ▼
   │   driver.build_phase 에서:
   │   if (!uvm_config_db#(virtual my_if)::get( this, "", "vif", vif ))
   │       `uvm_fatal("NOVIF", "vif not found")
   │
   │   ── get 의 시작 = this 의 full_name() = "uvm_test_top.env.agent.driver"
   │   ── 게시판에서 "uvm_test_top.env.agent.*" wildcard 매칭 → 발견 ✓
   │
   └────────────────────────────────────────────────────────┘
```
...
```
 ...
 │   │       null, "*", "vif", intf);             │
 │   │   ②   ↑       ↑      ↑     ↑              │
 │   │   ②   context inst   field value          │
 │   │   ③ run_test();                            │
 │   │ end                                         │
 │   └───────────────────────────────────────────┤
 └──────────────────────────────────────────────┘
                                   │
                                   ▼ run_test → uvm_test_top.build_phase 시작
 ┌──────────────────────────────────────────────┐
