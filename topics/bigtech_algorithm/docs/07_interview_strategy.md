...
코딩 인터뷰에서 코드가 정확히 작동하고 복잡도도 최적임에도 불합격 통보를 받는 경우가 실제로 있습니다. 피드백은 대개 이렇습니다.

> "후보는 30 분 동안 침묵한 채 코딩만 했습니다. 어떤 생각 과정인지 알 수 없었고, edge case (빈 입력, single element) 도 다루지 않았습니다."

면접관은 코드 한 줄이 아니라 네 가지 축으로 채점합니다.
...
Correctness 가 100점이어도 Communication 이 0점이면 합격선에 미달합니다. 면접관이 평가하는 것은 코드가 아니라 함께 일할 수 있는 사람인지이기 때문에, 침묵하는 천재보다 말하는 평범이 종종 합격합니다.
...
패턴을 빠르게 매핑하는 가장 실용적인 방법은 **문제에서 특정 키워드가 보이는 즉시 후보 패턴을 떠올리는 것**입니다. 이 키워드 → 패턴 대응은 Module 01~06 에서 배운 내용을 면접 상황에서 30 초 안에 작동시키기 위한 압축본입니다. 아래 표를 암기하기보다 각 패턴이 왜 그 키워드에 적합한지 이유를 이해하면 새로운 키워드 조합도 유연하게 대응할 수 있습니다.
