...
!!! danger "❓ 흔한 오해"
    ...
    **실제**: Communication, edge case, complexity 합의 없이 풀이 시작은 "완성" 해도 큰 감점. 다 못 풀어도 사고 흐름이 명확하면 통과 가능.

    **왜 헷갈리는가**: 코딩테스트(자동채점) 와 면접의 평가 기준 차이를 체감 못 할 때.

!!! warning "실무 주의점 — 입력 제약 / 엣지 케이스 명시 없이 코딩 시작"
    **현상**: 코딩은 빠르게 끝났는데 follow-up 에서 "N=0 이면?", "음수면?", "중복 있으면?" 질문이 들어오자 코드를 즉석에서 뜯어고치다 시간이 다 가고 신뢰가 무너진다.

    **원인**: clarify 단계에서 입력 범위 / 자료형 / 엣지 케이스를 칠판에 적지 않고 머릿속에서만 가정한 채 진행. 면접관과 합의된 명세가 없으니 follow-up 마다 재설계가 발생.

    **점검 포인트**: 코딩 전에 N 범위, 값의 범위/부호, 빈 입력/단일 원소/중복/오버플로우 가능성 5 가지를 한 줄씩 적었는가, 그리고 면접관의 동의를 받았는가.

!!! tip "💡 이해를 위한 비유"
    **면접 = 사고 과정 평가** ≈ **코드 결과보다 "어떻게 풀이를 만들어 가는지" 가 더 중요**

    Correctness + Complexity + Code Quality + Communication 의 4축. 다 풀어도 communication 부족이면 실패.
