...
```mermaid
flowchart LR
    subgraph PATHS["에러 발생 3 경로"]
        direction TB
        P1["① RDMASQDestroy(.err(1))<br/>→ driver.setErrState(qp)"]
        P2["② cq_handler 가 에러 CQE 수신<br/>→ setErrState(qp)"]
        P3["③ monitorErrCQ (백그라운드)<br/>ERR_CQ 폴링 → ②"]
    end
    subgraph GATES["격리 게이트 3 단위"]
        direction TB
        G1["① per-cmd<br/>expected_error<br/>범위: 1 verb"]
        G2["② per-QP<br/>isErrQP()<br/>범위: 그 QP 후속 verb 전부"]
        G3["③ per-component<br/>err_enabled<br/>범위: 모든 QP, 한 컴포넌트"]
    end
    subgraph CLEANUP["격리 후 chained 정리"]
        direction TB
        CL1["driver: outstanding flush<br/>completed_wqe_ap 차단"]
        CL2["comparator: flushQP()<br/>pending 큐 삭제"]
        CL3["c2h_tracker:<br/>is_err_qp_registered=1<br/>매칭 실패 skip"]
        CL4["sequencer:<br/>wc_error_status[qp]<br/>first error 보존"]
    end
    PATHS --> GATES --> CLEANUP

    classDef path stroke:#1a73e8,stroke-width:2px
    classDef gate stroke:#b8860b,stroke-width:2px
    classDef clean stroke:#137333,stroke-width:2px
    class P1,P2,P3 path
    class G1,G2,G3 gate
    class CL1,CL2,CL3,CL4 clean
```
