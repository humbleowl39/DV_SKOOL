...
```
   ┌─────── 에러 발생 3 경로 ────────┐    ┌─────── 격리 게이트 3 단위 ──────┐
   │                                 │    │                                 │
   │  ① RDMASQDestroy(.err(1))       │    │  ① per-cmd : expected_error    │
   │     → driver.setErrState(qp)    │    │     적용 범위: 1 verb           │
   │                                 │    │                                 │
   │  ② cq_handler 가 에러 CQE 받음  │    │  ② per-QP  : isErrQP()         │
   │     → setErrState(qp)           │    │     적용 범위: 그 QP 의 모든    │
   │                                 │    │                후속 verb        │
   │  ③ monitorErrCQ (백그라운드)     │    │                                 │
   │     → ERR_CQ 폴링 → ②로 진입    │    │  ③ per-component : err_enabled │
   │                                 │    │     적용 범위: 모든 QP, 한 컴포 │
   └─────────────────────────────────┘    └─────────────────────────────────┘

   격리 후 chained 정리:
       driver: outstanding flush, completed_wqe_ap 차단 (M04 게이트)
       comparator: flushQP() — pending 큐 삭제
       c2h_tracker: is_err_qp_registered = 1 — 매칭 실패 skip
       sequencer: wc_error_status[qp] 에 first error 보존
```
...
```
   t = T0       │  test seq: this.RDMAQPDestroy(.t_seqr(seqr), .qp_num(5), .err(1))
                │
   t = T0+1     │  driver.RDMASQDestroy 진입
                │      destroy_qp.setErrState(cmd.err)            ← vrdma_driver.svh:530
                │      qp[5].state = SQErr                          (per-QP 게이트 ON)
                │
   t = T0+2     │  driver 의 다음 EntryPoint(cmd_X) — qp_num=5
                │      chkSQErrQP(cmd_X) → qp[5].isErrQP()==1 → skip + warning
                │      (cmd_X 의 issued_wqe_ap.write 도 skip 안 됨, 발행은 함, 단 driver 가 무시)
                │
   t = T0+3     │  CQE 가 도착해도 outstanding 처리 중
                │      if(!t_qp.isErrQP()) completed_wqe_ap.write(cmd)  ← 차단! (M04 게이트)
                │      → scoreboard 가 정상 완료로 기록 안 함
                │
   t = T0+4     │  RDMAQPDestroy 가 각 횡단 컴포넌트에 deregister 알림
                │      1side_compare.deregisterQP(qp=5)
                │          if(qp.isErrQP() || err_enabled) flushQP(5)  ← :1312
                │          → write_pending_q[5], read_pending_q[5] 비움
                │      2side_compare.deregisterQP(qp=5) → flushQP(5)    ← :697
                │      imm_compare.deregisterQP(qp=5)   → flushQP(5)    ← :433
                │      c2h_tracker.deregisterQP(qp=5)
                │          is_err_qp_registered[node][5] = 1            ← :346,349
                │          → 이후 매칭 실패 발생해도 fatal 대신 skip
                │
   t = T0+5     │  test seq: t_seqr.clearErrorStatus(5)
                │      sequencer.wc_error_status[5].clear()
                │      (단, 이전 에러 정보가 필요했다면 t_seqr.wc_error_status[5][0] 으로 캡처해 둬야)
```
