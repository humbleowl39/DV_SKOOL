...
```
   원칙 1: Open-Closed       │  ✗ driver 안에 if(cc_enabled) hook       │  ✓ vrdma_cc_monitor 새 컴포넌트 추가
   원칙 2: Interface         │  ✗ AP 시그니처 타입 변경                  │  ✓ object 에 필드 추가, 새 AP 추가
   원칙 3: DRY via AP        │  ✗ driver 내부 변수 직접 read             │  ✓ 기존 AP 구독
   원칙 4: Stateless 보존    │  ✗ handler 에 카운터 추가                 │  ✓ sequencer 또는 새 stateful subscriber
```
