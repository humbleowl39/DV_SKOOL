...
```
                        DUT 의 C2H DMA 트랜잭션
                                  │
                                  ▼
                ┌─── vrdma_c2h_tracker.processC2hTransaction ───┐
                │                                                │
       ┌────────┼─① 주소가 m_qp_tracker[*][*].pa_queue 안에 있나? │
       │ NO     │              │ YES                              │
       │        │              ▼                                  │
       │        │  ─② 어느 QP 의 PA 인가? (cross-QP 충돌 검사) ─  │
       │        │              │                                  │
       │        │              ▼                                  │
       │        │  ─③ RC QP 라면 idx==0 이어야, OPS/SR 라면 ─    │
       │        │     idx 범위 내면 통과                            │
       │        │              │                                  │
       │        │              ▼                                  │
       │        │  ─④ DUT len <= cmd.transfer_size 인가? ───────  │
       │        │              │                                  │
       │        │       OK     ▼ FAIL                              │
       │        │              │                                  │
       └────────┼──▶ is_err_qp_registered[node][qp] ?              │
                │       YES   │   NO                              │
                │   silently  │  ▼                                │
                │   skip      │  F-C2H-MATCH-0001/0002/0003/0004  │
                │             │                                   │
                └─────────────────────────────────────────────────┘
                              │
                              ▼
                   `W-C2H-MATCH-0001~0004` (unprocessed PA dump)
```
...
```
   ┌─ 어느 실패인가? ─────────────────────────────────────┐
   │                                                       │
   │  ① PA 매칭 실패  (F-C2H-MATCH-0001/0002)              │
   │     └─ "expected PA 큐에 없는 주소에 DUT 가 씀"         │
   │     └─ root cause: PTW / QP routing / MR page table   │
   │                                                       │
   │  ② Ordering 위반 (E-C2H-MATCH-0001)                   │
   │     └─ "RC 인데 idx 0 가 아닌 위치의 PA 가 먼저 도착"  │
   │     └─ root cause: DUT RC out-of-order 처리           │
   │     └─ (OPS/SR 라면 idx 범위 내 OoO 정상)             │
   │                                                       │
   │  ③ 크기 초과   (F-C2H-MATCH-0003/0004/0005)            │
   │     └─ "DUT 가 transfer_size 보다 더 많이 씀"          │
   │     └─ root cause: DUT length 누적 / padding 오류      │
   └───────────────────────────────────────────────────────┘
```
