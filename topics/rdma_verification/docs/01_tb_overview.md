...
```
                          ┌──────────── vrdmatb_top_env ────────────┐
                          │                                          │
   ┌─── node[0] ───┐      │     ┌──── 횡단 검증 (cross-node) ───┐   │      ┌─── node[1] ───┐
   │ host mem      │ ───▶ │     │  data_env  : memory compare    │ ◀─── │ host mem      │
   │ ip shell      │      │     │  dma_env   : c2h_tracker       │      │ ip shell      │
   │ agent         │      │     │  ntw_env   : packet monitor    │      │ agent         │
   │  ├ driver     │      │     └────────────────────────────────┘      │  ├ driver     │
   │  ├ sequencer  │      │                                              │  ├ sequencer  │
   │  └ handlers   │      │     [node][qp] 키로 모든 트랜잭션 추적       │  └ handlers   │
   └───────────────┘      │                                              └───────────────┘
            │             │                                                       │
            └─────────────┴───── network (RDMA packet flow) ──────────────────────┘
```
...
```
   ...
   │ rdma_seqr       │         │               │         │                 │
   │   │             │         │               │         │                 │
   │   ▼ ① top_seq.RDMAWrite(.t_seqr=seqr[0])  │         │                 │
   │ driver          │         │               │         │                 │
   │   │ ② issued_wqe_ap.write(cmd) ─────────▶ write_handler              │
   │   │                       │   │           │         │                 │
   │   │                       │   ├──▶ 1side_compare (write 큐에 enqueue) │
   │   │                       │   └──▶ c2h_tracker  (expected PA 계산)    │
   │   │                       │               │         │                 │
   │   ▼ ③ DUT 에 doorbell      │               │         │                 │
   │ DUT IP_0 ─ pkt ─────── pkt_monitor ──────────────── DUT IP_1           │
   │                       │  (ntw_env)    │         │   │                 │
   │                       │               │         │   ▼ ④ payload DMA   │
   │                       │               │         │ host mem[1]         │
   │                       │               │         │   │                 │
   │                       │               │ ⑤ c2h_tracker 가 C2H 매칭     │
   │                       │               │         │   │                 │
   │                       │               │ ⑥ 1side_compare 가 mem[0] vs mem[1] 비교 │
   │                       │               │         │   │                 │
   │ ⑦ CQE ◀ cqe_ap ──── cq_handler ◀── pkt 도착 / completion │            │
   │   │                       │               │         │                 │
   │   ▼ ⑧ completed_wqe_ap.write(cmd) ─▶ data_scoreboard  │                │
   └─────────────────┘         └───────────────┘         └─────────────────┘
```
