...
RNM은 **단순화된 물리(simplified physics)**입니다. 1차 효과(이상적 VCO 주파수, 이상적 ADC 양자화)는 잘 잡지만 noise · jitter · settling 같은 2차 효과는 손으로 주입해야 합니다. 너무 단순하면 false-pass, 너무 정밀하면 회귀 비용이 폭발합니다.

### ② Equivalence 부재

Digital DV의 LEC(logic equivalence checking)에 해당하는 mixed-signal 도구는 성숙도가 낮습니다. spec(수식) ↔ behavioral 모델 ↔ Spice netlist 간 동등성을 자동으로 검증할 도구가 약해, 검증 엔지니어가 **assertion · coverage · cross-check**으로 신뢰성을 쌓아야 합니다.

### ③ X 부재 위험

`real`엔 X가 없습니다. uninitialized = `0.0`이라 **false-pass**가 잘 생깁니다. 모든 RNM 입력에 `bit valid` 동반 또는 driver-presence assertion을 강제해야 합니다 — Ch10의 "흔한 함정"에서 다시 다룹니다.

### ④ Multi-Driver Resolution

두 driver가 한 wire를 잡으면 digital은 X, analog는 물리 합성(KCL/KVL)이지만, plain `real`은 last-write-wins로 silently 마지막 값만 남습니다. **nettype + resolution function**으로 KCL/KVL을 명시해야 합니다.

### ⑤ Coverage 정의

"analog signal X가 충분히 swept 되었나"를 어떻게 보일까? cycle 기반 coverage는 의미가 약합니다. **range binning · threshold crossing · transition pattern**을 covergroup으로 변환해야 의미 있는 도달도가 됩니다.
