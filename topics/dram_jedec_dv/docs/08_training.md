...
> **순서 유연성**: 위 번호 순서는 *대표적 흐름*이며 절대 고정 순서가 아닙니다. spec은 필수 *의존 관계*(예: WCK2CK Leveling은 CBT 완료 후)만 규정하고, 그 제약을 어기지 않는 한 일부 단계의 순서·반복은 controller 구현 재량입니다(특히 DVFSC gear 전환 시 CBT/WCK2CK 일부를 재수행). 따라서 DV의 training FSM은 *허용된 순서 변형*을 reject하지 않도록, 고정 시퀀스가 아니라 *의존 관계 위반*을 기준으로 검증해야 합니다.
