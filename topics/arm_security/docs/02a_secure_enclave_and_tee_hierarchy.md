...
```mermaid
flowchart BT
    REE["<b>REE</b> (Linux/Android, NS-EL0/1)<br/>최하 — OS 해킹 = 일반 데이터 노출"]
    NSEL2["<b>Non-Secure EL2</b> (Hypervisor / KVM)<br/>VM escape 시 NS world 전체"]
    TZ["<b>TrustZone</b> (S-EL0/1)<br/>Trusted OS + TA<br/>TZ 뚫리면 Secure World 전체<br/>(BUT enclave key 는 살아남음)"]
    INT["<b>Internal Secure Enclave</b> (on-die)<br/>Apple SEP, Samsung SSP<br/>전용 processor + RAM<br/>사이드 채널 / RCE 둘 다 무력"]
    EXT["<b>External Secure Enclave</b> (별도 IC)<br/>TPM, NXP SE050, Infineon SLE97<br/>최고 — 물리 분리"]

    REE -- "stage 2 격리" --> NSEL2
    NSEL2 -- "NS bit" --> TZ
    TZ -- "Mailbox + 인증 토큰" --> INT
    INT -- "SPI Secure Channel<br/>(암호화 + MAC)" --> EXT
...
```mermaid
flowchart TB
    REE["<b>REE (Rich EE)</b><br/>일반 OS (Linux, Android) — NS-EL0/NS-EL1<br/>보안 레벨 최하"]
    NSEL2["<b>Non-secure EL2 (Hypervisor)</b><br/>VM 관리, VM별 리소스 분리<br/>여전히 Non-secure"]
    TZ["<b>Secure (TrustZone)</b><br/>Trusted OS + TA — S-EL0/S-EL1<br/>키 관리, 인증, FW 검증 (Module 01-02 범위)"]
    INT["<b>Secure Enclave (SoC Internal)</b><br/>전용 프로세서 + 전용 RAM · CPU 클러스터와 독립<br/>Key Box + Crypto Accelerator"]
    EXT["<b>Secure Enclave (External IC)</b><br/>별도 보안 칩 (SPI 연결) · Private Storage<br/>최상위 Root of Trust"]

    REE --> NSEL2 --> TZ --> INT --> EXT

    classDef low stroke:#5f6368,stroke-width:1px
    classDef mid stroke:#1a73e8,stroke-width:2px
    classDef sec stroke:#c5221f,stroke-width:2px
    classDef high stroke:#b8860b,stroke-width:3px
    class REE low
    class NSEL2 mid
    class TZ sec
    class INT,EXT high
...
flowchart TB
    subgraph SoC["SoC"]
        CPU["CPU Cluster<br/>(TrustZone 포함)<br/>NS-EL0/1/2, S-EL0/1"]
        IC["System Interconnect"]
        subgraph ISE["Internal Secure Enclave"]
            direction LR
            PROC["Processor<br/>(자체 ISA)"]
            RAM["Private RAM<br/>(SRAM, 수 KB~)"]
            KB["Key Box<br/>(키 저장)"]
            CA["Crypto Accel<br/>(AES/RSA/ECC)"]
            MB["Mailbox (APB)"]
            DMA["DMA"]
        end
        CPU --- IC --- ISE
    end
    EXT["External Secure Enclave<br/>(보안 칩, 별도 IC)"]
    STO["Storage<br/>(Root of Trust)"]
    ISE -- "SPI (Secure Channel)" --> EXT
    EXT --- STO

    classDef cpu stroke:#1a73e8,stroke-width:2px
    classDef enc stroke:#c5221f,stroke-width:2px
    classDef ext stroke:#b8860b,stroke-width:3px
    class CPU,IC cpu
    class PROC,RAM,KB,CA,MB,DMA enc
    class EXT,STO ext
```
...
```mermaid
flowchart TB
    subgraph PE["Enclave Processing Element"]
        SECRET["비밀 데이터 (평문)"]
        CA["Crypto Accelerator<br/>(내부에서 암호화)"]
        AXI["AXI Master<br/>(AxPROT=Secure)"]
        SECRET --> CA --> AXI
    end
    DRAM["DRAM<br/>(암호문만, 물리 덤프해도 평문 없음)"]
    AXI -- "암호문만 기록" --> DRAM

    classDef enc stroke:#c5221f,stroke-width:2px
    classDef ext stroke:#1a73e8,stroke-width:2px
    class SECRET,CA,AXI enc
    class DRAM ext
```
