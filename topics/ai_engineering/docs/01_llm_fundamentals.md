...
!!! tip "💡 이해를 위한 비유"
    **LLM = 자기회귀 토큰 생성기** ≈ **매우 빠른 추론 인턴 — 한 글자(토큰) 씩 "가장 likely 한 다음 글자" 를 결정**

    한 번 호출 = "다음 토큰 1개 확률 분포 → 샘플링" 의 반복. 글이 자연스러운 이유는 attention 으로 이전 컨텍스트 모두 참조하기 때문.
...
!!! danger "❓ 흔한 오해"
    ...
    **실제**: LLM 은 답을 외운 게 아니라 패턴 기반 다음 토큰 예측. 학습 데이터의 통계적 분포를 따른 generate 이지 lookup 이 아님.

    **왜 헷갈리는가**: "AI 가 알고 있다" 라는 marketing 표현 + 답이 정확할 때 외운 것 같은 인상.
---
