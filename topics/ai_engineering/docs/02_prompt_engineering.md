...
```
   ...
   │ "코드 짜줘"│  │ Role+Ctx+│   │ Few-shot │
   │           │   │ Task+Fmt │   │ + CoT    │
   └────┬─────┘   └────┬─────┘   └────┬─────┘
        │              │              │
        ▼              ▼              ▼
   generic        구체적         단계 추론 + 일관 형식
   (low quality)  (mid)          (high quality, ↑ tokens)
        ●              ●              ●
        └─ 가중치 동일, 입력 분포만 변경 → 출력 분포가 달라짐 ─┘
```
...
```
   ┌─── 동일 모델 (Claude Sonnet, T=0) ───┐
   │                                       │
   │   Prompt A (naive)                    │   Prompt B (구조화)
   │   "UVM 테스트벤치 만들어줘"            │   [Role][Context][Task][Constraint][Format]
   │           │                            │           │
   │           ▼ ① tokenize (~6 tok)        │           ▼ ① tokenize (~150 tok)
   │           ▼ ② attention                │           ▼ ② attention (richer routing)
   │           ▼ ③ generate                 │           ▼ ③ generate
   │           │                            │           │
   │           ▼                            │           ▼
   │   ④ 일반적 코드, port 추측             │   ④ 정확한 port + 컨벤션 준수
   │   ⑤ $display 사용 (금지된 패턴)        │   ⑤ `uvm_info 사용
   │   ⑥ factory 등록 누락                  │   ⑥ factory + config_db get
   │   ⑦ 컴파일 실패 60%                   │   ⑦ 컴파일 통과 87%
   │                                       │
   └───────────────────────────────────────┘
```
