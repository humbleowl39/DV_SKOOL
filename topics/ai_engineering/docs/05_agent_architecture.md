...
flowchart TB
    U["User<br/>목표 입력"]
    subgraph AG["Agent — loop until 목표 달성 / max_steps / budget / abort"]
        direction TB
        L["LLM (brain)<br/>- 추론, 계획, 의사결정<br/>- JSON tool_use 응답"]
        M["Memory<br/>(short / long / working)"]
        T["Tool dispatcher<br/>read_file<br/>run_simulation<br/>search_faiss<br/>..."]
        ENV["실제 환경<br/>(FS / VCS / DB)"]
        L -- "Thought" --> M
        L -- "Action (tool_use)" --> T
        T --> ENV
        ENV -- "Observation (tool result)" --> M
        M -. "context" .-> L
    end
    U --> L
```
...
sequenceDiagram
    ...
    participant T as Tool (read_file)
    ...
    T-->>A: 'UVM_ERROR @ 1500ns scoreboard.sv(142):<br/>expected=0xDEAD actual=0xBEEF'
    ...
    T-->>A: 'if (expected_data == actual_data) ...'
    A->>A: Observation: == 사용 (4-state X 처리 안 됨)
    A->>G: check

    Note over A: Iteration 3 (terminal)
    A->>A: Thought: '충분한 정보 수집. 결론 보고'
    A-->>U: '근본원인: scoreboard.sv:142 의 ==<br/>가 X 를 mismatch 로 처리 못 함. === 로 변경.<br/>수정: line 142, == → ===<br/>검증: mrun test --test_name <foo> --seed 0'

    Note over G: Guards (모든 iteration 체크)<br/>step_count ≤ max_steps (10)<br/>total_tokens ≤ budget (50K)<br/>같은 (tool, args) 반복 금지 (loop detection)
```
...
flowchart TB
    subgraph AG["Agent"]
        direction TB
        L["LLM (두뇌)<br/>추론, 계획, 의사결정"]
        subgraph CAPS["네 가지 능력"]
            direction LR
            C1["Tools"]
            C2["Planning"]
            C3["Memory"]
...
```mermaid
flowchart TB
    A["Actor: 행동 수행<br/>코드 생성 / 디버그 분석 / 테스트 작성"]
    E["Evaluator: 결과 평가<br/>컴파일 성공? 테스트 통과? 품질 충족?"]
    R["Self-Reflection: 실패 원인 분석<br/>'config_db 가 env 아닌 test 레벨 set'<br/>→ 교훈 메모리 저장"]
...
```mermaid
flowchart TB
    LEAD["Lead Agent<br/>전체 검증 전략 수립"]
    RA["RTL Analyzer<br/>DUT 인터페이스 / FSM 분석"]
    RT["Tool: RTL 파서, FSDB 리더"]
    TB["TB Generator<br/>UVM 컴포넌트 코드 생성"]
    TT["Tool: 코드 생성, 컴파일러"]
    DG["Debug Agent<br/>시뮬레이션 실패 분석"]
    DT["Tool: 로그 파서, 파형 분석기"]
    CG["Coverage Agent<br/>커버리지 갭 분석 / 테스트 생성"]
...
```mermaid
flowchart LR
    H["MCP Host<br/>(Claude Code 등)"]
    S["MCP Server<br/>(도구)"]
    H <-- "MCP Protocol<br/>JSON-RPC 2.0" --> S
```
...
```mermaid
flowchart TB
    IN["입력<br/>RTL 인터페이스 정의 변경"]
    S1["1. RTL 파서 (Tool)<br/>RTL 포트 변경 감지"]
    S2["2. 템플릿 엔진 (Tool)<br/>UVM Agent / Driver / Monitor 템플릿 적용"]
    S3["3. LLM<br/>포트별 Driver 로직 생성"]
    S4["4. 컴파일러 (Tool)<br/>생성 코드 컴파일 검증"]
    S5["5. LLM<br/>컴파일 에러 자동 수정"]
    OUT["6. 최종 결과물 출력"]
