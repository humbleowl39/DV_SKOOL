...
```
   질문 도착 (30 초 안에 답변 시작)
        │
        ▼
   카드 ① "Formal 강한 영역" 으로 점프
        │
        ▼
   골든 답변: "FSM, 프로토콜, FIFO, Arbiter, 보안 — 상태 공간 관리 가능"
        │
        ▼
   추가 보강: "Mapping Table IP 는 5 가지 property 로 검증" (이력서 연결)
```

### 시나리오 B — 코드 리뷰 중: "이 SVA 가 맞는가?"

```
   리뷰어 시야: assert property (req |-> ack);
        │
        ▼
   카드 ④ "함정 체크" 의 "1-cycle latency 설계인데 |-> 사용?" 항목
        │
        ▼
   확인: req → ack 는 1 cycle latency → |=> 또는 |-> ##1 이 맞음
        │
        ▼
   추가 확인: cover 짝 있는가? (vacuous 방지) — 카드 ④ 의 두 번째 항목
```

### 시나리오 C — 새벽 디버그: "전부 BOUNDED 만 나옴"

```
   상황: prove -all 결과가 모두 BOUNDED
        │
        ▼
   카드 ⑤ "Convergence" 순서로 점프
        │
        ▼
   1) Blackbox: 데이터 경로 모듈을 abstract -module
        │ (없으면 ↓)
   2) Abstraction: counter / data width 축소
        │ (없으면 ↓)
   3) Helper: 도달 불가능 상태 invariant 추가
        │ (없으면 ↓)
   4) Assume: 입력 환경 제약 (단, cover 짝 필수)
        │
        ▼
   대부분 1~3 단계에서 PROVEN 도달
```
