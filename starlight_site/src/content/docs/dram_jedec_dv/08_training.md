...
_왜 핀(lane)마다 따로 training 해야 하는가_ 는 PCB 배선의 물리에서 나옵니다. 여러 DQ lane 을 controller 에서 DRAM 까지 잇는 배선은 보드 위에서 완벽히 똑같은 길이로 깔 수 없습니다 — 패키지 ball 위치, 우회 경로, layer 전환 때문에 lane 마다 길이와 임피던스가 미세하게 다릅니다. 신호가 배선을 지나는 시간은 그 길이·임피던스에 비례하므로, 같은 순간 controller 가 내보낸 비트들이 DRAM 핀에는 lane 마다 _다른 시각_ 에 도착합니다(이 차이가 PCB skew). 게다가 고속에서는 이 skew 가 데이터 유효 윈도우(수십 ps)에 견줄 만큼 커집니다. 따라서 모든 lane 에 하나의 공통 delay 를 적용하면 어떤 lane 은 윈도우 중앙을, 다른 lane 은 가장자리를 찍게 됩니다. 각 lane 의 도착 시각이 개별적으로 다르므로, 최적 sampling delay 도 lane 마다 따로 측정해 맞춰야(per-pin/per-lane training) 모든 lane 이 동시에 자기 윈도우 중앙을 찍을 수 있는 것입니다.
...
> **이 챕터의 주축은 LPDDR5**입니다. LPDDR5는 **모든 JEDEC DRAM 중 training 단계가 가장 많습니다** — 그 핵심 이유는 데이터용 고속 클럭 `WCK`가 명령용 `CK`에서 분리되었기 때문입니다. 그래서 LPDDR5에만 있는 두 가지가 training 서사의 중심이 됩니다:
>
> 1. **WCK2CK Leveling** — WCK ↔ CK 정렬. *LPDDR5 고유* 단계이며, DVFSC로 gear(WCK:CK 비율)가 바뀔 때마다 다시 수행해야 합니다.
> 2. **CBT(Command Bus Training) Mode1 / Mode2** — LPDDR5의 CA[6:0]가 **단일종단(single-ended) 다중사이클**이라, DDR5의 차동 신호보다 정렬이 까다로워 두 단계의 CBT가 *필수*입니다.
>
> LPDDR5는 training 시 **내부(internal) Vref**를 사용합니다(외부 Vref 핀에 의존하지 않음). 아래에서는 LPDDR5 training을 기준 서사로 먼저 세우고(§5), 그보다 *단순한* DDR5 CA(CS)/DQS training(§3)을 **비교**로 제시합니다. DDR4(§2)는 역사적 기준점입니다.
...
_왜 0→1 전이점이 최적의 정렬점인가_ — 그 전이점이 바로 DQS 의 rising edge 가 CK 의 rising edge 와 _겹치는_ 순간이기 때문입니다. WL 동안 DRAM 은 매 CK rising edge 에 DQS 값을 sample 해 그대로 DQ 로 돌려줍니다. DQS delay 가 작아 DQS 가 CK 보다 _빠르면_, CK edge 가 올 때 DQS 는 아직 LOW 라 DRAM 은 '0' 을 보고합니다. delay 를 조금씩 늘려 DQS 를 점점 뒤로 밀다 보면, 어느 순간 DQS 의 rising edge 가 CK edge 와 정확히 만나는 지점을 지나며 sample 값이 '0' 에서 '1' 로 바뀝니다 — 이 0→1 전이점이 곧 "DQS edge = CK edge" 인 정렬 지점입니다. write 데이터는 이 정렬을 기준으로 전송되므로, 여기에 맞추면 DQS 가 데이터 비트 한가운데를 찍도록 정렬되어 setup/hold 마진이 양쪽으로 최대가 됩니다. 그래서 전이가 일어나는 첫 tap 을 최적 delay 로 채택하는 것입니다.
...
## 3. DDR5 Training (비교) — CA(CS)/DQS 중심, WCK 없는 단순한 흐름

> 출처: JESD79-5C.01 v1.31 §3.5.4 (MR3 DQS), §3.5.27~3.5.33 (MR25~MR31 Read Training)

LPDDR5(§5) 대비 **비교 기준**으로 DDR5 training을 봅니다. DDR5는 단일 CK + DQS 구조라 **WCK2CK Leveling 단계가 없고**, CA 버스도 차동·2-cycle이라 LPDDR5의 단일종단 CBT보다 정렬이 단순합니다. 핵심은 CS/CA Training과 DQS Training(MR3), 그리고 Read Training(MR25~MR31)·DCA·DFE입니다 — 단계 수는 LPDDR5보다 적습니다.
...
## 5. LPDDR5 Training (주축) — WCK2CK Leveling + CBT Mode1/2가 핵심

> 출처: JESD209-5C §4.2

이 챕터의 **핵심 절**입니다. LPDDR5의 *WCK* 클럭이 *CK와 분리*되었으므로, WCK 관련 training 단계가 *대거 추가*되어 **JEDEC DRAM 중 training 단계가 가장 많습니다**. LPDDR5만의 두 혁신을 먼저 짚습니다:

- **WCK2CK Leveling (§5.3)** — *LPDDR5 고유*. WCK를 CK에 정렬. DVFSC gear 전환 시 WCK:CK(2:1↔4:1)가 바뀌므로 재수행 필요. DDR5에는 대응 단계가 없습니다.
- **CBT Mode1 / Mode2 (§5.2)** — CA[6:0]가 *단일종단 다중사이클*이라 두 단계 CBT가 필수. Mode2는 Mode1보다 정밀한 보정을 수행합니다.

LPDDR5는 training 전 구간에서 **내부 Vref**를 사용합니다(외부 Vref 핀 비의존). 나머지 DQ/Write/Read training은 DDR5와 개념이 겹치지만, 위 두 단계가 LPDDR5 training 서사의 중심축입니다.

_왜 WCK 를 CK 에서 분리했는가_ 는 전력 대비 대역폭의 trade-off 에서 나옵니다. 클럭을 고속으로 토글시키는 것은 그 자체로 전력을 먹습니다 — 클럭 트리 전체의 capacitance 를 매 에지마다 충방전해야 하기 때문입니다. 그런데 메모리 동작을 뜯어 보면 _고속이 꼭 필요한 신호_ 와 _그렇지 않은 신호_ 가 나뉩니다. 명령·주소(CA)는 데이터에 비해 양이 적고 매 beat 마다 새 값을 쏟지 않으므로 **저속 클럭(CK)으로도 충분** 합니다. 반대로 대역폭을 직접 결정하는 것은 데이터(DQ)이므로 **데이터 클럭만 고속화** 하면 됩니다. CK 하나로 명령과 데이터를 모두 동기화하던 LPDDR4 방식은, 데이터를 빠르게 하려면 명령버스 클럭까지 덩달아 빨라져 불필요한 전력을 태웁니다. LPDDR5 는 CK(저속, 명령용)와 WCK(고속, 데이터용, CK의 2×/4×)를 분리해 — 명령버스는 느린 클럭으로 전력을 아끼고 데이터버스만 빠르게 돌려 대역폭을 얻는 — "필요한 곳에만 고속" 전략을 취한 것입니다. 그 대가로 두 독립 클럭의 위상을 맞추는 WCK2CK leveling 이라는 새 training 단계가 생겼습니다.
...
:::note[순서 유연성]
위 번호 순서는 *대표적 흐름*이며 절대 고정 순서가 아닙니다. spec은 필수 *의존 관계*(예: WCK2CK Leveling은 CBT 완료 후)만 규정하고, 그 제약을 어기지 않는 한 일부 단계의 순서·반복은 controller 구현 재량입니다(특히 DVFSC gear 전환 시 CBT/WCK2CK 일부를 재수행). 따라서 DV의 training FSM은 *허용된 순서 변형*을 reject하지 않도록, 고정 시퀀스가 아니라 *의존 관계 위반*을 기준으로 검증해야 합니다.
:::
...
### 5.3 WCK2CK Leveling (§4.2.5) — LPDDR5 고유 단계

이 단계는 **DDR5에 대응물이 없는 LPDDR5만의 training**입니다. WCK가 CK와 정확히 정렬되어야 *CAS WCK2CK Sync 비트*가 의미를 가짐. WCK 주파수는 CK의 2× 또는 4× (CKR/gear 설정에 따라). **DVFSC로 gear가 바뀌면 WCK:CK 비율이 변하므로 이 leveling을 다시 수행**해야 합니다:
...
- **LPDDR5 training이 주축** — JEDEC DRAM 중 단계가 가장 많음. 핵심 혁신은 *WCK2CK Leveling*(LPDDR5 고유, DDR5에 없음)과 *CBT Mode1/Mode2*(단일종단 CA[6:0] 정렬). LPDDR5는 *내부 Vref* 사용. DVFSC gear 전환 시 WCK2CK/CBT 일부 재수행.
- training 단계 순서는 *대표 흐름*일 뿐 — spec은 *의존 관계*만 강제하고 그 외 순서는 구현 재량. FSM은 고정 순서가 아니라 *의존 관계 위반*을 검증.
- DDR4 training (역사적 기준): Write Leveling + MPR-based DQ training + DQ Vref Training (3가지 핵심).
- DDR5 training (비교): 위 + *CS Training*, *Read Training Mode (MR25~MR31)*, *DCA Training*, *DFE Training*. 단일 CK + DQS라 WCK2CK 단계 없음 — LPDDR5보다 단순.
