...
여기서 **DV**(design verification, 설계가 스펙대로 동작하는지 시뮬레이션으로 검증하는 일)와 **DRAM**(dynamic RAM, 전하를 가둔 축전기로 1비트를 저장해 주기적 재충전이 필요한 휘발성 메모리)을 먼저 기억해 둡시다. DRAM 검증에서 발생하는 버그의 상당수는 **스펙의 corner 조항**(흔치 않은 조건에서만 적용되는 예외 규칙)에서 발생합니다. 요약본이나 애플리케이션 노트에는 공통 케이스만 기술되는 경우가 많고, 특정 기능을 활성화했을 때만 적용되는 타이밍 예외나 카운터 임계 조건은 스펙 원문의 주석(NOTE) 형태로만 남아 있을 때가 있습니다. 실제 예를 들면, DDR5(DDR의 5세대)의 `tCCD_L_WR2`는 일반적인 `tCCD_L`과 별도로 정의된 파라미터(`t`로 시작하는 이름은 두 동작 사이 최소 시간 간격 같은 타이밍 제약을 뜻함)인데 요약본에서 누락되기 쉽고, LPDDR5(저전력 DDR의 5세대)에서 `Write Link ECC`(데이터 전송선에서 생긴 오류를 정정하는 부호)가 활성화될 때 `tWR`(write recovery, 쓰기 후 안정화에 필요한 최소 시간) 적용 규칙이 달라지는 내용은 스펙 §9.2.1.2에만 명시되어 있습니다. DDR5 `RFM`(Refresh Management, 특정 row가 과도하게 열리는 것을 막는 보호용 명령) 명령은 `RAA Counter`(특정 row가 열린 횟수를 누적하는 카운터)가 임계치에 도달한 시점에 반드시 전송해야 하며 이를 어기면 **Rowhammer**(같은 row를 반복 접근하면 인접 row의 비트가 뒤집히는 취약점) 보호가 무력화됩니다.
