---
title: "Ch01. DRAM 기본 원리와 JEDEC 표준 지형도"
---
...
- 용어집 사전 학습: [DRAM, SDRAM, DDR, Bank, Row, Column](../appendix_b_glossary/)

## 1. 왜 DV 엔지니어가 스펙을 직접 읽어야 하는가

> "스펙 요약본은 빠르게 이해하기 좋지만, 검증 환경의 진실은 스펙 원문에 있다."

여기서 **DV**(design verification, 설계가 스펙대로 동작하는지 시뮬레이션으로 검증하는 일)와 **DRAM**(dynamic RAM, 전하를 가둔 축전기로 1비트를 저장해 주기적 재충전이 필요한 휘발성 메모리)을 먼저 기억해 둡시다. DRAM 검증에서 발생하는 버그의 상당수는 **스펙의 corner 조항**(흔치 않은 조건에서만 적용되는 예외 규칙)에서 발생합니다. 요약본이나 애플리케이션 노트에는 공통 케이스만 기술되는 경우가 많고, 특정 기능을 활성화했을 때만 적용되는 타이밍 예외나 카운터 임계 조건은 스펙 원문의 주석(NOTE) 형태로만 남아 있을 때가 있습니다. 실제 예를 들면, LPDDR5(저전력 DDR의 5세대)에서 `Write Link ECC`(데이터 전송선에서 생긴 오류를 정정하는 부호)가 활성화될 때 `tWR`(write recovery, 쓰기 후 안정화에 필요한 최소 시간) 적용 규칙이 달라지는 내용은 스펙 §9.2.1.2에만 명시되어 있고, LPDDR5의 WCK2CK 정렬 조건은 gear(WCK:CK) 설정에 따라 달라져 요약본에서 놓치기 쉽습니다. 비교 기준이 되는 DDR5(DDR의 5세대)에서도 `tCCD_L_WR2`는 일반적인 `tCCD_L`과 별도로 정의된 파라미터(`t`로 시작하는 이름은 두 동작 사이 최소 시간 간격 같은 타이밍 제약을 뜻함)인데 요약본에서 누락되기 쉽습니다. **Rowhammer**(같은 row를 반복 접근하면 인접 row의 비트가 뒤집히는 취약점) 보호의 경우 LPDDR5는 `ARFM/DRFM`, DDR5는 `RFM`(Refresh Management, 특정 row가 과도하게 열리는 것을 막는 보호용 명령) 명령으로 대응하며, 어느 쪽이든 카운터가 임계치에 도달한 시점에 반드시 대응 명령을 전송하지 않으면 보호가 무력화됩니다.

DV 엔지니어가 스펙 요약본만 보면 위와 같은 corner들을 놓치게 되고, **scoreboard**(DUT의 실제 출력과 기대값을 비교하는 검증 컴포넌트) · **SVA**(SystemVerilog Assertion, 신호가 지켜야 할 규칙을 코드로 적어 위반 시 자동 보고하는 검사문) · **coverage**(검증이 어떤 시나리오를 얼마나 건드렸는지 측정하는 지표)가 모두 통과하는데도 실제 silicon(완성된 실물 칩)에서 fail이 나는 최악의 상황이 발생합니다. 시뮬레이션에서 잡지 못한 버그는 검증 비용이 수십 배로 뛰기 때문에, 스펙 원문을 직접 확인하는 습관은 선택이 아닌 필수입니다.
...
DRAM cell(메모리의 1비트를 저장하는 최소 단위 회로)은 **1 transistor + 1 capacitor (1T1C)** 구조입니다. 단 하나의 **access transistor**(셀을 외부와 연결할지 끊을지 결정하는 스위치 역할의 트랜지스터)와 단 하나의 **storage capacitor**(전하를 담아 비트값을 저장하는 축전기)로 1비트를 저장하는 이 극단적 단순함이 **SRAM**(static RAM, 트랜지스터 6개로 비트를 붙들어 재충전이 필요 없지만 면적이 큰 메모리)(6T) 대비 셀 면적을 1/6 수준으로 줄여, GB급 집적을 가능하게 한 출발점입니다.
...
DRAM에 무언가를 읽고 쓰려면 항상 정해진 명령(command) 순서를 따릅니다. 표의 명령들을 먼저 풀어 두면 — **ACT**(activate, 한 row를 골라 row buffer로 끌어올려 "여는" 명령), **RD/WR**(read/write, 열린 row buffer에서 column 하나를 골라 읽거나 쓰는 명령), **PRE**(precharge, 열린 row를 닫고 다음 접근을 준비하는 명령), **REF**(refresh, 새어 나가는 전하를 다시 채워 데이터를 보존하는 재충전 명령)입니다. 여기서 **bank**(독립적으로 한 row를 열어 둘 수 있는 DRAM 내부 구획 — 여러 bank가 있으면 한 bank를 닫는 동안 다른 bank를 동시에 다룰 수 있음)는 명령이 작용하는 대상 구획을 가리킵니다.
...
JEDEC(Joint Electron Device Engineering Council)은 DRAM 표준의 사실상 단일 출처입니다. DV 엔지니어가 마주치는 주요 시리즈는 두 갈래입니다. 본 자료의 주축은 **JESD209 (LPDDR) 계열, 특히 모바일 SoC 메인 메모리인 LPDDR5/5X**이며, JESD79 (DDR5) 계열은 서버/PC 비교축으로 함께 봅니다. 아래에서는 먼저 메인스트림 DDR 계보를 비교 기준으로 정리한 뒤, 주축인 LPDDR 계열을 다룹니다.

### 3.1 JESD79 — 메인스트림 DDR (서버/데스크탑, 비교축)

아래 표의 **데이터 레이트**는 **MT/s**(mega-transfers per second, 초당 백만 번의 데이터 전송 — DDR은 클럭의 상승·하강 양 edge에서 전송하므로 클럭 주파수의 2배가 됨) 단위로 적습니다. 핵심 특징 칸에 처음 나오는 약어들을 풀어 두면: **ODT**(on-die termination, 신호 반사를 줄이려 칩 안에 내장한 종단 저항), **CRC**(cyclic redundancy check, 전송 데이터에 검사 비트를 붙여 오류를 탐지하는 방식), **CA Parity**(command/address 신호의 패리티 검사 — 명령·주소 전송 오류 탐지), **FGR**(fine granularity refresh, refresh를 잘게 쪼개 한 번에 멈추는 시간을 줄이는 방식), **Bank Group**(여러 bank를 묶은 상위 그룹 — 다른 그룹의 bank끼리는 더 짧은 간격으로 연속 접근 가능)입니다.
...
### 3.2 JESD209 — Low Power DDR (모바일/임베디드, 본 자료의 주축)
...
> **DV 시사점**: 동일한 "DDR" 이라 부르더라도, JESD79 vs JESD209는 *서로 다른 표준*입니다. 동일 vendor가 둘을 모두 만들고 controller IP도 둘을 모두 다루지만, 검증 환경은 *별도* 입니다. 본 자료는 **LPDDR5/5X (JESD209-5C) 검증을 주 대상**으로 삼고, DDR5는 동일 개념이 서버/PC 환경에서 어떻게 달라지는지를 보는 비교 기준으로 활용합니다.
...
표를 읽기 전에 칸에 나오는 용어들을 풀어 둡니다 — **DIMM**(dual in-line memory module, 여러 DRAM 칩을 한 기판에 얹어 메인보드 슬롯에 꽂는 메모리 모듈), **Vdd/Vddq**(칩 코어 전원전압 / DQ 입출력 전원전압), **Burst Length(BL)**(한 번의 RD/WR 명령으로 연속해 주고받는 데이터 묶음의 길이 — BL8이면 8연속), **MR**(mode register, 동작 모드·타이밍·기능을 설정하는 칩 내부 설정 레지스터), **PMIC**(power management IC, 전원전압을 모듈 위에서 직접 만들어 공급하는 전력관리 칩), **DFE**(decision feedback equalization, 고속 신호의 번짐을 보정하는 수신단 회로)입니다.
...
5. **DCA (Duty Cycle Adjuster)** — 고속 신호에서 clock/strobe(strobe는 데이터가 유효한 순간을 알려주는 기준 신호)의 **duty cycle**(한 주기에서 신호가 high인 시간의 비율, 이상적으로 50%)이 50%에서 벗어나면 **eye**(여러 신호 파형을 겹쳐 그렸을 때 가운데 비는 눈 모양 영역 — 넓을수록 0/1 판별 여유가 큼)가 비대칭해집니다. DCA는 이를 fine-tune하는 회로로, MR42~MR48과 MR103~MR254 영역에서 DQ 핀별로 설정합니다.
...
표의 새 약어들을 미리 풀어 둡니다 — **WCK**(write clock, 데이터 전용으로 분리한 빠른 클럭), **CBT**(command bus training, 명령·주소 신호의 타이밍을 칩과 맞추는 훈련 절차), **DVFS**(dynamic voltage frequency scaling, 부하에 따라 전압·주파수를 실시간으로 바꿔 전력을 아끼는 기법), **PASR**(partial array self refresh, 안 쓰는 메모리 영역의 refresh를 꺼 전력을 아끼는 기능), **PARC**(per-bank refresh 관련 절전 제어), **Self Refresh**(외부 명령 없이 칩 스스로 refresh를 수행하는 저전력 대기 모드)입니다.

| 항목 | LPDDR4 (JESD209-4E) | LPDDR5/5X (JESD209-5C) |
|---|---|---|
| 데이터 레이트 (typical) | 1600~4266 MT/s | 3200~9600 MT/s+ |
| Clock 구조 | 단일 CK 1:1 | **CK(저속 명령용) + WCK(고속 데이터용)** 분리, WCK:CK=2:1 또는 4:1 |
| Vddq | 1.1V (LPDDR4) / 0.6V (LPDDR4X) | **0.5V** (LPDDR5X에서 일부 0.3V) |
| Bank 구조 | BG 없는 8 banks | **MR로 모드 선택: BG mode(4 BG×4=16) / 8B mode(8) / 16B mode(16)** |
...
상용 메모리 컨트롤러 IP (예: Synopsys DesignWare, Cadence Denali, ARM CMN)는 *configurable*로 여러 스펙을 한 **RTL**(register-transfer level, 하드웨어의 동작을 레지스터·논리 단위로 기술한 설계 코드 — 검증 대상 그 자체)에서 지원합니다.
...
아래 표의 **TB**(testbench, DUT에 자극을 주고 응답을 검사하는 검증 환경)와 **VIP**(verification IP, 특정 프로토콜 검증을 위해 재사용 가능하게 만든 검증 컴포넌트 묶음), **agent**(한 인터페이스의 자극 생성·구동·관찰을 담당하는 UVM 검증 컴포넌트)를 염두에 두고 읽으면 됩니다. 여기서 **DUT**(design under test, 지금 검증하려는 대상 설계)는 보통 메모리 컨트롤러 RTL입니다.
...
- 모든 SystemVerilog 예제는 **SystemVerilog-2017 + UVM 1.2**(Universal Verification Methodology, 재사용 가능한 검증 환경을 만드는 SystemVerilog 표준 클래스 라이브러리) 기반
...
:::tip[Q1. 다음 controller IP 검증 상황을 보고, 어떤 JEDEC 스펙을 적용해야 하는지 분류하시오.]
...
- JEDEC 표준은 두 갈래: **JESD209 (LPDDR, 저전력 — 본 자료의 주축)** / **JESD79 (DDR, 메인스트림 — 비교축)**.
- 주축인 LPDDR4 → LPDDR5의 결정적 변화: WCK 분리(CK는 명령용, WCK는 데이터용), DVFS, Link ECC, ARFM/DRFM, per-pin DFE.
- 비교축인 DDR4 → DDR5의 결정적 변화: 2-cycle command, DFE, RFM, on-die ECC, MR 0~254 영역.
...
- 다음 챕터: [Ch02. 패키지·핀아웃·어드레싱](../02_package_pinout_addressing/)
