---
title: "Ch04. Mode Register 깊이 분석"
---
...
- [Ch03. 초기화·Reset·Power 시퀀스](../03_init_reset_power/)
...
:::note[이 챕터의 주축은 LPDDR5 — DDR5 MR 맵은 비교 대상]
본 교재는 **LPDDR5를 주축**으로 하고, DDR5(서버/PC), LPDDR4(직전 세대), DDR4(역사)를 비교축으로 둡니다. §2의 방대한 MR 맵은 *DDR5* 의 것이며, "MR을 카테고리로 빠르게 찾는 방법론"의 사례로 읽으십시오. **LPDDR5 MR의 성격**은 다음과 같습니다.

- **인코딩 원조**: MRA(MR 주소) + OP(payload)를 command bus에 packed하는 *LPDDR 스타일* 인코딩은 LPDDR이 먼저 썼고 DDR5가 따라왔습니다 (§8.1).
- **ODT가 단순**: LPDDR5는 DDR5의 RTT_NOM / RTT_WR / RTT_PARK 같은 다양한 ODT termination 세트를 갖지 않습니다(저전력·PoP 특성상 termination이 단순). DDR5 MR32~MR40의 풍부한 ODT 맵은 LPDDR5에 그대로 대응되지 않습니다.
- **LPDDR5 고유 제어 추가**: 대신 **WCK 관련 설정**(WCK:CK gear, WCK2CK), **DVFSC gear / FSP**(Frequency Set Point별 MR set), **bank mode 선택**(BG 모드 16뱅크 / 8B 모드 8뱅크 / 16B 모드 16뱅크)을 MR로 제어합니다. 이는 DDR5에 없는 축입니다.
- **CBT용 physical MR**: §6의 3 physical MR 구조도 LPDDR5 고유입니다.
:::
...
| MR Read | MPR-based (간접) | MRR (직접) | **MRR** (직접, 원조) | **MRR** (직접) |

> **MRR은 DDR5의 신기능이 아닙니다.** 직접 MR 읽기(MRR)는 **LPDDR4(2014)가 먼저 도입**했고, DDR5는 이를 **나중에(2020) 채택**했습니다. DDR4만 MPR 우회 방식이었고, LPDDR 계열은 처음부터 직접 MRR을 썼습니다. 즉 DDR5의 MR 접근 방식 자체가 *LPDDR 스타일을 따라온 것*입니다 (§8.1 스펙 원문 인용 참조).

DDR4는 MPR(Multi-Purpose Register)을 통해 우회해야 했는데, MR 내용을 DQ로 직접 읽어내는 경로가 없었기 때문입니다. LPDDR4/LPDDR5와 DDR5의 MRR 덕분에, controller는 현재 MR 값을 직접 readback하고 RAL(Register Abstraction Layer)의 mirror value와 비교 검증할 수 있습니다.
...
### 6.1 LPDDR5 MR이 제어하는 고유 축 (DV 관점)

LPDDR5에서 DV가 *DDR5와 다르게* 챙겨야 할 MR 제어 축은 다음입니다.

| 제어 축 | 내용 | DDR5 대비 |
|---|---|---|
| **WCK 설정** | WCK:CK gear(2:1 / 4:1), WCK2CK 관련 | DDR5엔 WCK 자체가 없음 |
| **DVFSC / FSP** | Frequency Set Point별 *별도 MR set* — gear 전환 시 전환 | DDR5엔 없음 |
| **Bank mode** | BG 모드(16뱅크) / 8B 모드(8뱅크) / 16B 모드(16뱅크) 선택 | DDR5는 고정 8 BG×4=32뱅크 |
| **ODT** | 단순 (RTT_NOM/WR/PARK 다양성 없음) | DDR5 MR32~40의 풍부한 ODT 맵 |

DV 시사점: LPDDR5 RAL은 *FSP별 MR shadow* 를 모델링해야 하며, FSP 전환 후 mirror가 해당 FSP set으로 스위칭되는지 검증해야 합니다. Bank mode MR은 *주소 디코딩 자체*를 바꾸므로 scoreboard의 address mapping과 직결됩니다.

---

## 7. 대표 문제 — DDR5 MR4 decode dry-run + RAL update

:::tip[Q. DDR5 controller가 MR4=8'b001_01_010 (OP[7:5]=001, OP[4:3]=01, OP[2:0]=010) 을 MRW 하려 한다. (1) 각 필드의 의미를 해석하고 (2) RAL을 통해 이 값을 write하는 코드를 작성하고 (3) 후속 RD/WR이 normal-temp range에서는 정상이지만 extended-temp range에서 timing violation이 생긴다면 어디를 의심해야 하는가?]
:::
<details>
<summary>풀이 (해석 + RAL 코드 + debug 사고)</summary>
...
_왜 DDR3/4 방식에서 LPDDR 방식으로 바꿨는가_ 는 핀 절감과 확장성 때문입니다. DDR3/4 의 전통적 방식은 MR 설정값을 _주소·뱅크 핀(BA/A 버스)_ 에 직접 실어 보냈습니다 — MR 종류와 값을 전용 address 핀 패턴으로 표현하는 구조라, MR 이 늘거나 payload 가 넓어질수록 더 많은 핀이 필요했습니다. 반면 LPDDR 방식은 MR 주소(MRA)와 payload(OP)를 모두 **command bus 인코딩 안에 packed** 합니다 — 정보를 별도 주소핀이 아니라 명령버스의 비트 패턴(여러 cycle 에 걸쳐)으로 담는 것입니다. DDR5 는 이미 2-cycle command 로 명령버스를 시간축으로 확장해 둔 상태이므로, 같은 인코딩 공간에 8-bit MRA + 8-bit OP 를 얹으면 _추가 핀 없이_ 최대 256개의 byte-wide MR 까지 깔끔히 확장됩니다. 즉 명령버스 packed 방식이 모바일에서 검증된 핀 효율·확장성을 DDR5 의 늘어난 MR 수요에 그대로 가져온 것입니다.
...
> 위 인용의 **UI**(unit interval, 한 비트가 DQ에 실려 있는 최소 시간 — DDR에서는 보통 half-tCK)는 burst의 한 beat 시간 단위를 가리킵니다.
...
> 여기서 **3DS**(3D stacked, 여러 DRAM die를 TSV로 수직 적층해 한 패키지로 만든 구성)와 **logical rank**(적층된 각 die를 별도의 rank처럼 다루는 논리 단위)를 알아 둡니다.
...
_왜 MRR/MRW 가 all-bank-idle 을 요구하는가_ — MR 변경은 DRAM 의 _내부 타이밍·구성_ 자체를 바꾸기 때문입니다. CL, BL, Vref, ODT, preamble 같은 설정은 read/write 동작이 어느 타이밍에 데이터를 내보내고 받을지를 결정하는 _전제_ 입니다. 만약 어떤 bank 에 row 가 열려 있거나 burst 가 진행 중인 상태에서 이 전제를 바꿔 버리면, 이미 그 전제 위에서 시작된 동작(예: 진행 중인 read 의 CL 카운트)과 새로 바뀐 설정이 어긋나 데이터가 깨집니다. 그래서 JEDEC 은 "모든 bank 가 idle(= 진행 중인 access 없음)" 일 때만 MR 을 건드리도록 강제해, 변경 시점에 _의존하는 진행 중 동작이 하나도 없도록_ 보장합니다. MR 변경과 활성 동작의 정합이 깨질 여지를 원천 차단하는 것입니다.
...
- DDR5의 MR은 *MR0~MR254*까지 확장. **인코딩 방식이 LPDDR style로 변경** — 8-bit MRA + 8-bit OP가 *command bus에 packed* (DDR5가 *LPDDR을 따라온 것*).
- **MRR(직접 MR 읽기)은 DDR5 신기능이 아님**: LPDDR4(2014)가 원조, DDR5는 후행(2020). DDR4만 MPR 우회.
- **LPDDR5(주축) MR 성격**: DDR5의 RTT_NOM/WR/PARK 같은 ODT 다양성은 *없고*, 대신 **WCK gear / DVFSC-FSP / bank-mode(16·8·BG)** 제어가 추가됨. CBT용 *3 physical MR*도 LPDDR5 고유.
...
- 이전: [Ch03. 초기화·Reset·Power](../03_init_reset_power/)
- 다음: [Ch05. Command·Truth Table·Burst Operation](../05_commands_burst/)
- 부록: [JEDEC Spec 빠른 참조](../appendix_a_quick_reference/)
- 퀴즈: [Ch04 퀴즈](../quiz/ch04_quiz/)
