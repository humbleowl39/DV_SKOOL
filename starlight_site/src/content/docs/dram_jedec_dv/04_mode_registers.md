...
_왜 DDR5 에서 MRR 을 직접 명령으로 만들었는가_ 는 DDR5 가 DRAM 으로부터 읽어야 할 정보가 폭발적으로 늘어난 데서 나옵니다. DDR4 시절 MR 은 대부분 host 가 _써넣고 끝나는_ 설정값이라, 굳이 되읽을 일이 적어 MPR 우회로도 충분했습니다. 그러나 DDR5 는 (1) on-die ECC 의 에러 통계(MR16~20), (2) RFM 의 RAA counter·thermal 상태(MR4 의 TUF), (3) per-DQ DFE·DCA training 결과 같이 **DRAM 내부에서 동적으로 갱신되는 상태값** 을 host 가 수시로 회수해야 합니다. 이런 readback 수요가 일상화되자 MPR 을 거치는 우회 경로는 느리고 번거로워, MR 주소를 지정해 곧바로 DQ 로 내용을 받는 전용 read 경로(MRR)를 명령 집합에 넣는 편이 합리적이 된 것입니다.
