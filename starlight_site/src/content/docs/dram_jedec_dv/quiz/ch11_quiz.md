---
title: "Ch11 퀴즈 — DV 프로젝트 End-to-End"
---
...
:::tip[Q1. LPDDR5 controller IP의 V-Plan에서 *가장 먼저* 정의해야 할 것은? `(Evaluate)`]
- A. UVM TB 코드
...
:::tip[Q2. UVM driver 가 LPDDR5 명령/데이터 발급 시 핵심 동작은? `(Understand)`]
- A. 명령과 데이터를 모두 단일 CK 도메인에서 1 cycle에 전송
- B. 명령은 저속 CK에 CA[6:0] 다중사이클로, 데이터는 고속 WCK 도메인으로 구동
- C. 데이터는 CK rising edge로만 전송
- D. CA만 전송, CS_n은 무관
:::
<details>
<summary>정답: B</summary>

**Why**: LPDDR5 driver는 명령/어드레스를 저속 **CK**(차동) 기준으로 CA[6:0]에 여러 cycle에 걸쳐 single-ended로 구동하고, 데이터는 별도의 고속 **WCK** 도메인으로 구동해야 합니다. 두 클럭이 WCK2CK leveling으로 정렬되어 있어야 데이터 타이밍 기준이 맞습니다. A(단일 CK 1 cycle)는 LPDDR5의 WCK/CK 분리 구조를 무시하므로 틀렸고, C(CK edge로 데이터 전송)는 데이터가 WCK 기준임을 무시합니다. D(CS_n 무관)는 CS_n이 명령 경계를 정의하므로 틀렸습니다. driver가 CA 다중사이클과 WCK 도메인을 정확히 구동하지 않으면 DRAM과 monitor가 명령/데이터를 잘못 해석해 후단 검증이 전부 무너집니다. (참고 — DDR5라면 CS_n 2 cycles 연속 LOW + 각 cycle별 CA로 2-cycle command를 구동) (Ch11 §11.3.2)

</details>
:::tip[Q3. Rowhammer scenario sequence의 *핵심 단계*는? `(Apply)`]
- A. Random WR 만 발급
...
:::tip[Q4. LPDDR5 환경을 DDR5에서 옮겨올 때(또는 LPDDR5 TB를 구성할 때) *추가/변경*되는 컴포넌트 설명으로 옳지 *않은* 것은? `(Remember)`]
- A. WCK_t/c 핀 추가 (데이터용 고속 클럭)
- B. CBT 시퀀스 + WCK2CK Leveling 추가
- C. *Bank Group이 항상 4개로 고정 축소*
- D. Link ECC encoding 모델 추가 (MR22)
:::
<details>
<summary>정답: C</summary>

**Why**: Bank Group이 "항상 4개로 고정 축소"된다는 C는 틀린 설명입니다. LPDDR5는 MR로 BG mode(4 BG×4=16뱅크)·8B mode(8뱅크)·16B mode(16뱅크) 중 하나를 선택하는 유연한 구조이지 고정 축소가 아닙니다. A(WCK_t/c 핀 추가)는 LPDDR5의 별도 데이터 클럭 도입에 따른 실제 변경이고, B(CBT + WCK2CK Leveling)는 LPDDR5 고유의 training 절차이며, D(Link ECC encoding 모델, MR22로 설정)도 LPDDR5에서 실제로 필요한 컴포넌트이므로 모두 옳습니다. (Ch11 §11.7)

</details>
## 단답형

:::tip[Q5. SVA `bind` 모듈을 사용해 protocol checker를 부착할 때 *parameter화*가 중요한 이유는? `(Apply)`]
:::
<details>
...
:::tip[Q6. *Coverage closure*가 *Tier 3 (directed)*에서만 가능한 *그러나 Tier 2*에서는 부족한 이유는? `(Analyze)`]
:::
<details>
<summary>예시 답안</summary>
...
:::tip[Q7. LPDDR5 controller IP 검증의 *Coverage Report* 결과:]

```
cmd_cg:        100.0%
timing_cg:      92.3%   HOLE: cp_gap.min_spec for tCCD_L
mr_cg:          88.5%   HOLE: cp_mr.link_ecc[MR22] enable states
refresh_cg:     75.0%   HOLE: cp_raa.near_threshold * cp_rfm.issued
training_cg:   100.0%
ecc_cg:         87.5%   HOLE: cp_err.double_bit_detected (Link ECC)
...
:::
<details>
<summary>풀이 (Coverage hole filling plan)</summary>
...
**Hole 2: `cp_mr.link_ecc[MR22] enable states`**

- Test: `test_link_ecc_mr22_walk`
- 시나리오: Init 후 MR22(Link ECC 설정)의 enable/disable 및 옵션 조합을 *다양하게 write* → MRR로 readback 확인
- RAL을 통해: `ral.MR22.set(8'h..); ral.MR22.update(status);` 로 각 enable state walk
- Sample point: `mr_walk_cg` / `link_ecc_cg` 에 각 enable state sample
- 함정: Link ECC enable 후 parity는 RDQS_t로 전달됨 — WCK/training이 완료된 뒤에만 의미 있는 데이터 검증 가능 — test order 주의
...
- Hole 2: 2 days (Link ECC enable state walk + WCK/training 의존성)
