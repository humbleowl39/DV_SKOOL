---
title: "Module 08 — Quick Reference Card"
---

:::tip[사용 목적]
이 페이지는 **참조용 치트시트** 입니다. 정독 자료가 아니라 _인터뷰 / 코드 리뷰 / 디버그_ 중에 빠르게 펴보는 용도. 새로 배우는 사람은 [Module 01-07](../01_architecture_and_phase/) 부터 보고 마지막에 이 페이지로 정리.
...
:::
:::note[사전 지식]
- [Module 01-07](../01_architecture_and_phase/) 학습 완료 후 이 카드를 보면 효과 극대화
:::
---
...
면접에서 5 분 안에 답해야 할 때, 코드 리뷰에서 _"이 부분 안티패턴인 것 같은데 어떤 거였더라"_ 의 순간, 디버그 중 _"raise/drop 의 정확한 패턴이 뭐였더라"_ 의 순간 — 이 한 페이지로 즉시 해결. 각 절은 Module 01-07 의 특정 섹션과 연결되어 있으므로, 한 줄 요약으로 답이 부족하면 연결된 본문으로 바로 이동하면 됩니다.
...
| 주제 | 핵심 포인트 | 자세히 |
|------|------------|------|
| 클래스 계층 | `uvm_object` (데이터) / `uvm_component` (인프라, Phase 있음) | [M01](../01_architecture_and_phase/) |
| Phase 순서 | build (Top→Down) → connect (Bot→Up) → run (병렬) → check/report | [M01](../01_architecture_and_phase/) |
| Agent | Driver + Monitor + Sequencer, Active/Passive | [M02](../02_agent_driver_monitor/) |
| Driver | Transaction → Pin-level 변환만. DUT 로직 금지 | [M02](../02_agent_driver_monitor/) |
| Monitor | Pin-level → Transaction 변환 + Analysis Port broadcast | [M02](../02_agent_driver_monitor/) |
| Sequence | 시나리오 로직. `start_item → randomize → finish_item` | [M03](../03_sequence_and_item/) |
| Virtual Seq | 멀티 Agent Sequence 조합 (시스템 시나리오) | [M03](../03_sequence_and_item/) |
| config_db | set/get 으로 설정 전달. Config Object 패턴 권장 | [M04](../04_config_db_factory/) |
| Factory | `type_id::create` + Override 로 코드 수정 없이 교체 | [M04](../04_config_db_factory/) |
| TLM | Analysis Port (1:N broadcast), seq_item_port (Driver↔Sqr) | [M05](../05_tlm_scoreboard_coverage/) |
| Scoreboard | Reference Model + 비교 + check_phase 잔여 확인 | [M05](../05_tlm_scoreboard_coverage/) |
| Coverage | Covergroup + Coverpoint + Cross. Closure = 검증 완전성 | [M05](../05_tlm_scoreboard_coverage/) |
| 실무 패턴 | 자주 쓰는 패턴 / Phase·config_db·Factory 안티패턴 | [M06](../06_practical_patterns/) |
| RAL | 이름 접근 + mirror, read/write·peek/poke·get/set·update·mirror, adapter, explicit prediction | [M07](../07_register_layer_ral/) |
...
아래 7가지는 "왜 그렇게 됐는가" 를 묻는 후속 질문에 연결되는 신호탄입니다. 한 줄 답 뒤에 Module 본문의 Design rationale 을 덧붙일 수 있어야 깊이가 보입니다.
...
:::caution[실무 주의점 — run_phase 와 sub-phase 혼용]
...
**실제**: 골든 룰은 _시작 신호_ 일 뿐. 면접관은 보통 _"왜 그렇게 됐는가"_ 를 묻고, 그 _왜_ 에 대한 깊이 (Module 01-07 의 design rationale) 가 답에 보여야 합니다. 룰은 _지표_, 깊이는 _자료_.<br>
...
**실제**: 치트시트는 _기억을 빠르게 끄집어내는 도구_ 이지 _학습 도구_ 가 아닙니다. Module 01-07 의 _why_ 가 머릿속에 없으면 새 상황에서 _어느 패턴을 꺼낼지_ 결정 못 함. 치트시트는 _index_, 본문은 _content_.<br>
...
- **이 카드는 _index_, Module 01-07 이 _content_** — 치트시트만 보고 환경 구축 시도 금지.
- **트리거 매핑 (§6)** 으로 어느 상황에 어느 절을 보면 되는지 즉시 결정.

### 7.1 자가 점검

:::tip[🤔 Q1 — 카드 trigger 매핑 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — 환경 구축 한계 (Bloom: Evaluate)]
...
:::caution[코스 마무리]
8 개 모듈을 마치셨습니다. 다음을 권장합니다:

1. **퀴즈 풀어보기** — 챕터별 5-8 문항으로 이해 점검: [퀴즈 인덱스](../quiz/)
2. **글로서리 스캔** — 모르는 용어 없는지: [용어집](../glossary/)
