...
|---|---|---|---|
| Factory Method / Abstract Factory | Creational | `type_id::create` + `uvm_factory` override | `oop_spec.md` §7 (factory = OCP) |
| Singleton | Creational | `uvm_factory`·`uvm_root`의 전역 유일 인스턴스 | (추론 — 전형적 UVM 구조) |
| Observer | Behavioral | `uvm_analysis_port`의 1:N broadcast | `oop_spec.md` §5.5, §7 (TLM = DIP) |
...
:::caution[근거 표기]
위 표에서 (추론)으로 표시한 행은 HDG 소스(`oop_spec.md`·`design_pattern_onboarding.md`)에 명시되지 않은, UVM 구조에 대한 일반적 패턴 해석입니다. factory=OCP와 TLM=DIP만 소스에 명시되어 있습니다 (`oop_spec.md` §7, §5.5).
:::
### 5.1 학습 순서 — 토대 → 분류 → 적용

패턴은 *토대 → 분류 → 적용* 순으로 익히는 것이 가장 자연스럽습니다. 이는 요리 비유 그대로입니다 — 도구와 사용법을 먼저 잡고 레시피로 넘어갑니다 (`design_pattern_onboarding.md` Steps).
...
- `design_pattern_onboarding.md` (Design Pattern — A GoF 23 Primer) — Overview, cooking analogy, Why use them, Classification(Creational/Structural/Behavioral 표), Steps
- `oop_spec.md` §5.5, §7 — TLM=DIP, factory=OCP (UVM 패턴 매핑 근거)
- Confluence: *Design Pattern* (refactoring.guru / GoF 기반)
