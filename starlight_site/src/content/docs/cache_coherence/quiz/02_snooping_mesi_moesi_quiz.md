...
모든 캐시가 공유 버스(또는 broadcast 인터커넥트)의 트랜잭션을 *엿듣습니다(snoop)*. 한 코어가 write 의도를 알리면(BusRdX/Upgrade), 같은 line 사본을 가진 나머지 캐시들이 그 신호에 반응해 자기 사본을 무효화(→ I)합니다. 그 결과 write 시점에 다른 쓰기 가능 사본이 남지 않아 "한 순간 한 writer"라는 SWMR이 자동으로 지켜집니다. 사서가 일일이 찾아다니지 않아도 모두가 버스를 듣고 있어 가능한 구조입니다 (출처: HDG §1.4).
