...
결과적으로 코어를 더 붙여도 coherence 트래픽이 대역폭을 잡아먹어 성능이 오히려 떨어집니다. 이 확장성 벽을 넘기 위해 등장한 것이 **directory**(어느 캐시가 어느 line을 가졌는지 적어 두는 장부)이며, 같은 일을 하는 **snoop filter**(불필요한 snoop을 걸러 내고 데이터를 가진 캐시에만 보내는 directory 구현)와 사실상 같은 개념입니다. directory는 "어느 캐시가 어느 line을 들고 있는가"를 *장부(ledger)* 로 추적해, 질문을 *해당 캐시에만* 보냅니다(**targeted snoop** — 사본을 가진 특정 캐시에만 보내는 snoop, 모두에게 뿌리는 broadcast의 반대). 여기서 **sharer**(특정 line의 사본을 가진 것으로 directory가 추적하는 상위 캐시)란 그 line을 들고 있는 캐시를 말합니다. 이것이 ARM ACE에서 CHI로, 그리고 **LLC**(Last Level Cache — 코어에서 가장 먼, 메모리 직전의 마지막 공유 캐시) 기반 directory로 진화한 핵심 동기입니다.
...
2. **그러나 broadcast는 코어 수에 따라 트래픽이 O(N)(코어 수 N에 정비례해 커진다는 뜻 — N이 2배면 트래픽도 2배)으로 폭발** → 모두에게 묻지 말고 *아는 캐시에만* 물어야.
...
trade-off의 본질은 "정보를 얼마나 정확히, 얼마의 비용으로 들고 있느냐"입니다. full bit-vector(코어마다 1비트씩 둬서 사본 보유 여부를 표시하는 방식)는 정확하지만 코어 수에 비례해 **SRAM**(캐시·directory에 쓰는 빠르지만 비싼 on-chip 메모리)을 먹고, limited pointer(sharer 몇 개만 포인터로 저장하는 방식)는 싸지만 sharer가 한도를 넘으면 broadcast로 fallback(예외 상황에서 단순하지만 비싼 방식으로 되돌아감)해 일시적으로 snooping의 단점을 되살립니다. coarse-grained는 그룹 단위로 묶어 절약하지만 그룹 내 일부 캐시에 불필요한 snoop이 갑니다. 어떤 표현을 쓰든 *정확성(빠뜨림 없음)* 은 절대 양보할 수 없고, *효율(over-snoop 최소)* 만 trade 대상입니다.
