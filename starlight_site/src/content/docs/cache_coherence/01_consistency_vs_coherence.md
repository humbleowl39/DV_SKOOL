...
두 코어가 하나의 물리 메모리를 공유하고, 각자 L1 캐시(코어 바로 옆에 붙은 가장 작고 빠른 1차 캐시)를 둡니다. Core A가 변수 `flag`를 1로 쓰는데, 그 쓰기가 A의 로컬 캐시에만 머무릅니다. 이때 Core B가 `flag`를 읽으면 자기 캐시(또는 **DRAM**(Dynamic RAM — 칩 밖에 있는 크고 느린 주 메모리, 모든 코어가 공유))에 남아 있는 옛날 값 0을 읽습니다. 여기서 **dirty**(캐시에 쓴 값이 아직 메모리에 반영되지 않아 캐시본이 메모리보다 최신인 상태)란 바로 A의 캐시처럼 "캐시에만 최신값이 있는" 상태를 말합니다.
...
**uniprocessor**(코어가 하나뿐인 단일 프로세서) 시절에는 이런 일이 없었습니다. 캐시도 **OoO**(out-of-order — 명령을 프로그램에 적힌 순서와 다르게 실행해 성능을 올리는 기법) 실행도 모두 "단일 스레드 입장에서 순차 실행처럼 보이도록" 설계되어, 그 illusion(겉으로는 순차로 보이게 만든 환상)이 깨지지 않았기 때문입니다. 그런데 1980~90년대에 여러 코어가 단일 주소 공간을 공유하는 **SMP**(Symmetric Multiprocessing — 여러 코어가 하나의 메모리를 대등하게 나눠 쓰는 구조)로 넘어오면서 이 illusion이 산산조각 났습니다. Core A의 갱신이 Core B에게 자동으로 전파되지 않으면 데이터가 조용히 오염됩니다.

이 문제를 하드웨어가 자동으로 막아 주는 메커니즘이 **Cache Coherence**(여러 캐시에 흩어진 한 주소의 사본을 자동으로 맞춰 stale 값을 못 읽게 하는 하드웨어 메커니즘)입니다. 그리고 "**barrier**(memory barrier/fence — "이 지점 이전 메모리 접근이 모두 끝난 뒤에 이후 접근을 진행하라"고 강제하는 순서 명령)를 실행하면 그 이후 다른 코어가 내 쓰기를 본다"처럼 프로그래머가 의존하는 *순서 규칙*은 **Memory Consistency**(로드/스토어가 어떤 순서로 보이는지를 규정하는, 프로그래머에게 공개된 계약)가 정의합니다. 이 둘을 섞으면 "coherence를 켰는데 왜 순서가 안 맞지?" 같은 잘못된 질문을 하게 됩니다.
...
- **Data-Value Invariant:** 새 epoch(한 주소가 특정 코어에 의해 읽기/쓰기되는 한 구간 — 소유자가 바뀌면 새 epoch이 시작됨) 시작 시점의 값은 직전 read-write epoch 종료 시점의 값과 일치해야 합니다. 즉 무효화 후 다시 읽으면 *반드시 최신값*을 받습니다.
...
3. **Multi-Core SoC & 이종(HSA) 시대:** CPU·**GPGPU**(General-Purpose GPU — 그래픽이 아닌 범용 연산에 쓰는 GPU)·**NPU**(신경망 연산 전용 가속기)가 한 칩에 → 단순 bus(여러 부품이 트랜잭션을 공유하는 공용 통신 선로) snooping을 넘어 **directory 기반**(누가 어느 사본을 가졌는지 장부로 추적하는 방식)과 인터커넥트 주도(ARM **ACE**(AXI Coherency Extensions — AMBA 버스에 coherence를 더한 확장), **CHI**(Coherent Hub Interface — ARM의 차세대 coherent 인터커넥트)) 구조로 진화.

### 4.3 HSA 예시 — consistency는 가정, coherence는 부담

CPU와 GPGPU가 같은 물리 메모리와 가상 주소 공간(**Shared Virtual Memory** — CPU와 가속기가 같은 포인터 값으로 같은 메모리를 가리킬 수 있게 한 공유 가상 주소 공간)을 공유하는 **HSA**(Heterogeneous System Architecture — CPU·GPU·NPU처럼 서로 다른 처리 요소가 한 메모리를 공유하는 이종 시스템 구조)에서 분리가 가장 선명해집니다.

- **프로그래머의 가정 (Consistency):** 과거엔 CPU→GPU 워크로드 핸드오프에 명시적 cache flush(캐시에 든 dirty 값을 메모리로 강제로 내보내는 동작) + **DMA**(Direct Memory Access — CPU를 거치지 않고 장치가 메모리를 직접 읽고 쓰는 방식) copy가 필요했습니다. HSA에서는 OpenCL/CUDA 코드가 consistency 모델에 의존해 단순히 가정합니다 — *"CPU가 행렬을 메모리에 쓰고 sync barrier를 실행하면, GPGPU는 그 포인터를 즉시 읽어 갱신된 값을 본다."*
- **하드웨어의 부담 (Coherence):** 그 가정이 깨지지 않도록, 인터커넥트(부품들을 잇는 통신망)가 무거운 일을 합니다. GPGPU가 행렬을 읽으려 하면 인터커넥트가 트랜잭션을 가로채 CPU의 L1/L2를 **snoop**(다른 캐시의 트랜잭션을 엿들어 자기 사본을 무효화/공급하는 동작)하고, dirty 사본이 있으면 그걸 추출해 *DRAM을 우회*하여 GPGPU로 직접 전달합니다. 소프트웨어는 이 복잡한 거래를 전혀 보지 못합니다.

---

## 5. 디테일 — 경계가 흐려지는 지점

consistency와 coherence는 개념적으로 분리되지만 구현에서는 한 팀으로 묶입니다. 예를 들어 **store buffer**(코어가 쓴 값을 캐시에 곧장 넣지 않고 잠시 모아 두는 작은 큐 — 쓰기 지연으로 stall을 줄임)는 consistency 모델(쓰기를 지연시켜 순서를 완화)에 영향을 주지만, 그 buffer가 drain(buffer에 쌓인 쓰기를 비워 캐시로 내보냄)되어 캐시에 반영될 때는 coherence 프로토콜이 SWMR을 강제합니다. 약한(weak) consistency 모델일수록 하드웨어는 더 공격적으로 재배치할 수 있고, 프로그래머는 더 많은 barrier로 순서를 *명시적으로* 요구해야 합니다. 강한(예: SC, sequential consistency) 모델일수록 하드웨어가 더 보수적으로 동작합니다.
...
검증 관점에서 이 경계는 중요합니다. coherence checker는 "단일 line의 사본이 SWMR/Data-Value invariant를 위반하는가"를 보고, consistency checker(예: **litmus test**(여러 코어가 동시에 메모리에 접근하는 아주 짧은 코드 조각으로, 메모리 모델이 허용하는 결과만 나오는지 시험하는 표준 테스트) 기반)는 "여러 주소에 걸친 로드/스토어 결과가 모델이 허용하는 outcome 집합 안에 있는가"를 봅니다. 둘은 서로 다른 reference model을 요구합니다 (추론).
...
| 증상 | 1차 의심 | 어디 보나 |
|---|---|---|
| 두 마스터(master — 버스에서 트랜잭션을 능동적으로 일으키는 주체, 예: CPU·DMA)가 같은 주소를 다른 값으로 읽음 | SWMR 위반 — 두 writer 동시 존재 | per-line state 추적, snoop 응답 로그 |
