---
title: "Module 01 — MMU 기본 개념 및 주소 변환"
---
...
:::
:::note[TLB 가 1 cycle 에 hit 하는 _물리적_ 이유 — CAM]
①의 TLB lookup 이 "1 cycle path" 로 끝나는 건 마법이 아니라 회로 구조 때문입니다. 일반 SRAM 은 주소를 주면 그 주소의 한 칸을 읽지만, TLB 는 반대로 _내용(ASID+VPN)을 주고 어느 칸이 그것과 같은지_ 를 찾아야 합니다. 이를 위해 fully-associative TLB 는 **CAM (Content-Addressable Memory)** 으로 만들어지는데, CAM 셀은 저장값과 검색값을 _모든 엔트리에서 동시에_ 병렬 비교하는 비교기를 칸마다 내장합니다. 그래서 entry 수가 몇 개든 검색이 한 번의 비교 동작으로 끝납니다.

대가는 면적과 전력입니다. 엔트리마다 비교 회로가 붙으므로 CAM 은 같은 용량 SRAM 보다 훨씬 비싸고, 동시에 켜지는 비교기 때문에 전력도 큽니다. 이것이 **micro-TLB(L1 TLB)를 수십 엔트리 규모로밖에 못 키우는 하드웨어 제약**입니다 — 더 키우면 1 cycle 에 못 끝내거나 전력이 폭발합니다. 그래서 큰 용량은 set-associative SRAM 기반 L2 TLB 로 따로 두는 2단 구조가 나옵니다(자세히는 [Module 03](../03_tlb/)).
:::
---
...
#### offset 이 변환되지 않는다는 사실의 진짜 함의 — VIPT 캐시 인덱싱

offset 이 변환을 거치지 않는다는 것은 단순한 편의가 아니라, L1 캐시 설계 전체를 떠받치는 메커니즘입니다. 이유를 따라가 보면 — 캐시는 주소를 받으면 _어느 set 을 볼지_ 정하는 index 비트와 _그 set 안에서 어느 way 인지_ 가리는 tag 비트로 주소를 쪼갭니다. 만약 index 비트가 변환되는 영역(VPN)에서 나온다면, 캐시는 TLB 변환이 _끝나야_ 비로소 어느 set 을 읽을지 알 수 있어 TLB 와 캐시가 직렬로 묶입니다.

그런데 offset 은 VA 든 PA 든 _값이 같습니다_. 따라서 캐시의 index 를 offset 비트 안에서만 뽑으면, 캐시는 VA 의 offset 으로 _지금 당장_ set 을 읽기 시작하면서 동시에 TLB 가 PA(=tag)를 변환하게 할 수 있습니다. 이것이 **VIPT (Virtually Indexed, Physically Tagged)** 이고, TLB lookup 과 L1 set 접근을 _병렬_ 로 돌려 변환 latency 를 캐시 접근 뒤에 숨기는 핵심 트릭입니다.

여기서 page 크기가 캐시 geometry 와 묶이는 근본 이유가 나옵니다. index + offset 이 한 page 안에 들어가려면 다음 부등식이 성립해야 합니다(추론적 정리이나 표준적 결과):

```
(L1 cache size) / (associativity) ≤ page size
즉  set 수 × line 크기 ≤ page 크기
```

이 한계를 넘으면 index 비트가 offset 영역을 벗어나 VPN(변환 대상)으로 침범하고, 그러면 같은 PA 가 서로 다른 VA index 로 두 곳에 캐싱되는 _aliasing_ 이 생깁니다. 그래서 4KB page + 64B line 환경에서 VIPT L1 을 키우려면 set 수가 아니라 _associativity_ 를 올려야 하는 제약이 따라옵니다 — page 크기를 16KB/64KB 로 키우면 같은 way 수로도 더 큰 L1 을 VIPT 로 유지할 수 있는 것도 이 부등식의 직접적 결과입니다.
...
(실제 ARMv8 의 AP/UXN/PXN/AF/NG 인코딩은 [Module 02 §5](../02_page_table_structure/) 에서 정밀하게 다룸)
...
이 indirection 이 _왜_ 존재하는지는 PTE 의 비트 예산을 따져 보면 분명해집니다. PTE 는 64 비트 안에 출력 주소, valid/table 구분, 권한(AP/UXN/PXN), AF, shareability, NS 까지 전부 욱여넣어야 하는 _극도로 빽빽한_ 워드입니다. 만약 8-bit attribute 인코딩을 _매 PTE 마다 통째로_ 박는다면 8 비트를 영구히 잡아먹습니다. 대신 자주 쓰는 메모리 타입은 시스템 전체에서 몇 종(Normal WB, Device-nGnRnE 등)뿐이라는 관찰을 이용해, 8 종을 MAIR 에 한 번만 정의해 두고 PTE 에는 그중 하나를 가리키는 **3-bit index** 만 둡니다. 8 비트를 3 비트로 줄여 PTE 당 5 비트를 회수하는 것 — 이것이 indirection 의 설계 동기입니다. CPU 의 register-number 가 register 내용을 직접 담지 않고 _번호_ 만 담는 것과 같은 절약 기법입니다.
...
_왜_ identity mapping 이 선택이 아니라 필수인지는 실행의 연속성 관점에서 봐야 합니다. MMU 를 켜는 그 명령어 자체도 어딘가의 주소에서 fetch 되어 실행 중이고, 그 다음 명령어의 주소(PC+4 등)는 이미 SCTLR write 이전에 _VA 기준으로_ 계산되어 fetch 파이프라인에 in-flight 상태로 들어가 있습니다. enable 이 발효되는 순간, 이 in-flight VA 들이 갑자기 변환 대상이 됩니다. 만약 그 VA 가 가리키는 곳의 매핑이 _VA == PA_ 가 아니라면, enable 직전엔 PA=X 에서 멀쩡히 돌던 코드 흐름이 enable 직후엔 PA=Y(전혀 다른 곳)로 점프해 버립니다. 즉 identity mapping 의 역할은 "보안" 이 아니라 **enable 경계를 가로지르는 실행 흐름이 같은 물리 위치에서 끊김 없이 이어지도록 보장하는 다리** 입니다. 이 다리를 깐 뒤 흐름이 정상 VA 공간으로 점프(예: kernel 의 high-half 주소로)하고 나면, 부트용 identity 매핑은 나중에 회수해도 됩니다.
...
:::caution[실무 주의점 — MMU Enable 직후 ISB 누락]
...
:::
:::tip[🤔 Q2 — TLB 가 캐시하는 것 (Bloom: Evaluate)]
...
- 📝 [**Module 01 퀴즈**](../quiz/01_mmu_fundamentals_quiz/)
- ➡️ [**Module 02 — Page Table Structure**](../02_page_table_structure/): 4-level walk 의 비트 분할, PTE descriptor format, granule trade-off, PWC 까지.
