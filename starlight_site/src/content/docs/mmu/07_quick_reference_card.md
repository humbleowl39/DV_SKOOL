...
:::
:::note[사전 지식]
- [Module 01 — MMU 기본](../01_mmu_fundamentals/) ~ [Module 06 — DV 방법론](../06_mmu_dv_methodology/) 완주 권장
...
:::caution[실무 주의점 — Identity Mapping 범위 부족으로 MMU Enable 후 즉시 Fault]
...
면접에서 MMU 관련 질문을 받으면 두 가지를 항상 갖추어야 합니다. 하나는 기능적 설명(무엇을, 어떻게), 다른 하나는 정량적 수치(왜 중요한가). 아래 12 개 룰은 그 두 요소를 짝지은 것입니다. 각 항목은 단독 답변으로도 쓸 수 있고, "이 외에 알고 계신 것이 있나요?" 라는 follow-up 에 대한 확장 포인트로도 쓸 수 있습니다.
...
아래 공식의 `T_eff` 는 **EAT**(Effective Access Time, 실효 접근 시간 — hit 와 miss 를 비율로 가중 평균한 평균 변환 시간)이고, `Hit_Rate` 는 적중률, `T_miss` 는 miss 1회당 추가 시간입니다.
...
면접에서 가장 자주 보이는 패턴은 절반만 맞는 답변입니다. "MMU 는 주소 변환을 한다" 는 틀리지 않지만 권한 검사와 캐시 속성 제어가 빠졌고, "TLB 를 크게 하면 성능이 좋아진다" 는 lookup latency 증가라는 부작용을 무시합니다. 아래 표의 왼쪽 열이 자기 입에서 나오는 순간 오른쪽 열로 즉시 교정할 수 있어야 합니다.
...
:::
:::tip[🤔 Q2 — 카드 사용 trigger (Bloom: Evaluate)]
...
1. **퀴즈 풀어보기** — [퀴즈 인덱스](../quiz/)
2. **글로서리 점검** — [용어집](../glossary/)
