---
title: "Module 05 — Performance Analysis"
---
...
:::
:::note[사전 지식]
- [Module 01-04](../01_mmu_fundamentals/)
...
:::note[bimodal 분포가 평균을 무의미하게 만드는 _통계적_ 근거]
평균과 표준편차가 분포를 잘 요약하는 것은 데이터가 _하나의 봉우리_ 를 가진 단봉(unimodal) 분포일 때입니다. 그런데 MMU latency 는 hit(~1 cycle)과 miss(~수백 cycle)라는 **두 개의 봉우리** 를 가진 bimodal 분포이고, 두 모드의 위치가 _수백 배_ 떨어져 있습니다. 이 경우 평균은 두 봉우리 _사이의 빈 골짜기_ 어딘가를 가리키게 되어, **실제로는 거의 아무 translation 도 그 평균 근처 값을 갖지 않습니다** — 평균이 "전형적인 값" 을 대표하지 못합니다.

표준편차도 무력해집니다. 표준편차는 _대칭적이고 봉우리 하나_ 인 산포를 가정한 척도인데, bimodal 에서는 분산이 "평균에서 얼마나 흩어졌나" 가 아니라 "두 봉우리가 얼마나 멀리 떨어졌나" 를 반영해 버립니다. 게다가 miss 라는 소수의 극단값이 분산에 _제곱_ 으로 기여하므로(편차의 제곱 합), 분산은 miss 비율의 작은 변화에도 요동쳐 안정적인 요약이 되지 못합니다.

그래서 분포의 모양을 가정하지 않는 **순위 기반 통계(percentile)** 가 필요합니다. P99 는 "99% 의 요청이 이 값 이하" 라는 _사실 그 자체_ 를 보고하므로, 분포가 몇 개의 봉우리를 갖든 꼬리가 어디서 시작하는지를 왜곡 없이 드러냅니다 — 이것이 SLA 를 평균이 아니라 percentile 로 정의하는 통계적 이유입니다.
:::
...
:::note[Ideal Model 의 "이론 최적 교체" 가 무엇인가 — Bélády(OPT)]
Ideal Model 의 miss ratio 0.5% 를 "이론 최적" 이라 부를 때, 그 기준은 막연한 이상이 아니라 **Bélády 의 OPT(또는 MIN) 교체 알고리즘** 입니다. OPT 는 "교체해야 할 때, _앞으로 가장 오랫동안 다시 쓰이지 않을_ 엔트리를 버린다" 는 규칙이고, 어떤 교체 정책도 이보다 낮은 miss ratio 를 낼 수 없다는 것이 증명되어 있습니다 — 즉 OPT 는 _도달 가능한 하한(lower bound)_ 입니다.

그런데 OPT 는 **미래의 접근 순서를 미리 알아야** 결정을 내립니다. 실제 하드웨어(LRU, PLRU, random)는 미래를 볼 수 없으므로 OPT 를 _구현할 수 없습니다_ — OPT 는 오프라인에서 trace 전체를 본 뒤에야 계산되는 이론적 기준선입니다. 이 사실이 검증에 주는 의미가 중요합니다: **DUT 가 Ideal(OPT 기준) 보다 miss 가 많은 것은 버그가 아니라 정상** 입니다. DUT 는 미래를 모르는 현실 정책을 쓰기 때문입니다. 따라서 Dual-Reference 비교에서 봐야 할 것은 "gap 이 0인가"(불가능)가 아니라 **"gap 이 같은 정책의 합리적 범위 안인가, 혹은 정책 결함을 시사할 만큼 비정상적으로 큰가"** 입니다. OPT 와의 거리는 DUT 정책이 _얼마나 개선 여지가 있는지_ 의 척도로 읽어야 합니다.
:::
...
| TLB 모델 | 있음 (DUT와 동일 크기/정책) | 무한 TLB (Miss = 0) 또는 이론 최적(Bélády OPT) |
...
#### 왜 utilization 0.95 부근에서 latency 가 발산하는가 — 큐잉의 근본

위 오해 5 의 `latency = 1/(μ-λ)` 공식이 _왜_ 그렇게 행동하는지는 큐잉 이론으로 풀어야 보입니다. 핵심 도구는 **Little's Law** 입니다 — 시스템에 머무는 평균 요청 수 `L`, 도착률 `λ`, 평균 체류 시간 `W` 사이에 **L = λ × W** 가 항상 성립합니다. 이것은 throughput(λ)과 latency(W)가 _독립이 아니라 큐 길이 L 을 통해 묶여 있다_ 는 것을 말해 줍니다.

이제 utilization `ρ = λ/μ` (도착률 ÷ 처리율)를 생각하면, walk engine 같은 단일 서버 큐에서 대기 시간은 대략 `W ∝ 1/(1-ρ)` 로 늘어납니다. ρ 가 0.5 → 0.9 → 0.95 → 0.99 로 갈 때 `1/(1-ρ)` 는 2 → 10 → 20 → 100 으로 _기하급수적_ 으로 폭증합니다. 직관적 이유는 이렇습니다 — 도착이 무작위(burst 섞임)라서, 서버가 거의 항상 바쁜(ρ→1) 상태에서는 새 요청이 도착할 때마다 _앞선 요청들의 잔여 처리 시간_ 을 통째로 기다려야 하고, 그 줄이 비워질 틈이 점점 사라집니다. ρ=1 에 닿으면 줄이 _이론상 무한_ 으로 자라 latency 가 발산합니다.

이것이 검증에 주는 통찰은 결정적입니다 — **tail latency 의 원천은 capacity miss(TLB 가 작아서) 만이 아니라 _큐 포화_ 도 있다** 는 점입니다. miss ratio 가 spec 안이어도, walk engine 이나 memory port 의 utilization 이 0.9 를 넘는 워크로드에서는 큐 대기만으로 P99 가 폭발할 수 있습니다. 그래서 성능 디버그 시 "miss 가 늘었나" 와 "어떤 자원의 utilization 이 1 에 가까워졌나" 를 _분리해서_ 봐야 하며, 후자라면 TLB 를 키워도 소용이 없고 _서버(walk port/대역폭)를 늘리거나 도착을 평활화_ 해야 합니다.
...
:::caution[실무 주의점 — ASID 고갈 시 전체 TLB Flush로 성능 절벽]
...
:::tip[🤔 Q1 — P99 vs P50 측정 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — TLB miss penalty 분해 (Bloom: Evaluate)]
...
- 📝 [**Module 05 퀴즈**](../quiz/05_performance_analysis_quiz/)
- ➡️ [**Module 06 — MMU DV Methodology**](../06_mmu_dv_methodology/): 지금까지의 _측정_ 을 _체계적인 검증 환경_ 으로 — Custom Thin VIP, SVA bind, Constrained Random.
