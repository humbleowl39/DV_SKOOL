---
title: "Module 04 — Quick Reference Card"
---
...
:::
:::note[사전 지식]
- [Module 01-03](../01_soc_top_integration/) — 본문 학습 후 사용
:::
---
...
SoC Top 에서 오류가 발생했을 때 무작위로 탐색하면 cascading 에러에 휩쓸립니다. 다음 순서를 따르면 원인을 체계적으로 좁힐 수 있습니다.

먼저 **FIRST error 를 확인** 합니다 — 시간순으로 가장 먼저 나타난 에러에 집중하고 그 이후 에러는 cascading 결과로 보류합니다. 그 다음 에러 위치가 **IP 내부인지 연결 지점인지** 를 판별합니다. IP 내부라면 IP-level DV 의 회귀 문제이고, 연결 지점이라면 통합 버그입니다. 통합 버그로 판별되면 **해당 연결의 RTL** (`soc_top.sv` 의 포트 매핑) 을 직접 확인합니다. 이어서 **스펙 (IP-XACT / Config JSON) 과 실제 RTL 을 비교** 해 어느 쪽이 틀렸는지 확인합니다. 원인을 찾았다면 수정 후 반드시 **SVA 를 추가** 해 동일 결함의 재발을 방지합니다.
...
면접에서 SoC Top 검증 경험을 설명할 때 다음 일곱 가지 논리 구조를 준비해 두면 어떤 질문이 와도 30 초 안에 핵심을 전달할 수 있습니다.

**Top vs IP** 에 대한 질문이 오면 "IP 는 부품 정상성을, Top 은 조립 정상성을 검증한다 — 통합 버그는 IP 검증 범위 밖이다" 로 시작합니다. 연결 정보가 IP TB 안에 없다는 구조적 이유를 덧붙이면 설득력이 높아집니다.

**CCTV 방법론** 을 물으면 "293 개, 216 개, 96.30%" 라는 세 숫자로 임팩트를 먼저 제시하고 방법론을 설명합니다. 숫자가 선행되면 면접관의 집중도가 높아집니다.

**소규모 역설** 은 반직관적 인사이트이므로 강조할 가치가 있습니다. "소규모 프로젝트의 Gap Rate 가 더 높다 — 엔지니어 수가 적어 교차 검증이 이루어지지 않기 때문이다" 를 정량 데이터 (4.99% vs 2.75%) 와 함께 제시합니다.

**IP-XACT 한계** 를 묻는다면 "구조적 정보만 있고 시맨틱이 없다 — Hybrid Extraction 이 차별점이다" 로 답합니다. "AXI Master 포트가 있다 → sysMMU 검증이 필요하다" 의 판단 간극을 설명하면 완성됩니다.

**TB Top 재사용** 에 대해서는 "Config (JSON) 기반 자동 구성 덕분에 프로젝트마다 TB 를 처음부터 재작성하지 않아도 된다" 가 핵심입니다. Layer 1 (Config) 만 교체하면 Layer 2/3 가 그대로 동작한다는 구조를 보여줍니다.

**Formal + Simulation Hybrid** 는 "Formal 은 구조적 연결 완전성을, Simulation 은 동적 동작 정확성을 담당한다 — 둘 중 하나만으로는 부족하다" 로 정리합니다.

**sysMMU 핵심 시나리오** 는 "Bypass → Enable 전환 중 진행 중인 트랜잭션이 보호되는지 여부 — 이것이 누락되면 간헐적 Silicon 버그의 원흉이 된다" 로 마무리합니다.
...
:::caution[실무 주의점 — 주소 디코더 범위 중첩 무음 응답]
...
:::
:::tip[🤔 Q2 — Common Task 의 가치 (Bloom: Evaluate)]
...
[퀴즈](../quiz/) · [용어집](../glossary/) · 다음: [UVM](../../uvm/), [AI Engineering](../../ai_engineering/).
