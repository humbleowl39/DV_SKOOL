---
title: "Module 01 — SoC Top Integration"
---
...
먼저 용어부터 풀어둡니다. **SoC**(System-on-Chip — CPU·GPU·메모리 컨트롤러·여러 주변 장치를 하나의 칩에 통합한 것), **IP**(Intellectual Property block — SoC 를 구성하는 재사용 가능한 설계 블록 하나, 예: CPU·DMA·카메라 처리기), **DV**(Design Verification — 설계가 의도대로 동작하는지 검증하는 작업), **integration**(여러 IP 를 한 칩으로 연결·조립하는 단계)을 다룹니다. **tape-out**(설계를 공장에 넘겨 실제 칩을 찍어내기 시작하는 시점), **silicon**(그렇게 만들어진 실제 칩)이라는 말도 곧 나옵니다.

SoC 출시 직전, 모든 IP 의 IP-level DV(개별 IP 하나만 따로 검증하는 단계)가 통과하고 통합 후 system 시뮬레이션도 통과했습니다. Tape-out 후 6 주가 지나 silicon 이 도착해 부팅을 시도했을 때 **fail** 이 떨어졌습니다. 원인을 추적해 보니 IP A 의 _interrupt_(어떤 IP 가 "처리해 달라"고 CPU 에 보내는 비동기 알림 신호)가 _wrong IRQ controller port_(IRQ controller = 여러 IP 의 인터럽트를 모아 CPU 로 분배하는 장치; 그 입력 포트가 잘못 연결됨)에 연결된 **1 줄 connectivity bug**(connectivity = IP 들 사이의 신호 배선 연결 — 그 한 줄이 틀림) 였습니다. IP 자체는 정상적으로 IRQ 를 발생시켰기 때문에 IP-level DV 는 이 오류를 발견할 수 없었습니다 — 연결 정보는 IP 단독 테스트벤치 안에 존재하지 않기 때문입니다.
...
**SoC Top-level DV** = 건물들이 _도시로 조립된 뒤_ 의 준공 검사 — 도로(interconnect — IP 들을 잇는 버스/배선 네트워크)가 끊기지 않았는지, 지번(memory map — 각 IP 가 차지하는 주소 영역의 배치도)이 맞는지, 119 신고망(interrupt)이 올바른 소방서로 가는지, 정전(power domain off — 칩을 전원 영역으로 나눠 일부만 끄는 것) 시 옆 건물에 X 전류가 흘러들어가지 않는지.
:::
...
CCTV / 영상 SoC 의 가장 단순한 시나리오. **CMOS image sensor**(빛을 디지털 픽셀로 바꾸는 카메라 센서)가 한 장의 **1920 × 1080 RAW10**(가로 1920 × 세로 1080 픽셀, 픽셀당 10비트의 가공 전 원본 영상) frame(영상 한 장)을 찍어 → **ISP**(Image Signal Processor — 센서의 거친 원본 픽셀을 보기 좋은 컬러 영상으로 다듬는 전용 처리기)가 RGB(빨강·초록·파랑 색 표현)로 변환 → **Video codec**(영상을 압축/복원하는 IP) (**H.264 encoder** — 널리 쓰이는 영상 압축 표준 H.264 로 인코딩=압축하는 장치) 이 압축 → **DDR**(흔히 쓰는 외부 메인 메모리, DRAM 의 한 종류)에 **bitstream**(압축된 영상 비트 데이터의 연속 흐름) 적재 → Display IP 가 preview 로 다시 읽어 LCD 로 출력. 이 데이터 흐름의 통로로 **MIPI CSI-2**(카메라 센서와 칩을 잇는 표준 인터페이스)와 **AXI**(AMBA AXI — ARM 이 만든, 칩 내부 IP 들이 데이터를 주고받는 대표적 버스 프로토콜)가 등장합니다. 이 한 frame 의 여행에서 **IP 간 연결 / 라우팅 / 도메인 경계가 어떻게 동시에 검증되는지** 따라가 봅니다.
...
아래 표에 나오는 약어를 먼저 풀어둡니다. **GIC**(Generic Interrupt Controller — ARM SoC 의 인터럽트 컨트롤러, 위에서 말한 IRQ controller 의 구체적 구현), **SPI**(여기서는 Shared Peripheral Interrupt — 여러 IP 가 공유하는 인터럽트 입력 번호; SPI[12]=12번 입력선), **MC**(Memory Controller — DDR 메모리에 대한 읽기/쓰기를 관리하는 IP), **AXI-S**(AXI-Stream — 주소 없이 데이터를 한 줄로 흘려보내는 AXI 변형, 영상 픽셀 스트림용), **AXI-MM**(AXI Memory-Mapped — 주소를 지정해 메모리처럼 읽고 쓰는 일반 AXI), **sysMMU**(System Memory Management Unit — CPU 외의 IP 가 메모리에 접근할 때 가상 주소를 실제 주소로 바꿔주고 영역을 격리하는 장치), **frame buffer**(완성된 영상 한 장을 담아 두는 메모리 영역), **MIPI DSI**(칩과 디스플레이 패널을 잇는 표준 인터페이스), **CDC**(Clock Domain Crossing — 주파수가 다른 두 클럭 영역 사이로 신호가 건너갈 때 생기는 동기화 문제), **PIXCLK/AXICLK**(각각 픽셀 처리용·버스 데이터용 클럭).
...
| ⑤ | Display IP | DDR 에서 frame buffer read | display 의 read 경로가 codec write 경로와 _coherent_(coherency=일관성; 한쪽이 쓴 최신 값을 다른 쪽이 그대로 보는 것) 한 view |
...
**Formal**(Formal Verification — 시뮬레이션처럼 값을 넣어 보는 대신, 수학적으로 "모든 경우에 성질이 성립함"을 증명하는 검증 기법) **(JasperGold Connectivity)**(JasperGold = Cadence 의 대표적 formal 도구) 은 연결 스펙 (CSV/JSON) 에서 **property**(설계가 항상 지켜야 하는 성질을 형식적으로 적은 것) 를 자동 생성해 "IP_A.data_out 이 IP_B.data_in 에 연결됨" 을 exhaustive(빠짐없이 전수) 하게 증명합니다. 입력 조합을 모두 탐색하므로 시뮬레이션이 미처 닿지 못한 경우도 커버합니다.

**Simulation**(설계에 실제 자극을 넣고 출력을 관찰해 검증하는 방식) 은 IP_A 에서 특정 패턴을 출력하고 IP_B 에서 동일 패턴이 수신되는지를 동적으로 확인합니다. 타이밍·핸드셰이크처럼 Formal 로 표현하기 어려운 동작을 보완합니다.

**DFT Scan**(DFT = Design For Test, 테스트하기 쉽게 넣어 둔 회로) 은 scan chain(칩 안의 플립플롭들을 한 줄로 꿰어 값을 밀어 넣고 빼낼 수 있게 한 사슬)을 통해 신호 값을 직접 관찰합니다. 접근하기 어려운 내부 노드를 검사할 때 사용합니다.

검증 대상으로는 데이터 버스 (AXI / **AHB**(AMBA 의 구형 고속 버스) / **APB**(AMBA 의 단순·저속 버스, 주로 레지스터 설정용)), 인터럽트 (IP → GIC), DMA(Direct Memory Access — CPU 를 거치지 않고 IP 가 직접 메모리를 옮기는 방식) 요청 (IP → **DMAC**(DMA Controller — 그 DMA 전송을 수행하는 IP)), Clock/Reset 트리, Power 스위치 제어 신호가 포함됩니다.
...
이 배치가 실제로 옳은지 확인하려면 네 가지를 검증해야 합니다. 먼저 각 IP 의 Base Address(그 IP 가 차지하는 주소 영역의 시작 주소)에 실제로 접근했을 때 정상 응답 (**OKAY** — AXI 버스에서 "요청을 정상 처리함"을 뜻하는 응답 코드) 이 돌아오는지 확인합니다. 그 다음 할당되지 않은 주소에 접근하면 **DECERR**(Decode Error — "그 주소에 해당하는 IP 가 없다"는 버스 에러 응답)가 반환되는지 봅니다. 범위 중첩이 없는지도 확인해야 합니다 — 두 IP 의 주소 범위가 겹치면 DECERR 대신 OKAY 가 돌아오되 엉뚱한 IP 에 쓰이는 silent corruption(에러 없이 조용히 잘못된 곳에 써져 데이터가 오염되는 것)이 발생하기 때문입니다. 마지막으로 **IP-XACT**(IEEE 1685 — IP 의 주소·포트·인터럽트 등을 기계가 읽을 수 있는 XML 로 적은 표준 메타데이터 형식) 메타데이터와 실제 RTL(Register Transfer Level — 하드웨어 설계를 기술한 코드, 곧 실제 회로) 의 주소 디코더(들어온 주소를 보고 어느 IP 로 보낼지 고르는 회로)를 비교하여 문서와 구현이 일치하는지 확인합니다.

#### 3. Clock/Reset Verification

Clock/Reset 검증의 핵심 질문은 "각 IP 에 올바른 클럭이 공급되고, reset 이 의존성 순서에 따라 해제되는가?" 입니다.

Clock 관점에서는 각 IP 가 요구하는 주파수를 실제로 받고 있는지, idle(놀고 있는 상태) 시 clock gating(안 쓰는 회로의 클럭을 멈춰 전력을 아끼는 기법)이 정상 동작하는지를 확인합니다. Reset(회로를 초기 상태로 되돌리는 신호) 관점에서는 해제 순서가 핵심입니다. 다음 순서가 지켜져야 합니다.
...
여기서 **PLL**(Phase-Locked Loop — 기준 클럭에서 원하는 주파수의 안정적 클럭을 만들어내는 회로)이 "Lock" 됐다는 것은 그 출력 클럭이 안정화됐다는 뜻이고, **Bus Fabric**(여러 IP 를 잇는 버스 연결망 전체)이 먼저 살아나야 메모리에 접근할 길이 열립니다. 이 순서가 뒤집히면 CPU 가 MC 보다 먼저 reset 에서 벗어나 아직 초기화되지 않은 DRAM 에 접근을 시도하고, AXI **SLVERR**(Slave Error — 응답하는 IP(slave)가 요청을 처리할 수 없을 때 내는 버스 에러)가 발생하면서 boot 시점에 hang(시스템이 멈춰 진행 못 하는 상태)이 걸립니다. IP 단독 환경에서는 이 순서가 단순화돼 있어 위반이 드러나지 않지만, SoC Top 에서는 여러 reset 도메인이 얽혀 있기 때문에 명시적 검증이 필요합니다. Reset 후 모든 IP 의 레지스터가 스펙에서 정의한 기본값으로 복원됐는지도 함께 확인합니다.

#### 4. Interrupt Routing Verification

Interrupt Routing 검증의 핵심 질문은 "IP 가 발생시킨 인터럽트가 GIC 의 올바른 SPI 번호를 거쳐 지정된 CPU 에 도달하는가?" 입니다.

검증해야 할 항목은 SPI 번호의 정확성, 트리거 타입 (**Edge/Level** — 신호가 바뀌는 순간(edge)에 반응할지, 신호가 높게 유지되는 동안(level) 반응할지) 의 일치, 보안 그룹 (**Secure/Non-Secure** — ARM 의 신뢰 영역 구분; 보안 인터럽트와 일반 인터럽트를 분리) 설정, 우선순위, 그리고 마스킹(특정 인터럽트를 일시적으로 막는 것) 동작입니다. 이 중 하나라도 어긋나면 IP 는 인터럽트를 정상적으로 발생시키지만 **ISR**(Interrupt Service Routine — 인터럽트가 들어왔을 때 CPU 가 실행하는 처리 함수)이 전혀 실행되지 않거나 엉뚱한 핸들러로 dispatch(처리 함수로 넘겨짐) 됩니다.
...
Power On/Off 시퀀스가 정확해야 하고, 꺼진 도메인의 IP 에 접근할 때 적절한 에러 응답이 돌아와야 합니다. 특히 **Power Isolation**(전원 격리) 이 중요합니다 — **isolation cell**(전원이 꺼진 IP 의 출력선을 잡아 정해진 값으로 고정해, 꺼진 회로의 불확정 출력이 켜진 쪽으로 새지 않게 막는 회로) 이 제대로 활성화돼야만 꺼진 IP 의 출력이 0 (또는 latch 된 값 — 직전 값을 붙잡아 둔 것) 으로 clamp(특정 값으로 고정) 되며, isolation cell 이 misconfigured(잘못 설정됨) 이면 X(0 인지 1 인지 모르는 불확정 값)가 버스로 전파돼 downstream(그 신호를 받는 하류 쪽) 전체가 알 수 없는 상태로 빠집니다. 이 X-propagation(X 가 회로를 타고 번지는 현상) 은 논리 회로 특성상 시뮬레이션에서 즉시 터지지 않는 경우도 있어서 실리콘에서야 발견되는 silent bug 의 대표 사례입니다. 검증 대상 상태 전이는 Active(동작) → Idle(대기) → Retention(전원은 거의 끄되 레지스터 값만 보존) → Off(완전 차단) 이며, **DVFS**(Dynamic Voltage and Frequency Scaling — 부하에 따라 전압·주파수를 동적으로 조절해 전력을 아끼는 기법) 동작도 함께 확인합니다.

:::note[X 가 _왜_ 게이트를 타고 퍼지는가 — 4-state 시뮬레이터의 전파 규칙]
"X 가 버스로 전파" 는 회로 레벨로 보면 4-state(0/1/X/Z) 시뮬레이터의 명확한 규칙에서 나옵니다. X 는 "0 인지 1 인지 모름" 을 뜻하며, 시뮬레이터는 게이트마다 _입력이 X 일 때 출력이 결정되는지_ 를 따집니다. 핵심은 **흡수(controlling) vs 전파** 입니다:

- **AND 게이트**: 한 입력이 0 이면 다른 입력이 X 여도 출력은 _0_ 으로 _확정_ — 0 이 X 를 흡수합니다. 그러나 다른 입력이 1 이면 출력은 `1 AND X = X` 로 전파됩니다.
- **OR 게이트**: 한 입력이 1 이면 출력은 _1_ 로 확정(1 이 흡수). 다른 입력이 0 이면 `0 OR X = X` 로 전파됩니다.

즉 _제어값(controlling value: AND 의 0, OR 의 1)_ 이 들어오면 X 가 막히지만, _비제어값_ 만 있으면 X 가 출력으로 새어 나갑니다. 꺼진 도메인의 floating 출력이 X 가 되면, 그 X 가 downstream 조합 논리를 따라 위 규칙대로 번져 결국 flip-flop 에 X 가 래치되고, 그 시점부터 시스템 상태가 "알 수 없음" 으로 오염됩니다. isolation cell 의 역할이 바로 이 출발점에서 X 를 _제어값_ (0 또는 1)으로 clamp 해 전파 자체를 차단하는 것입니다. 그래서 §7.1 Q2 의 SVA 가 `=== 1'bx` 를 잡아내는 것이 핵심 — `==` 가 아니라 `===` 를 쓰는 이유도 X 를 값으로 _구별해서_ 비교하기 위함입니다.
:::
...
이 구조에서 CPU Model 은 BootROM(전원을 켜면 가장 먼저 실행되는 칩 내장 부팅 코드), BL2(Boot Loader 2단계 — 그다음으로 실행되는 부트로더) 등의 FW(Firmware — 하드웨어에 박혀 실행되는 저수준 소프트웨어)를 실행하여 실제 부팅과 동일한 흐름을 시뮬레이션합니다. FW 실행 없이 레지스터 접근 시나리오만 필요할 때는 AXI Master **VIP**(Verification IP — 특정 프로토콜을 검증하려고 미리 만들어 둔 재사용 검증 컴포넌트) 으로 대체할 수 있습니다. 외부 메모리와 디바이스는 BFM (Bus Functional Model — 실제 RTL 대신 버스 동작만 흉내 내는 가벼운 모델) 으로 대체해 sim 속도를 확보합니다.

### 5.4 코드 예시 — Connectivity 검증 SVA

**SVA**(SystemVerilog Assertions — 설계가 지켜야 할 시간적 성질을 코드로 적어, 위반 시 자동으로 에러를 내게 하는 SystemVerilog 의 검증 구문) 로 "이 두 신호가 항상 같아야 한다" 같은 연결 규칙을 직접 기술합니다. 아래에서 `property`(성질 정의)와 `assert property`(그 성질을 강제·감시) 가 한 쌍으로 쓰입니다.
...
:::note[Formal 이 "exhaustive 증명" 을 _어떻게_ 하는가 — 연결은 조합 등식이라 가능]
"입력 조합을 모두 탐색" 이라는 말이 시뮬레이션 수십억 번을 돌린다는 뜻은 아닙니다. Formal 도구는 RTL 을 _수학적 모델_ 로 바꾼 뒤, property 의 _반례(counterexample — 성질을 깨뜨리는 입력 예)가 존재하는지_ 를 SAT(논리식의 참 할당이 있는지 푸는 엔진)/BMC(Bounded Model Checking — 정해진 사이클 수만큼 회로를 펼쳐 반례를 찾는 기법) 엔진으로 푸는 방식입니다. 핵심은 **connectivity property 가 _조합 등식_** 이라는 점입니다 — `ip_a_irq_out == gic_spi[47]` 은 시간에 걸친 시퀀스가 아니라 _매 사이클_ 두 신호가 같아야 한다는 단일-사이클 관계입니다. 그래서 도구는 "이 두 신호가 달라지는 입력 할당이 단 하나라도 있는가?" 를 묻고, SAT 엔진이 그런 할당을 _못 찾으면_ 모든 입력에서 등식이 성립한다고 _증명_ 됩니다. 입력 N 비트면 시뮬레이션은 2^N 경우를 일일이 돌려야 하지만, SAT 는 논리식의 구조를 이용해 대부분의 경우를 한꺼번에 가지치기하므로 실제로 전수 열거를 하지 않고도 전수와 _동등한_ 결론을 냅니다.

이 때문에 단순 wire 연결 검증에는 _unbounded_ 증명(시간 무한 전개)이 필요 없습니다 — 연결은 상태에 의존하지 않는 즉시 관계라, BMC depth 1 (또는 조합 등가성 검사) 만으로 충분합니다. 반대로 §6 오해 2 가 지적하듯, CDC·handshake·AXI ordering 처럼 _여러 사이클에 걸친_ 시간적 성질은 이 단일-사이클 등식으로 표현되지 않아 simulation 이 보완해야 합니다.
:::
...
**UVM**(Universal Verification Methodology — SystemVerilog 기반의 표준 검증 프레임워크) 환경은 몇 가지 정해진 부품으로 짜입니다. **agent**(한 인터페이스를 구동·관측하는 묶음, 보통 driver+monitor+sequencer), **scoreboard**(DUT 출력과 기대값을 비교해 맞는지 판정하는 컴포넌트), **checker/monitor**(버스나 신호를 감시해 규칙 위반을 잡는 컴포넌트), **coverage**(검증이 어떤 경우를 실제로 밟았는지 집계하는 측정), **config**(IP 목록·주소맵 등 환경 설정을 한 곳에 모은 객체)가 그것입니다. 아래에서 `uvm_config_db`(컴포넌트 사이로 설정 객체를 전달하는 UVM 의 중앙 저장소)로 config 를 내려보냅니다.
...
:::caution[실무 주의점 — 인터럽트 SPI 번호 1씩 오프셋 뒤바뀜]
...
:::tip[🤔 Q1 — Connectivity 자동 검증 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Power domain bug (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — Scan 시 SoC 검증 (Bloom: Evaluate)]
...
→ [Module 02 — Common Task & CCTV](../02_common_task_cctv/): SoC Top 검증 항목 중 _모든 IP 에 공통 적용되는 것_ 을 매트릭스로 추적. DVCon 2025 의 Gap 자동화.

[퀴즈 풀어보기 →](../quiz/01_soc_top_integration_quiz/)
