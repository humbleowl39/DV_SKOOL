...
:::
:::note[사전 지식]
- [Module 01-02](../01_tcp_ip_and_toe_concept/)
...
당신의 TOE 가 Linux peer 와 통신한다고 해봅시다. 정상 데이터 전송은 잘 됩니다. 그런데 packet loss 가 발생하는 순간 connection 이 끊깁니다.

추적해 보면 원인이 명확합니다. 여기서 **peer**(피어; 통신 상대편 끝단 — 여기서는 Linux 호스트)와 **RFC**(Request For Comments; 인터넷 프로토콜을 규정하는 IETF 표준 문서)를 먼저 잡고 가면 좋습니다. Linux 는 손실을 감지하면 **SACK**(Selective Acknowledgment; "여기까지 연속" 만 알리는 보통 ACK 과 달리, 띄엄띄엄 받은 구간을 콕 집어 알려 빠진 부분만 재전송하게 하는 옵션) block 을 담아 ACK 를 보내는데, 당신의 TOE 가 이를 그냥 무시합니다. Linux 는 TOE 가 SACK 를 보고 손실된 구간만 선택적으로 재전송할 것을 기대하는데 그게 오지 않으니, 결국 timeout 을 거쳐 connection 을 drop 합니다. 원인은 TOE 의 **RFC 2018 SACK 미구현** 입니다. Linux peer 는 RFC 를 지키는데 당신의 TOE 가 지키지 않은 것입니다.

이것이 TCP **interop**(interoperability, 상호운용성; 서로 다른 구현끼리 표준만 지키면 문제없이 통신되는 성질) 의 어려움입니다. SACK (RFC 2018), **PAWS**(Protection Against Wrapped Sequences; sequence number 가 한 바퀴 돌아 옛 패킷과 번호가 겹칠 때 timestamp 로 걸러 내는 보호 — RFC 1323), Congestion Control (RFC 5681) 등 수십 개의 RFC 가 모두 정확히 구현되어야 다양한 peer 와 호환됩니다. 그래서 검증의 90 % 는 packet loss, OOO, duplicate, timeout, zero window 같은 abnormal scenario 에서의 _복구 동작_ 검증에 집중됩니다.
...
검증의 90 % 는 이 5 가지 기능의 _상태 조합_ 을 커버하는 일입니다. 즉 이 모듈의 어휘 — RTO, **SRTT**(smoothed RTT; 흔들리는 왕복시간을 평탄화한 평균), **cwnd**(congestion window; 혼잡을 피하려 한 번에 보낼 수 있게 허용된 양), **ssthresh**(slow-start threshold; 지수 증가를 멈추는 경계), SACK block, PAWS — 가 이후 **거의 모든 DV scoreboard**(DUT 출력과 기댓값을 비교하는 검증 컴포넌트) **/ SVA**(SystemVerilog Assertion; "이 조건이 항상 성립해야 한다"를 코드로 박아 자동 검사하는 단언) **/ coverage bin**(검증이 어떤 상황을 실제로 밟았는지 세는 칸) 에 등장합니다.
...
가장 단순한 시나리오. ESTABLISHED 연결에서 4 개의 데이터 segment 를 연속 전송 → 두 번째가 wire 에서 손실 → peer 가 **dup ACK**(duplicate ACK; 같은 ack 번호를 반복해 "기대하던 조각이 빠졌다"고 알리는 중복 확인) 을 보내옴 → **Fast Retransmit**(RTO 만료를 기다리지 않고, dup ACK 가 3 개 쌓이면 빠진 segment 를 즉시 재전송하는 기법).

아래 NewReno 는 congestion control 알고리즘의 한 종류로 §5.5 에서 자세히 다룹니다.
...
# Note over TX: snd_una=11460\nretx buf [10000,11460) 해제
# Note over TX: dup_ack_count=1\n(seg3 받았지만 11460 이 빠짐)
# Note over TX: dup_ack_count=2
# Note over TX: dup_ack_count=3\n⑤ Fast Retransmit trigger:\ncwnd = cwnd/2\nssthresh = cwnd\nretx_seq = 11460\nretx buffer 에서 seg2 사본 꺼냄
# Note over TX: snd_una=15840\nretx buf [11460,15840) 해제\ndup_ack_count=0\ncwnd 는 ssthresh 로 정상화 (NewReno)
TX -> Wire: "t=0 · seg1 (seq=10000, len=1460)"
Wire -> Peer: "받음"
TX -> Wire: "t=0+ · seg2 (seq=11460, len=1460)"
# unparsed: Wire--XPeer: 손실!
TX -> Wire: "t=0+ · seg3 (seq=12920, len=1460)"
Wire -> Peer: "받음 (OOO)"
TX -> Wire: "t=0+ · seg4 (seq=14380, len=1460)"
Wire -> Peer: "받음 (OOO)"
Peer -> TX: "t=1ms · ACK(ack=11460) ① 정상 ACK" { style.stroke-dash: 4 }
Peer -> TX: "t=2ms · ACK(ack=11460) ② DUP ACK #1" { style.stroke-dash: 4 }
Peer -> TX: "t=3ms · ACK(ack=11460) ③ DUP ACK #2" { style.stroke-dash: 4 }
Peer -> TX: "t=4ms · ACK(ack=11460) ④ DUP ACK #3 ⭐" { style.stroke-dash: 4 }
TX -> Wire: "t=4ms+ · seg2' (seq=11460, len=1460)"
Wire -> Peer: "받음"
Peer -> TX: "t=5ms · ACK(ack=15840) ⑥ recovery" { style.stroke-dash: 4 }
```
...
| **Congestion Control** | cwnd 검사, **slow start**(연결 초기에 cwnd 를 매 ACK 마다 빠르게 키우는 단계)/cong avoid 단계 갱신 | **ECN**(Explicit Congestion Notification; 패킷을 버리는 대신 헤더 비트로 "혼잡하다"를 알리는 표시), dup ACK 검출 | cwnd, ssthresh, cong_state, **W_max**(직전 손실 직전의 cwnd 최댓값) |
...
위 다이어그램의 **FSM**(Finite State Machine; 연결이 가질 수 있는 상태들과 그 사이의 전이 규칙을 정한 유한 상태 기계)은 TCP 연결의 일생입니다. 마지막 전이의 **2MSL**(2 × Maximum Segment Lifetime; 네트워크에 떠도는 패킷이 사라지기까지의 최대 수명의 2배 — TIME_WAIT 에서 이 시간을 기다린 뒤 완전히 닫음)은 늦게 오는 패킷이 새 연결을 오염시키지 않도록 보장합니다.

HW 에서 이 FSM 의 모든 전이를 정확하게 구현해야 합니다. 단순히 ESTABLISHED → FIN_WAIT_1 처럼 정상 경로만이 아니라, 잘못된 trigger — 예를 들어 LISTEN 상태에서 FIN 이 오는 것 — 도 무시하거나 RST 로 응답하는 동작까지 포함해서 검증 대상입니다.

11 개 state 에서 나올 수 있는 모든 합법적 전이와 모든 invalid trigger 에 대한 응답 — 이것이 Module 04 FSM coverage 의 모집합입니다. 이 중 하나라도 누락되면 실제 운영 중 예상치 못한 state 에 빠질 위험이 있습니다.
...
네트워크에서는 패킷이 반드시 순서대로 도착하지 않습니다. 예를 들어 seg1~seg5 를 보냈는데 seg3, seg1, seg5, seg2, seg4 순서로 들어올 수 있습니다. 이때 TOE 는 OOO(Out-of-Order) 버퍼를 두어 기대 Seq 보다 앞서 도착한 세그먼트를 임시로 저장해 둡니다. seg1 (Seq=0) 이 도착하면 그것을 즉시 전달하고, 버퍼에 이미 저장되어 있던 seg2 도 이어서 전달합니다. 이처럼 "기대 Seq 와 일치하면 즉시 전달, 불일치하면 OOO 버퍼에 저장" 을 반복하면서 호스트에는 항상 순서대로 데이터가 올라갑니다.

HW 구현에서는 Linked list 또는 Bitmap 으로 수신된 Seq 범위를 추적하여 어느 구간이 채워졌는지 관리합니다.
...
# Note over TX: 정상
# Note over TX: ACK 수신 →\n재전송 버퍼에서 해당 데이터 해제
# Note over TX: 재전송 필요 (Timeout)
# Note over TX: 타이머 만료
# Note over TX: 재전송 필요 (Fast Retransmit)
# Note over TX: Duplicate ACK 3 개
TX -> RX: "DATA(seq=100)"
RX -> TX: "ACK(ack=600)" { style.stroke-dash: 4 }
# unparsed: TX-XRX: DATA(seq=100) — 손실
TX -> RX: "DATA(seq=100) 재전송"
TX -> RX: "DATA(100), DATA(600), DATA(1100)"
RX -> TX: "ACK(600), ACK(600), ACK(600)" { style.stroke-dash: 4 }
...
TOE HW 가 연결별로 RTO 를 동적으로 계산해야 한다. SW 에서는 커널이 하지만, TOE 에서는 **고정소수점 연산(fixed-point; 소수점 위치를 정수 비트 안에 고정해, 비싼 부동소수점 회로 없이 소수를 다루는 방식 — 예 Q16.16 은 정수부 16비트·소수부 16비트)으로 HW 구현**한다. 아래 식의 **EWMA**(Exponentially Weighted Moving Average; 최근 값에 더 큰 가중치를 주며 옛 값은 지수적으로 잊는 이동 평균)는 RTT 추정의 핵심 기법이다.
...
HW 구현의 핵심은 α=1/8, β=1/4 라는 계수가 비트 시프트로 표현된다는 점입니다 (÷8 = >>3, ÷4 = >>2). 덕분에 곱셈기 없이도 EWMA 계산이 가능합니다. 수치 정밀도를 위해서는 고정소수점 (예: Q16.16) 형식으로 SRTT 와 RTTVAR 를 저장하며, 이 값들은 연결별로 Connection Table 에 기록됩니다.
```
...
수신자가 **Window Size**(수신측이 받을 수 있는 버퍼 여유 — 송신측은 이만큼까지만 보낼 수 있음) 로 "내 버퍼에 여유 공간이 이만큼 있다" 알림. 여유가 0 이 되면 **Zero Window**(수신 버퍼가 꽉 차 "지금은 보내지 마라"를 뜻하는 window=0 상태)가 되고, 송신측은 전송을 멈춘 뒤 **Window Probe**(zero window 가 풀렸는지 주기적으로 1 byte 를 찔러 확인하는 작은 패킷)로 재개 시점을 탐지합니다.
...
# Note over TX: 전송 재개
RX -> TX: "ACK(ack=600, window=8192)" { style.stroke-dash: 4 }
RX -> TX: "ACK(ack=600, window=0) — Zero Window!" { style.stroke-dash: 4 }
RX -> TX: "Window Update(window=4096)" { style.stroke-dash: 4 }
```
...
위 세 알고리즘은 cwnd(혼잡 window)를 키우는 방식이 다른 혼잡 제어 변형입니다 — **Reno**(가장 단순한 표준), **NewReno**(Reno 의 손실 복구를 개선한 버전), **Cubic**(cwnd 를 시간의 3차 함수로 키워 고속·장거리 망에서 대역폭을 잘 채우는 Linux 기본). 여기서 **BDP**(Bandwidth-Delay Product; 대역폭 × 왕복지연 — "회선에 동시에 떠 있을 수 있는 데이터량", 이 값이 커야 큰 window 가 의미 있음)가 알고리즘 선택의 기준입니다.

실무에서는 대부분의 TOE 가 **NewReno 를 기본**으로 지원하고, 고성능 제품은 **Cubic 을 추가** 합니다. 알고리즘 선택을 런타임에 SW 로 설정할 수 있게 하면 워크로드에 따라 유연하게 전환할 수 있어 선호됩니다. 단 Cubic 을 HW 로 구현하려면 3 차 함수를 **LUT**(Look-Up Table; 미리 계산한 값을 표로 저장해 두고 입력으로 찾아 쓰는 방식 — 복잡한 연산을 회로 대신 메모리 조회로 대체) 또는 근사 연산으로 처리해야 하므로, 면적 예산이 빠듯한 경우 NewReno 만 지원하는 설계도 현실적인 선택입니다.
...
Connection Table 에 TX 용과 RX 용 Scale Factor 를 각각 저장해야 하는 이유는 두 방향의 Scale 이 독립적으로 협상되어 서로 다를 수 있기 때문입니다. Window 값을 읽거나 쓸 때마다 시프트 연산으로 실제 크기로 변환합니다.
...
SACK 의 HW 구현에서 까다로운 부분은 가변 길이 블록 파싱입니다. Options 필드 안에 최대 4 개의 SACK block 이 들어오는데, 이를 파싱하여 어느 Seq 범위가 수신 완료되었는지를 Scoreboard 자료구조에 기록합니다. 이후 재전송 엔진은 Scoreboard 를 보고 아직 수신 확인되지 않은 구간만 선별해서 재전송합니다 — 불필요한 재전송이 없으므로 대역폭 낭비가 줄어듭니다.
...
:::caution[실무 주의점 — RTO 타이머와 Fast Retransmit 동시 트리거]
...
:::
:::tip[🤔 Q2 — Cubic vs Reno (Bloom: Evaluate)]
...
→ [Module 04 — TOE DV Methodology](../04_toe_dv_methodology/): 위 5 대 기능을 실제로 검증하는 UVM 환경 — Host agent / Network agent / Reference model / Scoreboard / SVA / Coverage.

- 📝 [**Module 03 퀴즈**](../quiz/03_toe_key_functions_quiz/)
