---
title: "Module 05 — Quick Reference Card"
---
...
:::
:::note[사전 지식]
- [Module 01-04](../01_tcp_ip_and_toe_concept/)
:::
---
...
| 상황 | 한 줄 답변 | Deep-dive |
|---|---|---|
| "TOE 가 뭐예요?" | "TCP/IP 의 stateful HW offload — CPU 부하 80–90 % 감소, 100 Gbps 라인레이트, ~µs latency" | [M01](../01_tcp_ip_and_toe_concept/) |
| "TOE 와 TSO 차이?" | "TSO 는 segmentation 만, TOE 는 connection state 까지 — TSO ⊂ TOE" | [M01 §5.6](../01_tcp_ip_and_toe_concept/#56-toe-적용-후-효과) |
| "Connection 100 만 개 어떻게?" | "Hot 은 SRAM, cold 는 DRAM 의 LRU swap. 4-tuple hash O(1) lookup" | [M02 §5.5](../02_toe_architecture/#55-메모리-아키텍처--버퍼와-테이블-배치) |
| "RTO timer 100 만 개 어떻게?" | "Hashed Timing Wheel — tick 당 _현재 슬롯_ 만 검사 → O(1)" | [M02 §5.4](../02_toe_architecture/#54-타이머-관리-아키텍처--수백만-연결의-rto) |
| "Fast Retransmit 언제?" | "Dup ACK 3 개 도착 시 RTO 만료 기다리지 않고 즉시 재전송 + cwnd halve" | [M03 §5.3](../03_toe_key_functions/#53-retransmission--재전송) |
| "RTO 어떻게 계산?" | "RFC 6298 — SRTT/RTTVAR EWMA, RTO = SRTT + 4×RTTVAR. α=1/8, β=1/4 → bit shift" | [M03 §5.3](../03_toe_key_functions/#53-retransmission--재전송) |
| "Window Scale 왜 필요?" | "헤더 16 비트 = 64 KB 한도. 100 Gbps × 1 ms RTT = 12.5 MB BDP → 부족. Scale 로 최대 1 GB" | [M03 §5.6](../03_toe_key_functions/#56-tcp-options--toe-가-처리해야-하는-확장) |
| "Reactive Agent 가 뭐?" | "Monitor 가 DUT 출력 캡처 → Responder 가 _보고나서_ ACK/SYN-ACK 만듦. Stateful 프로토콜은 pre-programmed 불가" | [M04 §5.4](../04_toe_dv_methodology/#54-network-agent-설계--에러-주입) |
| "TOE 검증 4 축?" | "Protocol / Functional / Performance / Error Recovery — 하나라도 빠지면 미완성" | [M04 §4.1](../04_toe_dv_methodology/#41-검증-4-축) |
| "SVA 가 vacuous pass 라는데?" | "antecedent 가 한 번도 발생 안 함 → cover property 의 hit 로 확인" | [M04 §5.3](../04_toe_dv_methodology/#53-sva-systemverilog-assertions--프로토콜-준수-검증) |
...
| 카드 항목 | 해당 Module |
|---|---|
| TOE 한 줄 정의, Offload 스펙트럼 | [M01](../01_tcp_ip_and_toe_concept/) |
| TX/RX path, Connection Table, Timer Wheel, Memory hierarchy | [M02](../02_toe_architecture/) |
| 5 대 기능, RTO 공식, Cubic, Options | [M03](../03_toe_key_functions/) |
| UVM env, Reference, Scoreboard, SVA, Coverage | [M04](../04_toe_dv_methodology/) |
...
| 신호 | 어느 module 로 |
|---|---|
| "왜 stateful 이 stateless 보다 비싼지 정량 비교" | [M01 §5](../01_tcp_ip_and_toe_concept/#5-디테일--tcpip-스택-tcp-기능-toe-효과) |
| "Connection Table 의 hash 충돌 처리 디테일" | [M02 §5.3](../02_toe_architecture/#53-connection-table--엔트리-구조와-lookup) |
| "Timer Wheel 의 cascade 동작" | [M02 §5.4](../02_toe_architecture/#54-타이머-관리-아키텍처--수백만-연결의-rto) |
| "Cubic 의 W_max 회복 곡선 수식" | [M03 §5.5](../03_toe_key_functions/#55-congestion-control--혼잡-제어) |
| "Karn's Algorithm 정확한 정의" | [M03 §5.3](../03_toe_key_functions/#53-retransmission--재전송) |
| "Reactive Agent 의 Driver/Monitor/Responder 분리" | [M04 §5.4](../04_toe_dv_methodology/#54-network-agent-설계--에러-주입) |
| "SVA 의 vacuous pass 회피 구체 코드" | [M04 §5.3](../04_toe_dv_methodology/#53-sva-systemverilog-assertions--프로토콜-준수-검증) |
...
:::
:::tip[🤔 Q2 — TIME_WAIT 의 _목적_ (Bloom: Analyze)]
