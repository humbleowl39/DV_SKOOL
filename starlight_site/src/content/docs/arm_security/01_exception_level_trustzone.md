---
title: "Module 01 — Exception Level & TrustZone"
---
...
두 축은 **독립**입니다. EL3 만 항상 NS=0 으로 고정되고(전환 게이트), 나머지 EL 은 Secure 와 Non-Secure 둘 다 가능합니다. 그 결과 표면상 4 × 2 = 8 mode 가 존재합니다.
...
:::caution[실무 주의점 — SCR_EL3.NS 비트 전파 누락]
...
:::tip[🤔 Q1 — World 전환 추적 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — NS bit 전파 검증 (Bloom: Apply)]
SoC verifier 입장에서, SCR_EL3.NS 가 _AXI bus 의 AxPROT[1]_ 으로 정확히 전파되는지 검증해야 합니다. 어떤 시나리오를 구성하겠습니까?
...
→ [Module 02 — 월드 전환 & SoC 보안 인프라](../02_world_switch_soc_infra/): SCR_EL3.NS 가 실제로 어떻게 모든 SoC 컴포넌트 (TZPC, TZASC, GIC, SMMU, cache) 에 전파되는지, BL31 이 SMC handler 에서 무엇을 하는지.

[퀴즈 풀어보기 →](../quiz/01_exception_level_trustzone_quiz/)
