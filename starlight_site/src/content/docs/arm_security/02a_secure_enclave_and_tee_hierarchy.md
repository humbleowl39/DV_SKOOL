...
External Secure Enclave 는 SPI/I2C 같은 외부 버스로 SoC 와 연결됩니다. PCB 트레이스를 거치기 때문에 오실로스코프나 로직 분석기로 도청하거나, IC 를 물리적으로 교체하는 공격이 가능합니다. 이 위협을 막으려면 버스 위를 흐르는 메시지가 암호화·인증·freshness 세 가지를 모두 갖춰야 합니다. 암호화(AES-CCM/GCM)가 도청을 차단하고, CMAC/HMAC 이 변조나 IC 교체를 감지하며, 세션 키와 시퀀스 번호가 같은 메시지의 재전송(replay) 을 차단합니다. 또한 양쪽이 서로의 정체를 검증하는 상호 인증이 있어야 IC 교체 공격이 무의미해집니다.
