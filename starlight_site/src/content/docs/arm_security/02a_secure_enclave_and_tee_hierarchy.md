---
title: "Module 02A — Secure Enclave & TEE Hierarchy"
---
...
:::
:::note[사전 지식]
- [Module 01 — Exception Level & TrustZone](../01_exception_level_trustzone/)
- [Module 02 — World Switch & SoC Security Infra](../02_world_switch_soc_infra/)
...
**TrustZone**(ARM CPU 내장 하드웨어 보안 확장 — 같은 코어를 보안/일반 두 "세계"로 나눔)으로 결제 모듈을 보호했다고 해도, 2018 년 **Spectre**(투기적 실행의 부수 효과로 격리를 우회하는 CPU 취약점) 같은 CPU 버그가 발견되면 **캐시 측면 채널**(side-channel — 데이터를 직접 읽지 않고 캐시 hit/miss 같은 실행 흔적으로 비밀을 추론하는 경로)을 통해 Non-Secure 코드가 Secure 메모리를 읽어낼 수 있습니다. 실제로 2017 년 **CVE**(공개 취약점에 붙는 표준 식별 번호)-2017-15361(Infineon, TrustZone 안의 RSA 키 생성 약점), 2018 년 Spectre/Meltdown(캐시 측면 채널로 격리 우회), 2020 년 SMASH(TrustZone **Rowhammer** — 인접 메모리 셀을 반복 접근해 비트를 뒤집는 공격), 2022 년 ARMv9 **CCA**(Confidential Compute Architecture — ARMv9 의 새 격리 확장) bug(**PSCI**(전원 관리를 EL3 에 요청하는 표준 호출) handler 의 메모리 corruption) 처럼 TrustZone 의 격리가 우회된 사례는 꾸준히 등장했습니다.

이 사례들이 보여주는 것은 TrustZone 만으로는 충분하지 않다는 사실입니다. 가장 중요한 비밀(**device root key**(기기마다 고유하고 영구적인 최상위 마스터 키), **biometric template**(지문·얼굴 등 생체 정보를 저장한 기준 데이터) 등)은 Main CPU 와 cache, **TLB**(주소 번역 결과를 캐싱하는 작은 표), **branch predictor**(분기 방향을 미리 예측하는 회로) 같은 마이크로아키텍처 자원조차 공유하지 않는 별도 하드웨어에 보관해야 합니다. 이것이 **Secure Enclave**(별도 프로세서·전용 RAM·전용 암호엔진으로 물리적으로 격리된 보안 영역) 의 출발점입니다. Secure Enclave 는 독립된 소형 processor, 전용 RAM, 자체 boot ROM 으로 구성되며, Main CPU 가 망가져도 Enclave 는 안전합니다. 두 사이의 통신은 queue 기반 **Mailbox**(서로 격리된 두 영역이 메시지를 주고받는 전용 하드웨어 통로) 로만 이루어지고, 모든 메시지는 **Secure Channel Protocol**(SCP03/SCP11 — 외부 버스 위 메시지를 암호화·인증하는 표준)로 암호화됩니다. Apple Secure Enclave Processor(SEP), Google Titan M, Samsung Knox 가 이 모델의 대표 구현입니다.

Module 01-02 에서 _"TrustZone 이 Secure World 를 격리한다"_ 라고 했지만, **TrustZone 자체가 뚫리면 어떻게 되는가?** 라는 질문에는 답이 없습니다. 실제 사례 — Spectre 류의 캐시 부채널, **OP-TEE**(대표적 오픈소스 Secure World OS) 의 **RCE**(Remote Code Execution — 공격자가 원격에서 임의 코드를 실행하게 되는 취약점) 취약점, 또는 Trusted OS bug — 가 발생하면 Secure World 의 _모든_ 키와 데이터가 노출됩니다.
...
:::tip[🤔 잠깐 — 왜 Enclave 가 _별도 processor_ 필요?]
...
가장 직관적인 시나리오. OP-TEE (S-EL1) 가 RCE 로 장악된 상태에서, 공격자가 결제 마스터 키를 탈취하려 합니다. 마스터 키는 Internal Secure Enclave 의 **Key Box**(키만 보관하는 enclave 내부 전용 저장소) 에 있고, S-EL1 은 키를 _직접_ 읽을 수 없으며 Mailbox 로만 _서명 결과_ 를 요청할 수 있습니다. (여기서 **TA** = Trusted App, Secure World 안에서 도는 보안 앱.)
...
Module 01 의 두 World (S/NS) 만으로는 부족했습니다. 실제 SoC 는 **여러 TEE**(Trusted Execution Environment — 일반 OS 와 격리된 신뢰 실행 환경) **가 보안 레벨이 다른 채로 공존** 합니다. 가장 바깥의 일반 영역은 **REE**(Rich Execution Environment — Android·Linux 같은 일반 OS 가 도는, TEE 의 반대편 영역)라 부릅니다.
...
→ 보안 부팅의 **Chain of Trust**(신뢰 사슬 — 부팅 각 단계가 다음 단계를 서명 검증한 뒤에야 넘겨주는 구조; Module 03 에서 자세히) 에서 "BL2 가 BL3 를 검증하듯" 각 TEE 가 상대방을 검증하는 구조.
...
TrustZone 은 강력하지만, 그 격리의 전제가 "같은 CPU 와 메모리 서브시스템을 공유한다" 는 것입니다. 이 공유가 곧 한계의 근원입니다. Trusted OS (OP-TEE) 가 S-EL1 에서 실행되는 복잡한 소프트웨어라는 점, Secure 와 Non-Secure 가 동일한 CPU 코어의 cache·TLB·branch predictor 를 쓴다는 점, 그리고 TA 가 늘수록 공격 표면도 늘어난다는 점이 서로 맞물려 아래 네 가지 한계를 만들어 냅니다.
...
- **무슨 키인가 — 대칭 MAC vs 비대칭 서명**: 두 갈래가 있습니다. (1) **대칭 MAC**(Message Authentication Code — 비밀키로 메시지에 붙이는 위변조 검증 태그; 예: CMAC/HMAC) — enclave와 정당한 요청자가 _사전 공유한 비밀키_ `K`로 토큰을 계산·검증. 빠르고 작지만 양쪽이 같은 비밀을 가져야 함. (2) **비대칭 서명** — OEM/요청자의 _private key_ 로 서명하고 enclave가 대응 _public key_ 로 검증. 키 공유가 필요 없고 부인방지가 되지만 무거움. "OEM 키로 서명된 토큰"이라는 서술은 (2)에 가깝지만, 세션 내 요청마다는 (1) 대칭 MAC를 쓰는 혼합도 흔합니다.
- **무슨 값을 덮나**: 반드시 `op`(무슨 동작)와 `args`(어느 키 인덱스 등)를 포함해야, 공격자가 토큰은 그대로 두고 _요청 내용만 바꾸는_ 변조를 막습니다(토큰이 요청에 _바인딩_).

**nonce 가 freshness 를 보장하는 방식.** 토큰 입력에 매 요청 _새로 발급되는 **nonce**_(number used once — 한 번만 쓰고 버리는 무작위 값; 재전송을 막는 일회용 표식)(또는 단조 증가 sequence number — 매번 1씩 커지는 메시지 일련번호)를 넣습니다. enclave는 (a) 자신이 방금 발급한 challenge nonce인지, 또는 (b) sequence가 직전보다 큰지를 확인합니다. 그러면 공격자가 _과거에 정당했던 토큰_ 을 그대로 재전송해도(replay), 그 토큰은 _옛 nonce_ 로 계산된 값이라 enclave가 기대하는 현재 nonce와 안 맞아 MAC/서명 검증에서 떨어집니다. 공격자는 키가 없어 _새 nonce에 맞는_ 토큰을 다시 만들 수도 없습니다. 즉 "nonce를 인증값 입력에 묶음 + 키 비밀"이 합쳐져 replay가 토큰째 무효가 됩니다(§6의 "token verify disable", "sequence number 검증 누락" 항목이 이 검증을 점검).
...
| 복호화 키를 Enclave 에 저장 | **ROTPK**(Root Of Trust Public Key — 부팅 검증의 최상위 신뢰 기준이 되는 공개키) 를 **OTP**(One-Time Programmable — 한 번만 굽고 변경 불가한 메모리) 에 저장하는 것과 동일 원리 |

### 5.9 Secure Channel Protocol — External Enclave 와의 통신

External Secure Enclave 는 **SPI/I2C**(칩과 칩을 잇는 느린 직렬 통신 버스 — 보드 위 배선으로 신호가 노출됨) 같은 외부 버스로 SoC 와 연결됩니다. PCB 트레이스를 거치기 때문에 오실로스코프나 로직 분석기로 도청하거나, IC 를 물리적으로 교체하는 공격이 가능합니다. 이 위협을 막으려면 버스 위를 흐르는 메시지가 암호화·인증·**freshness**(이 메시지가 옛 것의 재사용이 아니라 "새로" 온 것임을 보장하는 성질) 세 가지를 모두 갖춰야 합니다. 암호화(**AES-CCM/GCM** — 암호화와 무결성 검증을 함께 하는 대칭 암호 모드)가 도청을 차단하고, **CMAC/HMAC**(비밀키로 메시지에 붙이는 위변조 검증 태그 방식) 이 변조나 IC 교체를 감지하며, 세션 키와 시퀀스 번호가 같은 메시지의 **재전송(replay)**(가로챈 정당한 메시지를 그대로 다시 보내 속이는 공격) 을 차단합니다. 또한 양쪽이 서로의 정체를 검증하는 상호 인증이 있어야 IC 교체 공격이 무의미해집니다.
...
- **패턴 A — 사전 공유 키 기반 KDF (대칭, SCP03류)**: 양쪽이 이미 _공유한 정적 비밀키_ 를 두고, 세션마다 서로 무작위 **challenge**(상대가 진짜 키를 가졌는지 떠보려고 보내는 무작위 질문값)를 교환한 뒤, `세션키 = **KDF**(Key Derivation Function — 하나의 키/비밀에서 용도별 키를 안전하게 뽑아내는 함수)(정적키, host_random ‖ card_random ‖ 라벨)` 로 _그 세션 전용 키_ 를 파생합니다. 매 세션 random이 달라 세션키가 매번 달라지고(같은 정적키라도), challenge가 양쪽 random을 모두 포함하므로 한쪽이 마음대로 세션키를 고정할 수 없습니다.
- **패턴 B — 키 교환 (비대칭, SCP11류)**: **ECDH**(Elliptic-Curve Diffie-Hellman — 비밀을 직접 주고받지 않고 양쪽이 같은 공유 비밀을 계산해 내는 키 합의 방식) 같은 키 합의로 _공유 비밀_ 을 즉석에서 만들고 거기서 세션키를 파생합니다. 정적 공유 비밀이 없어도 되고 **전방 비밀성**(forward secrecy — 나중에 장기 키가 털려도 과거 세션은 못 푸는 성질)에 유리.
...
이것은 차량 보안의 **SecOC**(Secure Onboard Communication — 차량 내부 통신 메시지에 MAC+Freshness 를 붙여 위변조를 막는 표준) (**CAN**(Controller Area Network — 차량 내 ECU 들을 잇는 표준 버스) 메시지에 MAC + Freshness) 와 동일한 원리를 물리 인터페이스 레벨에 적용한 것입니다.
...
**실제**: TEE 는 _ARM TrustZone 기반_, **SGX**(Software Guard Extensions — Intel 의 enclave 기술) 는 _Intel enclave_ (runtime 격리 + memory 암호화), **SEV**(Secure Encrypted Virtualization — AMD 의 VM 메모리 암호화 기술) 는 _AMD VM 단위 암호화_ — 카테고리가 다릅니다. 모델/위협/검증 방법이 모두 다릅니다.<br>
...
**실제**: 두 enclave 는 _다른 위협 모델_ 을 다룹니다. Internal 은 SW 공격 / 캐시 부채널 방어, External 은 _물리적 IC 교체 / **decapping**(칩 패키지를 화학적으로 벗겨 내부 회로를 직접 들여다보는 물리 공격)_ 방어. 각각이 독립적 **anti-tamper**(물리적 침입·변조를 탐지하면 비밀을 지워 버리는 방어 장치) 를 제공.<br>
**왜 헷갈리는가**: "보안 칩이 두 개면 중복" 같은 인상.
:::
:::danger[❓ 오해 4 — 'Mailbox 면 자동으로 안전']
**실제**: Mailbox 는 _통신 경로_ 일 뿐. 인증 토큰 검증, replay 방어 (sequence number), **TOCTOU**(Time-Of-Check to Time-Of-Use — 검사 직후 사용 전에 값이 바뀌어 검사를 무력화하는 취약점) 방어 (copy-then-validate) 가 모두 enclave FW 책임입니다. mailbox 가 있다고 보안이 끝난 게 아닙니다.<br>
...
:::tip[🤔 Q1 — Enclave 분리 결정 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Side-channel 분석 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — Mailbox 검증 (Bloom: Evaluate)]
...
→ [Module 03 — Secure Boot Connection](../03_secure_boot_connection/): TrustZone / Enclave 의 _존재 자체_ 가 부팅 시점에 어떻게 establish 되는가. Chain of Trust + ROTPK + Anti-Rollback + Measured Boot.

[퀴즈 풀어보기 →](../quiz/02a_secure_enclave_and_tee_hierarchy_quiz/)
