---
title: "Module 02 — World Switch & SoC Security Infra"
---
...
여기서 **NS bit**(모든 버스 트랜잭션에 붙는 1비트 보안 표식 — 0=Secure 세계, 1=Non-Secure 세계), **DMA**(Direct Memory Access — CPU 를 거치지 않고 주변장치가 직접 메모리를 읽고 쓰는 것), **IRQ**(Interrupt ReQuest — 하드웨어가 CPU 에 일으키는 일반 인터럽트)를 전제로 설명이 이어집니다.

답은 **5 개의 분산 컴포넌트** 가 각자 NS bit 를 _다른 단위_ 로 검사:

| 컴포넌트 | 검사 단위 | 막는 것 |
|---------|---------|--------|
| **TZPC**(TrustZone Protection Controller — 주변장치를 보안/일반으로 분류) | Peripheral 단위 | UART/GPIO 등 IP 의 secure/non-secure 격리 |
| **TZASC**(TrustZone Address Space Controller — DRAM 영역을 보안/일반으로 분할) | DRAM region 단위 | DDR 의 _주소 범위_ 별 격리 |
| **GIC**(Generic Interrupt Controller — ARM 표준 인터럽트 컨트롤러) | Interrupt 단위 | IRQ 의 secure/non-secure 분류 |
| **SMMU**(System MMU — DMA 마스터의 메모리 접근을 번역·차단) | DMA stream 단위 | DMA master 의 격리 (PCIe, GPU 등) |
| **Cache** | Cache line 단위 | L1/L2 의 NS tag(캐시 라인마다 그 줄을 채운 접근이 보안인지 일반인지 표시하는 비트) |

**모든 5 가 _동시에_ 작동해야** 안전. 하나라도 누락 = 격리 실패 = 보안 사고.

Module 01 에서 _"NS bit 가 모든 transaction 에 함께 흐른다"_ 라고 했지만, **누가 그 bit 를 set 하고, 어느 컴포넌트가 그 bit 를 보고 차단하는지** 는 아직 안 봤습니다. 이 모듈이 그 답입니다 — **BL31**(EL3 에 상주하는 Secure Monitor 본체 — 두 세계 전환을 전담하는 보안 펌웨어)의 **SMC**(Secure Monitor Call — 하위 EL 이 EL3 를 부르는 전용 명령) handler 가 **SCR_EL3.NS**(EL3 보안 설정 레지스터의 NS 비트 — 현재 세계를 정하는 원천 값)를 toggle 하면, downstream 에서 TZPC (peripheral), TZASC (DRAM), GIC (interrupt), SMMU (DMA), cache (NS tag) 가 각자 다른 단위로 같은 NS bit 를 _존중_ 합니다. (**EL3** = ARMv8 권한 등급 중 최고 단계, 보안 모니터 전용.)
...
가장 단순한 시나리오. NS-EL1 Linux kernel 의 **OP-TEE**(대표적 오픈소스 Secure World OS) driver 가 `SMC #0` 을 발행해 S-EL1 OP-TEE 에 진입했다가 돌아옵니다. 사이에 BL31 이 두 번 (NS→S, S→NS) 호출되며, 각 호출마다 **GPR**(general-purpose register, 범용 레지스터) + **sysreg**(system register — CPU 동작을 제어하는 특수 레지스터) 컨텍스트를 정확히 swap 합니다. (여기서 **TA** = Trusted App, Secure World 안에서 도는 보안 앱.)
...
| ② | NS-EL1 | OP-TEE message buffer 작성, X0 = function id | **SMCCC**(SMC Calling Convention — SMC 호출 시 어느 레지스터에 무엇을 담을지 정한 규약) 규약 |
| ③ | NS-EL1 | `smc #0` 실행 | HW 가 즉시 trap(HW 가 실행을 멈추고 상위 EL 핸들러로 강제 전환) to EL3 |
| ④ | EL3 (BL31) | NS context save: x0~x30, SP_EL1, ELR_EL1(복귀 주소 저장 레지스터), SPSR_EL1(복귀 시 되살릴 프로세서 상태 저장 레지스터), **NEON/FP/SVE**(부동소수점·벡터 연산용 레지스터 묶음 — 여기에도 비밀이 남을 수 있음), TPIDR_EL*, sysreg | 누락 시 secure state 누설 |
| ⑤ | EL3 | `MSR SCR_EL3, x?` 로 NS bit 0 set | 이 cycle 부터 outgoing **AxPROT[1]**(AXI 버스 주소채널의 보호 신호 비트 — 이 값이 곧 NS bit; 0=Secure, 1=Non-secure)=0 |
| ⑥ | EL3 | 이전에 보관한 S context (있으면) restore — 첫 진입이면 OP-TEE entry 로 set | 양방향 swap |
| ⑦ | EL3 | SPSR_EL3 ← S-EL1h, ELR_EL3 ← OP-TEE entry → `ERET`(Exception Return — 상위 EL 에서 하위 EL 로 복귀하는 명령) | S-EL1 진입 |
...
여기서 **PSTATE**(현재 프로세서 상태 — 현 EL·인터럽트 마스크 등)와 **VBAR_ELn**(각 EL 의 exception 핸들러들이 모인 벡터 테이블의 기준 주소 레지스터)이 핵심 레지스터입니다.
...
아래 규약에서 몇 용어가 처음 나옵니다 — **Fast Call**(즉시 처리되고 바로 반환하는 호출) vs **Yielding Call**(오래 걸려 중간에 양보·재개되는 비동기 호출), **SiP**(Silicon Provider — 삼성·퀄컴 등 칩 제조사가 정의하는 벤더 전용 호출 영역), 그리고 **PSCI**(Power State Coordination Interface — CPU 전원 켜기/끄기·절전을 EL3 에 요청하는 표준 SMC 서비스).
...
인터럽트는 발생원에 따라 종류가 나뉩니다 — **SGI**(Software Generated Interrupt — 코어가 다른 코어에게 보내는 인터럽트), **PPI**(Private Peripheral Interrupt — 코어마다 따로 있는 로컬 인터럽트), **SPI**(Shared Peripheral Interrupt — 여러 코어가 공유하는 일반 주변장치 인터럽트), **LPI**(Locality-specific Peripheral Interrupt — PCIe 등이 메모리 쓰기로 보내는 MSI 방식 인터럽트). **FIQ**(Fast Interrupt — 보안/우선 인터럽트용으로 IRQ 와 구분되는 별도 신호선).
...
- GICv3 는 **MPIDR**(Multiprocessor Affinity Register — 각 코어의 고유 식별 번호를 담은 레지스터) (Affinity 0~3 — 코어를 클러스터·소켓 등 계층으로 구분하는 좌표) 로 코어를 식별.
...
1. **INTID 별 Group 을 두 비트가 결정**. GIC는 인터럽트 번호(**INTID** — Interrupt IDentifier, 각 인터럽트에 붙은 고유 번호)마다 두 개의 비트 배열을 둡니다 — `GICD_IGROUPRn` (또는 SGI/PPI는 GICR쪽)의 해당 비트 = **Group 1인가 0인가**, `GICD_IGRPMODRn`의 해당 비트 = **Secure인가 NS인가**(group modifier). 이 둘을 조합해 INTID마다 {Group 0, Group 1 Secure, Group 1 Non-secure}가 정해집니다(표의 3종이 이 두 비트의 조합).
...
1. DMA Master 가 트랜잭션 발행 (VA 또는 **IOVA** — IO Virtual Address, 디바이스가 보는 가상 주소로 실제 PA 로 번역되어야 함).
2. SMMU 가 Stream ID 로 디바이스 식별 (Stream ID = 어떤 Master 가 보냈는지, HW 적으로 결정).
3. Stream ID → Context 매핑: 각 디바이스 (Stream) 에 독립된 페이지 테이블 할당.
4. Stage 1: IOVA → **IPA**(Intermediate Physical Address, 중간 물리 주소 — 한 번 더 번역되는 가짜 물리 주소) (디바이스 드라이버가 설정) / Stage 2: IPA → PA (Hypervisor 가 설정).
...
아래에서 몇 개념이 등장합니다 — **캐시**(자주 쓰는 메모리 값을 CPU 옆에 임시 보관하는 빠른 저장소), **TLB**(Translation Lookaside Buffer — 주소 번역 결과를 캐싱하는 작은 표), **VMID**(Virtual Machine IDentifier — TLB 엔트리가 어느 VM 것인지 구분하는 번호), **사이드 채널 공격**(데이터를 직접 읽지 않고 실행 시간·캐시 흔적 등 부수 정보로 비밀을 추론하는 공격; **Prime+Probe**·**Spectre** 가 그 예), **투기적 실행**(speculative execution — 분기 결과를 미리 추측해 앞질러 실행하는 성능 기법, Spectre 의 빌미).
...
- **①→② 사이**: **ISB**(Instruction Synchronization Barrier — 이 지점 이후 명령을 새 상태로 다시 가져오게 강제하는 동기화 명령) 같은 context-synchronizing 동작이 없으면, CPU가 이미 _앞질러 페치/투기 실행_ 해 둔 후속 명령이 _옛 NS 상태_ 로 동작할 수 있습니다([Module 03](../03_secure_boot_connection/)의 "ISB 필요 이유"와 같은 뿌리). 그래서 SCR_EL3.NS write 직후 ISB로 동기화 경계를 박습니다.
- **②→③→④ 사이 (회로적 race)**: 동기화가 됐더라도, "PE의 NS 상태"라는 _내부 상태_ 가 메모리 접근의 `AxPROT[1]` _버스 신호_ 로 구동되기까지 파이프라인·**LSU**(Load/Store Unit — 메모리 읽기/쓰기 명령을 실제 버스 트랜잭션으로 내보내는 회로)·인터커넥트(칩 안에서 부품들을 잇는 버스 망)의 단계를 거칩니다. 만약 어느 단계의 래치가 NS 변경을 한 사이클 늦게 반영하면, 그 한 사이클 동안 _새 세계의 명령이 옛 NS attribute로_ (또는 그 반대로) 트랜잭션을 발행하는 window가 생깁니다 — 예: Secure→NS로 막 바뀌었는데 첫 트랜잭션이 아직 `AxPROT[1]=0`(Secure)로 나가면, NS 코드가 Secure 권한으로 한 번 접근하는 틈이 됩니다.
...
레지스터 X0~X7 로 파라미터를 전달합니다. 최대 8 × 64 bit = 64 bytes 이므로 소량 요청/응답에 적합하고, 별도 메모리 공유가 없어 **TOCTOU**(Time-Of-Check to Time-Of-Use — 검사한 시점과 실제 사용 시점 사이에 값이 바뀌어 검사를 무력화하는 취약점) 위험이 없습니다.
...
:::caution[실무 주의점 — SMC 후 register save/restore 부족으로 secure state 누설]
...
:::caution[실무 주의점]
- BL31 의 context list 가 ARM revision 마다 (NEON, SVE, **MTE**(Memory Tagging Extension — 메모리에 태그를 붙여 잘못된 포인터 접근을 잡는 확장), **MPAM**(Memory System Resource Partitioning and Monitoring — 캐시·대역폭을 작업별로 나눠 주는 확장) 등 추가) 늘어납니다 — 새 ISA extension 추가 시 _BL31 context 코드 업데이트_ 가 필수 체크 항목.
...
:::tip[🤔 Q1 — 5 컴포넌트 누락 진단 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — SMC handler 검증 (Bloom: Apply)]
...
→ [Module 02A — Secure Enclave & TEE Hierarchy](../02a_secure_enclave_and_tee_hierarchy/): TrustZone 의 한계 (cache 부채널, Trusted OS 취약점) 를 어떻게 별도 processor + 전용 RAM 으로 보완하는가, 그리고 둘이 _상호 불신_ 관계로 공존하는 모델.

[퀴즈 풀어보기 →](../quiz/02_world_switch_soc_infra_quiz/)
