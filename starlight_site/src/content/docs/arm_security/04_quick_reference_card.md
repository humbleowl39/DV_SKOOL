...
:::
:::note[사전 지식]
- [Module 01-03](../01_exception_level_trustzone/) (모든 학습 모듈 완료 후 사용)
:::
---

## 1. Why care? — 이 카드가 왜 필요한가

면접/리뷰/디버그의 _압축된 시간_ 에는 4 개 모듈을 다 펼쳐볼 수 없습니다. 그런데 실제로 디버그를 할 때 필요한 것은 "이 증상이 나오면 어느 표를 봐야 하는가" 라는 라우팅 직관입니다. NS world 가 Secure DRAM 을 읽었다면 TZASC 와 NS attribute 전파를 보고, GPU DMA 가 Secure 영역에 닿았다면 SMMU stream id 매핑을 의심하며, SMC 후 secure 레지스터 잔여 값이 보이면 BL31 context save 목록을 확인합니다. 이 라우팅이 즉시 나오려면 _질문 한 줄_ → _답 한 줄_ 의 거리를 좁히는 5 개 표 — EL/NS 매트릭스, 5 축 인프라, world switch 흐름, boot stage 매핑, anti-rollback — 가 머릿속에 정렬돼 있어야 합니다.

이 카드는 그 정렬된 형태를 _그대로_ 옮긴 것입니다. Module 01-03 에서 다룬 모든 구조를 _질문 패턴 별_ 로 다시 묶어, 실무 상황 (debug, review, interview) 에서 즉시 호출 가능하게 만든 것이 이 모듈의 본질입니다.
...
:::caution[실무 주의점 — Stage 2 translation 미설정으로 hypervisor 격리 실패]
...
:::
:::tip[🤔 Q2 — 5 축 디버그 매핑 (Bloom: Analyze)]
...
[퀴즈](../quiz/) · [용어집](../glossary/) · 다음: [Secure Boot](../../soc_secure_boot/), [Virtualization](../../virtualization/) (EL2 hypervisor).
