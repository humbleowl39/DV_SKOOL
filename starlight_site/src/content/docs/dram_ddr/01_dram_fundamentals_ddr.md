---
title: "Module 01 — DRAM Fundamentals + DDR4/5"
---
...
**Q: On-die ECC란?**
> "DDR5부터 DRAM 칩 내부에서 단일 비트 에러를 자동 수정하는 기능이다. 외부(MC/호스트)에서는 이 ECC 동작이 투명하다. DDR4에서는 ECC를 위해 72-bit DIMM(64 data + 8 ECC)이 필요했지만, DDR5는 On-die ECC가 기본 포함되어 별도 ECC DIMM 없이도 기본적인 에러 보호가 가능하다. 단, On-die ECC는 한 번에 일정 크기의 워드 단위(보통 128-bit 급, 정확한 폭은 벤더/세대별로 다름 — 확인 필요)로 묶어 그 안의 1-bit만 수정하므로, 서버 환경에서는 여전히 외부 SECDED ECC가 필요하다. **왜 비트마다가 아니라 워드 단위로 보호하는가** — SECDED 류의 패리티는 보호 워드가 작을수록 데이터 대비 패리티 비트의 비율(오버헤드)이 커진다. 예컨대 같은 1-bit 정정 능력을 얻는 데 필요한 패리티 비트 수는 워드 폭에 로그적으로만 늘어나므로, 워드를 넓게 묶을수록 bit당 저장·면적 오버헤드가 줄어든다. DRAM 은 면적이 곧 원가이므로, 정정 능력은 유지하면서 오버헤드를 낮추는 적당한 워드 폭을 골라 그 단위로 ECC를 거는 것이다."
