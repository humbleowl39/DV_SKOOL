---
title: "Module 04 — DRAM DV Methodology"
---
...
이유는 DRAM 의 구조적 특수성에서 나옵니다. tRC, tRCD, tRP, tRAS, tFAW 등 수십 개의 timing parameter 가 존재하고, 이 중 하나라도 특정 명령 조합에서 위반되면 실리콘에서 간헐적 fail 이 발생합니다. 더 어려운 것은 이 fail 이 silent — 장치는 동작하는 것처럼 보이지만 특정 address 의 데이터가 조용히 손상됩니다. ECC injection, refresh 누락, training 실패 복구 같은 stateful 시나리오는 무작위 입출력 테스트로는 재현조차 되지 않습니다. 2014 년에 알려진 Rowhammer 공격이 대표적인 사례입니다 — refresh interval 이 허용 범위 내라고 표시되지만 공격적으로 반복 접근하면 이웃 row 의 전하가 누설되어 비트 플립이 발생했고, 이 취약점이 발견되기까지 수년이 걸렸습니다.
...
ECC 검증의 목적은 단순히 "에러가 수정되는가" 를 확인하는 것이 아닙니다. On-die ECC 가 투명하게 동작하는지(외부에서 에러가 보이지 않는지), On-die ECC 가 커버할 수 없는 multi-bit 에러에서 외부 SECDED 가 올바르게 인터럽트를 발생시키는지, CA parity 에러가 주입됐을 때 MC 가 재발행 또는 에러 보고를 수행하는지 — 이 세 경로를 독립적으로 검증해야 합니다. 또한 MC 의 ECC Scrubbing 이 주기적으로 모든 주소를 순회하며 축적된 soft error 를 교정하는지, 그리고 Scrubbing 과 정상 트래픽이 경합할 때 순서가 올바른지도 확인 대상입니다.
