...
2. **Data integrity**: MC가 write한 데이터가 read 시 정확히 돌아오는지 확인한다. 여기에는 ECC 경로도 포함되므로 1-bit error injection 후 자동 수정, 2-bit error detection 같은 시나리오를 명시적으로 테스트해야 한다. LPDDR5에서는 셀 내부를 보호하는 **On-die ECC**와 DQ 전송경로를 보호하는 **Link ECC**(LPDDR5 고유)가 직교적으로 존재하므로, 두 경로에 각각 fault를 주입하는 시나리오를 분리해 검증한다.
...
- **(d) Behavioral Model error injection**: model에 1-bit fault를 심은 뒤 MC/DRAM이 SECDED ECC로 자동 수정해 원본 데이터를 돌려보내는지 확인한다. 수정 후 data가 원본과 일치하면 ECC 경로가 올바르게 동작하는 것이다. LPDDR5는 셀 fault(On-die ECC)와 DQ 라인 fault(Link ECC)를 구분해 주입하여 각 경로를 독립적으로 검증한다.
