...
DQS 는 DDR 데이터 버스의 "스트로브" 신호입니다 — DQ 가 언제 유효한지를 알려 주는 기준 클럭의 역할을 합니다. Write 경로에서는 MC 가 DQS 를 DQ 와 center-aligned 로 정렬하여 전송하고, DRAM 은 DQS 에지에서 DQ 를 샘플링합니다. Read 경로에서는 DRAM 이 DQS 를 DQ 와 edge-aligned 로 내보내고, MC/PHY 는 DQS 를 90° 지연시켜 DQ 의 중앙에서 샘플링합니다. Write 와 Read 의 DQS-DQ 관계가 서로 다르다는 점이 Training 을 방향별로 구분해야 하는 이유입니다.
...
고속 신호는 전송선 끝에서 임피던스 불일치를 만나면 반사가 발생하고, 그 반사파가 원래 신호에 간섭하여 데이터 오류로 이어집니다. 이 문제를 해결하려면 전송선의 임피던스와 동일한 저항으로 종단해야 하는데, DDR2 이전에는 PCB 에 외부 저항을 실장했습니다. DDR2 이후부터 DRAM 칩 내부에 이 종단 저항을 내장한 것이 ODT 입니다. ODT 를 내장하면 외부 부품이 줄어 PCB 가 간결해지고, 신호 경로가 짧아져 신호 품질도 향상됩니다. Multi-Rank 구성에서는 타겟 Rank 가 데이터를 구동하는 동안 비타겟 Rank 의 RTT_PARK(park termination) 를 활성화하는 것이 특히 중요합니다 — 비타겟이 floating 이면 그쪽에서 반사가 발생해 타겟의 eye 가 닫히기 때문입니다.
...
DDR5 4800 MT/s 이상에서는 비트 간격이 약 208 ps 로 극히 좁아집니다. 이 좁은 간격 때문에 이전 비트의 잔여 신호가 현재 비트와 겹치는 ISI(Inter-Symbol Interference) 가 심각해져, 수신단의 eye 다이어그램이 거의 닫힙니다. 이를 보상하지 않으면 데이터를 올바르게 샘플링할 수 없습니다. Equalization 은 이 eye 를 복원하는 기법입니다. CTLE 는 아날로그 필터로 채널의 고주파 감쇠를 보상하지만 노이즈도 함께 증폭한다는 단점이 있습니다. DFE 는 이미 결정된 이전 비트들의 ISI 기여분을 디지털로 빼는 방식이라 노이즈를 증폭하지 않으며, CTLE 와 상호 보완적으로 동작합니다. DDR5 에서 두 기법을 함께 사용하는 이유가 바로 이 상보 관계 때문입니다.
...
DDR4 3200 MT/s 의 데이터 유효 윈도우는 ~312 ps, DDR5 4800 MT/s 에서는 ~208 ps 입니다. 이 좁은 윈도우 안에서 정확히 샘플링해야 데이터가 정상입니다. 그런데 PCB 배선 길이는 byte lane 마다 mm 단위로 다르고, 온도와 전압 변동은 트랜지스터의 속도를 바꾸며, 크로스토크는 신호를 왜곡합니다. 이런 변수들이 모두 겹치면 고정된 타이밍 설정으로는 모든 lane 에서 윈도우 중앙을 찌르는 것이 불가능합니다. Training 은 이 문제를 "직접 측정해서 학습하는" 방식으로 해결합니다. Write Leveling, DQ Training, Eye Training, VREF Training 을 순서대로 수행하면 chip-specific, lane-specific, 심지어 온도-specific 한 최적 타이밍 값을 찾아 PHY 레지스터에 기록합니다.
...
DDR4 의 명령 버스는 RAS#, CAS#, WE# 같은 개별 핀 방식이라 속도가 상대적으로 낮았고 타이밍 마진이 충분했습니다. 그러나 DDR5 에서는 명령·주소를 CA[13:0] 14개 핀으로 멀티플렉싱하여 클럭 속도에 동기화하면서 CA 버스의 타이밍 마진이 크게 줄었습니다. 타이밍이 맞지 않으면 잘못된 명령이 DRAM 에 전달되고, 그 결과는 치명적인 오동작이 됩니다. CA Training(또는 CS Training) 은 MC 가 알려진 CA 패턴을 전송하고 DRAM 이 샘플링 결과를 DQ 로 돌려보내는 피드백 루프로 CA-CK 간 최적 정렬 지점을 찾는 절차입니다. LPDDR5 에서는 같은 개념을 CBT(Command Bus Training) 라 부릅니다.
