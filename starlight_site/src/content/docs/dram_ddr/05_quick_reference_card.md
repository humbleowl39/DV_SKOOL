---
title: "Module 05 — LPDDR5 Quick Reference Card"
---
...
- **Apply** LPDDR5 핵심 요약 표(vs 이전 세대 LPDDR4)를 면접 / 코드 리뷰 / 디버그에서 5초 이내 펼쳐 답변 골격을 구성한다.
- **Justify** MC / MI / BootROM / UFS 프로젝트 경험을 카드의 "이력서 연결" 와 정렬하여 진술한다 (실제 메모리 세대는 본인 프로젝트에 맞게 치환).
:::
:::note[사전 지식]
- [Module 01 — DRAM Fundamentals](../01_dram_fundamentals_ddr/) ~ [Module 04 — DRAM DV 방법론](../04_dram_dv_methodology/) 완주 권장
...
DRAM 4 모듈을 끝까지 읽으면 머릿속에 _세 묶음_ 이 남습니다. 첫째는 명령 시퀀스 `ACT → RD/WR → PRE → REF` 라는 stateful flow 이고, 둘째는 `tRCD / tCL / tRP / tRAS / tRC / tFAW / tRFC / tREFI` 라는 timing 묶음이며, 셋째는 **LPDDR5 의 정체성 축**(CK/WCK 분리 / bank mode(BG/8B/16B) / CBT·WCK2CK training / DVFSC gear / PASR / On-die+Link ECC)과 그에 대한 비교축(vs 이전 세대 LPDDR4)입니다. 실무와 면접에서는 이 세 묶음 중 어느 묶음의 어느 행을 5초 안에 꺼낼 수 있느냐가 숙련도를 가릅니다.
...
### 3.1 시나리오 A — Timing checker 가 `tRCD violation @ T=12345ns` 보고

```
   ┌─ 증상 ─────────────────────────────────────────────────┐
   │  Timing checker: ACT 후 RD 가 tRCD(ns) 최소값 전에 도착 │
   │  같은 (rank, bank) 에서 발생, LPDDR5 환경 (특정 gear)   │
   ...
   │                                                         │
   │  §5.5 면접 골든 룰 #8 "tCL-tRCD-tRP" 표기 (gear 의존)  │
   └────────────────────────────────────────────────────────┘
            │
            ▼
   ┌─ 본문으로 점프할 위치 ─────────────────────────────────┐
   ...
   │  Module 04 §5.7 →  MC 레벨 timing checker (절차형 SV)  │
   └────────────────────────────────────────────────────────┘
```

:::note[MC/서브시스템 레벨 timing checker 는 SVA 가 아니라 절차형 SV]
LPDDR5 timing 은 DVFSC gear 에 따라 tCK·WCK:CK 가 **런타임에 바뀌므로**, cycle-delay 연산자(`##N` / `[*N]`)의 피연산자가 elaboration-time 상수여야 하는 SVA 로는 고정 `##tRCD` 가 gear 전환을 따라가지 못합니다. 게다가 DRAM timing 은 본질적으로 절대 ns 로 규정됩니다. 따라서 MC/서브시스템 레벨의 **주 timing checker 는 절차형 SV** 로 짭니다 — 각 명령 이벤트마다 `$realtime` 을 기록하고, 경과 ns 가 현재 gear 의 스펙 ns 최소값 이상인지 assert 하는 gear-aware 검사입니다. SVA 는 **보완 역할** — 고정 클럭 도메인의 edge 관계와 cover property 에 씁니다. 자세한 timing 체크 구현은 [Module 04 §5.7 (LPDDR5 타이밍 체크)](../04_dram_dv_methodology/) 참조.
:::
...
| ④ | 본문 | Module 04 §5.7 (절차형 SV timing checker) 로 점프 | gear-aware ns 검사 로직 + 보완용 SVA cover 매핑 확인 |
...
```
   ...
   │                                                         │
   │  §5.6 "추가 핵심 용어" → tWTR, tRTW (R/W turnaround)   │
   └────────────────────────────────────────────────────────┘
            │
            ▼
   ┌─ 본문으로 점프할 위치 ─────────────────────────────────┐
...
```
   ┌─ 증상 ─────────────────────────────────────────────────┐
   │  일정 주기(tREFI)마다 수백 ns 동안 bank read 가 stall  │
   │  effective BW 가 평균 ~5% 감소, P99.9 latency 가 +1 µs │
   │  per-bank REF(REFpb) 활용 후 같은 워크로드에서 폭 감소  │
   └────────────────────────────────────────────────────────┘
            │
            ▼
   ┌─ 카드의 어느 표를 펼치는가 ────────────────────────────┐
   │  §5.2 "핵심 타이밍" → tREFI / tRFC                      │
   │  → tREFI = 3.9 µs (LPDDR5), tRFC 는 ns·밀도 의존        │
   │  → tRFC = REF → ACT 사이 (ns 단위, 단일 고정값 아님)   │
   │                                                         │
   │  §5.1 "핵심 정리" → "per-bank REF/PASR" 행              │
   │  → LPDDR5 per-bank(REFpb) + PASR ← 핵심                 │
   │                                                         │
   │  §5.5 면접 골든 룰 — All-bank vs per-bank REF 차이      │
   └────────────────────────────────────────────────────────┘
            │
            ▼
   ┌─ 본문으로 점프할 위치 ─────────────────────────────────┐
   │  Module 01 §4  →  Refresh 의 물리 (capacitor leak)      │
   │  Module 02 §6  →  Refresh 관리 (all-bank vs per-bank)   │
...
| ② | 카드 | LPDDR5 per-bank REF(REFpb) + PASR 행 회수 | _stall 폭이 줄어든 이유_ 진술 |
| ③ | 본문 | Module 02 §6 로 점프 | MC 가 per-bank vs all-bank REF 모드 선택 로직 |
...
| **세대/세그먼트축** | §5.4 LPDDR5 핵심 요약(vs LPDDR4), §5.6 추가 용어 | LPDDR5 정체성(WCK, bank mode, gear, PASR, Link ECC) + 이전 세대 대비 _변화_ 와 _이유_ |
| **검증/스토리축** | §5.7 실무 주의, §5.8 이력서 연결, §5.9 Samsung 위치 | 자기 경험 ↔ 면접 follow-up 매핑 |

면접에서는 _물리축 → 세대축 → 검증축_ 순으로 답변이 흐르면 면접관의 follow-up 이 쉬워집니다. 카드는 이 3-hop 을 한 페이지에 압축한 것. 예를 들어 "LPDDR5 의 핵심 특징?" 이라는 질문이 오면:

| ① 질문 도착 (~30초) | ② 카드 펼침 (~5초) | ③ 본문 점프 (~1–2분) |
|---|---|---|
| "LPDDR5 의 핵심 특징?" | §5.1 핵심 정리 + §5.4 vs LPDDR4 요약 표 | Module 01 §6 (세대 변화), Module 02 §6 (per-bank REF) |
...
- **MI/PHY** = LPDDR5 전기 신호 변환 + Training(타이밍 캘리브레이션)
...
| Prefetch | 내부 저속 → I/O 고속 차이 해결. LPDDR5: 16n(BL16/BL32) / 이전 세대 LPDDR4: 16n(BL16/BL32) |
| 클럭 (LPDDR5) | CK(명령용 저속, 차동) + WCK(데이터용 고속), WCK:CK = 2:1 또는 4:1 (gear 의존) |
| Bank Group | I/O 회로 공유 → 같은 BG: tCCD_L(느림), 다른 BG: tCCD_S(빠름) |
| LPDDR5 vs LPDDR4 | WCK(데이터 클럭 분리)·DVFSC gear·Link ECC = LPDDR5 추가 / LPDDR4 = BG 없는 8뱅크, WCK 없음 |
...
| Strobe / WCK | LPDDR5: WCK 기반 데이터 strobe (CK 와 분리) / 이전 세대 LPDDR4: DQS 기반 |
| ODT | 신호 반사 방지, RTT_NOM/RTT_WR/RTT_PARK, Multi-Rank에서 비타겟 Rank 중요 |
| DBI | 데이터 반전으로 스위칭 전력 절감, LPDDR5 지원 |
...
아래 값은 대부분 "두 명령 사이에 최소한 기다려야 하는 간격"입니다 — `t` 는 time, 이름은 "무엇→무엇" 사이를 가리킵니다(예: tRCD = ACT→RD 사이). 명령 약어는 ACT=Row 열기, RD/WR=읽기/쓰기, PRE=Row 닫기, REF=refresh. cycle 수(tCL/tRCD 등)는 tCK 단위이며 **gear(DVFSC) 의존** 이고, tRFC/tREFI 는 ns 단위로 규정됩니다.
...
| tREFI | REF 명령 평균 간격 (LPDDR5 = 3.9 µs) |
...
| REF | Refresh (데이터 보존) — LPDDR5: per-bank(REFpb) / all-bank |
| MRS | Mode Register Set (설정) — LPDDR5 bank mode/gear 등 선택 |
| MRR | Mode Register Read (직접 MR 읽기 — LPDDR4 부터 도입) |
| ZQ | 임피던스 캘리브레이션 |

LPDDR5 의 CA[6:0] 는 단일종단 다중사이클로 전송되므로 **CBT(Command Bus Training) 가 필수** 입니다.

### 5.4 LPDDR5 핵심 요약 (vs LPDDR4)

LPDDR5 를 이전 세대 LPDDR4 와 나란히 놓으면 "무엇이 새로 생겼는가" 가 한눈에 보입니다. 아래 값 중 cycle 수는 gear 의존이므로 표에는 정성적 특징 위주로 정리합니다.

| 축 | LPDDR5 (주축) | LPDDR4 (이전 세대) |
|---|---|---|
| 클럭 | CK + **WCK 분리** (WCK:CK = 2:1/4:1, gear 의존) | 단일 CK + DQS (WCK 없음) |
| 뱅크 구성 | MR 선택: BG(16) / 8B(8) / 16B(16) | BG 없는 8뱅크 |
| Prefetch/Burst | 16n, BL16/BL32 | 16n, BL16/BL32 |
| Refresh | per-bank(REFpb) + all-bank + **PASR** | all-bank + per-bank + PASR |
| tREFI | 3.9 µs (예시, 온도/밀도 의존) | 3.9 µs (예시, 온도/밀도 의존) |
| ECC | On-die(cell) + **Link ECC**(DQ link, 신규) | On-die(cell) 중심 |
| Training | **CBT + WCK2CK** + DQ/Write/Read | CBT + DQ/Write/Read (WCK2CK 없음) |
| 전력/주파수 | **DVFSC gear** (F0~F4, 동적 전압/주파수) | 고정 클럭 (동적 gear 없음) |
| 전압 | VDD2 ~1.05 V, VDDQ 0.5 V (예시) | VDDQ 0.6 V (LPDDR4X, 예시) |

- **LPDDR5 가 이전 세대(LPDDR4) 대비 추가한 것**: WCK(데이터 전용 고속 클럭) 분리, WCK2CK leveling, DVFSC gear, Link ECC, BG(bank group) 도입. Prefetch(16n)·PASR·CBT 는 두 세대 공통.
- **주의**: 위 전압·tREFI 는 대표 예시값이며, cycle 수(tRCD/tCL 등)는 gear 의존이라 절대 spec 값으로 인용하지 마세요.

### 5.5 면접 골든 룰

1. **WCK 분리 (LPDDR5)**: "명령(CK, 저속 차동)/데이터(WCK, 고속) 클럭 분리 → 데이터만 고속, 전력 절감. WCK:CK = 2:1 또는 4:1 (gear 의존)"
2. **Bank Mode (LPDDR5)**: "MR 로 BG 모드(4 BG×4=16뱅크)/8B 모드(8뱅크)/16B 모드(16뱅크) 선택 — 이전 세대 LPDDR4 는 BG 없는 8뱅크"
3. **CBT / WCK2CK Training (LPDDR5)**: "CA[6:0] 다중사이클 → CBT 필수, WCK2CK leveling 으로 WCK·CK 위상 정렬. WCK2CK 단계는 WCK 를 도입한 LPDDR5 에서 새로 추가된 것"
4. **DVFSC (LPDDR5)**: "동적 전압/주파수 gear(F0~F4) — gear 전환 시 WCK:CK 변경 → WCK2CK 재정렬 후에만 데이터 발행. 이 런타임 gear 변화 때문에 MC 레벨 timing 검사는 고정 cycle SVA 가 아니라 ns 기준 절차형 SV 로 해야 함"
5. **PASR (LPDDR5)**: "부분배열 Self-Refresh — 필요한 segment 만 보존, 나머지 비보존으로 저전력. LPDDR 계열 저전력 기능(LPDDR4/5 공통)"
6. **On-die vs Link ECC**: "On-die = cell 내부 비트 정정(투명). Link ECC = DQ 전송경로 보호(LPDDR5 신규). 둘은 직교 — 보호 대상이 다름"
...
| WCK2CK | WCK–CK 위상 정렬(leveling) — LPDDR5 신규, gear 전환 시 재정렬 |
...
| PASR | Partial Array Self-Refresh (LPDDR 계열 부분배열 보존) |
| Link ECC | DQ 전송경로 ECC — LPDDR5 신규, On-die ECC(cell)와 직교 |
| On-die ECC | cell array 내부 비트 정정(투명) — 디바이스 의존 |
| MRR | Mode Register Read (LPDDR4 부터 도입) |
...
:::caution[실무 주의점 — tFAW + Bank conflict 동시 발생 시 throughput cliff]
...
| LPDDR5 프로토콜 | "LPDDR5 의 핵심 특징은? (이전 세대 대비)" | WCK 분리, bank mode, CBT/WCK2CK, DVFSC gear, PASR, On-die+Link ECC |
...
위 표는 LPDDR5 주축 예시입니다. 본인 프로젝트의 **실제 메모리 세대와 실제 chip / measurement** 로 치환해서 진술하세요 — 그대로 인용하면 면접관이 즉시 감지합니다.
:::
...
|---|---|---|
| 면접관이 "LPDDR5 핵심 특징? (vs 이전 세대)" 30초 안에 답해야 | §5.4 요약 표 + §5.5 #1~#6 | Module 01 §6 |
| Timing checker: `tRCD violation @ T=...` | §5.2 tRCD + §5.1 Row Hit/Miss 행 | Module 01 §3, Module 04 §5.7 |
...
:::
:::danger[❓ 오해 2 — 'LPDDR5 가 이전 세대보다 모든 게 빠르다']
**실제**: LPDDR5 는 WCK 분리로 _데이터 전송률(bandwidth)_ 이 올라가지만, _CAS latency (절대 ns)_ 자체는 크게 줄지 않습니다. 작은 random access 가 많은 워크로드에서는 대역폭 이득이 잘 드러나지 않습니다. LPDDR5 의 실질 이득은 _WCK 데이터 클럭 분리 + BG(bank group) 도입_ 으로 parallelism 과 전송 효율이 올라가는 부분.<br>
**왜 헷갈리는가**: 데이터율(MT/s) 숫자가 커져서 "그만큼 latency 도 빨라진다" 로 단순화.
:::
:::danger[❓ 오해 3 — 'Refresh 는 무시할 만한 오버헤드다']
**실제**: tRFC × (1 / tREFI) ≈ 4~7% 의 시간이 refresh 에 사용. tail latency 에서는 refresh 가 P99.9 의 _주요 원인_. LPDDR5 의 per-bank refresh(REFpb) 가 _이 비용을 분산_ 해서 stall 폭을 줄이는 게 핵심 가치.<br>
...
**실제**: On-die ECC 는 _DRAM cell array 내부의 soft error_ 만 정정(투명). DQ 전송경로(link) 위의 비트 에러는 별도 보호가 필요 — LPDDR5 는 이를 위한 **Link ECC** 가 표준에 있으며(이전 세대 LPDDR4 대비 신규), On-die ECC 와 _직교_(보호 대상이 다름).<br>
...
|---|---|---|
| `tRCD violation` timing checker fail | 같은 bank 에 ACT→RD 너무 빠름 | §5.2 tRCD → Module 02 §3 (issue 로직) |
| 평균 BW 정상, 특정 패턴만 50% | tFAW window + bank conflict | §5.7 박스 → Module 02 §4 |
| Periodic stall ~tREFI(3.9 µs) 주기 | REF 처리 (all-bank) | §5.2 tREFI / tRFC → Module 02 §6 |
...
- **LPDDR5 정체성** — CK/WCK 분리, bank mode(BG/8B/16B), CBT·WCK2CK training, DVFSC gear, PASR, On-die+Link ECC. 이것이 이전 세대(LPDDR4) 대비 핵심 변화.
...
- LPDDR5 와 이전 세대(LPDDR4) 를 _섞어_ 인용하면 (예: WCK + LPDDR4 뱅크 구성) 면접관이 한 번에 신뢰 잃음. §5.4 표의 한 컬럼 안에서만 답.
:::
### 7.1 자가 점검

:::tip[🤔 Q1 — 3 결말 즉답 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — LPDDR5 의 진짜 이득 (Bloom: Evaluate)]
"LPDDR5 는 이전 세대보다 _대역폭이 훨씬 크다_" 라는 답변의 문제?
<details>
<summary>정답</summary>

대역폭은 _이론치_, 실측 이득은 전송 효율 + parallelism:
- **이론**: WCK 데이터 클럭 분리로 데이터율(MT/s) 이 크게 상승.
- **실측**: Random access pattern (DV workload) 에서는 bank conflict 가 bottleneck → 이론 대역폭의 일부만 사용.
- LPDDR5 의 _진짜_ 이득: WCK 분리로 명령/데이터 클럭 독립 + BG(bank group) 도입 → parallelism 상승, 전력 효율(DVFSC) 개선.
- 결론: bandwidth 답변 → "왜 더 빠른가" follow-up 시 WCK 분리 + BG parallelism 으로 확장.
...
**External**
- JEDEC JESD209-5 *LPDDR5/LPDDR5X SDRAM Specification*
