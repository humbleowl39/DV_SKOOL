...
- [Module 06 — Error Handling Path](../06_error_handling_path/) (`expected_error`, `setErrState`, `enable_error_cq_poll`)
- [Module 03 — Phase & Test Flow](../03_phase_test_flow/) (`try_once` 단발 폴링 패턴)
:::
:::caution[실무 주의점]
...
당신의 RDMA test 가 _unexpected error CQE_(에러를 알리는 완료 항목 — CQE 의 결과 상태가 success 가 아닌 경우):
```
WC status = WC_REM_ACCESS_ERR
QP = 3, opcode = WRITE
```

용어: **CQE**(Completion Queue Entry — 작업 완료 항목), **wc_status**(work completion status — CQE 에 담긴 결과 코드; success 인지 어떤 에러인지), **rkey**(원격 접근 열쇠) / **lkey**(로컬 접근 열쇠 — MR 등록 시 받는 두 권한 키), **promote**(여기서는 "이 에러는 의도된 것"이라고 미리 표시해 fatal 을 면제시키는 것).
...
`wc_status` 를 보면 진단 방향이 갈립니다. `WC_RETRY_EXC_ERR` 계열 (12, 13 — RETRY_EXC = 재전송 한도 초과, RNR_RETRY_EXC = "수신 준비 안 됨(Receiver Not Ready)" 재시도 한도 초과) 은 network 또는 timing 문제로 DUT 의 성능 한계를 나타내는 신호입니다. 반면 그 외의 모든 에러 코드는 대부분 TB sequence 오류이거나 promote 누락입니다. 따라서 wc_status 확인이 가장 먼저 해야 할 분기점입니다.

RDMA-TB 의 가장 강한 invariant(불변 조건 — 시뮬 내내 항상 참이어야 하는 규칙) 중 하나는 **"RETRY_EXC 계열을 제외한 모든 에러 CQE 는 정상 시뮬레이션에서 발생하면 안 된다"** 입니다. 이 invariant 가 깨지면 곧 DUT 버그이거나 TB 시퀀스가 잘못된 lkey/rkey/length 를 발행한 것 — 어느 쪽이든 **사람의 결정** 이 필요합니다. 그래서 TB 는 `F-CQHDL-TBERR-0003` 으로 fatal 을 띄워 시뮬을 멈춥니다.
...
```
   ...
   Step 4   DUT 측 rkey 검증 로직 추적
            ...
            fsdb 에서 DUT 의 RX RDMA READ 처리:
              ▶ RETH(RDMA Extended Transport Header — Write/Read 패킷에 붙어 원격 주소·길이·rkey 를 담는 헤더).RKey = 0x12345678 받음 OK
              ▶ DUT 의 MR table lookup → 0x12345678 매칭 OK
              ▶ DUT 의 access flag(MR 에 허용된 접근 종류 비트 — IBV_ACCESS_LOCAL_WRITE / REMOTE_READ / REMOTE_WRITE / REMOTE_ATOMIC) 검사 → IBV_ACCESS_REMOTE_READ 비트 == 0 !
...
이 chained 정리 메커니즘이 [Module 06](../06_error_handling_path/) 의 ErrQP 게이트.
...
:::
:::tip[🤔 Q2 — RETRY 폭주 (Bloom: Evaluate)]
...
- Network 의 _drop pattern_ (incast = 여러 송신자가 한 수신자로 동시에 몰려 버퍼가 넘침; PFC storm = Priority Flow Control 정지 신호가 연쇄 확산되는 현상).
- Receiver 의 _RNR_ (Receiver Not Ready — recv WQE 부족으로 받을 준비가 안 됨).
- DUT 의 _ACK 안 보냄_.
...
→ [Module 12 — Debug Cheatsheet](../12_debug_cheatsheet/): 4 개 디버그 케이스 (M08–M11) 의 통합 cheatsheet — 에러 ID → 모듈 → 첫 액션.

[퀴즈 풀어보기 →](../quiz/11_debug_unexpected_err_cqe_quiz/)
