...
:::
:::note[사전 지식]
- [Module 04 — Analysis Port Topology](../04_analysis_port_topology/) (comparator 가 구독하는 AP)
- [Module 06 — Error Handling Path](../06_error_handling_path/) (ErrQP 게이트가 검증 정합성에 미치는 영향)
- [Module 07 — H2C/C2H QID Reference](../07_h2c_c2h_qid_map/) (HW 인터페이스 단계 디버깅 도구)
:::
---
...
**byte 1024 = MTU**(Maximum Transmission Unit — 패킷 하나에 담을 수 있는 최대 페이로드 크기) **boundary**. 즉:
- Byte 0-1023: OK → 첫 packet 정상.
- Byte 1024+: corruption → 두 번째 packet 또는 그 이후.

가설: **WRITE_MIDDLE packet 처리 bug** 또는 **MR**(Memory Region — RDMA 가 직접 접근하도록 등록한 메모리 영역) **의 page boundary**(메모리를 일정 크기(예 4KB) 단위 페이지로 나눈 경계) **처리 (1024 = 1 page 일 가능성)** .
...
Data mismatch 는 RDMA 검증의 가장 빈번하고 까다로운 실패입니다. 원인이 SW 설정 (MR/QP/IOVA), HW 인터페이스 (C2H/H2C — 카드↔호스트 DMA 방향), DUT 데이터 경로 모두에 있을 수 있어 단계적 분리 (triangulation — 여러 지점의 정보를 교차해 원인 위치를 좁히는 것) 가 필수입니다.
...
```
   ...
   Step 4   DUT 의 PTE walk 추적
            (PTE = Page Table Entry, 페이지 테이블의 한 항목; PTW = Page Table Walk,
             가상주소→물리주소를 얻으려 다단 테이블 PD0/PD1/PD2 를 순서대로 따라가는 과정)
            ─────────────────────────────────────
...
> **등장 용어** — **comparator**(두 노드 메모리/데이터를 대조해 일치 여부를 판정하는 검증 컴포넌트), **SGE**(Scatter-Gather Element — 한 전송이 흩어진 여러 메모리 조각을 가리킬 때 그 조각 하나의 (주소, 길이) 기술 단위), **IOVA**(장치가 보는 가상 주소) → **PA**(실제 물리 주소) 변환, **page table**(가상→물리 변환표; RDMA 는 보통 **PD0/PD1/PD2** 다단 구조), **immdt**(immediate data — Send 에 끼워 보내는 작은 부가 데이터), **inline**(작은 데이터를 별도 fetch 없이 WQE 안에 직접 실어 보내는 방식), **OPS/SR**(Out-of-order Packet Sending / Selective Repeat — 패킷을 순서 무관하게 보내고 빠진 것만 골라 재전송하는 고성능 전송 방식).
...
5 단계는 **비용 순**으로 설계되어 있습니다. 로그와 SW 엔티티를 먼저 보는 것이 가장 빠르고, DUT 내부 fsdb 추적이 가장 비쌉니다. 앞 단계에서 원인이 잡히면 즉시 종료하고 뒤 단계로 가지 않습니다. 경험상 80% 이상의 케이스가 Step 2~3 에서 해결됩니다.
...
[Module 07 — QID Reference](../07_h2c_c2h_qid_map/) 의 매트릭스를 적용:
...
:::tip[🤔 Q1 — Mismatch byte 위치 해석 (Bloom: Analyze)]
...
→ [Module 09 — CQ Poll Timeout](../09_debug_cq_poll_timeout/): CQE 가 안 올 때.

[퀴즈 풀어보기 →](../quiz/08_debug_data_integrity_quiz/)
