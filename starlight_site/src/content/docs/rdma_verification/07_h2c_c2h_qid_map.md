...
:::
:::note[사전 지식]
- [Module 02 — Component 계층](../02_component_hierarchy/) (c2h_tracker 의 위치)
...
먼저 용어. **DMA**(Direct Memory Access — CPU 를 거치지 않고 장치가 메모리를 직접 읽고 쓰는 것), **H2C**(Host-to-Card — host 메모리에서 카드/DUT 방향, 즉 DUT 가 host 를 **읽는** 방향), **C2H**(Card-to-Host — DUT 가 host 메모리에 **쓰는** 방향), **QID**(Queue ID — 각 DMA 트랜잭션이 어느 큐/서브시스템에 속하는지 가리키는 식별 번호), **fsdb**(Synopsys 파형 덤프 파일 형식 — 시뮬레이션 신호 변화를 저장; Verdi 로 본다). **QDMA bypass interface**(host 와 카드 사이의 모든 DMA 가 흐르는 단일 큐 기반 DMA 인터페이스)는 그 통로입니다.
   ...
   Step 1     fsdb 에서 시점 t_timeout 직전 5 ms 의 H2C QID 14–17 검색
              ...
              결과: QID 14 에 valid pulse 0 회
                ...
                ▶ 가설: BAR4 SQ_DB 레지스터 쓰기 문제
                  (BAR = Base Address Register, PCIe 장치 레지스터가 host 주소공간에 매핑되는 베이스;
                   SQ_DB = Send Queue Doorbell, "SQ 에 새 WQE 를 넣었다"고 DUT 를 깨우는 레지스터)
                ▶ 다음 액션: RAL(Register Abstraction Layer — DUT 레지스터를 이름으로 읽고 쓰게 해주는 UVM 계층)
                   의 BAR4 write 시점과 DUT 의 SQ_DB capture 비교
...
> **용어** — **Requester**(통신을 먼저 거는 쪽, 즉 Write/Read/Send 를 발행하는 송신 측), **Responder**(요청을 받아 처리·응답하는 수신 측), **payload**(헤더를 뺀 실제 데이터 본문), **descriptor**(WQE 처럼 "무엇을 어디서 어디로 얼마나" 옮길지 적은 작업 기술 항목), **SQ**(Send Queue — 보낼 WQE 가 쌓이는 큐) / **RQ**(Receive Queue — 받을 WQE 가 쌓이는 큐), **PTW**(Page Table Walk — 가상주소를 물리주소로 바꾸려 페이지 테이블을 따라가는 과정) / **PTE**(Page Table Entry — 그 테이블의 한 항목), **CTRL_QP**(QP/MR/CQ 생성·삭제 같은 관리 명령을 나르는 제어용 QP).
...
:::note[메커니즘 — QID 가 어떻게 특정 descriptor ring 으로 라우팅되나]
QDMA(Queue-based DMA) 엔진에서 **QID 는 곧 라우팅 키** 입니다. 각 QID 는 host 메모리에 있는 _하나의 descriptor ring_(=queue) 과 1:1 로 묶여 있고, QDMA 는 DMA 요청에 실린 QID 를 보고 **어느 ring 의 descriptor 를 fetch 할지 / 어느 ring 에 결과를 쓸지** 를 결정합니다. 그래서 QID 는 두 정보를 인코딩한 것으로 읽으면 됩니다 — **(방향 H2C/C2H) × (queue index)**. 같은 bypass 인터페이스를 모든 DMA 가 통과해도, QID 값 하나가 "이 transaction 은 H2C 의 14번 ring = CMD WQE fetch ring" 처럼 _목적지 ring 을 직접 지정_ 하므로 서브시스템이 자동으로 갈립니다.

복수 채널(예: CMD H2C 14–17)이 존재하는 이유도 여기서 나옵니다 — 같은 용도(SQ WQE fetch)를 _여러 개의 독립 ring_ 에 나눠 두면, DUT 가 4개 ring 에서 병렬로 descriptor 를 fetch 해 처리량을 높일 수 있습니다. 디버그 관점에서 "QID=14 = CMD ring 0" 으로 즉시 분류되는 것은, QID→ring 매핑이 _상수로 고정_(`vrdma_defs.svh`)돼 있기 때문입니다. 따라서 fsdb 에서 QID 만 보면 어느 descriptor ring 이 동작했는지가 곧 어느 서브시스템인지로 1:1 환원됩니다.
:::
...
QID 는 단순한 채널 번호가 아니라 **디버그 분기점**입니다. 어떤 증상이 나타났을 때 "어느 서브시스템이 동작했나 / 안 했나?" 를 확인하는 데 QID 를 사용합니다. 아래 표는 증상별로 확인해야 할 QID 와 그 의미를 정리한 것입니다.
...
→ [Module 08 — Data Integrity Error](../08_debug_data_integrity/): 데이터 비교 실패 케이스 단계별 디버깅.

[퀴즈 풀어보기 →](../quiz/07_h2c_c2h_qid_map_quiz/)
