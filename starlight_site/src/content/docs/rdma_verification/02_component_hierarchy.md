...
이 한 줄에는 진단에 필요한 정보가 모두 담겨 있습니다. 첫 토큰 **F** 는 Fatal 심각도를, 두 번째 토큰 **CQHDL** 은 Completion Queue Handler(CQE — 작업 완료 항목 — 를 받아 처리하는 컴포넌트) 를 가리킵니다. 여기서 **CQE**(Completion Queue Entry — 한 작업이 끝났음을 성공/실패 상태와 함께 알리는 완료 항목)와 **CQ**(Completion Queue — 그 완료 항목들이 쌓이는 큐)가 등장합니다. 뒤에 붙은 **NODE_0**, **QP_3** 는 어느 노드의 어느 **queue pair**(QP — RDMA 통신 채널 하나; Send Queue + Receive Queue 한 쌍)에서 발생했는지를 알려줍니다. prefix 두 토큰만 읽으면 `lib/base/component/env/agent/handler/vrdma_cq_handler.svh` 의 QP_3 처리 로직으로 곧장 이동할 수 있고, 1 분 안에 진단이 시작됩니다. prefix 를 모르면 log 를 grep 하고 파일을 찾고 함수 호출 경로를 거슬러 올라가는 데만 30 분이 걸립니다. **Component hierarchy + error prefix 규약** 이 이 차이를 만들어 내며, 이것이 RDMA-TB 의 가장 큰 운영 가치입니다.
...
agent 는 RDMA-TB 의 핵심 워크호스입니다. 일반 UVM 에서 **agent**(한 인터페이스를 담당하는 검증 컴포넌트 묶음)는 보통 driver + monitor + sequencer 로 구성됩니다 — **driver**(시퀀스가 만든 추상 명령을 실제 신호/동작으로 DUT 에 인가하는 컴포넌트), **sequencer**(어떤 자극을 어떤 순서로 줄지 흐름을 통제하는 컴포넌트), **monitor**(인터페이스를 수동 관찰해 일어난 일을 transaction 으로 뽑아내는 컴포넌트). RDMA-TB 의 agent 는 monitor 대신 **handler**(특정 op 종류별로 들어온 것을 적절한 검증 컴포넌트로 넘기기만 하는 stateless forwarder)를 둡니다. 3가지 역할로 분리되어 있습니다.
...
:::note[두 UVM 기초 — config_db 경로 매칭 / factory override (디버그 체크리스트의 전제)]
이 컴포넌트들이 _어떻게 설정값을 받고 어떻게 교체되는가_ 는 두 UVM 메커니즘에 의존합니다.

**config_db 경로 매칭**: `uvm_config_db#(T)::set(scope, "field_name", value)` 로 넣은 값을, `get(scope, "field_name", var)` 가 꺼냅니다. 핵심은 **set 의 scope+field 문자열과 get 의 scope+field 문자열이 (hierarchy 경로 기준으로) 매칭돼야** 값이 전달된다는 점입니다 — set 쪽은 `"node[0].*"` 같은 와일드카드 경로를, get 쪽은 자기 component 의 실제 hierarchy 경로를 쓰므로, 둘이 어긋나면 get 이 _조용히 실패_ 하고 default 값이 남습니다 (위 체크리스트의 "build_phase 에서 cfg 가 null" 류 증상의 흔한 원인). 그래서 디버그 1순위가 "set 한 경로 문자열과 get 하는 component 의 실제 경로가 같은가" 입니다.

**factory override 가 구조를 바꾸는 원리**: UVM 컴포넌트/시퀀스를 `new` 가 아니라 `type_id::create()` 로 만들면, 생성 시점에 factory 가 _등록 테이블_ 을 조회합니다. 미리 `set_type_override`(전역) 또는 `set_inst_override`(특정 경로)를 걸어두면, factory 가 create 호출에 대해 **원래 타입 대신 지정한 派生 클래스를 반환** 합니다. 그래서 RTL/TB 코드를 안 고치고도 "이 자리의 sequence/driver 를 확장 버전으로 교체" 가 가능합니다 — 단, 반드시 `create()` 로 만들어야 factory 가 개입할 수 있습니다 (`new` 는 우회).
:::
...
> **RDMA 리소스 해부** — pool 이 관리하는 객체들은 RDMA 가 통신 전에 미리 만들어 두는 자원입니다. **QP**(Queue Pair, 통신 채널 = Send Queue + Receive Queue 한 쌍), **SQ**(Send Queue, 보낼 작업을 쌓는 큐), **RQ**(Receive Queue, 받을 작업을 쌓는 큐), **CQ**(Completion Queue, 완료 알림이 쌓이는 큐), **MR**(Memory Region, RDMA 가 직접 접근해도 되도록 등록·고정한 메모리 영역 — 등록 시 로컬용 **lkey**·원격용 **rkey** 두 열쇠를 받음), **PD**(Protection Domain, QP·MR 을 묶어 "이 도메인 안에서만 서로 접근 가능"하게 하는 보호 경계). **Fast Register**(verb 로 런타임에 빠르게 MR 을 등록하는 방식)와 그 추적용 식별자 `gen_id`(generation id — 같은 MR 이 재등록될 때마다 올라가는 세대 번호)도 함께 관리됩니다.

driver 가 RDMA verb 를 실행할 때마다 pool 이 갱신됩니다. `RDMAQPCreate` 가 완료되면 `qpool` 에 새 QP 객체가 등록되면서 동시에 `qp_reg_ap` AP 가 송출됩니다. 이 AP 를 구독하는 comparator 와 tracker 는 즉시 `[node][qp]` 슬롯을 준비해 이후 verb 를 받을 준비를 마칩니다. 마찬가지로 `RDMAMRRegister` 는 MR 풀에 등록과 함께 `mr_reg_ap` 를 송출해 c2h_tracker 가 IOVA(IO Virtual Address — 장치가 보는 가상 주소) → PA(Physical Address — 실제 물리 메모리 주소) 변환 테이블을 미리 구성하게 합니다. 마지막으로 `RDMAQPDestroy(.err(1))` 는 해당 QP 를 ErrQP 상태로 전이시키고, downstream 의 모든 comparator / tracker 에 deregister 알림을 연쇄적으로 보냅니다.
