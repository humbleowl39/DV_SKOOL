...
세 가지가 동시에 풀려야 했습니다. 첫째, **검색 속도**입니다. fail 후 첫 30 초가 디버그 방향 결정의 핵심이므로, M08–M11 을 일일이 펼쳐 읽지 않고 한 페이지에서 분기점을 결정해야 합니다. 둘째, **재사용성**입니다. QID, static flag, AP 의 핵심 표가 M07 / M04 / M06 에 흩어져 있어 필요할 때마다 여러 모듈을 왔다 갔다 해야 합니다. cheatsheet 가 이 요약본을 한 곳에 모아두면 반복 탐색 비용이 사라집니다. 셋째, **보일러플레이트**입니다. 의도된 에러 시퀀스의 4 단계 구조 (expected_error 설정 → CQ 폴링 → 에러 검증 → cleanup) 가 매 테스트마다 반복되므로, 복사·붙여넣기 가능한 미니 템플릿이 있으면 작성 시간과 실수가 동시에 줄어듭니다.
...
|--------------|---------|-----|--------|
| `E-DRV-TBERR-0001/0002` | `vrdma_driver` | [M09](../09_debug_cq_poll_timeout/) | QID 14–17 fetch 여부 확인 |
| `F-CQHDL-TBERR-0003` | `vrdma_cq_handler` | [M11](../11_debug_unexpected_err_cqe/) | `wc_status` 가 RETRY 계열인지 분류 |
| `E-SB-MATCH-*` | `vrdma_1/2side/imm_compare` | [M08](../08_debug_data_integrity/) | 첫 mismatch byte 위치 → page/SGE/MR boundary 비교 |
| `F-C2H-MATCH-0001/0002` | `vrdma_c2h_tracker` | [M10](../10_debug_c2h_tracker/) | `W-C2H-MATCH-*` unprocessed PA 리스트 캡처 |
| `E-C2H-MATCH-0001` | `vrdma_c2h_tracker` | [M10](../10_debug_c2h_tracker/) | RC vs OPS/SR — ordering 규칙 분기 |
| `F-C2H-MATCH-0003/0004/0005` | `vrdma_c2h_tracker` | [M10](../10_debug_c2h_tracker/) | DUT 가 expected 보다 더 많이 썼나 / addr 맞나 |
| `E-C2H-FLOW-*`, `E-C2H-CFG-*`, `F-C2H-TBERR-*` | `vrdma_c2h_tracker` | [M06](../06_error_handling_path/) | `RDMAQPDestroy(.err)` / lifecycle 오류 |
...
> 자세히: [Module 07 — H2C/C2H QID Reference](../07_h2c_c2h_qid_map/)
...
- 새 컴포넌트 추가 전: [Module 05 — 4 원칙](../05_extension_principles/) 체크리스트
- 시퀀스 작성 전: [Module 03 — Phase / 시퀀서 라우팅](../03_phase_test_flow/)
- AP 구독 위치: [Module 04 — AP Topology](../04_analysis_port_topology/)
- 에러 게이트 설계: [Module 06 — Error Handling](../06_error_handling_path/)
- QID 디버그: [Module 07 — QID Reference](../07_h2c_c2h_qid_map/)
...
:::
:::tip[🤔 Q2 — Cheatsheet 한계 (Bloom: Evaluate)]
...
[퀴즈 풀어보기 →](../quiz/12_debug_cheatsheet_quiz/)
