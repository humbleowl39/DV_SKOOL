...
:::
:::note[사전 지식]
- [Module 02 — Component 계층](../02_component_hierarchy/) (driver / cq_handler 위치)
...
먼저 등장 용어: **WQE**(Work Queue Entry — "이 verb 를 처리하라"고 하드웨어에 남기는 작업 지시 항목), **CQE**(Completion Queue Entry — 한 작업의 완료를 성공/실패와 함께 알리는 항목), **comparator**(두 노드의 메모리/데이터를 대조해 일치 여부를 판정하는 검증 컴포넌트), **tracker**(특정 종류의 트랜잭션을 추적·검증하는 컴포넌트), **scoreboard**(DUT 출력과 기대값을 비교하는 검증 컴포넌트). **driver/handler**(각각 자극을 DUT 에 인가하는 컴포넌트 / 들어온 것을 적절한 검증 컴포넌트로 넘기는 forwarder)도 자주 나옵니다.

RDMA-TB 에 "WRITE size 분포 통계" 같은 새 검증을 추가해야 한다고 가정해 봅시다. 가장 먼저 떠오르는 방법은 driver 코드 안에 hook(기존 코드 흐름 중간에 끼워 넣는 호출 지점) 을 끼워 넣는 것입니다.
...
**Analysis Port (AP)**(UVM 에서 한 컴포넌트가 만든 transaction 을 구독한 모든 컴포넌트에게 동시에 뿌리는 1:N 단방향 방송 통로 — 응답을 기다리지 않아 관찰/검증용으로 쓴다) 는 이 문제를 구조적으로 해결합니다. 송신 측은 `analysis_port`(또는 export), 수신 측은 `analysis_imp`(들어온 transaction 을 받아 자기 `write()` 함수로 처리하는 구현 포트)를 connect 해 둡니다. driver 는 아무것도 바꾸지 않고 이미 항상 broadcasting 하고 있습니다.
...
이 모듈을 건너뛰면 새 컴포넌트 작성 시 driver 내부에 hook 을 끼워 넣게 되어 [Module 05 4원칙](../05_extension_principles/) 을 동시 위반합니다. AP 5종 + 1 derived 만 외워두면 모든 후속 검증 컴포넌트의 위치가 자연스럽게 결정됩니다.
...
| T1 | driver | mr_reg_ap.write | c2h_tracker 가 IOVA(장치가 보는 가상 주소)→PA(실제 물리 주소) 사전 변환, scoreboard 가 lkey(로컬 접근 권한 열쇠)/rkey(원격 접근 권한 열쇠 — MR 등록 시 받는 두 키) 테이블 갱신 |
...
**(2) ErrQP**(Error QP — 에러가 났다고 마킹된 QP; 이후 이 QP 의 verb 는 정상 검증 대상에서 제외된다) **게이트는 `completed_wqe_ap` 에만 적용** — issued/cqe_ap 는 그대로 broadcast. comparator 는 cqe 받지만 cmd-cqe 매칭이 ErrQP 의 경우 자동 skip.
:::
---
...
> 이 분리가 중요한 이유: handler 에 state 를 추가하면 시퀀스 재사용·flush 시 stale state 가 누적됩니다. 자세한 이유는 [Module 05](../05_extension_principles/) #4 참고.
...
`isErrQP()` gate 가 핵심입니다. ErrQP 의 WQE 는 `completed_wqe_ap` 로 전달되지 않으므로 scoreboard 가 검증 대상에서 제외합니다. 이 동작이 [Module 06](../06_error_handling_path/) 의 정합성 보장 메커니즘입니다.
...
**실제**: driver 가 이미 5 AP 로 broadcasting 하고 있음. hook 추가는 (1) Open-Closed(확장에는 열려 있고 기존 코드 수정에는 닫혀 있어야 한다는 설계 원칙) 위반 (driver 코드 변경), (2) DRY(Don't Repeat Yourself — 같은 정보/로직을 중복하지 말라) 위반 (이미 있는 정보 재계산), (3) Interface Stability 위험 (port 시그니처가 흔들림) — 3 원칙 동시 위반. AP 구독으로 해결.<br>
...
SVA(SystemVerilog Assertion — 시뮬레이션 중 "이 조건이 항상 성립해야 한다"를 자동 검사하는 단언문): `assert property (main_verb |-> qp_key_in_comparator_exists)`.
...
→ [Module 05 — Adding New Components 4원칙](../05_extension_principles/): 위 토폴로지를 안전하게 확장하는 4 가지 원칙.

[퀴즈 풀어보기 →](../quiz/04_analysis_port_topology_quiz/)
