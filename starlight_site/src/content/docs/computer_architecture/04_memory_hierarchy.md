...
또한 캐시를 검증할 때 "왜 같은 주소가 어떤 때는 빠르고 어떤 때는 느린가"를 설명하려면 3C miss(compulsory/capacity/conflict)와 write-back 의 dirty 라인 동작을 알아야 합니다. write-back 캐시(쓰기를 일단 캐시에만 반영하고 메모리에는 나중에 한꺼번에 내려쓰는 방식)는 hit(찾는 데이터가 캐시에 있음) 시 메모리를 갱신하지 않고 dirty bit(이 캐시 줄이 메모리보다 최신임을 표시하는 비트)만 세우므로, eviction(캐시에서 한 줄을 내쫓는 것) 전까지 메모리는 stale(낡은 값) 합니다 — 이를 모르면 coherence(여러 캐시·코어가 같은 주소에 대해 일관된 값을 보게 하는 성질) 검증에서 기대값을 잘못 만듭니다.
