---
title: "Module 01 — Big-O Complexity & Pattern Thinking"
---
...
코딩 인터뷰에서 가장 먼저 주어지는 정보는 입력 크기와 시간 제한입니다. 여기서 **Big-O**(빅오 — 입력 크기 N이 커질 때 연산 횟수가 *어느 정도 빠르게 늘어나는지* 를 나타내는 표기. O(N)이면 N에 비례, O(N²)이면 N의 제곱에 비례)와 **N**(입력의 크기 — 배열 길이·노드 수 등)이라는 두 개념이 모든 분석의 기준이 됩니다. _N = 10⁶_, 제한 _2 초_ 라는 조건이 보이면 세 후보를 즉시 비교할 수 있습니다. O(N²) 풀이는 10¹² 연산으로 C++ 에서도 _~1000 초_ 가 걸려 제출 자체가 불가능하고, O(N log N) 은 약 2×10⁷ 연산으로 _~0.02 초_ 에 통과하며, O(N) 은 10⁶ 연산으로 _~0.001 초_ 입니다. 즉, **입력 크기를 보고 목표 복잡도를 30 초 안에 결정하는 것** 이 풀이의 절반입니다.
...
위 표에 나오는 패턴 이름은 이후 모듈에서 하나씩 다루며, 지금은 "이런 도구가 있다" 정도만 알면 됩니다 — **Binary search**(정렬된 데이터에서 범위를 절반씩 줄여 찾는 기법), **Hash map**(키→값을 평균 O(1)에 찾는 자료구조), **two pointers**(배열 위 두 인덱스를 움직여 구간을 탐색하는 패턴), **DP**(Dynamic Programming; 부분 문제 답을 저장해 중복 계산을 피하는 기법). **brute force**(무식한 방법 — 가능한 경우를 전부 시도하는 가장 단순한 풀이)는 거의 모든 문제의 출발점입니다.
...
알고리즘 분석은 _상수와 하위 항을 무시_ 해서 "스케일이 커질 때의 행동" 만 봅니다. 작은 N 에서 빠른 풀이 (e.g. O(n²) with small constant) 는 N=10 에서 좋아 보일 수 있지만, 실제 인터뷰/프로덕션 입력 (N=10⁶) 에서는 **TLE (Time Limit Exceeded)** 가 발생합니다. 그래서 모든 분석은 **worst-case asymptotic**(최악 입력에서 N이 무한대로 갈 때의 차수 — 가장 불리한 입력을 가정한 상한)을 기본으로 보며, 평균/best 는 _보너스 정보_ 로만 다룹니다.
...
3단계 흐름에서 핵심은 **비효율의 위치를 정확히 집어내는 것**입니다. "어떤 자료구조를 써야 하는가" 가 아니라 "어디서 불필요한 반복이 일어나는가" 를 먼저 찾은 다음, 그 반복을 제거할 수 있는 패턴을 선택하는 순서가 맞습니다. Two Sum 예시에서 볼 수 있듯이, 내부 루프의 검색 작업 하나를 O(1) 로 바꾸는 것만으로 전체 복잡도가 O(n²) → O(n) 으로 낮아집니다.
...
O(1) 패턴들은 입력 외에 추가 메모리를 거의 쓰지 않는 공통점이 있습니다. 포인터 두 개나 인덱스 변수 몇 개만으로 동작하기 때문인데, 이는 정렬된 입력이라는 구조적 조건이 이미 검색 범위를 좁혀주기 때문에 가능합니다. 반면 Hash Map 이나 BFS 큐가 O(n) 공간을 쓰는 이유는 "이전에 본 원소를 다시 참조해야 한다" 거나 "같은 거리의 노드를 한꺼번에 처리해야 한다" 는 알고리즘 구조상 중간 상태를 반드시 저장해야 하기 때문입니다.
...
### 5.45 재귀 복잡도 — Master theorem 직관

위 binary search 의 O(log n) 은 손으로 step 을 세어 얻었지만, 분할정복(divide & conquer) 재귀가 복잡해지면 매번 셀 수 없습니다. 이때 쓰는 도구가 **Master theorem** 입니다. 분할정복은 대부분 다음 점화식으로 표현됩니다:

```
T(n) = a · T(n/b) + f(n)
  a    = 한 번에 만드는 하위 문제 수
  n/b  = 각 하위 문제의 크기 (입력을 b 로 나눔)
  f(n) = 나누고 합치는 데 드는 비용 (재귀 밖의 work)
```

핵심 직관은 "**호출 트리를 그려 각 레벨의 work 를 합산** 한다" 는 §7.1 의 디버그 표 그대로입니다. 트리의 깊이는 입력을 b 로 계속 나누므로 `log_b n` 이고, 레벨 ℓ 에는 `aˡ` 개의 노드가 있습니다. 그래서 전체 비용은 _레벨별 work 의 합_ 으로 결정되며, "맨 위(루트의 f(n)) 가 지배하느냐 vs 맨 아래(잎 노드 aˡ 개) 가 지배하느냐" 의 경쟁으로 세 가지 경우가 갈립니다:

| 경우 | 어디가 지배 | 직관 | 예 |
|---|---|---|---|
| **1. 잎이 지배** (`f(n)` 이 잎 수 `n^(log_b a)` 보다 작게 자람) | 트리의 _맨 아래 레벨_ | 하위 문제 수가 폭발해 잎 노드 비용이 합을 지배 | `T(n)=2T(n/2)+O(1)` → 잎 n 개 → **O(n)** |
| **2. 균형** (`f(n)` ≈ 잎 수) | _모든 레벨이 비슷_ | 레벨마다 work 가 거의 같고 깊이가 `log n` | `T(n)=2T(n/2)+O(n)` → **O(n log n)** (merge sort) |
| **3. 루트가 지배** (`f(n)` 이 잎 수보다 크게 자람) | 트리의 _맨 위 레벨_ | 나누고 합치는 비용이 커서 루트의 f(n) 이 합을 지배 | `T(n)=2T(n/2)+O(n²)` → **O(n²)** |

binary search 는 `T(n)=1·T(n/2)+O(1)` 로, 하위 문제가 _한 개_ (a=1) 라 잎이 폭발하지 않고 깊이 `log₂ n` 만큼만 상수 work — 그래서 O(log n) 이 [Module 05](../05_tree_bfs_dfs_explained/) 의 트리 깊이 논리와 같은 뿌리에서 나옵니다. 정확한 비교는 `f(n)` 과 `n^(log_b a)` 의 성장 속도를 견주는 것이지만, 면접에서는 "잎/균형/루트 중 누가 지배하는가" 라는 위 세 직관만으로 대부분의 분할정복 복잡도를 즉답할 수 있습니다.
...
**실제**: 여기서 **amortized**(분할상환 — 여러 번의 호출에 비용을 평탄하게 나눠 본 평균 비용. 가끔 비싼 한 번이 있어도 전체 평균은 싸다)가 핵심입니다. `vector::push_back` 의 amortized O(1) 은 _N 번 호출의 평균_ 입니다. 한 번의 worst-case 는 재할당이 일어날 때 O(N). 면접관이 "한 번 호출의 worst-case 는?" 이라 되물으면 답이 막힙니다.<br>
...
:::
:::tip[🤔 Q2 — Space-time trade-off (Bloom: Evaluate)]
frequency 카운팅 문제를 풀 때, Hash map (O(N) time + O(N) space) 과 Sort (O(N log N) time + O(1) space) 중 어느 것이 더 적합한가?
...
→ [Module 02 — Array & Hash Map](../02_array_hashmap_explained/): O(N²) Brute Force 를 hash map 으로 O(N) 으로 떨어뜨리는 가장 빈번한 패턴.

[퀴즈 풀어보기 →](../quiz/01_big_o_explained_quiz/)
