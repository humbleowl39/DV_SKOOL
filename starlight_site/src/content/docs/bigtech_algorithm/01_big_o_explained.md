...
코딩 인터뷰에서 가장 먼저 주어지는 정보는 입력 크기와 시간 제한입니다. _N = 10⁶_, 제한 _2 초_ 라는 조건이 보이면 세 후보를 즉시 비교할 수 있습니다. O(N²) 풀이는 10¹² 연산으로 C++ 에서도 _~1000 초_ 가 걸려 제출 자체가 불가능하고, O(N log N) 은 약 2×10⁷ 연산으로 _~0.02 초_ 에 통과하며, O(N) 은 10⁶ 연산으로 _~0.001 초_ 입니다. 즉, **입력 크기를 보고 목표 복잡도를 30 초 안에 결정하는 것** 이 풀이의 절반입니다.
