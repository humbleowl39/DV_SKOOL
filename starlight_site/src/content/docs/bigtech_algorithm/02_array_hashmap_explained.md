---
title: "Module 02 — Array & Hash Map"
---
...
:::tip[🤔 잠깐 — Hash map 의 _worst-case_ O(1)?]
...
#### 그런데 lookup 이 _어떻게_ O(1) 인가 — hash → bucket 매핑의 실제 동작

"이름표만 보고 정해진 칸으로 직진" 이라는 §2 의 비유를 기계 수준으로 풀면 이렇습니다. Hash map 내부에는 길이 `table_size` 의 배열(버킷 배열)이 있습니다. key 가 주어지면 (1) **hash function 이 key 를 하나의 정수로 변환** 하고 (`hash("eat")` → 예: 3829183741), (2) 그 정수를 테이블 크기로 나눈 나머지로 **버킷 인덱스를 정합니다** (`hash(key) mod table_size` → 예: 인덱스 7). 그러면 그 버킷 7번 칸으로 _직접_ 가서 값을 넣거나 읽습니다. 배열 인덱싱은 위치 계산만으로 O(1) 이므로 (Module 01 참조), 전체 검색이 한 번의 hash 계산 + 한 번의 배열 접근, 즉 평균 **O(1)** 이 됩니다. 배열을 순회하지 _않는다_ 는 점이 핵심입니다.

collision(충돌) 은 서로 다른 두 key 가 같은 버킷 인덱스로 떨어질 때 발생합니다 — `mod` 로 무한한 key 공간을 유한한 `table_size` 칸으로 접기 때문에 비둘기집 원리상 피할 수 없습니다. 같은 칸에 걸린 원소들은 chaining(연결 리스트/트리) 이나 open addressing(다음 빈 칸 탐색) 으로 보관하므로, 한 버킷에 M 개가 쌓이면 그 안에서는 O(M) 까지 떨어집니다. **hash function 이 key 들을 버킷에 _고르게_ 흩뿌릴수록** 각 버킷의 M 이 작게 유지되어 평균 O(1) 이 성립하는 것입니다 (그래서 §7.1 Q2 의 "high bit 도 섞기" 가 중요).

#### 왜 평균이 _amortized_ O(1) 인가 — load factor 와 rehashing

문제가 하나 남습니다. 테이블 크기가 고정이면 원소가 늘수록 버킷당 평균 원소 수가 커져 collision 이 늘고 O(1) 이 깨집니다. 이를 막는 장치가 **load factor = (원소 수 / 버킷 수)** 입니다. 이 값이 임계치(예: Java HashMap 은 0.75) 를 넘으면 hash map 은 **테이블을 약 2배로 키우고, 기존 모든 원소를 새 크기로 다시 hash 해 재배치(rehashing)** 합니다. rehashing 한 번은 전 원소를 옮기므로 O(N) 비용이 들지만, 이 값비싼 연산은 _드물게_ 일어납니다 — 크기를 2배씩 키우면 N 개를 넣는 동안 rehashing 총비용을 모든 삽입에 분산했을 때 삽입당 평균은 상수에 수렴합니다. 이것이 [Module 01](../01_big_o_explained/) 의 dynamic array `push_back` 과 _정확히 같은_ amortized O(1) 논리입니다 (둘 다 "넘치면 2배로 키우고 복사"). 그래서 hash map insert/lookup 을 **평균(amortized) O(1)** 이라 부르고, rehashing 이 일어나는 그 순간만 일시적으로 느려집니다.
...
세 용법의 공통점은 **"나중에 빠르게 찾아야 하는 것을 key 로 저장한다"** 는 원칙입니다. 존재 확인 문제라면 배열 값 자체가 key 가 되고, 빈도 문제라면 동일 key 에 얼마나 자주 나타났는지를 세어야 하므로 value 가 카운터가 됩니다. 그룹핑 문제는 어떤 불변식(invariant) — 예를 들어 아나그램이라면 문자를 정렬한 결과 — 을 공유하는 원소들을 하나의 key 아래 묶어야 하므로, key 설계 자체가 풀이의 핵심이 됩니다.
...
:::tip[🤔 Q1 — Group Anagrams (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Hash collision (Bloom: Analyze)]
...
→ [Module 03 — Two Pointers & Sliding Window](../03_two_pointers_sliding_window_explained/): hash map 으로 풀리는 문제를 _O(1) 공간_ 으로 풀어내는 기법. 정렬 가정이 더해질 때 뭐가 좋아지는지.

[퀴즈 풀어보기 →](../quiz/02_array_hashmap_explained_quiz/)
