---
title: "Module 06 — Dynamic Programming"
---
...
:::note[왜 지수가 다항이 되는가 — 곱셈 카운팅 공식]
"각 부분문제를 한 번만 계산" 이 _왜_ O(N) 인지를 한 줄 공식으로 못박으면 모든 DP 의 복잡도가 즉답됩니다. memoization 후의 총 시간은 항상
...
DP 는 큰 문제를 작은 부분 문제로 나누고 그 결과를 저장해 재사용하는 기법입니다. 두 가지 조건이 모두 충족될 때만 DP 가 효과적입니다. 첫째로 **최적 부분 구조**(optimal substructure) — 큰 문제의 최적 답이 작은 문제들의 최적 답의 조합으로 결정론적으로 표현되어야 합니다. 둘째로 **중복 부분 문제**(overlapping subproblems) — 같은 작은 문제가 여러 경로에서 반복 등장해야 저장의 이득이 생깁니다. 둘 중 하나만 있으면 DP 가 아닌 다른 패턴(Greedy, Divide-and-Conquer)이 더 적합합니다.
...
이 순서에서 가장 중요한 단계는 2 번 — **코드 작성 전에 3 단계를 구두로 선언하는 것**입니다. state 정의를 먼저 말해야 하는 이유는 state 가 흔들리면 점화식이 흔들리고, 점화식이 흔들리면 base case 도 무너지기 때문입니다. 면접관 입장에서 dp[i] 의 의미가 명확히 선언되어야 점화식이 맞는지 검증할 수 있습니다. Bottom-up 을 면접에서 선호하는 이유는 for 루프가 재귀보다 재귀 깊이 한계(stack overflow)에서 자유롭고, 공간 최적화(배열 O(n) → 변수 O(1)) 를 자연스럽게 적용할 수 있어서입니다.
...
:::tip[🤔 Q1 — 0/1 Knapsack DP (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Greedy vs DP (Bloom: Evaluate)]
...
→ [Module 07 — Interview Strategy](../07_interview_strategy/): Module 1~6 의 도구를 _45 분 면접 시간_ 에 작동시키는 메타-스킬.

[퀴즈 풀어보기 →](../quiz/06_dynamic_programming_explained_quiz/)
