---
title: "Module 03 — Two Pointers & Sliding Window"
---
...
**Two Pointers** 는 _정렬된 입력의 monotonic(단조 — 한 방향으로만 일관되게 커지거나 작아지는) 성질_ 에 의존합니다. "합이 target 보다 크다 → right 를 줄이는 게 옳다" 는 _정렬_ 덕분에 보장됩니다. 정렬이 깨지면 패턴이 무너집니다.

왜 이 한 줄이 O(N) 을 만드는지 그 _불변량(invariant)_ 을 분명히 합시다. 정렬돼 있으므로 합이 target 보다 _크면_ 더 작은 합이 필요하고, 이는 오직 right 를 줄여서만 얻습니다 — 그런데 right 를 줄여 버린 그 칸은 _현재 left_ 와 짝지을 일이 다시는 없습니다(그 칸보다 큰 값과의 합은 이미 너무 컸으니까). 합이 _작으면_ 대칭으로 left 를 키우고, 그 칸도 다시 볼 일이 없습니다. 즉 **두 포인터는 한 번 지나친 위치로 절대 되돌아가지 않으며**(monotonic 이동), left 는 0→n 방향으로만, right 는 n→0 방향으로만 움직입니다. 그래서 _각 원소가 정확히 1회만 방문_ 되고 전체 비용이 left·right 이동 횟수의 합 = O(N) 이 됩니다. 이 "되돌아가지 않음" 불변량이 정렬 가정과 monotonic 비교에서 직접 따라 나오는 것이 핵심입니다.
...
이 패턴이 정렬된 배열에서만 동작하는 이유는 **포인터를 어느 방향으로 움직여야 조건에 가까워지는지를 정렬이 보장해 주기 때문**입니다. 합이 target 보다 크다면 오른쪽 포인터를 줄이는 것이 합을 확실히 줄인다는 것을 정렬이 보장하고, 합이 작다면 왼쪽 포인터를 늘리는 것이 합을 늘린다는 것을 역시 정렬이 보장합니다. 정렬이 없다면 포인터를 어느 방향으로 움직여야 목표에 가까워지는지 결정할 수 없어서 O(1) 공간의 이점을 포기하고 Hash Map 으로 돌아가야 합니다.
...
Sliding Window 는 "연속", "부분 배열", "부분 문자열", "최대/최소 길이" 키워드가 문제에 보이면 즉시 후보로 올립니다. 이 키워드들이 등장하는 문제에서 Brute Force 는 대부분 "모든 시작점에서 길이 k 까지 다시 합산" 하는 이중 루프 O(nk) 로 떨어지는데, Sliding Window 는 새 원소를 추가하고 오래된 원소를 빼는 O(1) 갱신으로 같은 효과를 내기 때문에 O(n) 으로 풀립니다.
...
**실제**: window 안에서 _max/min_ 같은 추가 query 가 필요하면 **deque**(double-ended queue, 양쪽 끝에서 모두 넣고 뺄 수 있는 큐) 같은 보조 자료구조가 필요해집니다. Hash Map + Sliding Window + Monotonic Deque 가 같이 등장하는 문제 (Sliding Window Maximum #239) 는 단순 SW 보다 어렵습니다.<br>
...
:::tip[🤔 Q1 — Longest substring without repeat (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Sorted array two sum (Bloom: Analyze)]
...
→ [Module 04 — Stack & Binary Search](../04_stack_binary_search_explained/): 정렬된 배열의 다른 활용 + "최근에 본 것" 문제의 표준 도구.

[퀴즈 풀어보기 →](../quiz/03_two_pointers_sliding_window_explained_quiz/)
