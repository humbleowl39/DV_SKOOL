...
:::tip[왜 "첫 도달 = 최단" 인가 — level invariant 의 증명적 근거]
"FIFO 라 거리 단조 증가" 를 한 단계 더 밀면 _첫 도달이 곧 최단_ 임을 증명할 수 있습니다. 핵심은 **level invariant** 입니다 — BFS 큐 안에는 항상 거리 `d` 인 노드들이 _모두 먼저_ 들어 있고, 그 뒤에 거리 `d+1` 노드가 들어옵니다. 이유는 거리 `d` 노드를 dequeue 할 때 그 이웃(거리 `d+1`)을 큐 _뒤_ 에 enqueue 하므로, 큐에 이미 남아 있던 다른 거리 `d` 노드들이 _먼저_ 빠진 뒤에야 거리 `d+1` 노드 차례가 옵니다. 즉 큐는 거리 순으로 정렬된 상태를 유지합니다(거리 `d` 가 _전부_ 빠진 뒤에 `d+1` 이 시작).

따라서 어떤 노드 X 를 _처음_ visit 하는 순간, 그보다 짧은 거리의 노드는 이미 모두 처리됐고 X 는 가능한 가장 이른(=가장 가까운) 차례에 도달한 것입니다 — 만약 더 짧은 경로가 있었다면 그 경로의 끝 노드가 _먼저_ 큐에서 나와 X 를 더 일찍 발견했을 것이므로 모순입니다. visited 표시로 재방문을 막으면 이 "첫 도달" 이 곧 최단 거리로 _확정_ 됩니다. (단 이 논변은 모든 간선 비용이 1 인 unweighted 그래프에서만 성립합니다 — 가중치가 있으면 Dijkstra 가 필요.)
:::
---
...
트리 문제를 받으면 코딩보다 먼저 **"DFS 인가 BFS 인가?"** 를 결정해야 합니다. 키워드가 "깊이/높이/경로" 라면 재귀가 자연스러운 DFS 를 선택하고, "레벨/최단" 이라면 거리 순서를 유지하는 BFS 를 선택합니다. 둘을 혼동하면 최단 경로에서 DFS 가 첫 도달 경로를 잘못 반환하거나, 경로 합 추적에서 BFS 가 경로 상태를 관리하기 어려워집니다.

DFS 를 선택했다면 **기저 조건 → 왼쪽/오른쪽 재귀 → 결합** 의 3 단계로 진입합니다. 기저 조건을 먼저 적는 이유는 null 노드에 대한 처리가 빠지면 거의 모든 DFS 가 런타임 에러로 죽기 때문입니다. DFS 순회 순서에 대해 면접에서 물어보면 Pre-order / In-order / Post-order 세 가지를 적용 목적과 함께 답하는 것이 좋습니다.
...
마지막으로 코딩 전에 엣지 케이스를 명시합니다. 빈 트리(null), 루트만 있는 단일 노드, 편향 트리(연결 리스트처럼 한쪽만 자식이 있는 경우), 그리고 음수 값이 포함된 경우를 확인합니다. 특히 Path Sum 에서는 경로 중간에 합이 target 에 도달하더라도 리프가 아니라면 정답이 아니라는 점에 주의해야 합니다.
...
:::tip[🤔 Q1 — Tree 의 최대 depth (Bloom: Apply)]
DFS 템플릿 한 줄?
...
:::
:::tip[🤔 Q2 — Cycle detection (Bloom: Analyze)]
...
→ [Module 06 — Dynamic Programming](../06_dynamic_programming_explained/): DFS + memoization 의 자연스러운 확장 — 부분 문제 답을 저장해 재사용.

[퀴즈 풀어보기 →](../quiz/05_tree_bfs_dfs_explained_quiz/)
