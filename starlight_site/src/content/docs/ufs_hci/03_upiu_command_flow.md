---
title: "Module 03 — UPIU & Command Flow"
---
...
:::
:::note[사전 지식]
- [Module 01-02](../01_ufs_protocol_stack/)
...
당신의 UFS storage 가 _32 command 동시_ in-flight(발행됐지만 아직 완료 응답이 안 온 처리 중 상태). 각 command 의 Task Tag(동시에 떠 있는 명령들을 구별하는 0~31 범위의 식별 태그 — queue depth 32 에 대응) _0-31_:
...
UFS 가 OoO(Out-of-Order — 발행 순서와 무관하게 먼저 끝난 명령부터) response 가능:
```
...
**Silent data corruption**(오류 신고 없이 데이터만 조용히 틀어지는 손상). _Same time same flow_ 에서만 발생, _race-dependent_, 재현 어려움.

Task Tag lifecycle(태그가 할당→사용→해제되는 생애 주기) 의 정확한 _spec compliance_(표준 준수) + DV scoreboard(기대값과 실제 출력을 비교하는 검증 컴포넌트) 의 _per-Tag tracking_ 이 _silent bug 의 안전망_.

**UPIU**(UFS Protocol Information Unit — 명령/데이터/응답을 담는 UFS 의 표준 통신 패킷)**는 UFS 의 통신 단위** 입니다. 모든 명령 / 응답 / 데이터가 UPIU 로 캡슐화되므로 검증의 거의 모든 시나리오가 UPIU 정합성 + flow 정확성으로 귀결됩니다. Command UPIU 한 번에 Response UPIU 가 항상 한 번 — 이 _request/response 짝_ 만 정확히 잡으면 어떤 SCSI 명령도 같은 패턴으로 검증할 수 있습니다.
...
H -> H: "[note] 같은 Task Tag 로 묶임 (single tag)"
```
...
D -> H: "Response UPIU" { style.stroke-dash: 4 }
H -> H: "[note] single tag, RTT 가 buffer 협상"
```
...
1. **고정 길이 header + 가변 length payload**(payload = 헤더를 뺀 실제 내용물) — Cmd (16 B CDB(Command Descriptor Block — SCSI 명령을 담는 표준 부호 블록)), Response (sense(실패 원인 상세 정보)), Data (~MTU(한 프레임 최대 바이트)), Query (parameter), TM (control) 이 모두 같은 12 B header 로 시작 → driver / HCI 가 _Transaction Type_(UPIU 종류를 나타내는 헤더 첫 필드) 만 보고 디스패치(종류별로 처리 경로를 갈라 보냄).
2. **Multi-packet 메시지** (READ 4 KB → Data-In × N) 의 식별 — Task Tag 가 모든 fragment(쪼개진 조각) 에 동일하게 붙어 reassembly(조각들을 원래 하나로 재조립) 가능.
3. **Data path 와 Task Mgmt(Task Management — 명령 중단·LUN reset 같은 작업 제어) 의 분리** — Abort / Reset 이 _별도 list (UTMRL — Task Mgmt 요청 descriptor 들의 리스트)_ 과 _별도 doorbell (UTMRLDBR)_ 로 가야 Transfer 가 stuck(처리가 멈춰 막힘) 됐을 때도 control 가능.
...
가장 단순한 시나리오. slot=5 의 READ 명령이 device 에서 응답 없이 stuck. SW 가 30 ms timeout 후 **Abort Task** 를 발행 → device 가 해당 task 를 취소 → Transfer slot 이 정리. 이 한 사이클의 두 list / 두 doorbell / 두 IRQ 흐름을 추적합니다. 아래에서 **UTMRD**(UTP Task Management Request Descriptor — Abort 같은 작업 관리 요청을 담는 descriptor; 데이터 전송용 UTRD 와 별도)와 **LUN**(Logical Unit Number — 한 device 안의 독립 저장 영역 번호)이 처음 등장하며, **OCS**(Overall Command Status — HCI 가 적는 최종 결과 코드)가 ABORTED 로 갱신되는 시점이 핵심입니다.
...
왜 애초에 완료가 *발행 순서와 어긋나(OoO)* 도착하는가? 물리적 근원은 명령마다 device 내부에서 걸리는 시간이 제각각이기 때문이다. 어떤 READ 는 데이터가 이미 캐시/버퍼에 있어 즉시 끝나지만, 다른 READ 는 실제 NAND 매체에서 읽어야 해 훨씬 오래 걸린다. 또한 서로 다른 LU(Logical Unit) 로 가는 명령은 각자 독립적으로 처리되어 진행 속도가 다르다. 그래서 나중에 발행된 빠른 명령이 먼저 발행된 느린 명령보다 먼저 완료될 수 있다 — 이것이 OoO completion 의 근원이다. 검증의 scoreboard 가 완료를 *발행 순서(FIFO)* 가 아니라 *Task Tag* 로 매칭해야 하는 이유가 바로 이 물리적 비결정성에 있다.
...
왜 WRITE 에는 RTT 라는 단계가 *필요* 한가? 근본 이유는 device 의 **write buffer 흐름제어** 다. device 가 host 의 쓰기 데이터를 받으려면 먼저 그 데이터를 담을 내부 버퍼가 있어야 하는데, 이 버퍼는 유한하고 한 번에 받을 수 있는 양이 제한된다. 만약 host 가 Cmd 직후 무작정 전체 데이터를 쏟아내면, device 버퍼가 넘쳐 데이터가 유실되거나 throttling 이 걸린다. RTT 는 device 가 "지금 이만큼(Data Transfer Count) 받을 준비가 됐다" 고 host 에 *전송 허가를 나눠주는* 신호다. 그래서 큰 WRITE 는 device 가 버퍼를 비우는 속도에 맞춰 RTT → Data-Out 을 여러 round 반복한다 — host 가 device 의 수용 능력을 넘지 않도록 device 가 페이스를 쥐는 구조다. READ 에 RTT 가 없는 이유도 같은 논리로 설명된다: READ 데이터를 받는 쪽은 host 이고, host buffer 의 위치·크기는 이미 PRDT 로 device 에 알려져 있어 별도 협상이 불필요하다.
...
RPMB(Replay Protected Memory Block — 키·인증서 같은 민감 데이터를 두는 보안 저장 영역) 가 *replay 공격*(과거에 오갔던 정상 요청을 그대로 재전송하는 공격) 을 막는 메커니즘은 **단조 증가 Write Counter + HMAC**(Hash-based Message Authentication Code — 비밀 키로 메시지의 위조 여부를 검증하는 인증 코드) 의 결합에 있다. replay 공격이란 공격자가 과거에 정상적으로 오갔던 쓰기 요청을 *그대로 다시 전송* 해 옛 상태를 되살리려는 시도다. RPMB 는 매 성공적인 쓰기마다 device 내부의 Write Counter 를 1 씩 *되돌릴 수 없게* 증가시키고, 쓰기 요청의 HMAC 계산에 *현재 counter 값을 포함* 시킨다. 그래서 공격자가 옛 요청을 재전송하면, 그 요청에 담긴 counter 는 이미 지나간 값이라 device 의 현재 counter 와 불일치하고, HMAC 검증도 실패한다 → device 가 거부한다. 즉 counter 가 "이 요청은 몇 번째 쓰기인가" 를 못 박고 HMAC 가 그것을 위조 불가능하게 봉인하므로, 한 번 쓴 요청은 두 번 통하지 않는다. (HMAC 자체는 *송신자 진위* 와 *내용 무결성* 을, counter 는 *신선도(freshness)* 를 담당한다 — 둘이 합쳐져야 replay 가 막힌다.)
...
에러 감지 경로:
  ...
  2. UPIU 레벨 에러
     ...
     - Residual Count ≠ 0 (불완전 전송 — 요청한 양보다 적게 전송됨; Residual = 못 채운 잔여 바이트 수)
...
두 번째는 **Background Operations(BKOPS)** 입니다. Garbage Collection(흩어진 유효 데이터를 모아 빈 블록을 회수하는 정리) 과 Wear Leveling(쓰기를 고르게 분산해 특정 셀이 빨리 닳는 것을 막는 기법) 같은 내부 유지보수 작업인데, Device 가 자발적으로 수행하기도 하고 Host 가 fBackgroundOpsEn 플래그를 Set 해 명시적으로 시작시킬 수도 있습니다. Host 는 bBackgroundOpsStatus Attribute 로 긴박도를 파악하며, Critical 상태를 무시하면 Write 성능이 급격히 하락합니다.
...
**실제**: 일반적인 driver / HCI 에서는 같게 운영하지만 (slot N → Task Tag N), spec 상 _필수가 아닙니다_. HCI 가 다른 매핑을 쓰면 scoreboard 의 매칭 키가 바뀌어야 함. 우리 사내 IP 기준은 _같다_ 로 가정하지만, 검증 시 매핑 invariant 를 명시적으로 assertion 으로 둬야 합니다. *왜 통상 1:1 로 묶는가* — slot 은 곧 "현재 in-flight 인 명령 한 건" 의 자리이고 Task Tag 는 "그 명령을 식별하는 라벨" 이다. 둘을 1:1 로 묶어 두면, 응답이 들고 온 Task Tag 만 보고 곧장 어느 slot(어느 UTRD·어느 buffer) 의 명령인지 추적할 수 있어 매핑 테이블이 필요 없어진다. 즉 추적 로직을 단순화하려는 *구현 편의* 가 1:1 관례의 동기이지, spec 의 강제가 아니다.<br>
...
**실제**: NOP(No Operation — 아무 일도 안 하고 링크 생존만 확인하는 ping 성 UPIU) 은 _UTP layer 까지_ 가 정상임을 알려줍니다. UniPro link 가 fluctuation 중이면 가끔 NOP 은 통과하지만 큰 Data-In 이 NAK(못 받았으니 재전송하라는 부정 응답) 으로 깨질 수 있음. 종합적인 link health 는 UE(UIC Error) 카운터 + AFC(Ack Flow Control — 수신측이 크레딧을 돌려주는 흐름제어 프레임) 통계 + actual transfer 로 판단.<br>
...
- **Sense Data**(명령 실패 시 device 가 반환하는 상세 원인 정보) = SCSI sense key + ASC/ASCQ(Additional Sense Code / Qualifier — 실패 원인을 단계적으로 좁히는 코드). 명령 실패 시 Response UPIU 에 동봉.

:::caution[실무 주의점 — Task Tag 재사용 시점 오해]
...
:::tip[🤔 Q1 — Task Tag lifecycle (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — OoO Response (Bloom: Analyze)]
...
→ [Module 04 — HCI DV Methodology](../04_hci_dv_methodology/): UPIU sequence 와 Task Tag lifecycle 을 어떻게 UVM env / scoreboard / SVA / coverage 로 검증할지.

[퀴즈 풀어보기 →](../quiz/03_upiu_command_flow_quiz/)
