...
검증 엔지니어가 **MMU**(memory-management unit, 메모리 관리 장치 — CPU 가 낸 주소를 실제 메모리 주소로 번역·보호하는 하드웨어)나 **IOMMU**(I/O memory-management unit, device 가 내는 주소를 번역·보호하는 I/O 측 MMU)를 다룰 때, 그 핵심은 *주소 번역*입니다. CPU(또는 device)가 낸 **logical/virtual address**(프로그램이 보는 가상의 주소)를 page table 을 거쳐 **physical address**(실제 메모리 칩 상의 주소)로 바꾸고, 그 과정에서 protection bit 으로 접근을 보호하는 일이죠. 이 모듈에서 다루는 page/frame·page table·TLB·protection bit 은 정확히 우리가 RTL 로 검증하는 MMU 의 구성요소입니다.
