...
이 모듈은 DV 엔지니어에게 가장 직접적입니다. 우리가 검증하는 거의 모든 IP — DMA 엔진, **NIC**(network interface card, 네트워크 카드 — 컴퓨터를 네트워크에 연결하는 하드웨어), 스토리지 컨트롤러 — 가 여기서 다루는 **controller**(device 의 복잡성을 떠안고 host 와 약속된 방식으로 대화하는 device 측 제어 하드웨어)**·memory-mapped I/O·interrupt·DMA** 구도 그대로이기 때문입니다. 레지스터에 값을 쓰는 것이 곧 MMIO 이고, 명령 완료를 알리는 것이 곧 interrupt 이며, 대량 전송을 떠넘기는 것이 곧 DMA 입니다.
...
위 표의 용어를 먼저 풀면: **HDD**(hard disk drive, 회전하는 자기 원판에 저장하는 기계식 디스크), **SSD**(solid-state drive, 움직이는 부품 없이 flash 메모리로 저장하는 장치), **NVM**(non-volatile memory, 전원이 꺼져도 데이터가 남는 메모리), **NAND**(SSD 가 쓰는 flash 메모리의 한 종류). HDD 가 느린 두 원인은 **seek time**(읽기 head 를 목표 트랙으로 옮기는 데 걸리는 시간)과 **rotational latency**(원하는 sector 가 head 밑으로 돌아올 때까지 기다리는 시간)이며, SSD 는 움직이는 부품이 없어 이 둘이 없습니다. **IOPS**(I/O operations per second, 초당 입출력 연산 수 — 높을수록 빠름), **NVMe**(NVM Express, SSD 를 PCI bus 에 직접 붙이는 고속 인터페이스 규약)도 함께 알아 둡니다.

NAND 의 까다로움: page 단위로 읽고 쓰지만 덮어쓸 수 없어 먼저 **block 단위 erase** 필요(erase 가 가장 느림); erase 반복하면 닳아(약 10만 **P/E cycle** — program/erase cycle, 쓰고 지우기를 반복할 수 있는 횟수 한계) 수명을 **DWPD**(drive writes per day, 하루에 드라이브 용량만큼 몇 번 쓸 수 있는가로 나타낸 수명 척도)로 잼.
