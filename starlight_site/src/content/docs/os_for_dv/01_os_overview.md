...
검증 엔지니어가 어떤 IP(intellectual property; 칩 안에 들어가는 재사용 가능한 하드웨어 블록 — 예: DMA 엔진, NIC)의 레지스터에 값을 쓰고 결과를 관찰할 때, 그 "기대값"은 결국 OS 가 그 하드웨어를 *어떻게 쓸 작정인가*에서 나옵니다. 예를 들어 어떤 control register 는 반드시 **kernel mode**(운영체제 핵심이 도는 최고 권한 실행 상태 — 모든 명령·자원에 접근 가능)에서만 쓸 수 있어야 하고, **user mode**(일반 응용 프로그램이 도는 제한된 권한 상태)의 user 프로그램이 그것을 건드리려 하면 하드웨어가 막아야 합니다. 만약 **DUT**(design under test, 검증 대상 설계 — 지금 우리가 만들어 검증하는 그 하드웨어)가 user mode 의 접근을 그냥 통과시킨다면, 단위 테스트는 PASS 로 보여도 그것은 보안 구멍입니다.
...
거대한 OS 는 모듈로 나눠 설계하며, kernel 을 엮는 방식이 여럿입니다. 각 방식은 성능과 확장성·안전성 사이의 trade-off 입니다. (아래 표에서 **context switch** 는 한 실행 흐름에서 다른 흐름으로 코어를 넘기며 상태를 저장·복원하는 동작 — M02 에서 자세히, **message passing** 은 프로세스끼리 메시지를 복사해 주고받는 통신 방식입니다.)
