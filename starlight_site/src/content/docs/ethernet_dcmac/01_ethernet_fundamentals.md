...
:::note[메커니즘 — scrambling 이 transition density 를 만들어 clock recovery 를 가능하게 하는 원리]
8b/10b 와 달리 64b/66b 는 payload 64비트를 _부호화하지 않으므로_, 데이터가 운 나쁘게 0 또는 1 이 길게 이어지는 패턴(long run, 예: 0x0000…)일 수 있습니다. 이게 왜 문제인가? 수신측 SerDes 에는 별도 clock 선이 없어서, **들어오는 데이터의 0↔1 _전이(transition)_ 시점에서 클럭을 복원** 합니다 (CDR, Clock-Data Recovery — PLL 이 전이 가장자리에 위상을 맞춤). 그런데 신호가 같은 값으로 오래 머물면 전이가 없어 CDR 이 위상 기준을 잃고, 동시에 신호가 한쪽으로 치우쳐 DC offset(불균형)이 생깁니다.

**self-synchronous scrambler** 가 이를 해결합니다: TX 가 payload 를 LFSR(선형 피드백 시프트 레지스터)이 만드는 의사난수 시퀀스와 XOR 해서 내보내면, 원래 데이터가 어떤 패턴이든 **출력은 통계적으로 0 과 1 이 자주 뒤섞여 long run 이 깨집니다** → 전이 밀도(transition density)가 확보됩니다. 그 결과 ① CDR 이 충분한 전이 가장자리로 클럭을 안정적으로 복원하고, ② 0/1 비율이 균형을 이뤄 DC balance 가 잡힙니다. RX 는 같은 LFSR 다항식으로 descramble 해 원본을 복원합니다 ("self-synchronous" 라 별도 seed 동기 없이 수신 비트만으로 LFSR 상태가 따라잡힘). 즉 scrambling 은 _데이터 내용과 무관하게_ 물리계층이 클럭과 균형을 잃지 않도록 보장하는 장치입니다.
:::
...
:::note[메커니즘 — alignment marker 가 lane skew 를 측정·보정하는 원리]
한 데이터 스트림을 여러 lane(예: 4×25G)에 round-robin 으로 쪼개 보내면, 각 lane 은 _물리적으로 다른 경로_(다른 SerDes, 다른 trace 길이, 다른 CDR)를 지나므로 도착 시점이 lane 마다 조금씩 어긋납니다 — 이를 **lane skew** 라 합니다. skew 가 있으면 RX 가 lane 들을 단순히 합칠 때 "원래 Block N 다음이 Block N+1" 이라는 순서가 깨집니다.

**alignment marker(AM)** 가 이 skew 를 _측정 가능한 기준점_ 으로 만듭니다. TX 는 각 lane 에 **정해진 주기(약 16K 블록마다)로, lane 마다 고유한 AM 패턴** 을 _동시에_ 삽입합니다. 송신 시점에는 모든 lane 의 같은 회차 AM 이 정렬돼 있습니다. RX 는 각 lane 에서 자기 AM 을 찾는데, skew 때문에 **AM 이 lane 마다 _다른 시각_ 에 도착** 합니다 — 이 도착 시점 차이가 곧 각 lane 의 skew 측정값입니다. 그러면 RX 는 **가장 늦게 도착한 lane 을 기준으로, 먼저 온 lane 들을 deskew buffer(FIFO)에 그만큼 지연**시켜 모든 lane 의 AM 이 같은 출력 시점에 정렬되게 맞춥니다. AM 의 _고유 패턴_ 덕분에 lane 순서(어느 게 lane 0 인지)도 함께 복원되므로, lane swizzle(순서 뒤바뀜)까지 바로잡힙니다. 정렬이 끝나면 AM 은 제거하고 원래 block 스트림을 순서대로 재조립합니다. 즉 AM 은 "여러 lane 을 다시 한 줄로 세우기 위한 _동기 마커_" 이고, deskew buffer 가 그 시간차를 흡수하는 장치입니다.
:::
...
:::note[메커니즘 — RS-FEC 가 에러를 "정정" 하는 원리, 그리고 CRC 와의 본질 차이]
CRC(FCS)는 _검출만_ 합니다 — 전체 비트열의 다항식 나머지를 비교해 "어딘가 틀렸다/안 틀렸다" 라는 _1비트 판정_ 만 내고, 틀렸으면 프레임을 통째로 버립니다 (어디가 틀렸는지는 모름). RS-FEC 는 여기서 한 걸음 더 나아가, **틀린 위치와 틀린 값을 대수적으로 _풀어내_ 복원** 합니다.

원리는 _잉여 정보(redundancy)_ 를 의도적으로 추가하는 것입니다. RS(544,514)는 514 개 데이터 심볼에 **30 개 패리티 심볼** 을 더해 544 심볼 codeword 를 만드는데, 이 패리티는 데이터를 특정 다항식에 대입했을 때의 값들로, 단순 합이 아니라 **여러 개의 독립적인 방정식(syndrome)** 을 형성하도록 설계됩니다. 수신측에서 codeword 에 오류가 있으면 이 syndrome 들이 0 이 아닌 값으로 나타나는데, **미지수가 "오류 위치" 와 "오류 크기" 인 연립방정식** 을 이 syndrome 들로 세워 풀면 — 충분한 패리티(=방정식)가 있는 한 — _어느 심볼이 얼마나 틀렸는지_ 를 정확히 계산해 원래 값으로 되돌릴 수 있습니다. 30 개 패리티로는 **최대 16 개 심볼** 까지 정정 가능합니다 (대략 "위치+값" 두 미지수당 패리티 2개가 필요한 구조). 그래서 SerDes 출력의 pre-FEC BER ~1e-5 정도의 산발적 오류는 _폐기·재전송 없이_ 그 자리에서 복구되어 post-FEC BER 이 ~1e-13 으로 떨어집니다. **요약: CRC = "틀렸나?" 검출, RS-FEC = "어디가 얼마나 틀렸나?" 를 풀어 정정** — 잉여 패리티가 방정식을 제공하기에 가능한 일입니다.
:::
