---
title: "Module 03 — DCMAC DV Methodology"
---
...
:::
:::note[사전 지식]
- [Module 01-02](../01_ethernet_fundamentals/) — frame, MAC/PCS 분담, DCMAC 5 블록
...
또한 DCMAC 은 TOE / IP / 다른 NIC subsystem 의 wire-side 끝단이라는 위치상 특수성이 있습니다. TB 가 발견하지 못한 bug 는 곧바로 wire 를 타고 나가 silicon 후 SerDes BER 통계로만 보이게 됩니다. **BER**(Bit Error Rate — 전송 비트 중 잘못 수신되는 비트의 비율) 1e-12 수준이면 하루에 frame 1 개 손실로 환산되어 사실상 발견 불가능한 단계까지 잠복할 수 있습니다.
...
- 통계 → **RAL**(Register Abstraction Layer — DUT 의 하드웨어 레지스터 맵을 UVM 안에 객체로 모델링해, 코드에서 `reg.write/read` 로 접근하고 기대값을 자동 추적(mirror)하게 하는 계층) 이 reg 카운터를 mirror, scoreboard 가 실제 frame count 와 비교.
...
| 최소 크기 연속 (최대 pps, packets/sec) | 최대 프레임 레이트 달성 |
...
아래 표에 나오는 레지스터 필드 _접근 정책_ 약어를 먼저 풀어 두면: **RW**(Read-Write — 읽고 쓰기 모두 가능), **RO**(Read-Only — 읽기만; 쓰기는 무시, 보통 상태/통계), **WO**(Write-Only — 쓰기 전용; 읽으면 0), **W1C**(Write-1-to-Clear — 해당 비트에 1 을 쓰면 0 으로 지워지는, 에러 플래그·인터럽트에 흔한 정책)입니다.
...
:::caution[실무 주의점 — Statistics Counter Clear-on-Read 레이스 컨디션]
...
:::tip[🤔 Q1 — Coverage bin 설계 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Line-rate + pause 동시 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — Counter snapshot atomicity (Bloom: Evaluate)]
...
→ [Module 04 — Quick Reference Card](../04_quick_reference_card/): 지금까지 3 모듈을 한 페이지로 압축한 cheat sheet — frame, AXI-S tuser, RS-FEC, 면접 골든 룰, MangoBoost 데이터 패스 한 장.

[퀴즈 풀어보기 →](../quiz/03_dcmac_dv_methodology_quiz/)
