...
출처: HDG `nvme_basics_usage.md` — "NVMe vs. SATA/SAS" 표.
...
NVMe의 우위는 단일 마법이 아니라 세 가지 설계 결정의 합입니다. 첫째는 **병렬성**으로, 큐를 코어·NUMA 노드별로 분산해 락 경합을 없앱니다. 둘째는 **경로 단축**으로, PCIe 위에 직접 얹혀 SATA/SAS 컨트롤러라는 중간 단계를 제거해 지연을 줄입니다. 셋째는 **효율적 명령 집합**으로, 회전 디스크 시절의 레거시 가정을 버리고 SSD에 맞는 최소한의 명령으로 CPU 오버헤드를 낮춥니다.
...
HDG의 "Why this matters for DV" 절은 세 가지를 명시합니다.

첫째, **NVMe는 PCIe 위에 얹히므로** controller 검증은 올바른 PCIe 기반 위에서만 의미가 있습니다. PCIe 트랜잭션 계층이 흔들리면 NVMe 레벨 디버그는 헛돕니다. ([PCIe 코스](../../pcie/) 참고.)
...
- HDG `nvme_basics_usage.md` — "Overview", "Key features", "NVMe vs. SATA/SAS", "Application areas", "Why this matters for DV"
- Confluence: *NVMe basics* (mangoboost wiki, page 40206969)
