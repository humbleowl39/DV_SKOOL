...
데이터센터에서 32코어 서버에 빠른 플래시 **SSD**(Solid-State Drive, 회전 디스크 없이 플래시 메모리로 데이터를 저장하는 드라이브)를 **SATA**(Serial ATA, HDD 시절에 표준화된 스토리지 연결 인터페이스)로 연결했다고 합시다. 매체 자체는 마이크로초 단위로 응답하는데, 막상 벤치마크를 돌리면 **IOPS**(Input/Output Operations Per Second, 초당 처리하는 I/O 명령 수 — 스토리지 처리량의 핵심 지표)가 기대만큼 안 나옵니다. 원인을 추적해 보면 32개의 코어가 동시에 I/O를 던지는데도 SATA는 큐가 **하나**뿐이고 그 안에 명령을 최대 **32개**까지밖에 못 넣습니다. 결국 모든 코어가 이 좁은 통로 하나 앞에 줄을 서고, 명령 하나하나가 SATA/**SAS**(Serial Attached SCSI, 엔터프라이즈 서버·디스크용 스토리지 연결 인터페이스) 컨트롤러라는 중간 단계를 거치며 지연이 쌓입니다. 매체는 놀고 CPU는 락(lock) 경합으로 시간을 버립니다.
...
**NVMe**(Non-Volatile Memory Express, 플래시 SSD를 위해 PCIe 위에서 새로 설계된 스토리지 인터페이스 표준)는 이 구조적 병목을 없애기 위해 만들어졌습니다. 큐를 **최대 64,000개**, 큐당 명령을 **최대 64,000개**까지 둘 수 있어 코어마다 자기 전용 큐를 가질 수 있고, SSD를 **PCIe**(PCI Express, CPU와 주변 장치를 잇는 고속 직렬 버스 — 그래픽카드·SSD 등이 붙는 통로) 버스에 직접 붙여 중간 컨트롤러 **hop**(데이터가 거쳐 가는 중간 경유 단계)을 제거합니다.
...
첫째, **NVMe는 PCIe 위에 얹히므로** controller 검증은 올바른 PCIe 기반 위에서만 의미가 있습니다. PCIe 트랜잭션 계층이 흔들리면 NVMe 레벨 디버그는 헛돕니다. ([PCIe 코스](../../pcie/) 참고.) 이 "PCIe 위에 얹힌다"는 말은 통지 경로에서 구체적으로 드러납니다 — 명령 큐 자체는 **host 메모리**(명령을 요청하는 쪽, 즉 CPU/소프트웨어의 메인 메모리)에 살고, host가 새 명령을 넣었다고 **controller**(SSD 안에서 명령을 받아 처리하는 제어 칩) 에게 알리는 신호(**doorbell**, 큐에 새 항목이 생겼음을 상대에게 통지하는 메커니즘)는 PCIe **MMIO write**(Memory-Mapped I/O write — 장치의 레지스터를 메모리 주소처럼 읽고 써서 제어하는 방식의 쓰기) 트랜잭션 한 건으로 전달됩니다. 즉 doorbell은 추상적인 "벨"이 아니라 controller의 **BAR**(Base Address Register, 장치의 레지스터들이 매핑되는 메모리 주소 영역) 영역에 대한 PCIe 쓰기입니다. 이 메커니즘은 [2장](../02_sq_cq_doorbell/)에서 자세히 다룹니다.

둘째, **높은 큐 수는 곧 동시성 검증**을 의미합니다. 멀티 큐 경합, doorbell 순서, completion queue 인터럽트를 반드시 자극해야 합니다. 단일 큐 **directed test**(검증자가 시나리오를 직접 정해 한 가지 경로만 콕 집어 확인하는 테스트)만으로는 NVMe의 실제 리스크를 못 잡습니다.
...
큐 *수*의 이득은 사실상 없습니다. **outstanding 명령**(이미 발행했지만 아직 완료 응답이 오지 않아 처리 중인 명령)이 항상 1건이면 큐가 1개든 64K개든 동시에 처리할 게 없기 때문입니다. NVMe의 큐 병렬성은 *여러 코어/스레드가 동시에 많은 outstanding I/O*를 던질 때 효과가 나타납니다. 다만 단일 스트림에서도 NVMe는 controller hop 제거에 따른 *지연* 이득은 여전히 가집니다 — 병렬성 이득과 지연 이득을 구분해야 합니다.
