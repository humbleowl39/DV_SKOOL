...
NVMe 명령은 평평한 하나의 집합이 아니라 세 가지 분류 — **Admin / IO / Fabric** — 로 나뉩니다(HDG `nvme_dv_reference.md` §2). controller를 *설정·관리*하는 명령(Admin), 실제로 *데이터를 옮기는* 명령(IO), 그리고 NVMe-oF 패브릭 환경에서만 쓰이는 *연결·속성* 명령(Fabric)입니다. 이 분류를 모르면 어떤 명령을 어느 큐로, 어떤 순서로 보내야 하는지 판단할 수 없습니다.
...
| Step | 분류 | 명령 예 | 핵심 |
|---|---|---|---|
| ① | Fabric | Connect | NVMe-oF에서 admin QP(qid=5)를 활성화 — 이후 admin cmd 처리 가능 (HDG §2.1) |
...
①의 Fabric 단계는 *NVMe-oF 환경에서만* 등장합니다. 로컬 PCIe NVMe에서는 admin 큐가 PCIe 초기화로 바로 서므로 Connect 없이 Admin부터 시작합니다. HDG의 NRT DV는 NVMe-oF이므로 Connect 후 admin QP(qid=5)가 활성화되는 흐름을 따릅니다.
:::
...
Admin 커맨드는 controller를 *설정·관리*합니다. 큐 관리(Create/Delete IO Queue), 디바이스 식별(Identify), 로그 취득(Get Log Page) 등이 여기 속합니다. NRT DV에서는 NVMe-oF Connect 후 admin QP(qid=5)가 활성화되면 admin 커맨드를 처리할 수 있습니다(HDG §2.1). 전체 opcode 정의는 Confluence *NVMe 개념 / Admin·IO 전송* 페이지를 참조합니다.

### 4.2 IO Commands

IO 커맨드는 실제 데이터를 옮깁니다. HDG §2.2의 분류와 NRT DV의 `io_type` knob 매핑은 다음과 같습니다.
...
NRT-TB의 IO 시퀀스는 `lib/vnrttb/include/seq_lib/vnrttb_nvme_io_virtual_sequence_lib.svh`에 있습니다(HDG §2.2). 검증에서는 단순 Write/Read뿐 아니라 Flush(데이터 영속화), Write Zeros(데이터 없이 0 기록), Compare(읽은 값 비교) 같은 변형과, `IO_FULL_RAND`로 opcode를 무작위 섞은 스트레스 시나리오를 모두 자극해야 합니다.

### 4.3 Fabric Commands

Fabric 커맨드는 **NVMe-oF 환경에서만** 사용하며, capsule transport(RDMA SEND/RECV) 위에서 동작합니다(HDG §2.3). 인코딩이 독특합니다: **opcode `0x7F`가 Fabric command의 마커**이고, opcode의 sub-field(sub-opcode)가 실제 명령을 식별합니다.
...
출처: HDG §2.3 표(Confluence DV 공간 *§6 Fabric Command Set* 및 하위 §6.1~6.6 링크).
...
opcode 0x7F는 "이것은 Fabric 커맨드"라는 마커일 뿐이므로, 다음으로 SQE 안의 **sub-opcode(fabric command type)** field를 보고 실제 명령을 결정해야 합니다. sub-opcode가 `0x01`이면 **Connect** — 큐 쌍(NVMe-oF에서는 RDMA QP 한 쌍)을 수립하는 명령입니다. Connect가 성공해야 그 위에서 admin/IO 커맨드를 주고받을 수 있습니다(HDG §2.3).
...
- HDG `nvme_dv_reference.md` §2 — "NVMe Commands" (2.1 Admin, 2.2 IO + `io_type` knob, 2.3 Fabric command 표 / opcode 0x7F)
- Confluence: *NVMe 개념 / Admin·IO 전송* (DV 한국어 정리) — admin/IO opcode 정의
- Confluence: *§6 Fabric Command Set* 및 §6.1~6.6 (DV 공간) — Connect/Disconnect/Property/Auth
- `NRT_TB/.../seq_lib/vnrttb_nvme_io_virtual_sequence_lib.svh` — IO 시퀀스
