---
title: "Module 06 — Strict vs Passthrough"
---
...
:::
:::note[사전 지식]
- [Module 04](../04_io_virtualization/) — I/O 가상화 기본 (emulation / virtio / SR-IOV)
- [IOMMU 코스](../../mmu/04_iommu_smmu/) — SMMU / VT-d 의 DMA 격리
- [Module 05](../05_hypervisor_types/) — Type 1 / Type 2 / KVM 의 trap 경로
:::
---
...
:::caution[Posted Interrupt 가 _왜_ hypervisor 경유를 없애나 — passthrough latency 이점의 핵심]
③ 에서 "VF 가 MSI-X 를 발사하면 Posted Interrupt 로 vCPU 가 hypervisor 경유 없이 직접 받는다" 고 했는데, 이것이 passthrough 가 _데이터뿐 아니라 인터럽트까지_ bare-metal 에 가까운 비결입니다. 그 메커니즘을 풀면 다음과 같습니다.

문제는 — DMA 는 IOMMU 로 hypervisor 를 우회했는데, _인터럽트_ 가 여전히 hypervisor 를 거치면 그 hop 비용이 남는다는 것입니다. 전통적 path 에서는 device 가 인터럽트를 쏘면 host(hypervisor)가 먼저 받아서, "지금 그 vCPU 가 돌고 있나, 어느 물리 CPU 에 있나" 를 판단하고 guest 에게 _가상_ 인터럽트(vIRQ)를 주입했습니다 — 이 판단·주입이 곧 VM Exit 한 번입니다.

**Posted Interrupt 는 이 주입을 HW 가 _메모리를 통해_ 직접 하게 만듭니다.** 각 vCPU 마다 메모리에 **posted-interrupt descriptor** 영역이 잡혀 있고, device 의 인터럽트는 interrupt remapping table 을 거쳐 "이 인터럽트를 그 vCPU 의 descriptor 에 _posting_(비트로 기록)하라" 로 바뀝니다. 대상 vCPU 가 지금 물리 CPU 에서 _실행 중_ 이면, HW 가 그 CPU 에 특수 통지를 보내 guest 가 **VM Exit 없이** 곧장 인터럽트를 받습니다. 즉 hypervisor 의 "받아서 판단해서 주입" 단계가 통째로 사라지고, _HW 가 vCPU 의 메모리 영역에 직접 인터럽트를 꽂는_ 것입니다.

핵심 인과 — DMA 는 IOMMU 가, 인터럽트는 Posted Interrupt 가 각각 hypervisor 를 우회하므로, passthrough 의 데이터 path 전체에서 hypervisor hop 이 _0_ 이 됩니다(§3 의 "0 hop"). 만약 Posted Interrupt 가 비활성이면 데이터는 직접 가도 인터럽트마다 VM Exit 이 돌아오므로(§6 오해 2), 고빈도 IRQ 워크로드에서 passthrough 의 이점이 절반으로 깎입니다 — 그래서 Posted Interrupt 는 passthrough latency 이점의 _절반을 책임지는_ 부품입니다.
:::
---
...
:::note[IOMMU 비용이 "setup 1회, runtime 0" 인 _이유_ — 그리고 streaming 이 왜 예외인가]
§3 note 가 "IOMMU 는 setup 때 한 번, runtime 0" 이라고 단언했는데, 그 _기전_ 은 page table 을 _미리 깔아두면_ 변환이 캐시로 끝난다는 데 있습니다.

passthrough 의 일반적 구성은 VM 부팅 시 VM 의 메모리 전체에 대한 IOVA→HPA 매핑을 IOMMU page table 에 **한 번 깔아두는** 것입니다(§3 의 `VFIO_IOMMU_MAP_DMA` 한 번). 이후 runtime 의 매 DMA 는 그 page table 을 _다시 깔지 않고_ 이미 있는 매핑을 쓰기만 하면 되고, 변환 결과는 **IOTLB(IOMMU 의 TLB)** 에 캐시되어 hit 시 ns 단위로 끝납니다. 즉 비용이 드는 순간은 _매핑을 만들거나 지우는_ 때(page table write + IOTLB invalidate)뿐이고, 매핑이 그대로 있는 한 변환 자체는 거의 공짜입니다 — 이것이 "runtime 0" 의 정체입니다.

이 인과를 뒤집으면 **streaming 워크로드의 병목** 이 보입니다. 보안을 극대화하려는 일부 구성은 "쓸 때만 매핑하고 끝나면 즉시 unmap" 하는 _dynamic mapping_ 을 씁니다(DMA 가 끝난 버퍼는 device 가 더는 못 보게). 이러면 buffer 마다 map→DMA→unmap 이 반복되고, 매 unmap 은 IOTLB invalidate 를 동반합니다. 고속 streaming(초당 수백만 buffer)에서는 이 map/unmap + invalidate 가 변환 자체보다 훨씬 비싸져 병목이 됩니다 — "runtime 0" 이 깨지는 유일한 경우입니다. 그래서 고성능 path 는 보통 매핑을 _재사용_ 하거나(static mapping, buffer pool) 한 번 깔고 두는 쪽을 택해 invalidate 를 피합니다.
:::
...
:::caution[실무 주의점 — IOMMU disable 실수로 DMA 가 host RAM 직접 access]
...
:::note[live migration 이 passthrough 에서 _구조적_ 으로 어려운 이유 — 그리고 vDPA 가 푸는 것]
"HW state 이전이 복잡" 이라는 결과만으로는 왜 strict 는 쉽고 passthrough 는 어려운지가 안 보입니다. 근본 차이는 **state 가 _어디_ 에 있느냐** 입니다.

live migration 은 한 호스트에서 다른 호스트로 VM 을 옮기는 것인데, 옮기려면 VM 의 _전체 state 를 capture_ 할 수 있어야 합니다. strict(virtio)에서는 device 가 SW(QEMU/vhost)로 emulate 되므로 ring index·in-flight 요청 같은 device state 가 _전부 hypervisor 가 볼 수 있는 메모리_ 에 있습니다 — 그래서 그냥 읽어서 옮기면 됩니다.

passthrough 에서는 그 state 가 **물리 device 내부** 에 있습니다. VF 의 TX/RX ring 의 현재 index, 아직 완료되지 않은 in-flight DMA, device 내부 레지스터·offload 상태 — 이것들은 device 칩 안에 있고 _hypervisor 에게 노출되지 않습니다_. hypervisor 는 그 device 를 VM 에 통째로 넘겨놨으므로 내부를 들여다볼 표준 방법이 없습니다. capture 가 불가능하면 destination 에서 그 상태를 복원할 수 없고, in-flight DMA 가 떠 있는 채로 옮기면 데이터가 유실됩니다 — 이것이 _구조적_ 어려움입니다.

**vDPA 가 푸는 것** 이 바로 이 지점입니다. vDPA 는 데이터 path 는 HW(VF)가 직접 처리해 passthrough 성능을 내되, **control plane 은 표준 virtio 인터페이스로 노출** 합니다. 즉 ring index 같은 상태를 hypervisor 가 virtio 의 표준 방식으로 읽고 쓸 수 있게 해, "passthrough 의 속도 + virtio 의 migratable 한 state 가시성" 을 동시에 얻습니다. migration 직전 device 를 quiesce 하고 그 표준화된 state 를 추출·복원하는 길이 열리는 것입니다. (오해 3 의 VFIO migration API 도 같은 목표를 device 별로 푸는 다른 접근입니다.)
:::
...
:::tip[🤔 Q1 — Strict vs Pass-through 선택 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — IOMMU 페이지 fault (Bloom: Analyze)]
...
→ [Module 07 — Containers & Modern Virtualization](../07_containers_and_modern/): VM 격리의 무게가 부담스러운 워크로드를 위한 namespace/cgroup 기반 가벼운 격리와, "container 의 속도 + VM 의 격리" 를 노린 microVM.

[퀴즈 풀어보기 →](../quiz/06_strict_vs_passthrough_quiz/)
