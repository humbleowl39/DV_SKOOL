...
**KVM**(Kernel-based Virtual Machine — Linux 커널에 모듈로 내장된 hypervisor) 은 이 분류의 경계를 흐립니다. Linux kernel module 형태로 host OS 위에서 동작한다는 점에서는 Type 2 처럼 보이지만, KVM 모듈이 로드되는 순간 Linux kernel 자체가 **VMX root mode**(VT-x 에서 hypervisor 가 도는 최상위 권한 모드 — Guest 는 non-root mode) 로 진입하여 hypervisor 역할을 직접 수행합니다. 결과적으로 구조는 Type 2 이지만 trap 경로는 Type 1 과 동일합니다. 이것이 **Hybrid** 라고 불리는 이유입니다.
...
가장 단순한 시나리오. **같은 사용자가 "VM 한 개 만들어 부팅"** 을 요청했을 때 ESXi (Type 1) 와 VirtualBox (Type 2), 그리고 KVM (hybrid) 에서 각각 어떤 path 로 처리되는지 비교합니다. 아래에서 **libvirt/virsh**(VM 을 만들고 관리하는 표준 도구·라이브러리)가 **QEMU**(VM 을 대신 띄우고 디바이스를 흉내 내는 user-space 프로그램)를 띄우고, QEMU 는 **ioctl**(user-space 가 커널 드라이버에 명령을 거는 시스템 콜)로 `/dev/kvm` 에 요청합니다.
...
**실제**: VirtualBox / VMware Workstation 같은 Type 2 는 _host OS 스케줄러가 vCPU 를 throttle_ 합니다. **SLA**(Service Level Agreement — "99.99% 응답" 같이 보장하기로 약속한 서비스 품질) 가 있는 서비스에서는 **jitter**(응답 시간이 들쭉날쭉 흔들리는 정도) 가 통제 불가능. Production 은 Type 1 또는 KVM (Hybrid) 만.<br>
...
| 증상 | 1차 의심 | 어디 보나 |
|---|---|---|
| 같은 코드가 ESXi 에선 빠른데 VirtualBox 에선 30% 느림 | Host OS 스케줄러 jitter (Type 2 특성) | `vmstat`/`top` host 측 CPU steal time(vCPU 가 실행되길 원했지만 host 가 다른 일을 하느라 뺏긴 시간), VM 의 `/proc/stat` steal time |
