---
title: "Module 01 — Virtualization Fundamentals"
---
...
Hypervisor 는 HW 와 Guest OS 사이에 위치하여 세 가지 역할을 동시에 수행합니다. HW 자원을 VM 별로 분할하고, Guest OS 가 특권 명령어를 실행하려 하면 trap 으로 가로채어 대신 처리하며, Stage-2 page table 로 VM A 가 VM B 의 메모리에 접근하지 못하도록 막습니다.
...
:::tip[🤔 Q1 — Popek & Goldberg 3 요건 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — VT-x ON 의 _필요_ 조건성 (Bloom: Evaluate)]
...
→ [Module 01A — System Architecture Evolution](../01a_system_architecture_evolution/): 가상화가 어디서 왔는지 — HW Only → MMU → IOMMU → Hypervisor 의 7 단계 진화로 전체 그림을 잡습니다.

[퀴즈 풀어보기 →](../quiz/01_virtualization_fundamentals_quiz/)
