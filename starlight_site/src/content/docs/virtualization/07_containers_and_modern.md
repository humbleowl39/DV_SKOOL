---
title: "Module 07 — Containers & Modern Virtualization"
---
...
- [Module 01-06](../01_virtualization_fundamentals/) — Hypervisor / trap / 격리 모델
:::
---
...
AWS Lambda — **serverless**(서버를 직접 띄워두지 않고, 요청이 올 때만 코드를 잠깐 실행하고 끝나는 방식). 사용자가 함수 호출 시 _AWS 가 즉시 VM 띄워서 실행_.

요구사항:
- **Cold start**(요청이 처음 와서 실행 환경을 새로 띄우는 데 걸리는 초기 지연): <500 ms (사용자가 _기다린다고 느낌_).
...
같은 "가상화" 라는 단어 아래 두 개의 완전히 다른 모델이 있습니다 — **kernel**(OS 의 핵심부 — 하드웨어를 직접 제어하고 프로세스·메모리를 관리하는 코어) 을 _분리_ 하는 VM 과 kernel 을 _공유_ 하는 **container**(host 의 kernel 을 같이 쓰면서 프로세스 단위로만 격리한 경량 실행 단위). 이 한 줄의 차이가 startup 시간 (분 vs ms), **density**(서버 한 대에 몇 개를 욱여넣을 수 있는지의 밀도), 격리 (강함 vs 약함), 보안 표면 (작음 vs 큼) 의 _모든 trade-off_ 를 결정합니다. 그리고 2017 년 이후 등장한 **microVM**(device 모델과 kernel 을 최소화해 빠르게 뜨는 아주 작은 VM; Firecracker, kata) 은 _container 의 속도_ 와 _VM 의 격리_ 를 동시에 얻으려는 hybrid — 이걸 모르면 **FaaS**(Function as a Service — 함수 단위로 실행하는 serverless 서비스, 예: AWS Lambda)·Fargate·GKE Sandbox 가 왜 그렇게 생겼는지 답이 안 나옵니다.

이 모듈을 건너뛰면 "Docker 가 VM 보다 빠른 이유" 같은 면접 질문에 **"OS 부팅이 없어서"** 라는 _문장 한 줄_ 로만 답하게 됩니다. 반대로 **namespace**(한 프로세스가 보는 시스템 자원의 범위 — PID·네트워크·파일시스템 등 — 를 격리하는 Linux 기능) + **cgroup**(control group — 프로세스 그룹의 CPU·메모리 사용량에 상한을 거는 Linux 기능) + kernel 공유 의 세 단어가 _어떤 격리 boundary 를 만들고 어디가 약한지_ 를 잡으면, container escape CVE(공개된 보안 취약점) 가 나올 때마다 _어느 boundary 가 깨졌는지_ 즉시 보입니다.
...
가장 단순한 시나리오. 사용자가 `docker run nginx` 한 줄을 입력했을 때 host kernel 안에서 _container 가 어떻게 만들어지고 nginx 가 어떻게 실행되는지_ step-by-step. 등장 도구: **containerd/runc**(container 를 실제로 만들고 실행하는 하위 런타임 — docker 는 이들을 호출), **OverlayFS**(여러 읽기 전용 layer 위에 쓰기 layer 를 겹쳐 한 파일시스템처럼 보이게 하는 기법), **pivot_root**(프로세스가 보는 루트 디렉토리를 container 전용으로 바꿔치기하는 시스템 콜).
...
> **Kubernetes**(컨테이너를 여러 서버에 자동 배치·관리하는 오케스트레이션 플랫폼; K8s)에서 **Pod**(함께 배치되는 컨테이너 묶음의 최소 단위)는 위처럼 Kata 에서 경량 VM 하나로 감싸집니다. **OCI**(Open Container Initiative — 컨테이너 이미지·런타임의 업계 표준)는 이런 런타임들이 서로 호환되도록 하는 규격입니다.
...
:::caution[실무 주의점 — kernel 공유로 인한 namespace escape (CVE-2022-0492 류)]
...
:::tip[🤔 Q1 — Container vs VM 격리 강도 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — Firecracker 의 가치 (Bloom: Evaluate)]
...
→ [Module 08 — Quick Reference Card](../08_quick_reference_card/): 전 모듈을 한 장으로 — 면접/실무에서 즉시 꺼내 쓸 표와 체크리스트.

[퀴즈 풀어보기 →](../quiz/07_containers_and_modern_quiz/)
