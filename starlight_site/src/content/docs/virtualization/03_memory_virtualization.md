---
title: "Module 03 — Memory Virtualization"
---
...
- [Module 01](../01_virtualization_fundamentals/), [Module 02](../02_cpu_virtualization/)
:::
---
...
Bare metal 에서 OS 는 전체 물리 메모리를 직접 파악하고 있으며, 프로세스의 가상 주소를 페이지 테이블로 물리 주소 하나로 바꿔주면 끝납니다. 변환이 1 단계로 충분한 이유입니다.
...
가상화 환경에서는 이 전제가 깨집니다. 각 Guest OS 는 자기가 0x0 부터 시작하는 연속된 물리 메모리를 독점한다고 믿지만, 실제로 여러 VM 이 동일 물리 장치를 나눠 씁니다. 두 VM 이 같은 물리 주소를 쓰면 데이터가 충돌하고, Guest OS 가 직접 페이지 테이블을 관리하면 다른 VM 의 메모리를 침해할 수도 있습니다.
...
Hypervisor 가 VA → PA 를 직접 변환하는 "그림자" 페이지 테이블을 유지합니다. Guest OS 의 페이지 테이블(VA → IPA)과 별도로, Hypervisor 가 VA → PA 를 담은 Shadow PT 를 따로 만들어 MMU 에는 Shadow PT 를 로드합니다. Guest OS 는 자기 페이지 테이블을 자유롭게 수정한다고 생각하지만, MMU 가 실제로 사용하는 것은 Hypervisor 가 관리하는 Shadow PT 입니다.
...
:::tip[🤔 Q1 — Shadow Page Table 폐기 이유 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — TLB invalidate 시점 (Bloom: Apply)]
...
→ [Module 04 — I/O Virtualization](../04_io_virtualization/): CPU + Memory 의 가상화 모델은 잡았으니, 이제 _디바이스_ 가상화 — Emulation / VirtIO / SR-IOV / Pass-through.

[퀴즈 풀어보기 →](../quiz/03_memory_virtualization_quiz/)
