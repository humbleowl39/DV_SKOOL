---
title: "Module 02 — CPU Virtualization"
---
...
- [Module 01](../01_virtualization_fundamentals/), [Module 01A](../01a_system_architecture_evolution/)
:::
---
...
Hypervisor 를 설계한다는 것은 Guest OS (Linux) 가 자기가 ring 0 (kernel mode — CPU 의 최고 권한 모드) 에 있다고 믿으면서 동작해야 한다는 전제를 지키는 것입니다. 그런데 Guest 가 `CR3` (page table base, 즉 현재 주소 변환표의 시작 주소) 를 변경하면 hypervisor 의 메모리 매핑이 깨져 다른 VM 을 침해하고, `HLT`(halt — CPU 를 멈추는 명령) 를 실행하면 전체 시스템이 정지하며, `INVLPG`(invalidate page — 특정 주소의 TLB 항목을 비우는 명령) 를 실행하면 의도하지 않은 **TLB**(Translation Lookaside Buffer — 최근 주소 변환 결과를 캐시하는 고속 버퍼) 가 무효화됩니다. Guest OS 의 **특권 명령어**(privileged instruction — 커널만 실행할 수 있는, 하드웨어 핵심 상태를 바꾸는 명령)를 그냥 내버려 두면 격리 자체가 무너집니다.

소프트웨어로 이 문제를 푸는 첫 번째 시도는 Guest 코드를 실행 전에 스캔하여 **sensitive instruction**(하드웨어 상태를 바꾸거나 읽어서 가상화 시 hypervisor 가 가로채야 하는 명령)을 hypervisor 핸들러 호출로 동적 재작성하는 방법이었습니다. 이것이 **Binary Translation**(Guest 의 위험한 명령을 실행 직전 안전한 명령열로 바꿔치기하는 SW 기법) 입니다. 동작은 하지만 변환 오버헤드와 구현 복잡도가 크고, self-modifying code 처리가 어렵습니다. 두 번째 시도는 Guest OS 커널을 직접 수정하여 sensitive 명령어를 **hypercall**(Guest 가 hypervisor 에게 직접 일을 요청하는 호출 — 시스템 콜의 가상화 버전) 로 대체하는 **Para-virtualization**(Guest OS 를 가상화에 맞게 고쳐 trap 을 피하는 방식) 입니다. trap 오버헤드를 없앤 대신, 소스가 없는 Windows 처럼 변경 불가능한 OS 에는 적용할 수 없습니다.

**Intel VT-x** 와 **AMD-V**(각각 Intel·AMD 의 하드웨어 가상화 확장 — 서로 등가물) 는 이 문제를 하드웨어 레벨에서 근본 해결했습니다. CPU 에 _VMX root mode_ 와 _VMX non-root mode_ 를 추가하여 hypervisor 와 Guest 가 각자 자기 영역의 ring 0 을 갖도록 분리했습니다. Guest 는 non-root 의 ring 0 에서 자기가 최고 권한이라고 믿으면서 동작하고, sensitive instruction 이 실행되면 HW 가 자동으로 **VM Exit**(Guest 실행이 중단되고 hypervisor 로 제어가 넘어가는 전환) 을 발생시켜 root mode 의 hypervisor 가 가로채서 emulate 합니다. 일반 명령어는 그대로 native 실행됩니다.
...
:::tip[🤔 잠깐 — _VM Exit_ 의 _cost_ 가 얼마?]
...
X86: "x86 (VT-x) — 두 root 의 분리" {
  ROOT: "VMX root mode (Hypervisor)\nRing 0..3"
  NONROOT: "VMX non-root mode (Guest)\nRing 0..3"
  ROOT <-> NONROOT: "VMLAUNCH / VM Exit"
}

ARM: "ARM (EL2) — 처음부터 4 단계 EL" {
  EL3: "EL3 (Secure Monitor)"
  EL2: "EL2 (Hypervisor)"
  EL1: "EL1 (Guest OS)"
  EL0: "EL0 (User App)"
  EL3 <-> EL2: "exception level\ntransitions\n(HVC, ERET, ...)"
  EL2 <-> EL1: "HVC"
  EL1 <-> EL0: "SVC"
}
```
...
가장 단순한 시나리오. Guest Linux 가 부팅 중 `RDMSR(IA32_APIC_BASE)` 한 번을 실행하는 순간을 step-by-step. 여기서 **MSR**(Model-Specific Register — CPU 의 동작을 설정·조회하는 특수 레지스터 모음)은 특권 자원이라, Guest 가 **RDMSR**(read MSR — MSR 값을 읽는 명령)로 건드리면 가로채집니다. 읽으려는 대상은 **APIC**(Advanced Programmable Interrupt Controller — CPU 의 인터럽트를 관리하는 컨트롤러)의 base 주소입니다.
...
# Note over G: APIC base 읽으려 함\nRDMSR ECX=0x1B
G -> G: "① ECX = 0x1B 셋업"
G -> HW: "② RDMSR (sensitive)"
HW -> H: "③ VM Exit (reason=31, RDMSR)\nGuest RIP/RSP 저장\nVMCS guest-state 갱신"
H -> H: "④ MSR 0x1B emulate 대상?"
H -> H: "⑤ Hypervisor 가 값 fabricate\n(예: vAPIC base 0xFEE00000)"
...
Guest OS 는 원래 bare metal 에서 동작하도록 설계되었습니다. 즉, 자기가 가장 높은 권한을 갖고 있다고 가정합니다. 그 가정 아래에서 Guest OS 는 페이지 테이블 설정(CR3 / **TTBR** — ARM 의 Translation Table Base Register, x86 의 CR3 에 대응하는 page table 시작 주소 레지스터 쓰기), 인터럽트 활성화 / 비활성화, I/O 포트 접근, CPU 모드 전환 등을 자유롭게 수행하려 합니다.
...
ARM 은 처음부터 가상화를 고려한 **Exception Level**(EL — ARM 의 권한 레벨; 숫자가 클수록 높은 권한, EL0=응용 ~ EL3=보안 모니터) 설계. 레벨 간 전환은 전용 명령으로 일어납니다 — **SVC**(Supervisor Call, 응용→OS 시스템 콜), **HVC**(Hypervisor Call, OS→hypervisor 호출), **SMC**(Secure Monitor Call, 보안 세계 전환).
...
**실제**: Guest 가 tight loop 에서 CPUID(CPU 의 기능·식별 정보를 조회하는 명령 — 가상화에서는 VM Exit 유발), RDMSR, I/O port 읽기를 반복하면 초당 수십만 VM Exit 발생 가능. 성능이 native 의 50% 이하로 떨어짐.<br>
**왜 헷갈리는가**: "VT-x 가 효율적" 이라는 일반론. 실제로는 _Guest 코드 패턴_ 이 결정.
:::
:::danger[❓ 오해 5 — 'KVM 의 vCPU 는 thread 와 1:1 이다']
**실제**: **KVM**(Linux 커널에 내장된 hypervisor) 의 **vCPU**(virtual CPU — VM 에게 CPU 처럼 보이는 가상 CPU) 는 host 의 한 thread 에 매핑되지만, host scheduler 가 그 thread 를 _다른 물리 CPU 로 옮길 수 있다_. CPU pinning 을 안 하면 **NUMA**(Non-Uniform Memory Access — CPU 마다 가까운 메모리/먼 메모리가 나뉘어 접근 지연이 다른 구조) 횡단 액세스로 성능 저하.<br>
...
:::tip[🤔 Q1 — VM Exit 의 비용 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — VHE 의 효과 (Bloom: Evaluate)]
...
→ [Module 03 — Memory Virtualization](../03_memory_virtualization/): CPU 가 sensitive 명령을 trap 하는 모델은 알았으니, 이제 _메모리 주소 자체_ 가 어떻게 가상화되는지 — VA → IPA → PA 의 2-stage translation.

[퀴즈 풀어보기 →](../quiz/02_cpu_virtualization_quiz/)
