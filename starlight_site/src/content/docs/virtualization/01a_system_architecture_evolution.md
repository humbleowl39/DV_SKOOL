---
title: "Module 01A — System Architecture Evolution"
---
...
:::
:::note[사전 지식]
- [Module 01](../01_virtualization_fundamentals/) — 가상화의 3 대 요소
...
이 구조의 결정적 한계는 요구사항이 바뀌는 순간 HW 를 처음부터 다시 설계해야 한다는 점입니다. 그 불편이 쌓여 "고쳐 쓸 수 있는 로직" — 프로세서 — 의 필요성이 인정됐습니다.
...
그러나 범용 프로세서만으로는 영상처리 / 암호화 같은 연산 집약적 작업이 부족합니다. 이 성능 부족이 구체적인 제품 요구사항으로 드러나면서 전용 HW 가속기를 곁에 붙이는 다음 단계로 이어졌습니다.
...
이 구성의 한계는 세 방향에서 동시에 나타납니다. 확장성이 부족하고, 메모리 보호 기능이 없으며, 사용자 프로그램과 시스템 프로그램을 분리할 방법이 없습니다. 세 한계가 맞물리면서 OS 와 MPU 를 도입하는 단계 4 로 자연스럽게 이어졌습니다.
...
그러나 IOMMU 가 없다는 치명적 약점이 남아 있습니다. 그 약점이 운영 현장에서 DMA 보안 문제로 드러나면서 단계 6 의 IOMMU + PEs + LLC + HW Coherency 묶음이 한꺼번에 도입됐습니다.
...
SW 가상화 방식의 근본 한계는 모든 I/O 가 Hypervisor 를 경유해야 한다는 점에서 옵니다. HW 가속기 설정도 Hypervisor 가 대행하고, VM 수가 늘어날수록 오버헤드가 선형 증가합니다.
...
VM1: "VM1 Guest OS"
VM2: "VM2 Guest OS"
VM0 -> VF0: "pass-through" { style.stroke-dash: 4 }
...
:::
:::tip[🤔 Q2 — 7 단계 진화 ROI (Bloom: Evaluate)]
...
→ [Module 02 — CPU Virtualization](../02_cpu_virtualization/): 7 단계 중 단계 7 의 _CPU 부분_ 을 깊게 — Trap-and-emulate, VMX root/non-root, ARM EL2, VMCS, VHE.

[퀴즈 풀어보기 →](../quiz/01a_system_architecture_evolution_quiz/)
