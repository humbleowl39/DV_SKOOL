...
수천 개의 GPU/TPU 가속기와 분산 메모리(HBM, DDR5)를 동기화해 LLM을 학습한다고 합시다. 어느 노드의 한 **SoC**(System on Chip, 프로세서·메모리·가속기 등을 한 칩에 모은 시스템 칩) 안에서, 로컬 **SRAM**(Static RAM, 칩 안에 두는 빠른 휘발성 메모리 — 보통 캐시에 쓰임) 캐시의 한 워드에 정정 불가능한 2-bit 에러(uncorrectable error)가 발생합니다. 이 한 비트가 어떻게 되는지에 따라 결과가 갈립니다.
...
| **노화 가속 & 밀도** | sub-3nm + 2.5D/3D(CoWoS) 고밀도 + 100% 가동률 → 열·전압 droop(부하 급증 시 공급 전압이 순간 처지는 현상) → electromigration(전류가 금속 배선의 원자를 밀어내 끊거나 단락시키는 노화), thermal cycling(가열·냉각 반복으로 재료에 누적되는 피로) | transient/permanent fault 빈도 급증을 검출·격리로 흡수 |
| **SDC 방지** | 에러가 silicon에서 안 잡히면 오염 데이터가 추론 파이프라인으로 전파 | 알람 없이 모델 출력 무결성이 깨지는 최악의 escape |

여기서 두 종류의 고장을 구분해 두면 이후가 쉽습니다 — **transient fault**(일시적 고장; 우주선·노이즈로 비트가 한 번 뒤집혔다 다음 write에서 정상화되는, 회로 자체는 멀쩡한 soft error)와 **permanent fault**(영구 고장; 노화·물리 손상으로 해당 셀/배선이 계속 틀린 값을 내는 hard error)입니다. RAS는 둘 다 검출하되, 영구 고장은 해당 자원을 격리하는 식으로 다르게 대응합니다.
...
| 메커니즘 | 어디에 | 역할 |
|----------|--------|------|
| **ECC (SEC-DED)** | 온칩 L1/L2/L3 SRAM 캐시, register file(레지스터 파일 — CPU 코어 안의 작은 고속 레지스터 묶음), 외부 HBM/DDR5 인터페이스 | 1-bit를 on-the-fly로 정정(corrected error), 2-bit를 안전하게 검출·플래그 |
...
| 메커니즘 | 동작 |
|----------|------|
| **Error recording & telemetry** | 표준화된 RAS Node(Arm의 `ERR<n>STATUS` 레지스터 아키텍처). 에러 발생 시 HW가 type/failing address/timestamp를 memory-mapped 레지스터(메모리 주소를 읽고 쓰듯 접근하는 제어 레지스터)에 자동 기록하고, 외부 SCP(System Control Processor)나 BMC(Baseboard Management Controller)로 비동기 인터럽트(error logging)를 올림. telemetry란 이렇게 수집한 에러·상태 정보를 관리 주체로 보내는 원격 보고 데이터를 말함 |
| **Fault injection 모델** | pre-silicon(칩 제조 전, 시뮬레이션 단계) 검증과 post-silicon(실물 칩이 나온 뒤) 테스트 모두에 핵심. 특정 레지스터를 프로그래밍해 runtime에 가짜 에러를 주입(fault injection), 물리적 고장 없이 내부 RAS 로직·인터럽트·telemetry 경로가 올바로 동작하는지 검증 |
