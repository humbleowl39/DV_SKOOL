...
Fault injection은 "특정 레지스터를 프로그래밍해 runtime에 가짜 에러를 주입"하는 HW 기능입니다. 즉 _RTL을 건드리는 것이 아니라 칩이 제공하는 inject 레지스터를 시퀀스로 쓰는 것_ 이 정석입니다. 이 모듈은 그 검증 방법론을 다룹니다.
...
> 정확한 비트 필드·인코딩은 Arm® RAS System Architecture 사양에서 재확인이 필요합니다. 위 표는 아키텍처 수준의 일반 형태입니다(추론).

**inject 비트가 _실제 에러를 만드는_ 경로.** "inject enable을 켜면 다음 접근에서 가짜 에러"라는 결과 뒤의 메커니즘은 구현에 따라 둘로 나뉩니다(확인 필요). (1) **ECC check-bit 강제 플립형** — inject 레지스터에 "다음 write 시 코드 비트 일부를 일부러 틀리게 저장"하도록 마스크를 주면, 나중에 그 워드를 read할 때 _정상 디코더 경로_ 가 syndrome≠0을 만나 CE/UE를 _자연스럽게_ 보고합니다(검출 로직을 우회하지 않고 그대로 통과 — 검증 가치가 높음). (2) **fault 상태 비트 강제형** — 디코더 앞단에 inject 상태 비트를 두어, 다음 접근에서 디코더에게 "이번엔 CE다/UE다"를 직접 신호해 STATUS를 set시킵니다(코드워드를 실제로 망가뜨리진 않음 — 더 가벼우나 ECC 산술 경로 일부는 미자극). 핵심 차이: (1)은 _인코딩~디코딩 산술_ 까지 자극하고, (2)는 _보고 경로_ 만 자극합니다. 검증에서는 해당 SoC가 어느 방식인지 확인해, (2)라면 ECC 산술 자체는 [M02](../02_ecc_parity_poison/)식 직접 코드워드 손상으로 별도 자극해야 함을 알아야 합니다.
...
error record의 status 비트는 보통 **W1C(write-1-to-clear)** 입니다. 즉 1을 써야 _clear_ 됩니다.

**HW에서 W1C 비트가 동작하는 방식 — 비대칭 래치.** W1C 비트는 set과 clear의 _주체가 다른_ 비대칭 플립플롭입니다. **set은 HW만** — 에러를 검출한 RAS 로직이 내부적으로 1로 올립니다(SW가 1을 써도 set되는 건 아님, "1 write"는 _clear 명령_ 으로 해석). **clear는 SW만** — 버스로 그 비트에 1을 write할 때만 0으로 떨어집니다(0을 쓰면 무시 — 그래서 다른 비트를 건드리지 않고 특정 비트만 clear 가능). 회로적으로는 대략 `next = (hw_error_detected) ? 1 : (sw_write_data & sw_write_strobe) ? 0 : current` 형태입니다. **set과 clear가 같은 사이클에 충돌하면 보통 set(HW)이 우선** — 막 새로 난 에러를 놓치지 않기 위해, SW가 옛 에러를 clear하는 같은 순간 새 에러가 오면 비트는 1로 남습니다(추론: 일반 RAS 정책, 사양 확인 필요). 이 비대칭이 "에러는 HW가 자동 기록, SW는 처리 후 명시적으로 지운다"는 RAS 보고 모델을 회로로 구현한 것입니다.

여기서 RAL의 `update()`를 쓰면 함정에 빠집니다 — desired==mirror면 write를 생략하기 때문입니다. clear는 "같은 비트에 1을 다시 써야 의미"가 있으므로, `update` 대신 명시적 `write`를 써야 합니다. (이 함정은 [UVM M07 RAL](../../uvm/07_register_layer_ral/)의 update 주의와 동일한 패턴입니다.)
...
- 에러 주입은 시퀀스 레벨에서만 수행하고 RTL은 수정하지 않는 것이 원칙입니다.
