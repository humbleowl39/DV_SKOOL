---
title: "Module 03 — Embedding & Vector DB"
---
...
:::
:::note[사전 지식]
- [Module 01](../01_llm_fundamentals/) — context window 한계, KV cache 메모리
- [Module 02](../02_prompt_engineering/) — Few-shot, ICL
- 기본 선형대수 (벡터 내적, 노름)
...
:::tip[🤔 잠깐 — 왜 _BM25 키워드 검색_ 으로 안 되나?]
BM25(키워드 빈도 기반의 고전 검색 점수 함수 — 질의 단어가 문서에 얼마나 자주/희소하게 나오는지로 순위를 매김; TF-IDF 의 개선) 도 도서관 검색 표준. 여기서 TF-IDF(Term Frequency–Inverse Document Frequency — 흔한 단어는 깎고 드문 단어는 키워 단어 중요도를 재는 고전 가중치). 왜 _embedding_ 이 필요?
...
두 약어를 먼저 — **ANN**(Approximate Nearest Neighbor — 정확한 최근접 탐색 대신 약간의 정확도를 양보하고 "가까운 벡터"를 훨씬 빠르게 찾는 근사 검색), **FAISS**(Facebook AI Similarity Search — Meta 가 공개한 대규모 벡터 검색 라이브러리; §5.3).
...
순진하게 매 쿼리마다 _모든_ chunk 를 LLM 에 넣으면 ① context window 초과, ② 토큰 비용 폭증, ③ "lost in the middle" 로 정확도 저하 — 이 셋이 동시에 깨집니다. 그래서 (1) 의미 좌표(embedding) 로 압축하고 (2) 가까운 좌표만 빠르게 (ANN) 골라 (3) 그 부분만 LLM 에 주입하는 _3 단 분리_ 가 RAG 시스템의 표준이 되었습니다. 이 모듈은 (1)+(2) 의 인프라, [Module 04](../04_rag/) 가 (3) 까지 합치는 단입니다.

---

## 3. 작은 예 — `"TLB flush"` 쿼리 한 개를 end-to-end 추적

가장 단순한 시나리오. 사내 IP 스펙 1000 청크가 이미 인덱싱돼 있고, `"TLB flush"` 라는 쿼리 한 개를 던져 top-3 청크가 나오기까지를 추적합니다. 아래에 나오는 **cosine similarity**(코사인 유사도 — 두 벡터가 이루는 각도로 의미 가까움을 재는 값; 1 에 가까울수록 유사, 크기는 무시), **IVF**(Inverted File — 벡터를 미리 군집으로 나눠 일부 군집만 뒤져 검색을 빠르게 하는 인덱스; §5.3), **nprobe**(IVF 에서 검색할 때 실제로 뒤져 볼 군집 개수 — 클수록 정확·느림)를 먼저 알아 두면 됩니다.
...
:::note[왜 "비슷한 의미" 가 "가까운 벡터" 가 되는가 — 인과의 한 줄]
이 성질은 우연이 아니라 _loss 함수가 직접 강제_ 한 결과입니다. 위 InfoNCE 의 분자 `exp(sim(q, k⁺)/τ)` 를 키우려면 긍정 쌍 `(q, k⁺)` 의 유사도 `sim` 이 커져야 하고, 분모의 부정 쌍 항들은 작아져야 합니다. 학습 시 gradient 가 바로 이 방향 — _긍정 쌍의 두 벡터를 끌어당기고(거리↓), 부정 쌍은 밀어내도록(거리↑)_ — 으로 임베딩 파라미터를 움직입니다. 즉 "의미가 비슷하면 가깝다" 는 모델이 _그렇게 되도록 학습되었기 때문_ 이며, 그래서 §1 의 "TLB flush ↔ TLB invalidation" 같은 동의 표현이 같은 좌표 영역에 모이는 것입니다.
:::

### 4.4 ANN 알고리즘의 세 갈래

표의 세 갈래를 먼저 한 줄로 (메커니즘 상세는 바로 아래 소절) — **HNSW**(Hierarchical Navigable Small World — 가까운 벡터끼리 간선으로 이은 다층 그래프를 hop 하며 빠르게 찾는 인덱스), **PQ**(Product Quantization — 벡터를 조각내고 각 조각을 대표점 번호로 치환해 메모리를 수십 배 줄이는 압축), **Brute-force/Flat**(전수 비교 — 모든 벡터와 일일이 거리 계산; 정확하지만 느림).
...
#### HNSW 가 _어떻게_ 빠른가 — 다층 그래프 탐색의 기계적 동작

표의 "빠르고 정확" 은 결과일 뿐, 그 빠름이 어디서 오는지를 보면 ANN 의 핵심 직관이 잡힙니다. HNSW 는 벡터들을 **노드로, "가까운 벡터끼리 간선으로 잇는" 그래프** 로 만들고, 이 그래프를 _여러 층_ 으로 쌓습니다. 위층으로 갈수록 노드 수가 _기하적으로 줄어들어_ (예: 한 층 올라갈 때마다 약 1/m 로 감소), 최상위 층은 전체의 극소수 노드만 가진 _성긴_ 그래프가 됩니다. 비유하면 _고속도로(상위 층) → 국도(중간 층) → 골목(최하위 층)_ 의 계층 도로망입니다.

탐색은 다음과 같이 진행됩니다. (1) 최상위 층의 한 진입점에서 시작해, **현재 노드의 이웃 중 쿼리에 더 가까운 노드로 greedy 하게 hop** 합니다 — 더 가까운 이웃이 없으면 그 층의 지역 최적점에 도달한 것입니다. (2) 그 지점을 다음 (더 촘촘한) 층의 진입점으로 삼아 다시 greedy hop 합니다. (3) 최하위 층(모든 벡터가 있는 층)에 내려와서야 정밀하게 탐색해 top-k 를 추립니다.

왜 이게 O(N) 전수 검색보다 압도적으로 빠른가? 상위 층의 한 번의 hop 이 좌표 공간에서 _큰 거리_ 를 단번에 건너뛰기 때문입니다 — 고속도로로 대륙을 가로지른 뒤 골목에서만 미세 조정하는 셈입니다. 성긴 상위 층이 쿼리 근방으로 _대강 순간이동_ 시켜 주고, 촘촘한 하위 층이 _마지막 정밀화_ 만 담당하므로, 방문하는 노드 수가 전체 N 이 아니라 대략 **층 수 × 층당 hop 수 ≈ O(log N)** 규모에 그칩니다. 메모리가 큰 이유도 같은 메커니즘의 대가입니다 — 각 노드가 여러 이웃 간선 리스트를 (그리고 여러 층에서) 들고 있어야 그래프 hop 이 성립하기 때문입니다.

이것이 soc_integration 토픽의 [FAISS 유사 IP 검색](../../soc_integration_cctv/03_tb_top_and_ai/) 이 "embedding → nearest-neighbor" 한 줄로 끝낼 수 있는 근거입니다 — "가까운 벡터 = 비슷한 의미" 라는 §4.1 의 성질 위에, HNSW/IVF 가 그 가까운 벡터를 _전수 비교 없이_ 찾아 주기 때문입니다.

#### PQ 가 메모리를 1/8~1/32 로 줄이는 원리

PQ(Product Quantization) 의 절감도 결과가 아니라 메커니즘으로 이해할 수 있습니다. 핵심은 **하나의 긴 벡터를 여러 sub-vector 로 쪼갠 뒤, 각 조각을 "codebook 의 대표 벡터 인덱스" 한 개로 치환** 하는 것입니다. 예를 들어 768 차원 float32 벡터(= 768×4 = 3072 byte)를 8개의 96 차원 sub-vector 로 나누고, 각 sub-vector 공간에서 미리 K-means 로 256개 대표점(codebook)을 학습해 둡니다. 그러면 각 조각은 "256개 중 몇 번째 대표점에 가장 가까운가" 라는 _1 byte(8 bit)_ 인덱스로 표현되므로, 벡터 하나가 3072 byte → 8 byte 로 줄어듭니다. 원본을 버리고 _대표점 번호만_ 저장하기에 정확도는 일부 손실되지만(양자화 오차), 메모리가 수십 배 줄어 수십억 벡터를 단일 머신에 올릴 수 있게 됩니다. 그래서 압도적 규모에서는 IVF(후보 좁히기) + PQ(메모리 압축) 를 조합합니다.
...
여기서 **BGE**(BAAI General Embedding — 베이징 BAAI 가 공개한 고성능 오픈소스 임베딩 모델 계열; 로컬 배포에 자주 쓰임), **Sentence-BERT**(문장 전체를 한 벡터로 임베딩하도록 BERT 를 변형한 오픈소스 모델)가 표에 나옵니다.
...
FAISS 외에도 다양한 Vector DB 선택지가 있습니다. 무엇을 선택할지는 배포 환경(로컬/클라우드), 운영 복잡도, 검색 방식 요구에 따라 달라집니다.
...
| recall 은 좋은데 LLM 답변이 hallucinate | 검색 자체는 OK, LLM 단의 문제 → [Module 04](../04_rag/) §6 |
...
- **품질 평가** — top-k recall, MRR(Mean Reciprocal Rank — 정답이 검색 결과의 몇 번째에 나오는지를 1/순위 로 점수화해 평균; 정답이 위에 올수록 높음), nDCG(normalized Discounted Cumulative Gain — 관련도 높은 결과가 상위에 올수록 점수가 커지는 순위 품질 지표). retrieval 이 망가지면 LLM 이 아무리 좋아도 답이 망가진다.

:::caution[실무 주의점 — Embedding 모델 교체 시 기존 인덱스 전체 재구축 필요]
...
:::tip[🤔 Q1 — Chunking 전략 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Recall vs Precision (Bloom: Evaluate)]
...
:::
:::tip[🤔 Q3 — Embedding 모델 선택 (Bloom: Analyze)]
...
→ [Module 04 — RAG (Retrieval-Augmented Generation)](../04_rag/): embedding/vector DB 를 LLM 호출과 결합.

[퀴즈 풀어보기 →](../quiz/03_embedding_vectordb_quiz/)
