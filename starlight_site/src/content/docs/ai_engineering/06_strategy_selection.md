---
title: "Module 06 — Strategy Selection (Prompt vs RAG vs Fine-tune)"
---
...
:::
:::note[사전 지식]
- [Module 02](../02_prompt_engineering/) — prompt 5 축
- [Module 03](../03_embedding_vectordb/) — embedding, chunking
- [Module 04](../04_rag/) — RAG 4 단계
- [Module 05](../05_agent_architecture/) — Agent loop
...
사내 SystemVerilog 코딩 어시스턴트를 만들려 한다고 합시다. 가능한 옵션은 네 가지이며 비용·정확도·유지보수 측면에서 완전히 다른 profile 을 가집니다.

이 모듈은 앞서 따로 배운 세 도구 — **Prompt**(입력 설계만 바꿈), **RAG**(외부 지식을 검색해 붙임), **Fine-tune**(가중치를 추가 학습) — 중 _언제 무엇을_ 쓸지 고르는 의사결정이 주제입니다.
...
이 모듈은 [Module 02-05](../02_prompt_engineering/) 까지 따로 배운 4 가지 도구 (prompt / RAG / agent / fine-tune) 를 _하나의 의사결정 트리_ 로 합칩니다. [Module 07](../07_dv_application/) 의 모든 사례도 이 트리로 분류됩니다.

:::tip[🤔 잠깐 — 선택을 _되돌리기_ 의 비용?]
...
세 전략은 **상호 배타적이 아니라 보완**. _가장 싼_ 도구 (Prompt) 부터 시작해 한계가 보일 때만 다음 단계로 escalation(에스컬레이션 — 더 비싸지만 강력한 다음 수단으로 단계적으로 올라가기) 하는 것이 ROI 최적. Fine-tune 으로 시작하면 (1) 데이터 준비 수 주, (2) 학습 비용 수천~수만 달러, (3) 새 모델 출시마다 재학습 — 에 막대한 매몰 비용을 만들고 나서야 "사실 prompt 만으로도 됐는데" 깨닫게 됩니다.
...
표의 기법들을 먼저 한 줄로 — **LoRA**(Low-Rank Adaptation — 거대한 가중치 행렬은 얼린 채, 옆에 붙인 작은 저랭크 행렬만 학습해 전체의 0.1~1% 만 갱신하는 경량 파인튜닝; 아래 상세), **QLoRA**(LoRA 를 4-bit 양자화 모델 위에서 돌려 메모리를 더 줄인 버전), **Prefix Tuning**(가중치는 그대로 두고 입력 앞에 학습 가능한 가상 토큰 벡터만 붙여 행동을 바꾸는 방식).
...
Fine-tuning 을 선택하기 전에 다음 다섯 가지 상황을 먼저 확인해야 합니다.

데이터가 500개 미만이라면 Fine-tuning 은 오히려 역효과입니다. 적은 데이터로 학습하면 과적합이 쉽게 일어나고, 이 경우 Few-shot + RAG 가 훨씬 효과적입니다. 지식이 자주 변한다면 — 예를 들어 IP 스펙이 매주 갱신된다면 — Fine-tuning 마다 재학습이 필요하므로 RAG 로 인덱스만 갱신하는 방식이 압도적으로 유리합니다.

코드 생성, 로그 분석, 검색 등 태스크가 다양하다면, 특정 패턴에 특화된 Fine-tuned 모델보다 범용 모델에 태스크별 프롬프트를 조합하는 편이 훨씬 유연합니다. 보안 제약도 중요한 판단 기준입니다. Fine-tuned 모델에는 기밀 데이터가 가중치에 영구적으로 새겨지므로, 모델이 유출되면 데이터도 함께 노출됩니다. RAG 는 검색 시에만 데이터에 접근하고 모델에는 저장되지 않으므로 보안 위험이 낮습니다. 마지막으로, 기반 모델이 자주 업그레이드된다면 Fine-tuning 을 매번 재실행해야 하지만, Prompt + RAG 는 모델을 즉시 교체할 수 있습니다.
...
:::caution[실무 주의점 — Fine-tune 후 기반 능력 손상(Catastrophic Forgetting)]
...
:::
:::tip[🤔 Q2 — RAG vs Long-context (Bloom: Evaluate)]
...
:::
:::tip[🤔 Q3 — Hybrid 의 함정 (Bloom: Analyze)]
...
→ [Module 07 — DV/EDA Application](../07_dv_application/): 위 전략을 DV/EDA 워크플로에 적용.

[퀴즈 풀어보기 →](../quiz/06_strategy_selection_quiz/)
