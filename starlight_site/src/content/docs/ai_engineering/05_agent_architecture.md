---
title: "Module 05 — Agent Architecture"
---
...
:::
:::note[사전 지식]
- [Module 02](../02_prompt_engineering/) — system prompt, structured output
- [Module 04](../04_rag/) — RAG 4 단계
- 기본 함수 호출 / API 디자인 감각
:::
---
...
simulation 이 실패한 상황에서 다음 task 를 LLM 한 번 호출로 풀려고 한다면 어떻게 될까요.

```
"이 log 파일을 분석해서 root cause 찾고 fix 제안해줘"
```

단일 호출로는 세 가지 이유에서 불가능합니다. 첫째, **파일을 읽지 못합니다** — LLM 은 텍스트 입력만 받으므로 log 가 1 MB 라면 전체를 prompt 에 직접 붙여야 하는데 context 한계를 초과합니다. 둘째, **mrun 명령을 실행하지 못합니다** — 가설을 검증하려면 실제 시뮬레이션 실행이 필요하지만 단일 LLM 은 외부 세계에 영향을 줄 수 없습니다. 셋째, **여러 시도를 반복하지 못합니다** — 한 응답은 한 번의 추론이며, 시도 1 이 실패해도 시도 2 로 자동 전환되지 않습니다.

해법은 **Agent 패턴**입니다 (**Agent** = LLM 을 "두뇌"로 삼아, 스스로 **Tool**(도구 — 파일 읽기·명령 실행 같은 외부 동작 함수)을 골라 호출하고 그 결과를 보고 다음 행동을 정하는 루프형 시스템).
...
DV / 코딩 / 분석 자동화 등 거의 모든 실무 응용은 결국 agent 형태로 수렴하며, [Module 07](../07_dv_application/) 의 모든 적용 사례도 agent 골격을 가집니다.

:::tip[🤔 잠깐 — Agent vs Chained Prompts 의 차이?]
...
가장 단순한 시나리오. 사용자 목표 = "이 시뮬레이션 실패를 디버그하라". 도구 = `read_file`. **ReAct**(Reasoning + Acting — "생각(다음에 뭘 할지 추론) → 행동(도구 호출) → 관찰(결과 확인)"을 번갈아 반복하는 가장 기본적인 agent 패턴) 패턴으로 loop 를 한 번 돌립니다.
...
일반 LLM 호출은 단방향입니다. 사용자가 질문을 던지면 모델이 텍스트로 답하고 끝입니다. 외부 세계에 영향을 주거나 새로운 정보를 스스로 수집하지 못합니다.

Agent 는 이 구조를 루프로 바꿉니다. 사용자가 "목표" 를 제시하면, LLM 이 계획을 수립하고 → 도구를 호출하고 → 결과를 관찰하고 → 다음 행동을 결정하는 과정이 반복됩니다. 파일 읽기, 코드 실행, API 호출 같은 실제 행동이 포함되며, 목표가 달성됐다고 판단하면 비로소 최종 보고를 내놓습니다. 바로 이 "관찰 → 결정" 의 동적 루프가 일반 LLM 과 Agent 의 본질적 차이입니다.

이 차이를 다섯 축으로 정리하면 다음과 같습니다.
...
세 패턴은 "언제 계획하고 언제 행동하는가" 라는 타이밍 차이에서 갈립니다. 상황에 따라 유연하게 도구를 선택해야 한다면 ReAct 가, 긴 다단계 작업을 추적 가능하게 실행해야 한다면 Plan-and-Execute 가, 검색과 행동을 함께 자율화하려면 Tool-Augmented RAG 가 적합합니다.
...
**Function Calling**(함수 호출 — LLM 이 "이 도구를 이런 인자로 부르겠다"를 자유 텍스트가 아니라 정해진 구조의 JSON 으로 출력하게 하는 방식)이 핵심입니다. 도구는 **JSON Schema**(JSON 의 필드 이름·타입·필수 여부를 미리 규정한 형식 명세)로 정의됩니다.
...
Reflexion 이 효과적인 이유는 "같은 실수를 반복하지 않는다" 는 단순한 원리 때문입니다. 생성 → 평가 → 교훈 저장 → 개선의 루프를 거칠수록, 초기에 놓쳤던 `config_db set` 레벨 오류나 factory 등록 누락 같은 패턴이 메모리에 쌓여 다음 시도에서 회피됩니다. HumanEval 코드 생성 벤치마크에서 Pass@1 기준 68% → 91% 향상이 보고되었으며, DV 에서는 컴파일 실패 → 원인 분석 → 수정 → 재컴파일 루프를 사람 개입 없이 자동화하는 데 활용됩니다.
...
이 구조에서 각 하위 Agent 는 자신의 전문 도구만 가지고 독립적으로 동작합니다. RTL Analyzer 가 인터페이스를 분석하는 동안 Coverage Agent 가 커버리지 갭을 동시에 찾을 수 있어 병렬화가 자연스럽습니다. 결과는 모두 Lead Agent 에 보고되고, Lead Agent 가 추가 분석이 필요한지 또는 완료로 판단할지를 결정합니다. 단, 이 통신 자체가 LLM 호출이라는 점에서 비용과 지연이 발생하므로 전문성 분리가 진짜 필요한 경우에만 가치가 있습니다 (§6 오해 2 참조).
...
**MCP**(Model Context Protocol — LLM 호스트와 도구 서버가 주고받는 메시지 형식을 표준화한 규약; 도구를 한 번 만들면 여러 agent/IDE 에서 재사용)는 그 메시지 전송에 **JSON-RPC 2.0**(JSON 으로 "이 함수를 이 인자로 호출해 달라"를 주고받는 표준 원격 호출 규약)을 씁니다.
...
:::caution[실무 주의점 — Agent Infinite Loop 로 인한 비용 폭주]
...
:::tip[🤔 Q1 — Tool 정의의 명확성 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Multi-agent 통신 비용 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — Guard rail 설계 (Bloom: Evaluate)]
...
→ [Module 06 — Strategy Selection](../06_strategy_selection/): prompt vs RAG vs fine-tune 결정 기준.

[퀴즈 풀어보기 →](../quiz/05_agent_architecture_quiz/)
