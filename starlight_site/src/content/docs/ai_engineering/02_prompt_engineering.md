---
title: "Module 02 — Prompt Engineering & In-Context Learning"
---
...
:::
:::note[사전 지식]
- [Module 01](../01_llm_fundamentals/) — 자기회귀 생성, context window, KV cache
...
:::tip[🤔 잠깐 — Few-shot 이 zero-shot 보다 _왜_ 잘 동작?]
...
예시를 전혀 주지 않고 바로 "다음 SystemVerilog 코드의 버그를 찾아줘" 라고 하는 것이 Zero-shot 입니다. 이는 모델이 학습 분포 전체의 "평균" 같은 응답을 생성하게 만들어 일반적으로 품질이 낮습니다.

여기에 2~5개 정도의 (버그 코드 → 수정 코드) 예시를 먼저 보여주고 그 뒤에 실제 작업을 요청하면 Few-shot 이 됩니다. 모델이 예시 패턴을 attention 으로 인식하고 그 "sub-distribution" 쪽으로 routing 되기 때문에 Zero-shot 보다 정확도가 10~30% 향상되는 경우가 많습니다. 수십 개의 예시를 제공하는 Many-shot 은 이를 더욱 강화하지만, context window 비용이 증가하므로 Few-shot 만으로도 대부분의 DV 작업에는 충분합니다.

### 5.2 Chain-of-Thought (CoT)

"이 UVM 에러의 원인은?" 이라고 바로 물으면 모델은 즉답을 내놓지만 종종 부정확합니다. 반면 "1단계: 에러 메시지의 핵심 키워드 추출 → 2단계: 관련 UVM 컴포넌트 식별 → 3단계: 가능한 원인 나열 → 4단계: 가장 가능성 높은 원인과 근거 제시" 처럼 단계별 분석을 명시적으로 요청하면 정확도가 올라갑니다. 이것이 Chain-of-Thought (CoT) 입니다.

CoT 가 효과적인 이유는 두 가지입니다. 첫째, LLM 이 "중간 추론 과정" 을 토큰으로 생성하면서 다음 토큰 예측의 확률 분포가 더 좁은 올바른 영역을 향하게 됩니다. 둘째, 복잡한 문제를 작은 단계로 분해함으로써 각 단계에서 모델이 집중해야 할 정보가 명확해집니다. 다만 CoT 는 _복잡한 추론_ 에서만 유효하며, 단순 분류 task 에서는 토큰 비용만 늘리고 효과가 미미하거나 오히려 떨어질 수 있다는 점에 주의해야 합니다 (§6 오해 2 참조).
...
:::caution[실무 주의점 — Context Window 초과 시 무음 잘림(Silent Truncation)]
...
:::tip[🤔 Q1 — Prompt 디버깅 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — Prompt Chaining vs Single Mega-prompt (Bloom: Evaluate)]
...
→ [Module 03 — Embedding & Vector DB](../03_embedding_vectordb/): context 를 외부에서 끌어오기 위한 검색 기반.

[퀴즈 풀어보기 →](../quiz/02_prompt_engineering_quiz/)
