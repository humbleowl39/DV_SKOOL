...
같은 LLaMA-2 13B 모델을 두 사용자에게 서비스한다고 가정합니다. 사용자 A 는 500 토큰짜리 요약 작업을 돌리고 응답이 1 초 안에 돌아오는 반면, 사용자 B 는 32K 토큰짜리 long-document QA 를 요청해 응답이 30 초에 **GPU 비용 80 배** 가 납니다.

같은 모델인데 왜 이런 격차가 생기는가. **답은 KV cache** 입니다. LLaMA-2 13B 의 KV cache 는 _토큰 당 ~1 MB_ 이고 [Introl 2025], 32K context 라면 **32 GB 의 GPU 메모리** 가 _한 sequence_ 에만 할당됩니다. Batch=32 로 운영하면 **1 TB** 가 됩니다.
