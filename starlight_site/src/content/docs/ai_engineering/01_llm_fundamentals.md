---
title: "Module 01 — LLM Fundamentals"
---
...
먼저 등장하는 단어부터 풀어 둡니다. **LLM**(Large Language Model — 방대한 텍스트로 학습해 "다음에 올 단어"를 확률로 예측하는 거대 신경망)이 이 코스의 주인공이고, **토큰**(token — 모델이 다루는 텍스트의 최소 조각; 단어·subword·글자 단위로 잘린 한 덩어리)이 그 입출력의 기본 단위입니다. **GPU**(Graphics Processing Unit — 수천 개 코어로 행렬 연산을 병렬 처리하는 칩으로, 신경망 학습/추론의 표준 하드웨어)가 이 연산을 실제로 돌립니다.

같은 LLaMA-2 13B 모델(13B = 파라미터, 즉 학습으로 정해지는 모델 내부 가중치가 130억 개)을 두 사용자에게 서비스한다고 가정합니다. 사용자 A 는 500 토큰짜리 요약 작업을 돌리고 응답이 1 초 안에 돌아오는 반면, 사용자 B 는 32K 토큰짜리 long-document QA(긴 문서를 통째로 넣고 질문하는 작업)를 요청해 응답이 30 초에 **GPU 비용 80 배** 가 납니다.

같은 모델인데 왜 이런 격차가 생기는가. **답은 KV cache**(Key-Value cache — 이미 처리한 토큰들의 attention 중간 계산값을 저장해 두어 매 토큰마다 재계산을 피하는 메모리 버퍼; §5.4 에서 상세) 입니다. LLaMA-2 13B 의 KV cache 는 _토큰 당 ~1 MB_ 이고 [Introl 2025], 32K context(context = 모델이 한 번에 보는 입력 토큰 묶음, 즉 "지금까지의 대화·문서 전체")라면 **32 GB 의 GPU 메모리** 가 _한 sequence_(sequence = 한 요청의 토큰 열) 에만 할당됩니다. Batch=32(batch = 여러 요청을 한 번에 묶어 동시에 처리하는 단위) 로 운영하면 **1 TB** 가 됩니다.
...
이후 모든 모듈 (Prompt / RAG / Agent / Strategy) 은 한 가정에서 출발합니다 — **"LLM 은 magic box 가 아니라 _자기회귀 토큰 생성기_ 다"**(자기회귀 autoregressive = 방금 만든 토큰을 입력에 도로 붙여 그 다음 토큰을 만드는 식으로 한 개씩 이어 가는 생성 방식). KV cache 메모리가 왜 폭증하는지, context window(context window = 모델이 한 번에 받아들일 수 있는 최대 토큰 수의 상한) 가 왜 비싼지, batch 가 왜 throughput(throughput = 단위 시간당 처리한 토큰/요청 수, 즉 처리량) 을 결정하는지, MoE(Mixture of Experts — 입력마다 일부 전문가 서브네트워크만 켜서 연산을 아끼는 구조; §5.5) 가 왜 "총 파라미터 ≠ 활성 파라미터" 인지 — 모두 이 한 가정의 파생입니다. 참고로 latency(지연 — 한 요청이 응답을 받기까지 걸리는 시간)와 throughput 은 서로 다른 축입니다.
...
:::tip[🤔 잠깐 — KV cache 메모리는 _왜_ 그렇게 크나?]
...
한 번 호출 = "다음 토큰 1개의 확률 분포 → 샘플링" 의 **반복**. 글이 자연스러운 이유는 attention(어텐션 — 각 토큰이 다른 모든 토큰과의 "관련도"를 계산해, 관련 깊은 토큰의 정보를 더 많이 끌어오는 메커니즘) 으로 이전 컨텍스트 _전체_ 를 매 step 참조하기 때문.
:::
...
Transformer(트랜스포머 — attention 을 핵심 블록으로 쌓아 시퀀스를 처리하는 신경망 구조로, 현대 LLM 의 표준 골격; 2017 년 제안)도 _하늘에서 떨어진_ 게 아닙니다. 두 순진한 시도가 _구체적으로 어디서 막혔는지_ 의 결과물입니다.

**시도 1 — "RNN/LSTM 으로 sequence 처리"** (1997-2017) — RNN/LSTM(토큰을 하나씩 순서대로 읽으며 "이전까지의 요약(hidden state)"을 다음 단계로 넘기는 옛 순차 신경망)

순차적으로 처리하면 자연스럽다. 결과:
- **Vanishing/exploding gradient**(gradient = 손실을 줄이려고 각 가중치를 어느 방향으로 얼마나 바꿀지 알려 주는 학습 신호; 이것이 0 으로 사라지거나 무한대로 폭주하면 학습이 안 됨): 100+ 토큰 시퀀스에서 학습 불가능 → BPTT(시간을 거슬러 올라가며 gradient 를 전파하는 RNN 학습법) 가 깊은 timestep 의 gradient 를 잃거나 폭주.
...
가장 단순한 시나리오. 사용자 입력 `"Write a UVM"` 을 받아 LLM 이 다음 토큰 하나 (`" test"`) 를 예측해 응답하는 한 사이클을 8단계로 추적합니다. 아래 단계에 나오는 용어를 미리 풀어 두면 — **embedding**(임베딩 — 토큰을 의미가 담긴 실수 벡터로 바꾼 표현; 비슷한 의미일수록 벡터가 가까움), **d_model**(모델이 토큰 하나를 표현하는 벡터의 차원 수), **RoPE**(Rotary Position Embedding — 토큰의 위치를 Q/K 벡터의 회전으로 주입하는 현 표준 방식; §5.2), **output head**(마지막 hidden 벡터를 전체 어휘에 대한 확률 분포로 바꾸는 마지막 층), **argmax/sampling**(확률 분포에서 가장 큰 것을 고르거나(argmax) 확률대로 뽑는(sampling) 다음-토큰 선택).
...
아래 그림의 블록을 미리 풀어 두면 읽기 쉽습니다 — **FFN**(Feed-Forward Network — attention 뒤에 붙어 각 토큰 벡터를 독립적으로 한 번 더 비선형 변환하는 작은 2층 신경망), **Residual Connection**(잔차 연결 — 층의 입력을 출력에 그대로 더해 주어 깊은 신경망에서도 gradient 가 잘 흐르게 하는 우회로), **LayerNorm**(Layer Normalization — 한 토큰 벡터의 값들을 평균 0·분산 1 로 정규화해 학습을 안정화하는 연산), **Positional Encoding**(위치 인코딩 — attention 자체는 순서를 모르므로 "몇 번째 토큰인지"를 따로 더해 주는 신호).
...
이 한 식이 (1) 모든 토큰 쌍의 관련성을 계산하고 (2) 관련성에 비례해 정보를 가중합산합니다. 여기서 **softmax**(소프트맥스 — 임의의 실수 점수 묶음을 모두 0~1 사이이고 합이 1 인 확률분포로 바꾸는 함수)는 점수를 "가중치 비율"로 변환하는 장치이고, `√d_k`(d_k = Query·Key 벡터의 차원) 로 나누는 것은 dot-product(내적 — 두 벡터의 성분을 곱해 더한 값으로, 여기서는 두 토큰의 유사도 점수) 가 너무 커져서 softmax 가 saturate(한쪽으로 쏠려 거의 0/1 로 굳음) 되는 것을 막는 안전장치.

:::note[왜 softmax 가 "관련성 가중합" 이 되는가 — 볼록결합의 관점]
"관련성에 비례해 가중합산" 이라는 말의 메커니즘은 softmax 의 성질에서 정확히 나옵니다. `QKᵀ / √d_k` 는 각 토큰 쌍의 _점수(score)_ 일 뿐이라 음수·양수가 섞인 임의의 실수입니다. 여기에 softmax 를 씌우면 모든 값이 _0~1 사이로_ 바뀌고 _합이 정확히 1_ 인 확률분포가 됩니다 (관련 높은 쌍일수록 큰 가중치). 그 분포로 `V` 들을 가중합하면 결과는 V 벡터들의 **볼록결합(convex combination)** — 즉 "관련성이라는 확률로 섞은 평균 벡터" 가 됩니다. 가중치 합이 1 이라는 점이 핵심입니다 — 정보를 _증폭하거나 소실시키지 않고_ 비율대로만 섞기 때문에, 관련 토큰의 V 가 결과를 지배하면서도 스케일이 보존됩니다. 이것이 "정보 손실 없이 관련성대로 정보가 모인다" 는 직관의 수학적 근거입니다.
:::
...
**핵심 메커니즘 — "여러 관점" 은 차원을 _쪼개서_ 만든다.** 직관만 보면 head 를 늘릴수록 연산이 h 배로 불어날 것 같지만 실제로는 그렇지 않습니다. multi-head 는 `d_model` 차원을 _그대로 복제_ 하는 것이 아니라 **h 개로 분할** 합니다. 즉 각 head 는 `head_dim = d_model / h` 차원만 봅니다 (예: `d_model=4096`, `h=32` 이면 각 head 는 128 차원). 각 head 가 자기 128 차원 부분공간에서 독립적으로 `Q·Kᵀ → softmax → ·V` 를 수행하고, h 개 head 의 출력(각 128 차원)을 `Concat` 하면 다시 `h × head_dim = d_model` 차원으로 복원됩니다. 마지막에 `W_O` (d_model × d_model) 로 한 번 더 섞어 head 간 정보를 통합합니다.

이 "쪼개기" 덕분에 (1) 총 연산량은 single-head 와 거의 같으면서 (2) 각 head 가 서로 다른 부분공간 = 서로 다른 _관점_ (문법/의미/위치)을 학습할 수 있습니다. 그리고 이 `head_dim` 이 바로 §1.1·§5.4 의 KV cache 비용 계산에 들어가는 `d_head` 입니다 — GQA/MQA(§5.3) 가 "Q head 는 h 개, K/V head 는 g 개로 공유" 해서 메모리를 아낄 수 있는 것도, head 가 이렇게 분할된 독립 단위이기 때문입니다.

### 4.5 추론 비용을 줄이는 세 갈래

아래 표에 처음 나오는 기법들을 한 줄씩 — **GQA/MQA**(Grouped/Multi-Query Attention — 여러 Q head 가 K/V 를 공유하게 해 KV cache 메모리를 줄이는 기법; §5.3), **Flash Attention**(attention 을 GPU 의 빠른 SRAM 안에서 블록 단위로 계산해 느린 메모리 왕복을 줄이는 알고리즘; §5.3), **HBM/SRAM**(HBM = GPU 의 크지만 느린 주메모리, SRAM = 작지만 빠른 온칩 메모리), **Quantization**(양자화 — 가중치를 더 적은 비트로 표현해 메모리/연산을 줄이는 기법; §5.6).
...
Tokenization(토크나이즈 — 텍스트를 모델이 처리할 토큰 열로 쪼개는 과정)의 방식들입니다. 표의 **OOV**(Out-Of-Vocabulary — 어휘 사전에 없는 단어; 통째로는 표현 못 해 처리에 문제가 생기는 경우)와 **BPE**(Byte-Pair Encoding — 자주 함께 나오는 글자 쌍을 반복 병합해 서브워드 어휘를 만드는, 가장 널리 쓰는 방식)가 핵심입니다.
...
모델 크기가 특정 임계점을 넘으면 갑자기 나타나는 능력:
  ...
  ~10B: 기본 언어 이해, 간단한 코드
  ~50B: CoT 추론(Chain-of-Thought, 답 전에 중간 추론 단계를 풀어 쓰게 하는 기법 — Module 02), 복잡한 코드 생성
...
**Phase 3 — RLHF / DPO (Human Alignment)**: 사람이 선호하는 출력 vs 비선호 출력의 쌍을 학습해 안전성·유용성·정확성을 강화합니다. 이 단계를 거쳐야 비로소 "실용적인 AI 어시스턴트" 가 됩니다. RLHF 는 별도의 Reward Model 을 요구하는 반면, 최근의 DPO/ORPO ([Module 06](../06_strategy_selection/) §5.5) 는 이 단계를 더 단순하게 만들었습니다.
...
:::caution[실무 주의점 — KV Cache 미활성화 시 추론 지연 폭증]
...
:::tip[🤔 Q1 — KV cache 비용 계산 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — Greedy vs Sampling 선택 (Bloom: Apply)]
...
:::
:::tip[🤔 Q3 — Context window 확장 trade-off (Bloom: Evaluate)]
...
→ [Module 02 — Prompt Engineering & In-Context Learning](../02_prompt_engineering/): 같은 모델로도 결과를 바꾸는 입력 설계 기법.

[퀴즈 풀어보기 →](../quiz/01_llm_fundamentals_quiz/)
