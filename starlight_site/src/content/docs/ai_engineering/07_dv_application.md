---
title: "Module 07 — DV/EDA Application"
---
...
:::
:::note[사전 지식]
- [Module 01–06](../01_llm_fundamentals/) 의 LLM / Prompt / RAG / Agent / Fine-tuning 기본
...
DV 엔지니어의 하루를 들여다보면 8 시간 업무 가운데 7 시간이 반복 작업으로 채워집니다. 오전 4 시간은 새 IP spec 30 페이지를 읽고 **V-Plan**(Verification Plan — "이 IP 에서 무엇을 검증해야 하는가"를 항목별로 나열한 검증 계획서) 항목 100 개를 식별하는 일이고, 점심 후 2 시간은 **regression**(리그레션 — 여러 테스트를 한꺼번에 자동 실행해 회귀(이전엔 되던 게 깨졌는지)를 잡는 일괄 실행) 20 개 fail 의 log 를 분석해 root cause(근본 원인)를 분류하는 일이며, 오후 2 시간은 새 UVM testbench skeleton(테스트벤치 뼈대 코드)을 코딩합니다. 이 7 시간은 spec 읽기, log pattern matching, boilerplate 코딩 같은 기계적 작업입니다. 진짜 창의적인 시간 — 새 corner case 를 발견하는 일 — 은 겨우 1 시간입니다.
...
**총 시간 8 시간 → 3 시간**. 절감된 5 시간을 _더 많은 corner case 발견_ 에 투자.

**중요**: AI 는 _sign-off 책임_(sign-off = "이 설계는 검증을 통과했다"고 책임지고 최종 승인하는 것) 못 짐. AI 가 _제안_ → 사람이 _검수_ + _sign-off_. 이게 _augmentation_(증강 — AI 가 사람을 대체하는 게 아니라 사람의 생산성을 끌어올리는 보조 역할로 쓰는 모델) 모델.
...
:::tip[🤔 잠깐 — _왜_ AI 가 sign-off 못 하나?]
LLM 이 _점점 좋아지면_ 언젠가 sign-off 도 할 수 있을까?
...
1. **Hallucination 잔존**: 최신 모델도 _낮은 빈도_ 로 거짓말함. Tape-out 결정에 0.1% hallucination 도 허용 불가.
2. **책임 소재**: 칩에 bug 가 나면 _누가 책임지나_? AI 가 sign-off 하면 회사가 _법적_ 으로 곤란.
3. **Spec 의 변하는 영역**: 신제품 spec 이 _학습 데이터 외_ 일 가능성 큼 → AI 의 _자신감_ 이 _신뢰_ 와 비례하지 않음.

그래서 _augmentation_ 모델 (AI 가 후보 제시, 사람이 결정) 이 _안정성과 throughput 의 sweet spot_. 미래에는 어떨지 모르겠지만 _현재 (2026)_ 는 명확.
...
AI: "AI Augmentation Layer\nFAISS Index: IP 스펙 청크 / V-Plan 항목 / 과거 디버그 노트 / 코드 스니펫\n(사내 로컬 — 보안)"
...
이 세 제약의 교집합이 **로컬 RAG + 로컬 모델 (INT4 quantized — 가중치를 4-bit 정수로 양자화해 메모리·연산을 크게 줄여 사내 GPU 에서도 큰 모델을 돌릴 수 있게 한 것) + 인간 sign-off** 입니다.

---

## 3. 작은 예 — `run.log` 1개를 LLM 으로 triage 하는 한 사이클

가장 단순한 시나리오. 하나의 fail regression log 를 LLM 이 첫 에러 → 분류 → 수정 제안까지 6단계로 triage(트리아지 — 여러 실패를 빠르게 분류·우선순위화해 "어느 것부터 어떻게 볼지" 정하는 작업) 합니다.
...
E -> L: "① mrun fail log path\n(path/run.log)"
L -> L: "② log chunking (5K token)"
L -> L: "③ FAISS retrieve\nTop-3 similar past fails"
L -> L: "④ system prompt 조립\n+ log chunk + past fails"
L -> L: "⑤ LLM inference (T=0)\n→ first_error, classify, fix"
L -> E: "⑥ structured JSON\n→ reviewer GUI (시니어 확인)" { style.stroke-dash: 4 }
```
...
표의 검증 열에 나오는 **smoke sim**(스모크 시뮬레이션 — "일단 돌아가긴 하는지"만 빠르게 확인하는 최소 시뮬레이션)을 먼저 알아 두면 됩니다.
...
DV 의 반복 작업은 크게 다섯 유형으로 분류할 수 있고, 각각에 맞는 AI 접근법이 다릅니다. 수백 IP 의 스펙 문서에서 검증 항목을 추출하는 정보 과부하 문제는 RAG + LLM 이 가장 직접적입니다. 스펙 변경마다 UVM 환경을 수동으로 업데이트하는 반복 작업에는 Agent + 코드 생성 루프가 적합합니다. 검증 계획에서 항목을 3~5% 누락하는 인간 실수는 체계적 자동 스캔으로 보완하고, regression 실패 패턴 분류와 로그 분석은 LLM 분석이 즉각 투입 가능합니다. 시니어의 디버그 노하우는 RAG 기반 지식 베이스로 보존해 주니어가 활용할 수 있게 합니다.
...
이 분리가 깨지면 양쪽 모두 망합니다. AI 가 sign-off 를 담당하면 audit 통과가 불가능하고, 반대로 사람이 정보 추출·분류·1차 분석까지 전부 담당하면 throughput 이 개선되지 않습니다. AI 는 빠르고 지치지 않으며 편향 없이 전체를 스캔하는 역할을, 사람은 판단과 책임을 맡는 역할을 맡는 것이 이 모델의 핵심입니다.
...
:::caution[실무 주의점 — RAG 기반 DV 도우미의 Prompt Injection 위험]
...
:::tip[🤔 Q1 — AI 도입 우선순위 (Bloom: Evaluate)]
당신의 팀에 _AI 1 개 워크플로_ 만 도입 가능. 어떤 task 부터?

<details>
<summary>정답</summary>

선택 기준:
1. **반복성 높음** (매일/매주 발생) — 학습 곡선 흡수 시간 회수.
2. **명확한 input/output** — fuzzy 한 task 는 prompt engineering 비용 큼.
3. **검수 가능** — 사람이 _빠르게_ 결과 확인 가능해야 (AI 가 틀려도 catch).

가장 흔한 첫 선택: **Log analysis** (debug 단계) — 반복성 + 패턴 명확 + 검수 빠름. 두 번째: UVM testbench skeleton (boilerplate 많음).

</details>
:::
:::tip[🤔 Q2 — 보안 vs 정확도 (Bloom: Analyze)]
Claude API (정확도 95%) vs 사내 로컬 LLM (정확도 80%). 사내 IP 인데 _고객 데모용_ 으로도 사용. 어느 것?

<details>
<summary>정답</summary>

**두 가지 분리**:
- 사내 IP 코드 → 로컬 LLM 강제 (보안 first, 정확도 80% 받아들임).
- 고객 데모 / 일반 docs → API 사용 가능 (보안 risk 낮음, 정확도 우선).

Router 로 분기 — 입력에 IP code 포함 시 자동 로컬, 외 API. 이게 _hybrid_ 의 가치.

</details>
:::
:::tip[🤔 Q3 — 측정 metric (Bloom: Apply)]
당신은 _UVM TB 자동 생성 AI_ 를 도입. ROI 검증을 위해 어떤 metric 측정?

<details>
<summary>정답</summary>

4 가지 metric:
1. **시간 절감**: AI 사용 전후 testbench 작성 시간 (분 단위 측정).
2. **결함 탐지율**: AI 생성 TB 의 첫 시뮬에서 _얼마나 잡았나_ (vs 사람 작성 TB baseline).
3. **비용**: token 사용량 × 단가 + GPU 시간 (FT 의 경우).
4. **수정 비율**: AI 출력 중 _사람이 수정한 라인_ 비율 (낮을수록 좋음).

4 가지 모두 _수치화_ 해야 임원/팀에 가치 입증 가능. 정성적 "잘 동작하는 것 같다" 는 운영 부채.
...
- *AutoChip: LLM-Based VHDL Code Generation* — academia, 2024
