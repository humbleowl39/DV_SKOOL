---
title: "Module 04 — RAG (Retrieval-Augmented Generation)"
---
...
:::
:::note[사전 지식]
- [Module 02](../02_prompt_engineering/) — Few-shot, system prompt
- [Module 03](../03_embedding_vectordb/) — embedding, FAISS, chunking
...
사내 IP / 코드 / 문서를 LLM 으로 활용하려는 거의 모든 프로젝트의 표준 패턴이며, [Module 06](../06_strategy_selection/) 의 "Prompt vs RAG vs Fine-tune" 결정에서 항상 출발점이 됩니다.

:::tip[🤔 잠깐 — RAG 실패 시 _가장 자주_ 깨지는 단계는?]
...
| ② | embed 모델 | 쿼리 → 1024 차원 벡터 | 인덱싱 시 _같은_ 모델 ([Module 03](../03_embedding_vectordb/) §6 흔한 오해 1) |
...
RAG 파이프라인은 두 시간대에 걸쳐 작동합니다. 오프라인 단계에서는 나중에 빠르게 검색할 수 있도록 문서를 미리 가공해둡니다. IP 스펙 PDF, IP-XACT XML, 설계 문서, 테스트 플랜 등 다양한 소스를 수집하여 파싱하고, 의미 단위로 청킹한 뒤, 각 청크를 임베딩 벡터로 변환해 FAISS 인덱스에 저장합니다. 이 단계의 품질 — 청킹 전략, 파서의 정확성, 임베딩 모델의 선택 — 이 온라인 검색의 상한을 결정합니다.
...
**실제**: RAG 는 _지식 갱신_ 에 강하고, fine-tune 은 _형식/스타일 내재화_ 에 강합니다. 두 기법은 **보완**. 실무는 "prompt → RAG → 필요 시 FT" 순서로 escalation ([Module 06](../06_strategy_selection/)).<br>
...
- **Chunking** — 문서를 의미 단위로 자르는 것이 retrieval 품질의 출발점 ([Module 03](../03_embedding_vectordb/) §5.4).
...
:::caution[실무 주의점 — RAG 검색 실패가 Hallucination 으로 보이는 문제]
...
:::tip[🤔 Q1 — Retrieval 실패 vs Generation 실패 분리 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — Hybrid search 의 score fusion (Bloom: Apply)]
...
:::
:::tip[🤔 Q3 — RAG vs Fine-tune (Bloom: Evaluate)]
...
→ [Module 05 — Agent Architecture](../05_agent_architecture/): RAG 가 도구 호출과 결합되어 자율 agent 가 된다.

[퀴즈 풀어보기 →](../quiz/04_rag_quiz/)
