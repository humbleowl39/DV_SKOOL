---
title: "Module 01 — Formal Verification 기본 개념"
---
...
:::tip[🤔 잠깐 — Formal 이 _모든 IP_ 에 안 쓰이는 이유?]
...
:::note[Property 의 두 종류 — safety vs liveness]
Property 는 *무엇을 주장하는가* 에 따라 두 부류로 갈리며, 이 구분이 BOUNDED·`s_eventually`(Module 02) 가 왜 문제되는지의 토대입니다.

- **Safety property — "나쁜 일이 *절대* 일어나지 않는다."** 위반이 있다면 그것을 보여주는 *유한한* trace 가 반드시 존재합니다(나쁜 일이 일어난 그 순간까지만 보면 됨). 예: FIFO overflow 금지, 불법 상태 진입 금지, deadlock 으로 빠지는 전이 금지. §3 의 `!(wr_en && full)` 이 전형적인 safety 입니다.
- **Liveness property — "좋은 일이 *결국* 일어난다."** 예: req 가 들어오면 *언젠가는* ack 가 온다(`req |-> s_eventually ack`).

핵심 차이는 *위반을 유한 시간에 확정할 수 있는가* 입니다. safety 위반은 유한 trace 로 잡히지만, liveness 위반("좋은 일이 끝내 안 일어난다")을 확정하려면 *무한히 기다려도 안 온다* 는 것을 보여야 합니다 — 유한한 시뮬/유한한 unroll 로는 "아직 안 왔을 뿐" 인지 "영원히 안 올" 것인지 구별할 수 없습니다. 그래서 liveness 는 BOUNDED 와 시뮬레이션에서 특히 다루기 까다롭고, `s_eventually` 가 Formal 의 무한 시간 가정에서만 의미를 갖는 근본 이유가 됩니다(Module 02 §5.7).
:::
...
:::note[왜 도달 불가능 상태가 귀납 반례가 되는가 — inductive step 의 출발점]
이 함정의 뿌리는 inductive step 이 *어디서 출발하는가* 에 있습니다. base case 는 reset 직후라는 *실제로 도달하는* 한 상태에서 시작하지만, inductive step 은 "Property 가 성립하는 *임의의* 상태 N" 을 가정하고 N+1 을 따집니다 — 여기서 "임의의 상태" 는 RTL 의 상태 비트가 가질 수 있는 *모든 인코딩* 을 뜻하지, *실제로 도달 가능한* 상태만을 뜻하지 않습니다. 엔진은 reset 부터 시뮬레이션해 N 에 이르는 게 아니라, 상태 비트에 값을 *직접 부여*해 출발하기 때문입니다. 그래서 `state == 3'b101` 같이 정상 동작에서는 절대 진입할 수 없는 인코딩도 inductive step 의 합법적 시작점이 되고, 그 가짜 상태에서 N+1 의 불변이 깨지면 step 이 실패합니다 — 실제로는 없는 반례인데 말입니다. Helper invariant(`state inside {유효 인코딩}`)가 푸는 일이 바로 이것 — *그 가짜 시작점을 inductive step 의 가정에서 배제*해, 엔진이 도달 가능한 상태에서만 귀납하게 만드는 것입니다.
:::
...
```
RTL →(시간 전개/unroll)→ Boolean Formula →(SAT Solver)→ SAT (반례) / UNSAT (증명)
```

여기서 한 단계가 빠지면 안 됩니다 — SAT solver 는 *시간 개념이 없는* 조합 논리식만 풀 수 있는데, RTL 은 flip-flop 으로 *상태가 cycle 마다 바뀌는* sequential 회로입니다. 그래서 엔진은 먼저 회로를 **시간 전개(unrolling)** 합니다 — 같은 조합 로직을 cycle 0, 1, 2, … N 으로 *복제해 펼치고*, 각 cycle 의 flip-flop 출력을 다음 cycle 의 입력으로 연결해, "N cycle 동안의 동작 전체" 를 하나의 거대한 조합 식으로 만듭니다. 그 펼쳐진 식에 "이 안에서 Property 를 위반하는 입력 조합이 있는가?" 를 SAT 로 묻습니다. **BOUNDED N 이 "N cycle 까지만 증명" 인 이유가 바로 이 unroll 깊이** 입니다 — N 단계만 펼쳐 풀었다는 뜻이고, 무한 cycle 의 PROVEN 은 펼침을 무한히 늘리는 대신 induction(§4.3)으로 "임의의 N→N+1" 을 한 번에 닫아 얻습니다.
...
이 기법을 정확히 이해하려면 *입력 공간(input space) 모델* 을 먼저 잡아야 합니다. Formal 엔진은 기본적으로 DUT 입력이 *매 cycle 어떤 값이든 가질 수 있다* 고 가정하고 — 즉 가능한 모든 입력 시퀀스의 집합 전체를 탐색 대상으로 삼고 — 그 안에서 위반을 찾습니다. **`assume` 은 이 입력 집합을 *잘라내는*(가지치기) 도구** 입니다. `assume (addr < 256)` 은 "256 이상의 주소가 들어오는 시퀀스" 를 탐색 대상에서 통째로 제거합니다. 그래서 탐색 공간이 줄어 수렴이 쉬워지는 *동시에*, 위험의 근원도 여기서 옵니다 — 만약 실제 환경에서는 가능한 입력을 assume 이 잘라내 버리면(over-constraint), 엔진은 그 잘린 영역의 버그를 *볼 수 없는데도* PROVEN 을 보고합니다. 이것이 **false PROVEN** 입니다. 즉 assume 은 "엔진이 무엇을 보게 할지" 를 정하는 입력-공간 필터이고, 그 필터가 spec 보다 좁으면 안전이 *증명* 된 게 아니라 *가정* 된 것입니다 — 이 입력-공간 관점은 Module 03 에서 audit 전략으로 깊게 다룹니다.
...
:::caution[실무 주의점 — BOUNDED 를 PROVEN 으로 오해]
...
:::tip[🤔 Q1 — PROVEN vs BOUNDED (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — Formal 적용 결정 (Bloom: Evaluate)]
...
→ [Module 02 — SVA (SystemVerilog Assertions)](../02_sva/): 이 모듈에서 본 property 가 실제로 어떤 SVA 연산자로 표현되는지, Vacuous Pass 와 Bind 의 함정까지.

[퀴즈 풀어보기 →](../quiz/01_formal_fundamentals_quiz/)
