...
:::
:::note[사전 지식]
- [Module 01-03](../01_formal_fundamentals/) 학습 후 활용
:::
---
...
C: "골든 답변:\n'FSM, 프로토콜, FIFO, Arbiter, 보안\n— 상태 공간 관리 가능'"
D: "추가 보강:\n'Mapping Table IP 는 5 가지 property 로 검증'\n(이력서 연결)"
A -> B
B -> C
C -> D
```
...
B: "카드 ④ '함정 체크' 의\n'1-cycle latency 설계인데 |-> 사용?' 항목"
C: "확인: req → ack 는 1 cycle latency\n→ |=> 또는 |-> ##1 이 맞음"
D: "추가 확인: cover 짝 있는가? (vacuous 방지)\n— 카드 ④ 의 두 번째 항목"
A -> B
B -> C
C -> D
```
...
:::caution[실무 주의점 — Over-constraint 인 assume]
...
:::tip[🤔 Q1 — Convergence 6 단계 적용 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Over-constraint 검출 (Bloom: Evaluate)]
