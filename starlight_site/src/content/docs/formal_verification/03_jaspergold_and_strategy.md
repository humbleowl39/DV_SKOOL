---
title: "Module 03 — JasperGold & DV Strategy"
---
...
:::
:::note[사전 지식]
- [Module 01](../01_formal_fundamentals/), [Module 02](../02_sva/)
- 시뮬레이션 기반 검증 워크플로 이해
:::
---
...
:::note[Property 분할이 원본을 증명하는 *논리적 조건*]
"복잡한 property 를 sub-property 로 쪼개 각각 증명 → 원본 증명" 은 공짜가 아니라 한 가지 논리 조건을 만족할 때만 타당합니다 — **sub-property 들의 AND(논리곱)가 원본 property 를 *함의(imply)* 해야** 합니다. 즉 `P_sub1 && P_sub2 && … ⟹ P_orig` 가 성립해야, 각 sub 를 증명한 것이 원본 증명이 됩니다. 이 조건을 확인하지 않고 *임의로* 쪼개면 — 원본이 요구하던 어떤 경우를 어느 sub-property 도 커버하지 않는 *틈* 이 생겨, 모든 sub 가 PROVEN 인데도 원본은 실제로 증명되지 않은 상태(cover 누락)가 됩니다. 그래서 분할 후에는 각 조각의 cover 로 "원본이 묻던 시나리오가 어느 sub 에든 도달하는가" 를 점검해야 합니다.
:::
...
:::note[COI(Cone of Influence)란 무엇인가]
`check_coi` 를 여러 번 썼지만 정의를 먼저 못박아야 합니다. 한 property 의 **COI(Cone of Influence)는 그 property 의 값에 *영향을 줄 수 있는* 신호와 로직만 모은 회로의 부분 그래프** 입니다. property 에서 출발해 그것이 참조하는 신호 → 그 신호를 구동하는 로직 → 다시 그 로직의 입력… 을 역방향으로 추적해 닫은 집합입니다. COI 밖의 로직은 그 property 의 결과를 바꿀 수 없으므로 증명에 무관합니다. 이 개념이 두 가지 실무 판단의 토대입니다 — (1) **blackbox 후보 식별**: property 의 COI 에 *들어오지 않는* 큰 데이터 경로(MUL, 메모리)는 안심하고 blackbox 할 수 있습니다(아래 표). (2) **sign-off COI 검토**: blackbox 한 모듈이 정말 property 의 COI 밖인지 확인해야 — COI 안의 것을 blackbox 하면 증명이 그 부분을 안 본 채 나온 것이 됩니다. `check_coi -property <name>` 이 바로 이 부분 그래프를 보여 줍니다.
:::
...
:::note[추상화는 왜 *없는 버그* 를 만들어내나 — over-approximation 원리]
blackbox 의 false CEX 는 우연이 아니라 *모든 추상화에 공통된* 성질의 한 사례입니다. 추상화(blackbox·counter abstraction·data abstraction 모두)는 회로를 단순화하면서 **실제보다 *넓은* 동작 집합을 허용** 합니다(over-approximation) — blackbox 출력을 "어떤 값이든" 으로 두면 실제로는 불가능한 출력까지 포함하고, counter 를 3-bit 으로 줄이면 실제와 다른 wrap 동작이 끼어들 수 있습니다. 핵심 비대칭은 이것입니다 — 추상 모델은 실제의 모든 동작을 *포함* 하므로, 추상 모델에서 **PROVEN 이면 실제도 안전** 합니다(over-approx 가 안전 증명 방향으로는 건전). 그러나 추상 모델이 *추가로* 가진 가짜 동작에서 위반이 나오면, 실제에는 없는 **false CEX** 가 됩니다. 그래서 추상화 후의 CEX 는 항상 "이게 추상이 만든 가짜인가, 진짜인가" 를 먼저 의심하고, assume 으로 그 가짜 동작을 도로 잘라내는 것입니다.
:::
...
"자유 입력으로 교체" 의 기전은 이렇습니다 — cut point 는 지정한 신호의 **driver 로직을 끊어 버리고**(그 신호를 만들던 앞단 회로와의 연결을 제거), 대신 엔진이 *매 cycle 임의의 값* 을 그 신호에 주입하게 만듭니다. 즉 그 신호는 더 이상 회로가 계산하는 값이 아니라 *자유로운 1차 입력* 이 됩니다. 그 결과 앞단의 상태 공간이 탐색에서 빠져 뒷단을 독립적으로 검증할 수 있게 되지만, *대가* 도 함께 옵니다 — 끊긴 신호가 원래는 spec 에 의해 좁은 범위만 가졌다면, 자유 입력은 그 범위 밖의 값까지 주입해 false CEX 를 만듭니다. 그래서 cut 한 신호에는 `assume` 으로 *원래의 실제 출력 범위* 를 다시 명시해 줘야 합니다.

**blackbox 와의 차이** 는 *대상의 단위* 입니다 — blackbox 는 *모듈 전체* 를 비우고 그 모듈의 출력들을 자유롭게 만드는 것이고, cut point 는 *단일 신호* 의 driver 만 끊는 더 국소적인 도구입니다. 둘 다 "끊고 자유 입력화 → assume 으로 범위 재부여" 라는 같은 패턴을 따르지만, 모듈 단위냐 신호 단위냐가 다릅니다.
...
:::caution[실무 주의점 — Blackbox 후 false counterexample]
...
:::tip[🤔 Q1 — Convergence 전략 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Assume audit (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — Sign-off 5 기준 (Bloom: Evaluate)]
...
→ [Module 04 — Quick Reference Card](../04_quick_reference_card/): SVA 연산자, Convergence 순서, Sign-off 5 기준의 한 페이지 치트시트 — 면접/코드리뷰/디버그 중 빠른 확인용.

[퀴즈 풀어보기 →](../quiz/03_jaspergold_and_strategy_quiz/)
