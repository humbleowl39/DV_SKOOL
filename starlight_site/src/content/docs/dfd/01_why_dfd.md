...
새 SoC(System on Chip, 하나의 칩에 코어·메모리·주변장치를 모두 통합한 집적회로)의 첫 실리콘이 도착했습니다 — 여기서 **post-silicon**(제조가 끝나 실물 칩이 손에 들어온 이후 단계)이라는 말은 시뮬레이션이 아닌 진짜 칩을 다루는 국면을 뜻합니다. 보드에 올리고 전원을 넣었더니 부팅 로그가 어느 시점에서 멈추고 더 이상 진행되지 않습니다 — 이렇게 새 칩을 처음 살려 동작시키는 작업을 **bring-up**(첫 실리콘을 보드에서 단계적으로 깨워 동작 확인하는 과정)이라 부릅니다. RTL(Register Transfer Level, 레지스터·논리로 회로 동작을 기술한 하드웨어 설계 수준) 시뮬레이션이었다면 파형을 열어 PC(program counter, 다음 실행할 명령의 주소)가 어느 주소에서 무한 루프에 빠졌는지, 어느 핸드셰이크가 안 떨어졌는지 즉시 볼 수 있었을 겁니다. 그러나 실물 칩 안에서는 그 신호들이 외부 핀으로 나오지 않습니다.

이때 살아 있는 유일한 통로가 디버그 포트입니다. 디버거(디버그 동작을 지시하는 호스트 PC 측 도구)를 JTAG(JTAG; 소수의 직렬 핀으로 칩 내부에 접근하는 표준 테스트/디버그 포트 — N02에서 자세히) 핀에 연결하고 코어를 **halt**(코어 실행을 그 자리에서 멈추는 것)시킨 뒤, PC와 레지스터를 읽고, 시스템 메모리를 덤프하고, 한 명령씩 **single-step**(명령을 하나만 실행하고 다시 멈추는 것)으로 진행시켜 어디서 멈추는지 추적합니다. DFD(Design For Debug, 칩 내부를 외부에서 관찰·제어할 수 있게 설계 단계에 심어 두는 디버그 인프라)는 이 모든 동작이 _칩 설계 단계에서 미리 심어 둔_ 인프라 위에서 동작하게 만드는 일입니다. 디버그 인프라를 설계에 넣지 않으면, 칩이 죽었을 때 진단할 방법이 아예 없습니다.

### 1.2 DFD가 책임지는 두 가지 능력

칩 내부 가시성은 두 가지 서로 다른 능력으로 나뉩니다. 이 구분이 이후 모든 인증/보안 논의의 뼈대가 됩니다. 표를 읽기 전에 핵심 용어 셋만 먼저 풀면 — **breakpoint**(특정 주소의 명령에 도달하면 코어를 자동으로 halt시키는 표시), **trace**(코어가 실제로 실행한 명령·데이터의 흐름을 시간 순서대로 기록한 스트림), **PMU**(Performance Monitoring Unit; cache miss·실행 사이클 같은 성능 통계를 세는 카운터 모음)입니다.
...
바깥 **현관문(JTAG 핀)** 으로 들어와, **로비의 안내데스크(DAP)** 가 어느 층으로 갈지 정해 주면, **엘리베이터(MEM-AP가 내는 버스)** 를 타고, 실제 사무실(**CoreSight 컴포넌트와 시스템 메모리**)에 도착합니다. 디버거는 사무실 주소(레지스터 주소)만 알면 되고, 어느 문·로비·엘리베이터를 거쳤는지는 각 층이 알아서 처리합니다.<br>
여기서 **DAP**(Debug Access Port; 직렬 핀 접근을 칩 내부의 메모리-맵 버스 접근으로 바꾸는 다리 — N03), **MEM-AP**(Memory Access Port; DAP 안에서 실제 시스템 버스 트랜잭션을 발행하는 부품), **CoreSight**(Arm의 온칩 디버그·trace IP 모음 — 코어 halt, trace 수집 등을 실제 수행)가 처음 등장합니다.
:::
...
1. **핀은 최소여야 한다** → 칩 핀은 비싸고 귀합니다. JTAG 4~5핀(또는 **SWD**(Serial Wire Debug; JTAG와 같은 DAP를 단 2핀으로 노출하는 핀 절약형 디버그 프로토콜) 2핀)만으로 칩 전체 메모리 공간에 접근해야 하므로, 직렬 프로토콜(JTAG)을 메모리-맵 접근(DAP)으로 바꾸는 변환 계층이 필요합니다.
2. **디버거는 SoC 내부 구조를 몰라야 한다** → 디버거는 표준 **ADI**(Arm Debug Interface; DAP의 동작을 규정한 Arm 표준 사양) 프로토콜만 알고, 어떤 CoreSight 컴포넌트가 어디 있는지는 **ROM table**(칩 안 CoreSight 컴포넌트들의 주소 목록이 담긴, 칩이 스스로 알려 주는 디렉터리)을 읽어 _런타임에_ 발견합니다. 그래서 한 디버거가 여러 칩을 지원합니다.
...
가장 흔한 디버그 동작인 "코어 halt 후 PC 읽기"가 스택을 어떻게 통과하는지 단계로 봅시다. 아래 단계에 처음 나오는 부품들을 미리 풀어 두면 — **TAP**(Test Access Port; JTAG 핀을 받아 칩 내부 접근을 시퀀싱하는 제어 블록, 내부에 FSM이 있음 — N02), **DP**(Debug Port; DAP의 물리 측, JTAG/SWD를 처리하고 어느 AP를 쓸지 고름), **AP**(Access Port; DAP의 시스템 측, 실제 접근을 발행), **IR**(Instruction Register; "지금 무엇을 할지"를 담는 JTAG 명령 레지스터), **TAR/CSW/DRW**(MEM-AP의 세 레지스터 — 각각 목표 주소/전송 속성/데이터를 담음), **CTI**(Cross Trigger Interface; halt 같은 이벤트를 코어로 전달하는 CoreSight 제어 블록), **TDO**(Test Data Out; 칩에서 값이 직렬로 빠져나오는 JTAG 출력 핀)입니다. 자세한 동작은 N02/N03에서 다루고, 여기서는 흐름만 봅니다.
...
**보안 관점.** 디버그 인프라는 칩 내부를 들여다보는 강력한 통로이므로, 양산 칩에서 함부로 열리면 보안 키나 펌웨어가 유출될 수 있습니다. 그래서 DBGEN/NIDEN/SPIDEN/SPNIDEN 네 인증 신호(각 디버그 동작을 켜고 끄는 게이트 신호 — §4.3에서 분해)가 invasive/non-invasive × secure/non-secure 조합별로 디버그를 게이팅(통과 여부를 제어)하며, 이 신호들은 보통 **fuse**(제조 후 한 번 태워 영구히 값을 고정하는 일회성 메모리 — 보안 설정을 되돌릴 수 없게 박는 데 씀)나 **lifecycle**(칩이 개발→양산→폐기 중 어느 단계인지를 나타내는 상태 — 단계에 따라 디버그 허용 범위가 달라짐) 상태에서 구동됩니다.

**가상화 관점.** 여러 게스트가 한 SoC를 공유하는 환경에서는 디버그 권한도 가상화 경계를 존중해야 합니다. 어떤 게스트의 디버그가 다른 게스트나 **hypervisor**(여러 가상 머신을 한 하드웨어 위에서 격리해 돌리는 관리 소프트웨어 계층)의 상태를 볼 수 없어야 하며, **secure world**(Arm **TrustZone**에서 일반 코드와 분리해 보안 키·인증을 다루는 격리된 실행 영역)의 디버그는 SPIDEN/SPNIDEN으로 따로 게이팅됩니다.
...
네 신호의 이름은 곧 그 역할입니다 — **DBGEN**(Debug Enable; non-secure invasive 허용), **NIDEN**(Non-Invasive Debug Enable; non-secure trace/PMU 허용), **SPIDEN**(Secure Privileged Invasive Debug Enable; secure halt 허용), **SPNIDEN**(Secure Privileged Non-Invasive Debug Enable; secure trace 허용). 각 디버그 동작은 그에 맞는 인증 신호가 _enable_(켜져 허용 상태)되어 있어야만 동작합니다. 예를 들어 secure world의 코드를 halt하려면 SPIDEN이 켜져 있어야 하고, 이 신호가 fuse로 영구히 막혀 있으면 양산 칩에서는 secure halt가 원천 불가능합니다.
...
디버그가 가장 필요한 순간은 시스템이 비정상일 때입니다. 코어가 **전원 게이팅**(power-gating; 안 쓰는 블록의 전원을 잘라 전력을 아끼는 기법 — 꺼진 블록은 내부가 안 보임)으로 꺼져 있거나, 시스템 클럭이 멈췄거나, **warm reset**(전원은 유지한 채 논리 상태만 초기화하는 reset; 전원까지 내렸다 올리는 것은 **cold reset**)이 걸린 직후 같은 상황입니다. 만약 디버그 로직이 코어 전원/클럭에 종속되어 있다면, 정작 문제가 생긴 순간 디버그 통로도 같이 죽어 버립니다.

그래서 디버그 서브시스템은 보통 **독립적인 디버그 클럭(DBG clock; 코어 클럭이 멈춰도 따로 도는 디버그 전용 클럭)** 과 **always-on 전원 도메인**(코어가 꺼져도 항상 전원이 유지되는 영역)에 둡니다. 이렇게 해야 코어가 power-gating으로 꺼져 있어도 디버거가 DAP에 붙어 코어를 깨우거나 상태를 읽을 수 있습니다.
