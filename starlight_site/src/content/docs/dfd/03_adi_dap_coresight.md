...
DAP의 핵심 동작인 "MEM-AP를 통한 시스템 버스 write"를 단계로 봅시다. HDG의 typical access flow를 그대로 따릅니다.
...
:::note[여기서 잡아야 할 핵심]
디버거가 한 일은 DAP 레지스터 몇 개에 값을 쓴 것뿐인데(②~⑤), 그 결과 MEM-AP가 _진짜 시스템 버스 트랜잭션_을 발행합니다(⑥). 이 ⑥의 트랜잭션이 곧 [AMBA 코스](../../amba_protocols/)의 APB/AHB/AXI 트랜잭션과 동일합니다. 대부분의 CoreSight 컴포넌트는 **APB-AP**를 통해 접근됩니다(HDG §2).
:::
...
CoreSight는 디버그 동작을 네 신호로 게이팅합니다(HDG §3). 각 신호가 enable되어 있어야 해당 디버그가 동작합니다.
...
HDG §5의 DFD 통합 체크리스트입니다. 검증/통합 엔지니어가 새 SoC에서 점검해야 할 항목들입니다.
...
DAP/CoreSight 서브시스템을 검증할 때 핵심 시나리오는 다음과 같습니다(추론: HDG는 통합 체크리스트는 명시하나 구체적 testbench 시나리오는 명시하지 않음).
...
- `DFD` (Confluence import, `wiki/common/dfd_spec.md`) — §2 ADI/DAP(DP·AP·접근 flow·ROM table), §3 CoreSight 컴포넌트 카테고리·glossary·인증 신호, §4 3계층 상호작용, §5 통합 체크리스트
