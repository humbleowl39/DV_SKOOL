...
**"선이 없는데 어떻게 인터럽트인가" — MSI는 사실 _특정 주소로의 memory write_ 다.** 이것이 MSI의 가장 중요하면서도 가장 덜 직관적인 부분입니다. MSI의 "메시지"는 추상적인 신호가 아니라 **평범한 PCIe memory write 트랜잭션**입니다. 디바이스의 MSI capability 레지스터에는 OS가 enumeration 때 채워 넣은 두 값이 있습니다 — **message address**(어디에 쓸지)와 **message data**(무엇을 쓸지). 디바이스가 인터럽트를 걸고 싶으면, 그 _address_ 에 그 _data_ 를 쓰는 memory write 한 번을 발행합니다. 그게 전부입니다 — 디바이스 입장에서 인터럽트는 "특정 주소에 특정 값을 쓰는 store"입니다.

그러면 _그 write가 어떻게 인터럽트가 되는가_? message address는 임의의 DRAM 주소가 아니라, **인터럽트 컨트롤러가 디코드하는 특수 주소 영역**(x86이면 local APIC의 0xFEE0_0000 대역, ARM이면 GIC의 ITS doorbell 레지스터 주소)을 가리킵니다. 그 주소로의 write가 메모리 패브릭을 타고 인터럽트 컨트롤러에 도달하면, 컨트롤러는 "이건 DRAM 저장이 아니라 인터럽트 요청"으로 해석하고, write data(와 출처)에서 _어느 벡터/CPU_ 인지를 디코드해 코어에 인터럽트를 올립니다.
...
DEV -> WR: "인터럽트 발행 =\n그냥 store"
WR -> IC: "패브릭이 라우팅"
IC -> CPU: "decode(data) → vector"
```

이 메커니즘이 MSI의 장점을 설명합니다 — _물리 선이 필요 없으니_ 디바이스 수만큼 선을 깔 필요가 없고, write data를 바꾸면 _벡터를 무한히 다중화_ 할 수 있습니다(**MSI-X** 는 디바이스마다 여러 개의 address/data 쌍을 테이블로 두어, 한 디바이스가 여러 벡터를—예: 큐별로—분리해 발행). 검증에서는 "디바이스가 인터럽트 조건에서 _올바른 msg_addr 에 올바른 msg_data 를_ 담은 memory write 트랜잭션을 발행하는가"가 핵심 체크입니다 — 즉 MSI 검증은 인터럽트 선 검증이 아니라 _버스 write 트랜잭션의 payload 검증_ 입니다(§5.2 scoreboard).

ARM의 **GIC ITS(Interrupt Translation Service)** 가 이 라우팅의 구체입니다 — 디바이스는 doorbell 주소에 자기 **EventID** 를 write하고, ITS는 두 단계 테이블(디바이스별 **device table** → 그 디바이스의 **ITT(Interrupt Translation Table)**)로 _DeviceID + EventID_ 를 최종 **LPI(인터럽트 번호)** 로 변환해 코어로 보냅니다. 즉 "어느 디바이스의 어느 이벤트"가 "어느 인터럽트"가 되는지는 ITS의 테이블 lookup으로 결정되며, 이 테이블을 OS가 구성합니다 — MSI 라우팅이 _소프트웨어로 프로그래밍되는_ 실체입니다.
...
각 단계가 사이클을 더하므로, 인터럽트는 본질적으로 _즉시가 아니라_ "다음 명령 경계 + 파이프라인 정리 + 저장/벡터링"만큼 지연됩니다. 이 분해의 하드웨어 측 상세(특히 ARM의 단계별 진입 흐름)는 [cpu_arm — Caches & GIC](../../cpu_arm/06_caches_gic/)에서 다룹니다. latency가 중요한 실시간 시스템에서는 이 합을 줄이려 마스킹 구간을 짧게 유지하고(2번 대기 단축), 핸들러를 _빠른 진입_ 으로 설계합니다 — 그것이 §5.1 "top-half는 짧게"의 근거입니다.
...
### 5.2 bottom-half — 긴 일을 _어디로_ 미루나

§5.1의 "긴 일은 bottom-half로"는 한 줄이지만, 실제로는 _여러 종류_ 의 deferred work 메커니즘이 있고 선택이 중요합니다. 원칙은 **top-half(ISR)를 최대한 짧게** 유지하는 것입니다 — top-half가 도는 동안 (적어도 같은) 인터럽트가 막히고, 길면 latency·재진입·우선순위 역전을 일으키기 때문입니다. 그래서 acknowledge·필수 상태 갱신만 top-half에서 하고, 패킷 처리 같은 긴 일은 인터럽트가 _재활성화된_ 문맥으로 미룹니다.

| 메커니즘 | 실행 문맥 | 특징 |
|---|---|---|
| **softirq** | 인터럽트 문맥(인터럽트 직후, 인터럽트 enable 상태) | 가장 빠름, 정적으로 정의된 소수 종류, 같은 종류가 여러 코어에서 _동시_ 실행 가능 → 자체 동기화 필요 |
| **tasklet** | softirq 위에 구현 | 동적 생성 쉬움, 같은 tasklet은 한 번에 한 코어에서만 → softirq보다 쓰기 단순 |
| **workqueue** | _프로세스 문맥_(커널 스레드) | 잠들 수 있음(sleep 가능) — mutex·메모리 할당 등 blocking 작업 가능. 대신 스케줄링 지연 |
| **threaded IRQ** | 전용 커널 스레드 | 핸들러 본체를 스레드로 분리, 우선순위 부여·실시간성에 유리 |

핵심 구분: softirq/tasklet은 _인터럽트(atomic) 문맥_ 이라 **잠들 수 없습니다**(blocking 호출 금지), workqueue/threaded IRQ는 _프로세스 문맥_ 이라 잠들 수 있습니다. "이 deferred 작업이 blocking을 하는가"가 선택의 1차 기준입니다 — DMA 매핑·메모리 할당처럼 잠들 수 있는 일은 workqueue로, 짧고 non-blocking인 네트워크 후처리는 softirq/tasklet으로 갑니다.

### 5.3 spurious interrupt — 원인 없는 인터럽트의 처리

가끔 인터럽트 컨트롤러가 "인터럽트가 왔다"고 코어를 깨웠는데 _막상 원인 소스를 찾을 수 없는_ 경우가 있습니다 — **spurious interrupt** 입니다. 원인은 여럿입니다: 공유 level 선에서 한 디바이스가 acknowledge하는 _찰나_ 에 다른 관찰이 끼거나, level 선의 기생 용량/노이즈로 짧은 glitch가 잡히거나, 인터럽트가 마스킹되는 _순간_ 과 도착이 겹치는 race 때문입니다.

컨트롤러는 이를 위해 **특수 spurious INTID** 를 반환합니다 — ARM GIC는 "읽을 유효 인터럽트가 없음"을 INTID **1023**(spurious) 로 알립니다(외부 표준 지식). ISR(또는 GIC 처리 루틴)이 acknowledge 레지스터를 읽었는데 1023이 나오면, "실제 처리할 것이 없다"는 뜻이므로 _아무 일도 하지 않고 조용히 복귀_ 합니다 — 특히 spurious에는 EOI(end-of-interrupt)를 보내지 _않아야_ 합니다(존재하지 않는 인터럽트를 종료 처리하면 안 되므로). 검증에서는 (1) 위 race 상황을 의도적으로 만들었을 때 컨트롤러가 spurious INTID를 정확히 반환하는가, (2) 공유 선에서 _모든_ ISR이 자기 소스 아님(`IRQ_NONE`)을 반환하는 "전원 부정" 시나리오를 시스템이 spurious로 안전하게 흡수하는가를 봅니다.

### 5.4 DV 관점 — 인터럽트를 레지스터·드라이버 레벨에서 검증
