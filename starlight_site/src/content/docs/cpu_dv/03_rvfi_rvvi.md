...
| **모드** | `rvfi_mode`, `rvfi_ixl` | 실행 당시 privilege mode, XLEN(레지스터 폭 — 32/64-bit 중 무엇으로 실행 중인지) |
...
`rvfi_valid` 가 1 인 사이클이 정확히 _한 명령의 retire_ 를 의미합니다. superscalar(한 사이클에 여러 명령을 동시에 발행·retire 하는 코어 — 명령 처리 폭이 1 보다 큼) 코어는 한 사이클에 여러 명령을 retire 할 수 있으므로, RVFI 는 보통 _채널마다_ 또는 _retire port 마다_ valid 를 둡니다. `rvfi_order` 는 프로그램 순서를 나타내는 단조 증가 번호로, retire 가 여러 포트로 나뉘어도 비교기가 _프로그램 순서_ 를 복원하게 합니다.

**order 의 monotonicity 가 hole 을 드러낸다.** `rvfi_order` 의 계약은 단순한 "증가"가 아니라 _retire stream 에서 연속(gap 없음)_ 이라는 점입니다. retire 된 명령에만 order 가 부여되고, flush/squash(분기 예측 실패 등으로 잘못 가져온 추측 명령을 파이프라인에서 무효화·폐기하는 것)로 _폐기된_ 추측 명령에는 order 가 붙지 않습니다 — 즉 폐기는 order 를 _소비하지 않습니다_. 따라서 비교기가 받은 order 가 N, N+1, **N+3** 처럼 건너뛰면, 그것은 "N+2 명령이 retire 됐는데 RVFI 로 노출되지 않았다"는 _검증 hole_ 의 신호입니다. order 의 연속성 검사 자체가 "retire 한 모든 명령을 빠짐없이 봤는가"의 무결성 체크인 셈입니다(riscv-formal 의 `liveness`·`unique` 류 검사가 이 성질을 형식적으로 요구).
...
이 계약은 monitor 쪽에는 편하지만, _코어 설계자_ 에게는 분명한 결선 의무를 지웁니다 — **RVFI 신호는 반드시 ROB commit(retire) 신호에 결선해야** 합니다. 흔한 구현 실수는 execution 결과가 나오는 시점(예: ALU writeback, speculative wakeup)에 RVFI 를 일찍 토글하는 것입니다. 그러면 _나중에 flush 될 추측 명령_ 의 값이 RVFI 로 새어 나가, monitor 입장에선 "retire 되지도 않은 명령"이 노출되거나 추측 중간값이 보입니다. 이는 코어의 RVFI 구현 버그이지 비교 로직의 문제가 아니며, 증상은 "가끔 추측값처럼 보이는 rd_wdata" 또는 "flush 와 함께 사라지는 유령 retire"로 나타납니다. 올바른 구현은 ROB 엔트리가 _commit 포인터를 통과하는 그 사이클_ 에만 해당 명령의 architectural 효과를 RVFI 로 내보내는 것입니다.
...
### 5.5 RVFI 의 사각지대 — CSR·다중 메모리 접근 채널의 한계

RVFI 가 "retire 의 모든 것"을 노출한다고 가정하면 _조용한 비교 누락(blind spot)_ 에 빠질 수 있습니다. 채널 구조 자체에 한계가 있기 때문입니다.

- **CSR 변화의 노출은 구현 정의에 가깝다.** 기본 RVFI 신호 집합(메타·PC·레지스터·메모리·trap)에는 _전용 CSR write 채널이 명시적이지 않습니다_. 어떤 코어는 CSR 변화를 별도 확장 신호로 노출하지만, 표준이 모든 CSR 의 매 변화를 1:1 로 노출하도록 강제하지는 않습니다. 그래서 한 명령이 _여러_ CSR 을 부수효과로 바꾸거나(예: trap 진입 시 mepc/mcause/mstatus 동시 갱신), 명령이 직접 쓰지 않은 CSR 이 hardware 로 갱신되는 경우, RVFI 만으로는 그 변화가 비교에 안 잡힐 수 있습니다 — 비교기는 "본 것만" 대조하므로, 노출되지 않은 CSR 의 버그는 silent 하게 통과합니다.
- **한 명령의 다중 메모리 접근 표현이 제한적이다.** 기본 메모리 채널은 보통 명령당 하나의 접근(addr/rdata/wdata/mask)을 가정합니다. misaligned access 가 두 라인에 걸치거나, 한 명령이 여러 워드를 건드리는 경우(구현에 따라)를 단일 채널로 표현하기 어려워, 일부 접근이 비교에서 빠질 수 있습니다.

대응 원칙은 **"RVFI 가 노출하지 _않는_ architectural 변화는 비교 범위 밖"이라는 사실을 명시적으로 인지하고**, 빠지는 CSR·메모리 접근에 대해서는 (a) 코어의 RVFI 확장 신호를 추가로 결선하거나, (b) 해당 CSR 을 별도 RAL/probe 로 교차 확인하거나, (c) 최소한 coverage 로 "그 경로를 봤는지"를 측정해 blind spot 을 가시화하는 것입니다. "RVFI 가 통과시켰다 = 옳다"가 아니라 "RVFI 가 _본_ 범위에서 옳다"로 해석해야 합니다.
