...
**order 의 monotonicity 가 hole 을 드러낸다.** `rvfi_order` 의 계약은 단순한 "증가"가 아니라 _retire stream 에서 연속(gap 없음)_ 이라는 점입니다. retire 된 명령에만 order 가 부여되고, flush/squash 로 _폐기된_ 추측 명령에는 order 가 붙지 않습니다 — 즉 폐기는 order 를 _소비하지 않습니다_. 따라서 비교기가 받은 order 가 N, N+1, **N+3** 처럼 건너뛰면, 그것은 "N+2 명령이 retire 됐는데 RVFI 로 노출되지 않았다"는 _검증 hole_ 의 신호입니다. order 의 연속성 검사 자체가 "retire 한 모든 명령을 빠짐없이 봤는가"의 무결성 체크인 셈입니다(riscv-formal 의 `liveness`·`unique` 류 검사가 이 성질을 형식적으로 요구).
