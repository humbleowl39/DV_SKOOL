...
memory ordering 은 단일 코어 안에서도(load/store 재정렬), 특히 multi-hart 에서 _관찰 순서_ 가 메모리 모델(RISC-V RVWMO — RISC-V Weak Memory Ordering; 어떤 load/store 재정렬이 합법인지 정의한 RISC-V 의 메모리 일관성 규칙)을 지키는가입니다.(외부 표준 지식) coverage 는 _값_ 이 아니라 _순서 시나리오_ 를 본다: store-load forwarding, 두 hart 의 message-passing 패턴(한 hart 가 data 후 flag 를 쓰고, 다른 hart 가 flag 후 data 를 읽는) 등이 허용된 결과만 내는가. 이 영역은 litmus test(정형화된 작은 동시성 패턴)로 자극을 만드는 것이 표준입니다.(외부 표준 지식)
...
| **IRIW** (Independent Reads of Independent Writes) | 두 writer + 두 reader | 두 reader 가 두 store 의 순서를 _다르게_ 관찰할 수 있나 — multi-copy atomicity(한 store 가 모든 코어에게 _동시에_ 보이는가; 코어마다 다른 순서로 보이면 위반) |
