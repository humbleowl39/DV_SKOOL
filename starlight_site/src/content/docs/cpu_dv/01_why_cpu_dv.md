...
신규 RISC-V 코어를 검증하는 팀이 컴파일러 회귀 테스트, 부트로더, 몇 개의 벤치마크를 **directed test**(검증 엔지니어가 특정 시나리오를 노리고 손으로 짠 결정적 테스트) 로 돌려 모두 PASS 를 받았습니다. 자신감을 갖고 tape-out 했는데, 실리콘에서 특정 OS 커널이 간헐적으로 죽습니다. 추적해 보니 원인은 이랬습니다.
...
이 세 명령이 _특정 파이프라인 정렬_ 로 겹치면서, load-use forwarding(앞 명령의 결과를 레지스터 파일에 다 쓰기 전에 곧바로 뒤 명령에 전달하는 우회 경로 — load 결과를 바로 다음 명령이 쓸 때 동작) 이 한 사이클 어긋난 값을 CSR 에 쓰는 버그였습니다. directed test 들은 이 명령 _조합_ 과 _타이밍_ 을 우연히도 한 번도 만들지 않았습니다. 명령 자체는 각각 수천 번 실행됐지만, "load 직후 그 값을 CSR 에 쓰는, 그것도 특정 stall 정렬에서" 라는 _조합_ 이 빠졌던 것입니다.
