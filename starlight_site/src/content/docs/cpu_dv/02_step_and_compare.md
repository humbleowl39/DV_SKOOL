...
| **memory-mapped I/O 읽기** | 읽을 때마다 값이 바뀌어 ISS 가 예측 못 함 → RTL 이 읽은 값을 ISS 에 _주입_ |
| **self-modifying code** | 코드 영역에 store 가 생기면 ISS 의 명령 메모리도 같은 store 로 갱신해야 동일 fetch |

핵심 원칙: **architectural _의미_ 는 비교하되, _구현 정의·비결정_ 값은 RTL→ISS 로 동기화**해 비교 대상에서 빼거나 강제로 맞춥니다.

마지막 두 행은 "ISS 가 _원리적으로_ 예측할 수 없는 입력"이라 특히 까다롭습니다. **MMIO 읽기**는 같은 주소를 읽어도 매번 다른 값이 나올 수 있어(상태 레지스터, FIFO pop 등), ISS 가 자기 메모리에서 읽으면 RTL 과 어긋납니다 — 해결은 RTL 이 실제로 읽어온 load 결과를 ISS 의 그 load 에 _덮어쓰기(주입)_ 하는 것입니다(RVFI 의 `mem_rdata` 채널이 이 용도). **self-modifying code** 는 데이터 store 가 _이후 fetch 될 명령_ 을 바꾸는 경우로, ISS 가 옛 명령을 fetch 하면 발산합니다 — RTL 의 store 가 ISS 메모리에도 반영되어야 같은 명령을 보게 됩니다(여기에 더해 RISC-V 는 `fence.i` 전까지 가시성을 보장하지 않으므로, ISS 가 그 fence 의미까지 따라야 정확).
...
### 5.4 무엇을 비교하나 — architectural vs micro-architectural 경계

비교가 성립하려면 "무엇이 비교 대상인가"를 명확히 그어야 합니다. ISS 는 _architectural_ state 만 모델링하므로, RTL 의 _micro-architectural_ state 를 비교에 끌어들이면 반드시 false mismatch 가 납니다.

| 비교 대상 (architectural) | 비교 대상 _아님_ (micro-architectural) |
|---|---|
| GPR(x0–x31) retire 시점 값 | 파이프라인 래치(IF/ID, ID/EX …)의 중간 값 |
| 구현된 CSR 의 architectural 값 | 캐시 라인 상태·dirty 비트 |
| PC / next PC | 분기 예측기 상태(BHT/BTB) |
| 메모리(load/store가 닿은 값) | reorder buffer·reservation station 내용 |
| privilege 모드 | 물리 레지스터 번호(rename 결과) |

경계의 원리는 **"ISA 가 소프트웨어에 보장하는 것만 architectural"** 입니다. 프로그램이 직접 관측할 수 있는 상태(레지스터·메모리·PC·CSR)는 두 모델이 같아야 하지만, 캐시·예측기·파이프라인 래치는 _같은 architectural 결과를 내는 다른 구현_ 일 뿐이라 값이 달라도 정상입니다. micro-arch 상태는 비교가 아니라 _coverage·SVA_ 의 관찰 대상입니다(M07).

### 5.5 메모리는 _언제_ 비교하나 — load 즉시 vs store 지연

레지스터는 retire 시점 값이 곧 확정이라 비교가 단순하지만, 메모리는 store 가 retire 후에도 store buffer·캐시에 _머물다_ 늦게 메모리에 닿기 때문에 "언제 비교하나"가 문제입니다.

- **load 결과**는 즉시 비교 가능 — load 가 retire 하는 순간 읽어온 데이터(`rvfi_mem_rdata`)가 확정되므로 ISS 의 load 결과와 그 자리에서 대조합니다.
- **store 데이터**는 두 가지 모델로 다룹니다. (a) _retire 시점 비교_: store 가 retire 할 때 "주소 A 에 값 D 를 쓴다"는 _의도_(`rvfi_mem_waddr/wdata`)를 ISS 의 store 와 비교 — 실제 메모리에 닿는 시점과 무관하게 architectural 의미만 본다. (b) _최종 dump 비교_: 프로그램 끝에 RTL 메모리 전체를 ISS 메모리와 대조 — buffer 가 모두 drain 된 뒤라 안전하지만 first-divergence 의 정밀함은 잃는다.

표준은 (a)입니다 — retire 시점에 store _의도_ 를 비교하면 first-divergence 의 즉시성을 유지하면서, store buffer 의 타이밍(micro-arch)은 비교에서 자연히 빠집니다. 이때 RTL 측은 retire monitor 가 store 의 architectural 효과를 노출하는 **backdoor** 경로(메모리에 닿기를 기다리지 않고 retire record 에서 직접 addr/data 를 뽑음)를 쓰고, 실제 버스 트랜잭션이 메모리에 도착하는지는 별도 메모리 모델/scoreboard 가 추적합니다(M04).

### 5.6 divergence 후 resync — 한 run 에서 여러 버그 모으기

기본 정책은 "첫 divergence 에서 멈춤"이지만(5.1), 일부 환경은 divergence _후_ ISS 를 RTL 상태로 **강제 resync** 해 계속 진행시킵니다. 명령 N 에서 어긋나면, ISS 의 architectural state 를 RTL 의 (잘못된) state 로 덮어쓰고 N+1 부터 다시 비교하는 방식입니다.

| 정책 | 장점 | 단점 |
|---|---|---|
| **stop-at-first (기본)** | root cause 가 첫 명령 하나로 명확, 디버그 단순 | 한 run 에 버그 1개만 수집 |
| **resync-and-continue** | 한 회귀 run 에서 _복수_ 독립 버그를 한 번에 수집 → throughput↑ | resync 자체가 cascading 을 가릴 수 있고, "RTL 의 틀린 값으로 ISS 를 맞춤"이라 이후 비교의 의미가 약해짐 |

trade-off 의 핵심: resync 는 _독립적인_ 버그가 여럿일 때 회귀 효율을 높이지만, 첫 버그가 이후를 오염시키는 경우엔 resync 가 그 인과를 숨길 수 있어 항상 first-divergence 가 진짜 root cause 라는 보장을 잃습니다. 그래서 보통 bring-up·디버그 단계에선 stop-at-first, 대규모 회귀의 _수집_ 단계에선 resync 를 선택적으로 씁니다.

### 5.7 step-and-compare 가 특히 잘 잡는 버그
