...
#### write-allocate vs no-write-allocate — write miss 시 라인을 할당할까

write-back 캐시라도 _write miss_ 시 행동에 선택지가 있습니다. **write-allocate(WA)** 는 write miss 가 나면 해당 라인을 캐시로 가져온 뒤 그 위에 씁니다 — 곧 그 라인을 다시 읽거나 쓸 것이라는 locality 베팅입니다. **no-write-allocate(nWA)** 는 라인을 캐시에 안 들이고 write 를 하위 계층으로 바로 흘려보냅니다.

ARM 페이지 속성(`MAIR_EL1`)은 영역별로 이 정책을 고를 수 있고, 선택의 기준은 _접근 패턴_ 입니다. 일반 데이터는 쓰고 곧 다시 만질 가능성이 높아 WA 가 유리합니다. 그러나 **streaming write**(한 번 쓰고 다시 안 읽는 큰 버퍼 초기화·복사)에서는 WA 가 손해입니다 — 곧 안 쓸 라인을 굳이 캐시로 끌어와(read-for-ownership) _유용한 라인을 축출_ 하고 대역폭만 낭비하기 때문입니다. 이런 경우 nWA(또는 캐시를 거치지 않는 경로)가 캐시 오염을 막아 이롭습니다. 같은 맥락에서 `DC ZVA`(캐시 라인 통째 zero, §5.1) 같은 streaming 초기화 명령은 _기존 데이터를 읽어 올 필요가 없으므로_ 라인을 fetch 하지 않고 곧장 0 으로 채워, 대량 memset 에서 read-allocate 비용을 제거합니다. 검증·성능에서 "큰 버퍼 쓰기가 다른 데이터의 캐시를 밀어낸다"면 WA 정책과 streaming 패턴의 부조화를 의심합니다.
...
#### 왜 CMO 가 _다른 코어_ 에는 불필요한가 — coherence 가 자동 처리한다

여기서 자연스러운 의문이 듭니다 — DMA 와 stale 을 CMO 로 맞춰야 한다면, _다른 CPU 코어_ 끼리도 매번 CMO 가 필요할까요? 답은 **아니오** 이고, 그 이유가 핵심입니다. 같은 shareability domain 안의 코어들은 **하드웨어 cache coherence 프로토콜**이 _자동으로_ 정합성을 유지합니다 — SW 가 CMO 를 칠 필요가 없습니다.

작동 원리를 한 단락으로: 각 캐시 라인은 **MESI**(Modified / Exclusive / Shared / Invalid) 또는 그 확장인 **MOESI**(Owned 추가) 같은 상태기를 가집니다. 코어 A 가 어떤 라인을 쓰려 하면(M 상태로 가려 하면), 인터커넥트가 다른 코어들에게 **snoop**(질의)을 보내 그 라인의 사본을 _invalidate_ 시키고, 만약 다른 코어가 최신(dirty) 값을 가졌으면 그 값을 _전달_ 받습니다. 그래서 코어 B 가 같은 주소를 읽으면, 자기 캐시가 invalid 가 되었거나 coherence 가 최신 값을 넘겨 주어 _항상 최신_ 을 봅니다 — 코어 간에는 "stale 캐시"가 _구조적으로_ 생기지 않습니다.

DMA 가 CMO 를 필요로 했던 것은, 전통적 DMA 엔진이 이 coherence 프로토콜에 _참여하지 않아_(snoop 을 안 보내고 DRAM 을 직접 침) 캐시 밖에 있었기 때문입니다(M-hw_sw 의 IO-coherent DMA 는 이 참여 여부가 다름). 반대로 코어들은 모두 같은 coherence 도메인 안에 있어 자동 동기화됩니다. 그래서 멀티스레드 공유 변수의 정합성은 CMO 가 아니라 _배리어_(M04, 관측 _순서_ 문제)로 다루고, CMO 는 _coherence 밖의 관측자_(I-cache vs D-cache 의 self-modify, 비-coherent DMA)에만 씁니다. 이 구분을 놓치면 "코어 간인데 왜 CMO 가 안 먹히지"(사실 불필요) 또는 "DMA 인데 배리어만 쳤다"(사실 CMO 필요)로 헷갈립니다. coherence 프로토콜 상태기의 전체 동작은 [Cache Coherence 토픽](../../cache_coherence/)에서 다룹니다.
...
#### priority 와 preemption — "EOI 가 priority 를 내린다"의 실제 동작

§3.2 에서 `ICC_IAR1` read 가 acknowledge, `ICC_EOIR1` write 가 EOI 라고 했는데, 그 사이에 **priority** 가 어떻게 preemption 을 결정하는지가 핵심 메커니즘입니다. 두 개의 priority 값이 관여합니다.

- **`ICC_PMR_EL1`(Priority Mask Register)**: "이 코어가 받아들일 최소 우선순위"를 정하는 _마스크_ 입니다. PMR 보다 낮은 우선순위(숫자상으로는 GIC 에서 _작은 값이 높은 우선순위_)의 인터럽트는 아예 전달되지 않습니다 — SW 가 PMR 을 올리면 일정 수준 이하 인터럽트를 일괄 차단할 수 있습니다.
- **running priority**: 현재 _처리 중인_ 인터럽트의 우선순위입니다. `ICC_IAR1` 로 인터럽트를 acknowledge 하면 GIC 가 running priority 를 _그 인터럽트의 우선순위로 올립니다._

preemption 은 이 둘의 비교로 일어납니다. ISR 처리 중(running priority 가 올라간 상태)에 _더 높은 우선순위_ 인터럽트가 도착하면, 그것은 현재 running priority 보다 높으므로 현재 핸들러를 **선점(preempt)** 해 중첩 진입합니다 — 반대로 같거나 낮은 우선순위는 현재 ISR 이 끝날 때까지 대기합니다. 그래서 "**EOI 가 priority 를 내린다**"의 실체는 이것입니다: `ICC_EOIR1` 에 INTID 를 쓰면 GIC 가 _running priority 를 그 인터럽트 이전 수준으로 되돌려_, 비로소 그 이하 우선순위 인터럽트가 다시 전달될 수 있게 됩니다. EOI 를 빠뜨리면 running priority 가 높은 채로 남아 _그보다 낮은 모든 인터럽트가 영원히 차단_ 되어 hang 합니다(§6 체크리스트의 "인터럽트 한 번만 받고 멈춤"). 즉 IAR=priority 올림, EOIR=priority 내림, 그 사이 비교가 preemption — 이 한 사이클이 GIC 우선순위의 전부입니다.

#### ITS 의 변환 테이블 — DeviceID+EventID 가 LPI 가 되기까지

"ITS 가 DeviceID+EventID 를 LPI 로 변환"이 한 줄로는 마법처럼 들리지만, 실제로는 메모리에 있는 _두 단계 테이블 조회_ 입니다. PCIe 장치가 MSI 를 보내면 그 메시지에는 어느 장치인지(**DeviceID**, 보통 PCIe RID 에서 유도)와 어떤 이벤트인지(**EventID**, 장치 내 인터럽트 벡터 번호)가 실립니다. ITS 는 이를 다음으로 푼다고 이해하면 됩니다.

1. **Device Table**: DeviceID 로 조회해, 그 장치 전용의 **ITT(Interrupt Translation Table)** 위치를 얻습니다 — "이 장치의 인터럽트 매핑이 어디 있는지".
2. **ITT(Interrupt Translation Table)**: 위에서 얻은 ITT 안에서 EventID 로 조회해, 최종 **LPI INTID** 와 _목표 Redistributor(어느 코어)_ 를 얻습니다.

즉 DeviceID 가 "어느 장치의 표를 볼지"를, EventID 가 "그 표의 어느 줄을 볼지"를 정해, 둘의 조합이 _고유한 LPI 번호와 목적지 코어_ 로 매핑됩니다. 이 테이블 기반 구조 덕분에 _수천 개의 장치 × 다수 이벤트_ 가 만드는 방대한 MSI 를, 고정 배선 없이 메모리 테이블만 갱신해 유연하게 라우팅합니다 — 가상화에서 게스트별로 인터럽트를 재매핑하거나 장치를 다른 코어로 옮기는 것도 테이블 수정으로 됩니다. 검증에서 "MSI 가 엉뚱한 코어로 가거나 안 온다"면 Device Table/ITT 엔트리(DeviceID·EventID 매핑, 목적 Redistributor)부터 확인합니다.
