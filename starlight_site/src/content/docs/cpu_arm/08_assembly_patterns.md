...
NEON 은 32개의 **128비트** 벡터 레지스터 `V0~V31` 을 가지며, 같은 물리 레지스터를 element 폭에 따라 다르게 봅니다 (asm/SIMD ①).
...
`y[i] = a*x[i] + y[i]` 를 NEON(고정 4-lane)과 SVE(VL-agnostic)가 어떻게 다르게 쓰는지 보면 두 모델의 차이가 드러납니다.
...
- **Structured load `ld3`/`ld4`**: 메모리의 인터리브 데이터(RGB RGB …)를 로드하며 자동 deinterleave — `ld3 {v0.4s,v1.4s,v2.4s},[x0]` 한 명령으로 R/G/B 를 각 레지스터로 분리. AoS↔SoA 변환에 강력합니다 (asm/SIMD ⑧).
