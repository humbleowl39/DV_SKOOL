...
데이터센터 가속기 카드의 관리용 펌웨어를 검증한다고 합시다. 같은 C 소스를 한 SoC의 Cortex-A 코어용으로 빌드한 바이너리는 잘 도는데, 다른 SoC의 Cortex-M 기반 MCU로 가져가니 부팅조차 못 합니다. 원인은 코드 버그가 아니라 **ISA 프로파일**이 다르다는 데 있습니다. Cortex-A는 AArch64 + MMU를 갖춘 application 프로파일이지만, Cortex-M은 Thumb-2 전용에 MMU가 없는 microcontroller 프로파일이라 64-bit AArch64 바이너리를 아예 실행할 수 없습니다.
...
검증 관점에서 이 분류는 곧 환경 가정으로 직결됩니다. Cortex-A/Neoverse라면 MMU·다단계 주소 번역·SMP coherence를 전제해야 하고(M04–M06), Cortex-M이라면 그런 것들이 아예 없어 검증 시나리오가 단순해집니다.
