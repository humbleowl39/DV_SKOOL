...
:::note[여기서 잡아야 할 두 가지]
**(1) X30(LR)을 저장하지 않고 다른 함수를 호출하면 복귀 주소를 잃는다.** **`BL`**(branch-with-link — 분기하면서 복귀 주소를 LR에 저장하는 함수 호출 명령)이 X30을 새 복귀 주소로 덮어쓰기 때문 — 그래서 프롤로그가 먼저 LR을 스택에 옮긴다.<br>
...
#### PSTATE 는 하나의 주소 가능한 레지스터가 아니다 — 그 함의

위 표를 보면 PSTATE 를 하나의 레지스터처럼 그렸지만, 실제로는 **single addressable register 가 아닙니다.** PSTATE 는 코어의 여러 실행 상태 비트를 _논리적으로 묶어 부르는 이름_ 일 뿐, "PSTATE 라는 주소를 한 번에 읽는" `MRS x0, PSTATE` 같은 명령은 없습니다. 이것이 실무에 직접 영향을 줍니다 — PSTATE 의 내용을 보거나 바꾸려면 **field 별로 따로 접근**해야 합니다.

구체적으로 NZCV 는 `MRS x0, NZCV`, DAIF 는 `MRS x0, DAIF`, 현재 EL 은 `MRS x0, CurrentEL` 처럼 _각 필드가 별도의 이름(별칭 시스템 레지스터)_ 으로 노출됩니다. 쓰기도 마찬가지로 `MSR DAIFSet, #imm` / `MSR DAIFClr, #imm` 처럼 필드 단위 명령을 씁니다. 유일하게 통째로 다뤄지는 순간은 _예외 진입/복귀_ 인데, 이때는 HW 가 PSTATE 전체를 SPSR 로 자동 저장(진입)하고 `ERET` 가 SPSR 에서 자동 복원(복귀)합니다 — 즉 "통째로"는 SW 명령이 아니라 HW 자동 경로에서만 일어납니다. 검증·디버그에서 "PSTATE 를 한 줄로 못 읽는다"가 함정이 아니라 _설계상 필드별 접근이 정상_ 이라는 점을 알아야, 플래그/마스크 상태를 추적할 때 올바른 필드 별칭을 씁니다.
...
#### 왜 sign-extend 가 아니라 zero-extend 인가 — partial-register stall 회피

여기서 한 걸음 더 — _왜 하필 zero-extend_ 일까요? 부호 확장(sign-extend)이 아니라 0 확장을 택한 데는 마이크로아키텍처적 이점이 있습니다. 핵심은 **W 쓰기가 X 의 _전체 64-bit 를 명확히 정의_** 한다는 점입니다. 상위 32-bit 가 항상 0 으로 _확정_ 되므로, W 결과를 곧이어 X 로 읽어도 상위가 "이전 X 값의 잔재"인지 헷갈릴 여지가 없습니다 — 모든 32-bit 연산 결과가 _유일하게 결정된_ 64-bit 값이 됩니다.

이것이 막아 주는 것이 x86 류 ISA 에서 악명 높은 **partial-register stall** 입니다. x86 의 일부 부분 레지스터 쓰기(예: `AL`/`AX` 만 갱신)는 나머지 비트를 _이전 값으로 보존_ 하므로, 그 레지스터를 전체 폭으로 읽을 때 "방금 쓴 부분 + 옛날에 쓴 나머지"를 _합쳐야_ 합니다. OoO 코어에서는 이 합치기가 두 producer(부분 쓰기와 그 이전 전체 쓰기)에 대한 추가 의존을 만들어, 그 둘을 merge 하는 동안 파이프라인이 멈추는 stall 을 유발합니다. AArch64 의 zero-extend 규칙은 W 쓰기가 상위 비트를 _이전 값에 의존하지 않고_ 0 으로 덮으므로, 32-bit 연산 결과를 64-bit 데이터패스에서 _옛 상위 비트와 merge 할 필요 없이_ 처리합니다 — partial-register 의존 자체가 생기지 않아 stall 이 원천 제거됩니다. 즉 "일관성 확보"라는 결과 뒤에는 _부분 레지스터 merge 의존을 없애 OoO 효율을 지킨다_ 는 인과가 있습니다.
...
#### SP 16-byte 정렬은 하드웨어가 강제한다

SP 가 EL별로 banked 된다는 것과 별개로, AArch64 는 **SP 를 메모리 주소로 사용할 때 16-byte 정렬을 하드웨어로 강제**할 수 있습니다. `SCTLR_ELx` 의 SP-alignment-check 비트가 켜지면, SP 를 base 로 한 load/store(또는 SP 가 정렬되어야 하는 시점)에서 SP 하위 4비트가 0 이 아니면 **stack alignment fault** 로 trap 합니다. 이것은 소프트웨어 관례가 아니라 _아키텍처가 검사하는 규칙_ 입니다.

왜 강제하나. 16-byte 정렬은 단순히 미관 문제가 아니라 _**ABI**(Application Binary Interface — 컴파일된 코드끼리 함수 호출·레지스터·스택을 맞추기로 한 이진 수준 약속) 와 데이터패스의 전제_ 입니다 — `STP`/`LDP` 가 16-byte 쌍을 한 번에 옮기고, 함수 호출 규약(AAPCS64)이 호출 경계에서 SP 16-byte 정렬을 요구하며(M08), 정렬된 접근은 캐시 라인을 가로지르지 않아 빠릅니다. 정렬을 _소프트웨어 신뢰_ 에만 맡기면 스택을 망가뜨린 버그가 _조용히_ 잘못된 데이터로 번지지만, 하드웨어 검사가 있으면 misaligned SP 가 만들어지는 _그 순간_ 에 fault 로 잡혀 버그 지점이 명확해집니다. 그래서 M08 에서 프레임 크기를 항상 16의 배수(`#-32` 등)로 잡는 것은 _이 하드웨어 강제와 ABI 를 동시에 만족_ 시키기 위함입니다 — 검증에서 stack alignment fault 가 나면 프레임 크기가 16배수인지부터 봐야 합니다.
...
#### 시스템 레지스터는 왜 5개 필드로 인코딩되나 — 확장 가능한 이름 공간

GPR 은 `x0`~`x30` 처럼 5비트 한 필드로 번호를 매기지만, 시스템 레지스터는 그렇게 못 합니다 — 종류가 수백 개에 달하고 ISA 버전마다 늘기 때문입니다. 그래서 AArch64 는 시스템 레지스터를 **op0 / op1 / CRn / CRm / op2** 라는 _다섯 개의 인코딩 필드_ 조합으로 지정합니다. `MRS x0, SCTLR_EL1` 같은 어셈블리의 이름은 사람이 읽기 위한 별칭이고, 실제 인코딩은 이 5필드 좌표입니다.

이 다차원 인코딩의 의미는 **확장성**입니다. 5개 필드가 만드는 좌표 공간은 매우 넓어, ARM 이 새 아키텍처 버전(v8.1, v8.2, … v9)에서 _새 기능을 위한 시스템 레지스터를 추가_ 할 때 이 공간의 빈 좌표를 할당하면 됩니다 — 기존 인코딩과 충돌 없이, 디코더 구조를 바꾸지 않고 새 sysreg 가 끼어듭니다(M01 에서 본 LSE/PAuth/MTE 등은 모두 이렇게 새 시스템 레지스터를 동반). 또 어떤 코어가 그 기능을 미구현이면 해당 좌표 접근이 undefined 로 trap 되어, _기능 유무를 인코딩 좌표 단위로_ 게이팅할 수 있습니다. 즉 5필드 인코딩은 "시스템 레지스터를 끝없이 추가할 수 있는 확장 가능한 이름 공간"을 만든 설계이며, 그래서 같은 ARM 이라도 ISA 버전이 오를수록 sysreg 가 계속 늘어나는 것입니다(M05/M06 에서 MMU·GIC 의 다양한 `*_EL1` 레지스터로 다시 만납니다).
