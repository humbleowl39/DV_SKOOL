...
> **이 글의 핵심 용어** — **TLP**(Transaction Layer Packet — 읽기/쓰기 같은 요청을 담아 주고받는 패킷), **header**(패킷 앞부분의 제어 정보 — 종류·주소·길이 등), **payload**(실제 데이터 본문), **DW**(DoubleWord — 4 byte 단위; "3DW header" = 12바이트짜리 헤더), **Tag**(요청마다 붙이는 번호 — 응답을 그 요청에 짝지우는 열쇠), **Completion**(요청에 대한 응답 패킷, 약자 Cpl; 데이터를 실으면 CplD), **outstanding**(보냈지만 아직 응답을 못 받아 떠 있는 요청), **out-of-order**(보낸 순서와 다른 순서로 응답이 도착하는 것).
...
| **TC** (Traffic Class — 트래픽 우선순위 등급) | 3 | 0..7. VC (Virtual Channel — 한 물리 link 위의 독립 buffer 를 가진 가상 채널) 매핑에 사용 |
...
| **Last DW BE** | 4 | 마지막 DW 의 byte enable (BE = Byte Enable — 한 DW 안에서 어떤 byte 가 유효한지 1비트씩 표시) |
