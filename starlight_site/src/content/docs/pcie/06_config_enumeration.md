---
title: "Module 06 — Configuration Space & Enumeration"
---
...
> **이 글의 핵심 용어** — **Configuration Space**(각 device 가 가진 4 KB 설정 영역 — 이름·자원·기능을 SW 에 알리는 창구), **enumeration**(OS/firmware 가 트리를 훑어 device 를 발견하고 자원을 나눠 주는 부팅 초기 과정), **DFS**(Depth-First Search — 트리를 가지 끝까지 파고든 뒤 되돌아오는 순회 방식), **BDF**(Bus/Device/Function — 트리 안의 한 기능을 가리키는 주소, 예 `01:00.0`), **Vendor ID/Device ID**(제조사·제품 식별 번호 — driver 가 이걸로 짝을 찾음), **BAR**(Base Address Register — device 가 자기 MMIO 영역의 주소·크기를 알리는 레지스터), **Capability list**(지원 기능을 줄줄이 엮어 둔 목록), **ECAM**(Enhanced Configuration Access Mechanism — Configuration Space 를 메모리 주소에 매핑해 load/store 로 접근하는 방식), **MSI/MSI-X**(메모리 쓰기로 전달하는 인터럽트 방식), **`lspci`**(Linux 에서 PCIe device 목록을 보는 명령).
...
```c
// BAR sizing 의 pseudo code — 하드웨어 협조 기반
...
:::note[ECAM 주소 비트 폭은 왜 그 폭인가 — BDF 8/5/3 의 PCI 호환 뿌리와 ARI]
ECAM 주소식의 shift 값 — Bus `<<20`, Device `<<15`, Function `<<12` — 은 임의가 아니라 **BDF 의 비트 폭** 에서 곧장 나옵니다. Function 은 4 KB(=2^12) Configuration Space 한 칸을 가리키므로 `<<12`, Function 필드가 **3-bit**(8 functions)라 그 위 Device 는 `<<(12+3)=<<15`, Device 필드가 **5-bit**(32 devices)라 그 위 Bus 는 `<<(15+5)=<<20`, Bus 필드가 **8-bit**(256 buses)입니다. 즉 256 bus × 32 device × 8 function 이라는 구조는 BDF = **8/5/3 bit** 분할의 직접적 결과입니다.

그럼 왜 하필 8/5/3 일까요? 이것은 PCIe 가 *원래 PCI* 의 주소 모델을 **SW backward compatibility** 를 위해 그대로 물려받았기 때문입니다. legacy PCI 시절의 Configuration 메커니즘(CF8/CFC)이 Bus 8-bit·Device 5-bit·Function 3-bit 로 정의되어 있었고, PCIe 는 기존 OS·드라이버가 손대지 않고도 동작하도록 이 분할을 유지했습니다. 즉 8/5/3 은 기술적 최적이라기보다 *역사적 호환의 산물* 입니다.

이 호환성이 곧 한계가 됩니다 — Function 이 3-bit 라 한 device 가 최대 8 function 뿐입니다. SR-IOV 로 수백 개의 Virtual Function 을 만들려면 8 개로는 턱없이 부족합니다. 그래서 **ARI(Alternative Routing-ID Interpretation, §5.8)** 가 등장합니다 — ARI 는 (Device 5-bit + Function 3-bit)의 8-bit 를 *쪼개지 않고* 통째로 **8-bit Function 번호** 로 재해석해, 한 endpoint 가 256 function 을 가질 수 있게 합니다. 즉 ARI 는 "8/5/3 이라는 PCI 호환 분할이 SR-IOV 시대에 모자라서" 나온 직접적 후속 조치입니다 — ECAM 비트 폭의 역사적 제약과 ARI 는 한 인과 사슬로 이어집니다.
:::
...
| **Interrupt Pin / Line** | Legacy INTx (옛 PCI 의 전용 인터럽트 핀 방식) — modern 은 MSI/MSI-X 로 대체 |
...
여기서 **Prefetchable**(미리 읽어 와도 부작용이 없는 메모리 — read 가 상태를 바꾸지 않으므로 캐싱·prefetch 가 안전한 영역)은 보통 frame buffer 같은 순수 메모리에 붙고, read 자체가 동작을 유발하는 제어 레지스터에는 붙지 않습니다.

→ 64-bit BAR 는 **두 개의 BAR slot 차지** (BAR0+BAR1, BAR2+BAR3, BAR4+BAR5).

:::tip[BAR sizing 의 마법 — 하드웨어가 협력하는 절묘한 트릭]
...
:::note[CRS가 왜 필요한가 — link-up과 device-ready의 시간차]
CRS 의 존재 이유는 한 문장으로 압축됩니다 — **link 가 올라온 시점과 device 가 config 요청에 답할 준비가 된 시점이 다르기 때문** 입니다.

LTSSM 이 L0 에 도달했다는 것은 *물리·링크 계층* 이 통신 가능해졌다는 뜻일 뿐, device *내부* 가 완전히 초기화됐다는 보장이 아닙니다. 많은 device 는 link-up 후에도 firmware 를 ROM 에서 로드하거나, 내부 controller·메모리·sub-system 을 부팅하는 데 추가 시간이 걸립니다(특히 NVMe·복잡한 가속기). 그런데 OS 의 enumeration 은 link 가 올라오자마자 Vendor ID 를 읽으려 CfgRd 를 보냅니다 — 이 요청이 device 의 internal init *완료 전* 에 도착할 수 있습니다.

이 시간차에서 device 가 택할 수 있는 나쁜 선택지는 두 가지입니다: (1) 아무 응답도 안 해서 OS 가 timeout 으로 "device 없음" 판정, (2) 준비 안 된 상태의 쓰레기 값을 반환해 OS 가 오인. CRS 는 *제3의 정직한 응답* 입니다 — "나 여기 있는데, 아직 준비 중이니 잠시 후 다시 물어봐 줘". OS 는 CRS 를 받으면 0xFFFF(없음)로 오해하지 않고 backoff 후 재시도하며, device 가 init 을 끝내면 정상 CplD 를 받습니다. 즉 CRS 는 "느리게 깨어나는 device" 와 "곧장 묻는 OS" 사이의 시간차를 메우는 핸드셰이크입니다. 그래서 CRS retry 로직이 부실하거나 CRS 를 0xFFFF 로 오해하는 OS/firmware 가 boot hang 의 단골 원인이 됩니다.
:::
...
:::tip[🤔 Q1 — BAR sizing protocol (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — CRS handling (Bloom: Evaluate)]
...
→ [Module 07 — Power, AER, Hot Plug](../07_power_aer_hotplug/): Configuration Space 위에서 동작하는 PM / AER / Hot Plug Capability 의 운영 메커니즘.

[퀴즈 풀어보기 →](../quiz/06_config_enumeration_quiz/)
