...
> **이 글의 핵심 용어** — **SR-IOV**(Single Root I/O Virtualization — 한 물리 device 를 여러 가상 device 로 쪼개 VM 들이 나눠 쓰게 하는 표준), **PF/VF**(Physical/Virtual Function — 관리용 본체 function 과 거기서 복제된 경량 function), **VM**(Virtual Machine — 한 물리 머신 위에서 도는 가상 컴퓨터), **IOMMU**(I/O Memory Management Unit — device 의 DMA 주소를 변환·격리하는 하드웨어), **DMA**(Direct Memory Access — CPU 를 거치지 않고 device 가 메모리를 직접 읽고 쓰는 것), **ATS**(Address Translation Service — device 가 주소 변환 결과를 자기 캐시(ATC)에 보관해 IOMMU 조회를 줄이는 기능), **PASID**(Process Address Space ID — 한 device 안에서 여러 프로세스 주소공간을 구분하는 식별자), **P2P**(Peer-to-Peer — device 끼리 host 를 거치지 않고 직접 주고받는 것), **ACS**(Access Control Services — P2P 같은 경로를 허용/차단하는 보안 정책), **CXL**(Compute Express Link — PCIe 의 물리 계층 위에 캐시 일관성·메모리 확장을 더한 프로토콜), **utilization**(자원이 실제로 쓰인 비율).
...
PASID 덕분에 하나의 VF 가 서로 다른 VM 의 메모리를 격리된 상태로 동시에 DMA 할 수 있습니다. PASID 는 **SVM (Shared Virtual Memory)** 의 기반이기도 합니다 — accelerator 가 CPU 의 가상 주소 공간을 그대로 참조하는 시나리오에서 PASID 가 없으면 프로세스별 메모리 격리가 무너집니다. 단, PASID 는 별도 Extended Capability (0x0023) 로 device 와 IOMMU 양쪽이 모두 지원해야 하며, 한쪽이라도 빠지면 silent failure 로 이어집니다.
