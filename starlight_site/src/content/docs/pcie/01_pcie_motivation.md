...
> **이 글에 자주 나오는 용어 미리보기** — **PCIe**(PCI Express; CPU 와 GPU·SSD·네트워크 카드 같은 주변장치를 잇는 고속 직렬 인터커넥트 표준), **PCI**(PCIe 이전 세대의 병렬 버스 — 여러 장치가 한 배선을 공유), **lane**(송·수신 differential pair 한 쌍으로 이뤄진 PCIe 의 기본 전송 단위 — `x4`/`x16` 의 그 lane), **link**(양 끝 장치 사이를 잇는 lane 묶음 전체), **skew**(여러 배선의 신호 도착 시각 차이 — 클수록 데이터가 어긋남), **bus**(여러 장치가 데이터를 주고받는 공용 배선), **GT/s**(Giga-Transfers per second — lane 한 개가 1초에 보내는 심볼 전송 횟수, PCIe 세대별 속도 단위).
...
PCIe 의 발상은 lane 을 독립시키는 것이었습니다. 각 lane 은 별도 differential pair(두 가닥의 차이로 신호를 표현해 잡음에 강한 한 쌍의 배선)로 자체 clock 을 데이터 안에 임베드하고, lane 간 skew 는 packet 이 lane 끝에서 재조립될 때 처리되므로 문제가 되지 않습니다. N 개의 lane 은 단순히 단일 lane bandwidth 의 N 배가 되므로 완전히 scalable 합니다.
...
이후 모든 PCIe 모듈은 한 가정에서 출발합니다 — **"한 lane 의 직렬 전송을 여러 lane 으로 묶어 늘린다, 양 끝은 packet 단위로 layered protocol(여러 계층으로 책임을 나눈 통신 규약) 을 주고받는다"**. **TLP**(Transaction Layer Packet — 읽기/쓰기 같은 실제 요청을 담아 주고받는 패킷)가 왜 가변길이인지, **LTSSM**(Link Training and Status State Machine — link 를 켜고 상태를 관리하는 PHY 의 상태 기계)이 왜 11 state 나 필요한지, **AER**(Advanced Error Reporting — 에러를 표준 형식으로 분류·기록하는 PCIe 기능)의 error class 가 왜 셋으로 갈라져 있는지 — 전부 이 한 가정의 파생입니다.
...
PCI 구조에서는 한 device 만 송신 권한을 얻을 수 있어 나머지는 기다려야 했고, 공유 bus 의 부하가 올라갈수록 전체 속도가 떨어졌으며, 32-line skew 가 주파수 상한을 ~533 MHz 근방으로 묶어두었습니다. PCIe 는 이 구조를 뒤집어 모든 link 를 full-duplex(송신과 수신을 동시에 — 보내면서 동시에 받음) 로 만들고, **Switch**(여러 장치를 갈래로 펼쳐 잇는 PCIe 의 분기 장치)가 fan-out(하나의 link 를 여러 갈래로 나눠 펼치는 것) 을 담당하며, 각 lane 이 독립 **CDR**(Clock-Data Recovery — 수신한 데이터의 변화 지점에서 박자(clock)를 스스로 뽑아내는 회로)로 박자를 추출합니다.

세 가지 PCI 의 한계 — 공유 bus 부하, parallel skew, multi-drop(여러 장치가 한 배선에 매달려 붙는 연결 방식) arbitration(공유 자원을 누가 쓸지 정하는 중재) — 가 PCIe 에서 모두 사라지고, 그 자리를 **lane 독립 SerDes**(Serializer/Deserializer — 병렬 데이터를 직렬 신호로 바꿔 보내고 다시 병렬로 되돌리는 회로) **+ Switch fan-out + packet-switched layered architecture** 가 채웁니다.
...
가장 단순한 시나리오. CPU 가 NVMe device (x4 Gen3 **EP**(Endpoint — PCIe 트리 끝단의 실제 기능 장치, 여기선 SSD)) 에 **64 byte** Memory Write 를 한 번 발행합니다.

아래 흐름에 처음 나오는 약어들을 먼저 풀어 둡니다 — **RC**(Root Complex — CPU 와 PCIe 세계를 잇는 관문), **MMIO**(Memory-Mapped I/O — 장치 레지스터를 메모리 주소에 매핑해 일반 load/store 로 접근하는 방식), **MWr**(Memory Write — 메모리에 쓰는 TLP 종류), **TL**(Transaction Layer — TLP 를 만들고 푸는 최상위 계층), **DLL**(Data Link Layer — 패킷에 순번·검사값을 붙여 link 한 구간의 신뢰성을 책임지는 계층), **PHY**(Physical Layer — 실제 전기 신호로 비트를 보내는 최하위 계층), **Seq#**(Sequence Number — TLP 마다 붙이는 순번), **LCRC**(Link CRC — TLP 가 한 구간을 건너는 동안 깨지지 않았는지 확인하는 검사값), **Replay Buffer**(상대의 ACK 를 받기 전까지 보낸 TLP 를 보관했다가, 손상 신호가 오면 다시 보내려고 쥐고 있는 버퍼), **ACK/NAK DLLP**(잘 받았다/다시 보내라를 알리는 link 전용 짧은 패킷), **BAR**(Base Address Register — 장치가 자기 MMIO 영역의 주소·크기를 알리는 레지스터).
...
위 ④의 PHY 동작에 처음 나오는 용어를 풀면: **STP framing**(패킷 앞뒤에 시작·끝 표시를 붙여 경계를 알리는 것), **128b/130b encoding**(128비트마다 2비트짜리 표식을 덧붙여 보내는 부호화 — Gen3 이후), **scrambling**(데이터를 의사난수와 섞어 같은 패턴이 반복되지 않게 흩는 것), **byte stripe**(바이트를 여러 lane 에 번갈아 나눠 싣는 것), **retire**(보관하던 항목을 더 이상 쥐고 있을 필요가 없어 버퍼에서 비우는 것)입니다.
...
여기서 **BDF**(Bus/Device/Function — 트리 안의 한 기능을 가리키는 주소, 예: `01:00.0`)는 PCIe 장치를 식별하는 번지수입니다. **MRd/MWr**(Memory Read/Write), **IORd/IOWr**(I/O Read/Write), **Cfg**(Configuration access — 장치 설정 영역 접근), **Cpl**(Completion — 요청에 대한 응답), **Msg**(Message — 이벤트·인터럽트 등을 전하는 TLP)는 모두 TLP 의 종류입니다. 자세한 건 Module 03.
...
표에 나오는 부호화·신호 방식 용어: **8b/10b**(원본 8비트를 10비트로 부풀려 전기적 균형을 맞추는 초기 부호화 — 효율 80%), **128b/130b**(128비트마다 2비트만 덧붙이는 효율 높은 부호화 — 98.5%), **NRZ**(Non-Return-to-Zero — 한 심볼에 1비트를 싣는 기본 방식), **PAM4**(한 심볼에 2비트를 싣는 4-레벨 변조 — Gen6 부터), **EQ**(Equalization — 배선을 지나며 뭉개진 신호를 송·수신에서 보정하는 것), **FLIT**(Flow control unIT — Gen6 부터 고정 크기 프레임으로 묶어 보내는 단위).
