---
title: "Module 05 — Physical Layer & LTSSM"
---
...
LTSSM: "LTSSM" {
  ...
  Recovery: "Recovery\nEQ retrain · Gen change"
  TestReset: "Test / Reset\nDisabled · Loopback · Hot Reset"
  BringUp -> Steady -> PowerSaving
  BringUp -> Recovery -> TestReset
}

Stack -> LTSSM: { style.opacity: 0.0 }
```
...
PHY 를 하나로 보면 "그냥 bit 를 보내는 layer" 처럼 보이지만, 실제로는 처리하는 시간 스케일이 서로 다른 세 영역이 겹쳐 있습니다. 가장 바깥의 **Analog Front-End** 는 ps~ns 단위로 신호의 전압과 타이밍을 다룹니다 — Tx 가 pre-emphasis 로 channel 의 고주파 손실을 미리 보상하고, Rx 의 CTLE/DFE 가 ISI 를 제거합니다. 그 위의 **PMA** 는 PLL 로 클럭을 만들고 CDR 로 수신 신호에서 박자를 추출하는 ns~μs 수준의 동기화 영역입니다. 맨 위의 **PCS** 는 byte 를 10b 또는 130b 블록으로 인코딩하고 lane 에 분산시키는, 상대적으로 느린 디지털 처리 영역입니다. 이 세 sub-block 이 분리되어 있기 때문에 "link 가 안 올라온다"는 증상을 맞닥뜨렸을 때 신호 수준인지, 동기화 수준인지, 프레이밍 수준인지 좁혀 나갈 수 있습니다.
...
Gen3 부터 link 속도가 8 GT/s 를 넘으면서 channel 의 개인차가 결정적으로 중요해졌습니다. PCB 에서 RC 와 EP 사이의 trace 길이, connector 삽입 손실, 케이블 특성이 보드마다 다르기 때문에, 공장에서 고정된 EQ 설정 하나로 모든 channel 을 커버하기란 불가능합니다. 그래서 Gen3 부터는 **양 끝이 직접 실측하고 협상해서 channel 에 맞는 Tx FFE 를 결정하는 4 Phase EQ** 절차를 Recovery 안에서 실행합니다. Phase 0 에서 사전 정의된 preset 을 출발점으로 삼고, Phase 1 과 2 에서 양방향으로 Tx FFE 계수를 조율한 뒤, Phase 3 에서 합의된 값으로 안정화를 확인합니다. EQ 가 channel 에 맞지 않으면 BER 이 폭증하고, 그 결과로 Recovery 가 빈발하며, 최종적으로 throughput 이 저하됩니다.
...
Detect: "Detect\nReceiver detection (TX short pulse)"
Polling: "Polling\nTS1/TS2 교환 · lane 동기 · 속도 협상"
Configuration: "Configuration\nLink/Lane # · width · speed 결정"
L0: "L0\n정상 동작"
L0s: "L0s\n저전력 (단방향 quiesce)"
L1: "L1\n저전력 (양방향 quiesce)"
L2: "L2\nAux power만 (deep sleep)"
Recovery: "Recovery\nEQ retrain · Gen change"
Disabled: "Disabled"
Loopback: "Loopback"
HotReset: "Hot Reset"
...
Polling -> Configuration: "TS2 교환 완료"
Configuration -> L0
L0 -> L0s: "ASPM L0s"
L0s -> L0: "FTS"
L0 -> L1: "ASPM / PCI-PM"
L0 -> L2: "PCI-PM"
L1 -> Recovery: "Recovery 트리거"
Recovery -> L0: "EQ 성공 / Gen up"
Recovery -> Disabled: "실패"
Recovery -> Loopback: "실패"
Recovery -> HotReset: "실패"
```
...
:::tip[Bit Lock ↔ Symbol Lock — Polling 의 두 단계 박자 맞추기]
**Bit Lock (비트 박자 맞추기)**:
...
보드를 설계하다 보면 라우팅 편의상 lane 의 순서가 뒤집히거나, differential pair 의 극성이 반전되거나, lane 마다 PCB trace 길이가 조금씩 달라 도착 시간에 차이가 생기는 상황이 자주 발생합니다. PCIe 는 이 세 가지 현상을 모두 자동으로 보정합니다. **Lane Reversal** 은 Polling 단계에서 각 lane 의 TS1 패턴을 분석해 순서가 뒤집혔음을 감지하고 내부 매핑을 뒤집어 SW 에는 정상으로 보이게 합니다. **Polarity Inversion** 은 Receiver 가 수신 신호의 극성을 자동으로 감지해 반전 처리합니다. **Lane De-skew** 는 SKP Ordered Set 이 각 lane 의 도착 시간 차이를 lane 간 ~1.5 ns 이내로 보정합니다. 이 자동화 덕분에 보드 라우팅 자유도가 높아지지만, **PCB 의 length matching** 은 Gen5+ 에서 여전히 중요합니다 — 공차 한도를 넘으면 De-skew 로도 보정이 안 됩니다.
...
:::tip[🤔 Q1 — LTSSM stuck (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — Common vs SRIS Refclock (Bloom: Evaluate)]
...
→ [Module 06 — Configuration Space & Enumeration](../06_config_enumeration/): L0 도달 후 OS 가 device 를 발견하고 BAR 를 할당하는 표준 절차.

[퀴즈 풀어보기 →](../quiz/05_phy_ltssm_quiz/)
