---
title: "Module 04 — DLLP, Flow Control, ACK/NAK"
---
...
:::note[infinite credit의 용도와 위험]
무한 credit 의 *용도* 는 "항상 소비되는 트래픽에서 credit 회계로 인한 stall 을 아예 없애는 것" 입니다. 대표가 Completion 입니다 — requester 는 자신이 보낸 NP 의 응답으로만 completion 을 받으므로, 받을 completion 의 양을 이미 스스로 통제하고 있습니다(outstanding NP 수만큼만 옴). 즉 completion 은 "예상 밖으로 쏟아질" 일이 없고 requester 가 받는 즉시 소비합니다. 이런 트래픽에 굳이 credit 을 세어 UpdateFC 를 주고받으면 불필요한 오버헤드와 stall 위험만 생기므로, ∞ 를 광고해 "completion 은 credit 걱정 없이 언제든 보내라" 로 단순화합니다.

*위험* 은 "∞ 라고 광고했지만 실제 buffer 는 유한할 때" 입니다. ∞ 광고는 송신 측에게 "네 마음껏 보내라, 내가 다 받는다" 는 약속입니다. 그런데 receiver 의 실제 completion buffer 가 무한일 수는 없으므로, receiver 는 *자신이 outstanding 시킨 NP 수를 스스로 제한* 해서 "돌아올 completion 총량이 내 buffer 를 넘지 않도록" 보장해야 합니다(Module 03 의 Tag/completion buffer 사슬과 연결). 이 자기 규율이 깨지면 — 즉 buffer 가 감당 못 할 만큼 completion 이 몰리면 — overflow 가 나고, completion 은 흘릴 수도 재전송할 수도 없는 트래픽이라 이는 보통 **Fatal** 급 에러로 이어집니다. 정리하면 ∞ credit 은 "흐름 제어 stall 제거" 라는 이득과 "광고대로 정말 받을 수 있어야 한다(아니면 overflow=Fatal)" 라는 책임이 맞바꿔진 선택입니다.
:::
...
:::note[credit을 왜 절대(누적) 카운터로 보내나 — DLLP 유실 내성]
UpdateFC 가 "이번에 credit N 개 더 풀었다" 는 *증분(delta)* 이 아니라 "지금까지 누적 처리한 총합(modulo)" 이라는 *절대값(absolute/cumulative)* 을 보낸다는 점이 핵심입니다. 이 설계가 robustness 를 만듭니다.

DLLP 는 reliability 보호(ACK/NAK·replay)를 받지 않습니다 — UpdateFC DLLP 한 개가 PHY 에서 깨지면 그냥 *사라집니다*. 만약 credit 을 증분으로 보냈다면, 유실된 그 한 개의 "+N" 이 영영 반영되지 않아 sender 의 credit 회계가 receiver 와 *영구히 어긋납니다* — 갈수록 credit 이 모자란 것으로 착각해 결국 stall 합니다. 반면 절대 누적값을 보내면, 유실은 *자동 복구* 됩니다. 다음 UpdateFC 가 "그동안 처리한 새 총합" 을 다시 실어 오므로, sender 는 직전 값을 못 받았더라도 최신 총합 하나만 받으면 정확한 credit 상태로 동기화됩니다. 한 DLLP 의 유실이 *일시적 갱신 지연* 으로 끝날 뿐, 누적 오류로 굳지 않습니다.

즉 절대값 인코딩은 "신뢰성 없는 채널(DLLP)로 흐름 제어 정보를 나르되, 메시지가 가끔 사라져도 다음 메시지가 자동으로 메꾼다" 는 self-healing 메커니즘입니다. 그래서 UpdateFC 는 안심하고 가벼운(보호 없는) DLLP 로 보낼 수 있습니다.
:::
...
:::tip[🤔 Q1 — FC credit 계산 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Replay buffer overflow (Bloom: Analyze)]
...
→ [Module 05 — Physical Layer & LTSSM](../05_phy_ltssm/): DL_Inactive 로 빠진 link 가 어떻게 LTSSM Recovery 와 EQ 로 회복되는지. PHY 의 11 state 와 4 phase EQ.

[퀴즈 풀어보기 →](../quiz/04_dllp_flow_control_quiz/)
