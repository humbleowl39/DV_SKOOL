---
title: "Module 05 — Attack Surface & Defense"
---
...
#### DPA 가 _통계_ 로 키 비트를 추출하는 원리

SPA는 위 그림처럼 _한 번의_ 파형에서 곱셈/제곱 패턴을 눈으로 읽습니다. 하지만 방어가 들어가 한 트레이스로는 안 보일 때, DPA는 **수천 개의 트레이스를 통계로 갈라** 비밀을 끄집어냅니다. 절차는 이렇습니다.

1. **수천 회 측정**: 서로 다른 입력으로 crypto를 반복 실행하며 각 실행의 전력 트레이스를 수천 개 모읍니다.
2. **키 비트 가설 세우기**: 공격자가 "키의 이 비트(또는 바이트)는 X일 것"이라고 _가설_ 을 세우고, 그 가설이 맞다면 특정 중간값이 0인지 1인지를 입력별로 예측합니다.
3. **가설로 트레이스 분류·평균**: 그 예측에 따라 트레이스를 두 그룹(예측 중간값=0 / =1)으로 나눠 각각 평균을 냅니다.
4. **차이(또는 상관) 검사**: 가설이 _맞으면_ 두 그룹 평균 사이에 그 중간값이 처리되는 시점에 _유의한 전력 차이_ 가 나타납니다(올바른 분류이므로 실제 물리와 정렬). 가설이 _틀리면_ 분류가 무작위라 차이가 잡음 수준으로 묻힙니다. 차이(또는 가설 예측값과 측정 전력의 _상관계수_, CPA)가 가장 큰 가설이 정답 키 비트입니다.

핵심은 "한 트레이스의 작은 신호"가 노이즈에 묻혀도, _올바른 가설로 정렬해 수천 개를 평균_ 하면 신호는 쌓이고 노이즈는 상쇄돼 드러난다는 점입니다(평균화 이득). 그래서 더미 연산 같은 약한 균형화는 통하지 않고, 아래 masking이 필요합니다.

#### masking 이 부채널을 막는 원리 — 비밀의 random split

전력 균형화가 "신호를 가린다"면, masking은 _측정 전력과 실제 비밀의 통계적 상관 자체를 끊습니다_. 방법: 매 실행마다 새 난수 마스크 `m`을 뽑아, 비밀이 얹힌 중간값 `v`를 두(이상) 개의 share로 _쪼갭니다_.

```
  v  =  v_masked  XOR  m       (또는 산술 분할)
  계산을 v 자체가 아니라 (v_masked, m) 두 share로 따로 수행
  → 어느 share 하나의 전력도 v 와 무관(마스크 m 이 매번 무작위라 v_masked 는 균등 분포처럼 보임)
```

이렇게 하면 회로의 어느 한 지점에서 측정한 전력은 `v_masked`(또는 `m`)에만 의존하고, `m`이 매 실행 무작위라 그 측정값은 _진짜 비밀 `v`와 통계적으로 무상관_ 이 됩니다. DPA가 위 4단계에서 `v`에 대한 가설로 분류해도, 측정 전력이 `v`와 상관이 없으니 두 그룹 평균 차이가 안 생깁니다 — 즉 평균화 이득이 무력화됩니다. (1차 masking은 두 share를 동시에 보는 고차 DPA로 깰 수 있어, 강한 구현은 share 수를 늘립니다.)
...
#### SRAM Lock 이 TOCTOU 를 닫는 _회로적 지점_

"검증 후 영역을 쓰기 보호"가 실제로 무엇을 내리는지가 핵심입니다. SRAM lock은 SW가 마음만 먹어도 못 우회하도록, **버스 패브릭/메모리 컨트롤러 레벨의 write 권한 비트** 를 내립니다. 검증이 끝나면 BootROM이 lock 레지스터를 set하고, 그 비트가 해당 SRAM 영역으로 가는 _write 트랜잭션을 게이팅_ 합니다 — 이후 어느 마스터(CPU든, 특히 공격자가 모는 **DMA 엔진**이든)가 그 주소 범위에 write를 발행하면, 패브릭(또는 메모리 보호 유닛)이 주소·마스터를 검사해 **write를 거부**(error response)합니다. 즉 "검증된 image가 든 SRAM 영역"이 jump 시점까지 _물리적으로 read-only_ 가 되어, check와 use 사이의 race window가 닫힙니다. 중요한 건 이것이 SW 플래그가 아니라 _트랜잭션 경로 위의 권한 비트_ 라, 검증을 통과한 코드조차(또는 별개 DMA 마스터가) 그 영역을 못 덮어쓴다는 점입니다. 그래서 검증에서는 "lock set 이후 DMA write → DECERR/거부되는가"와 "lock이 jump _이전_ 에 걸리는가(타이밍)"를 음성 케이스로 자극합니다.
...
자세한 Active Driver 아키텍처는 [Module 07 (DV 방법론)](../07_bootrom_dv_methodology/) 에서.
...
:::caution[실무 주의점 — JTAG 가 production 출하 시 disable 되지 않음]
...
:::tip[🤔 Q1 — 글리치 surface 식별 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — TOCTOU 본질 (Bloom: Analyze)]
...
- 📝 [**Module 05 퀴즈**](../quiz/05_attack_surface_and_defense_quiz/)
- ➡️ [**Module 06 — Quick Reference Card**](../06_quick_reference_card/): 위 다섯 모듈을 한 페이지 치트시트로 응축.
