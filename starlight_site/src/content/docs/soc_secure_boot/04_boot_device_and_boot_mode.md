...
OTP 는 신뢰의 최상위 anchor 이지만 용량이 수 KB 수준으로 제한되어, 수십 개 이상의 Anti-Rollback 카운터나 대용량 인증서를 담기에는 부족합니다. RPMB 는 이 한계를 보완하기 위해 UFS/eMMC 컨트롤러 안에 내장된 특수 보안 파티션으로, 일반 storage 영역과 달리 HMAC 인증 + Write Counter 기반의 재전송 방지 메커니즘을 하드웨어 수준에서 제공합니다. 즉, 공격자가 과거의 쓰기 요청을 그대로 재전송해도 컨트롤러가 Counter 불일치를 즉시 탐지해 거부합니다.
