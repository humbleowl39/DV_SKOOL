---
title: "Module 04 — Boot Device & Boot Mode"
---
...
:::
:::note[사전 지식]
- [Module 01-03](../01_hardware_root_of_trust/) — RoT, chain, crypto
...
(위 표의 **Boot LU**/**LUN** = Logical Unit Number — 하나의 저장 장치를 여러 논리 영역으로 나눈 그 번호; UFS 는 부팅용 데이터를 별도의 Boot LU 에 둔다.) 장치별 초기화 복잡도의 차이는 단순히 명령어 수가 아니라 프로토콜 계층 수에서 비롯됩니다. SPI NOR 는 마스터가 명령 한 줄을 보내면 데이터가 바로 나오는 구조라 초기화가 수십 줄로 끝나는 반면, UFS 는 물리 계층(M-PHY) 부터 링크 계층(UniPro), 전송 계층(SCSI) 까지 세 계층을 순서대로 협상해야 데이터에 접근할 수 있습니다. eMMC 는 그 중간 — CMD 시퀀스로 카드를 선택하고 상태를 확인하는 과정이 필요하지만 UFS 보다 단순합니다. 모든 장치 초기화가 끝난 뒤의 서명 검증 단계는 장치 종류와 무관하게 완전히 동일합니다.
...
OTP 는 신뢰의 최상위 anchor 이지만 용량이 수 KB 수준으로 제한되어, 수십 개 이상의 Anti-Rollback 카운터나 대용량 인증서를 담기에는 부족합니다. RPMB 는 이 한계를 보완하기 위해 UFS/eMMC 컨트롤러 안에 내장된 특수 보안 파티션으로, 일반 storage 영역과 달리 **HMAC**(Hash-based Message Authentication Code — 비밀 키와 해시로 만드는 인증 태그, 데이터 변조 여부와 키 보유자가 보냈음을 동시에 보증) 인증 + Write Counter 기반의 재전송 방지 메커니즘을 하드웨어 수준에서 제공합니다. 즉, 공격자가 과거의 쓰기 요청을 그대로 재전송해도 컨트롤러가 Counter 불일치를 즉시 탐지해 거부합니다.
...
아래 표의 **VIP**(Verification IP — UFS/eMMC 같은 표준 프로토콜의 동작과 규칙 위반 검사를 미리 구현해 둔 재사용 검증 컴포넌트)는 프로토콜 준수 여부를 자동으로 점검합니다.
...
**실제**: RPMB 는 OTP 의 _보조_. RPMB key 자체는 HW unique key 에서 **KDF**(Key Derivation Function — 하나의 기준 비밀에서 용도별 키를 안전하게 파생하는 함수) 또는 OTP 의 RPMB key field — 즉 RPMB 의 root 는 여전히 HW. OTP 가 메이저 버전, RPMB 가 마이너 카운터의 _이중 구조_ 가 일반적.<br>
...
:::caution[실무 주의점 — Secondary boot 경로의 검증 누락 (test mode pin 우회)]
...
:::tip[🤔 Q1 — OTP > Pinstrap 우선순위 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — RPMB vs OTP counter (Bloom: Evaluate)]
...
- 📝 [**Module 04 퀴즈**](../quiz/04_boot_device_and_boot_mode_quiz/)
- ➡️ [**Module 05 — Attack Surface & Defense**](../05_attack_surface_and_defense/): 위에서 본 모든 path 를 _공격자 관점_ 으로 다시 — FI, side-channel, TOCTOU, JTAG.
