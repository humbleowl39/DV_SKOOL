---
title: "Module 01 — Hardware Root of Trust"
---
...
PROV -> BOOT: "chip 출하 후" { style.stroke-dash: 4 }
```
...
#### constant-time memcmp 가 timing leak 을 막는 원리

⑩에서 "_byte-wise constant-time_ 비교"라고만 적었는데, 왜 평범한 `memcmp`를 쓰면 안 되는지를 봅니다. 보통의 `memcmp`는 **첫 불일치 바이트에서 즉시 return**(early-out)합니다. 그러면 비교에 걸린 _시간이 일치한 prefix 길이에 비례_ 합니다 — 첫 바이트부터 틀리면 빨리 끝나고, 앞 31바이트가 맞으면 오래 걸립니다. 공격자는 후보 hash를 한 바이트씩 바꿔 가며 _응답이 느려지는 순간_ 을 측정해 "여기까지는 맞았다"를 알아내고, 바이트 단위로 정답을 좁힙니다(timing side-channel). constant-time 비교는 early-return을 _없애고_ 모든 바이트를 끝까지 누적합니다.

```c
// constant-time: 분기 없이 전체를 XOR 누적 — 시간이 입력값에 무관
int constant_time_memcmp(const uint8_t *a, const uint8_t *b, size_t n) {
    uint8_t diff = 0;
    for (size_t i = 0; i < n; i++)
        diff |= a[i] ^ b[i];   // 불일치가 하나라도 있으면 diff != 0, 그러나 루프는 항상 n번
    return diff;               // 0 == 완전 일치 (분기·조기탈출 없음)
}
```

핵심: `^`로 차이를 모아 `|=`로 누적하므로, 일치 prefix 길이와 무관하게 _항상 n바이트_ 를 돈 뒤에야 결과가 나옵니다. 시간이 비밀에 의존하지 않으니 timing으로 정답을 추론할 수 없습니다. (단, 컴파일러가 early-out으로 "최적화"해 버릴 수 있어 §7 실무 주의점처럼 어셈블리 확인이 필요합니다.)

#### glitch 이중 검증이 단일 글리치를 막는 정량 근거

(2)에서 "두 번 검증"이라 했는데, _왜_ 두 번이면 글리치에 강한지의 정량 직관입니다. 전압/클럭 글리치는 보통 _특정 한 순간(한 cycle 부근)의 단일 분기_ 만 교란합니다 — 정밀하게 타이밍을 맞춰 그 한 번의 `if (mismatch) halt`를 건너뛰게 만드는 것이 단일 글리치 공격입니다. 검증을 _시간적으로 분리된_ 두 지점에서 독립으로 수행하면, 공격자는 _같은 펄스로 두 분기를 동시에_ 뒤집어야 합니다. 두 검증 시점이 충분히 떨어져 있으면 한 글리치 펄스가 양쪽 모두에 정확히 작용할 확률은 각각을 뒤집을 확률의 _곱_ 으로 떨어집니다(예: 각 분기 성공 확률 p라면 둘 동시 ≈ p²이고, p가 작으므로 p²은 훨씬 더 작음). 즉 "한 펄스 = 한 분기"라는 글리치의 물리적 한계 때문에, 시간 분리된 이중 검증은 단일 글리치로 동시에 뚫기가 곱셈적으로 어려워집니다. 그래서 production BootROM은 각 단계 OK 플래그를 따로 두고 _서로 다른 시점에_ 두 번 확인합니다.
---
...
하나라도 빠지면 trust anchor 성립 불가입니다. BootROM 만 있고 OTP 가 비어 있으면 검증할 기준 — ROTPK hash — 자체가 존재하지 않으므로 서명 검증이 의미를 잃습니다. 반대로 OTP 만 있고 BootROM 이 변경 가능하다면, 공격자가 검증 함수 자체를 nop 으로 패치해 버릴 수 있으므로 OTP 에 올바른 hash 가 있어도 소용이 없습니다. 두 요소는 각각의 약점을 서로가 보완하는 관계입니다.
...
POR: "Power-On Reset"
S1: "1. CPU/보안 HW 초기화"
S2: "2. Boot Mode 결정 (OTP + Pinstrap)"
S3: "3. BL2 이미지 SRAM 로드"
S4: "4. BL2 서명 검증 (ROTPK → cert → image)"
S5: "5. BL2 Jump"
POR -> S1
S1 -> S2
S2 -> S3
...
ROM Patch 는 강력한 수단이지만, 그 자체가 새로운 공격 표면이 되지 않도록 네 가지 요건을 동시에 만족해야 합니다. 첫째로 패치 테이블 자체가 서명 검증의 대상이어야 합니다 — 패치가 ROTPK 체인으로 서명되지 않으면 공격자가 임의 코드를 패치 슬롯에 주입할 수 있습니다. 둘째로 패치 슬롯은 보통 8~16개로 유한하므로, 슬롯이 소진되면 더 이상 추가 패치가 불가능합니다 — 이것이 pre-silicon 검증을 철저히 해야 하는 경제적 근거이기도 합니다. 셋째로 패치 코드가 저장되는 Secure SRAM 의 예약 영역이 수 KB 로 제한되므로, 하나의 패치로 수정할 수 있는 로직의 크기에도 상한이 있습니다. 넷째로 패치에도 버전 관리가 필요합니다 — 이전 버전 패치로 롤백되면 이미 수정된 취약점이 재활성화될 수 있으므로 Anti-Rollback 검증을 패치 로딩 경로에도 적용해야 합니다.
...
**SRAM PUF 가 고유 비트를 만드는 물리 원리.** "전원 인가 시 셀 초기값이 칩마다 다르다"의 회로적 원인은 SRAM 셀의 **cross-coupled 인버터 쌍** 에 있습니다. SRAM 비트 셀은 두 인버터가 서로의 출력을 입력으로 받는 _쌍안정(bistable)_ 구조라, 안정 상태가 0과 1 두 개뿐입니다. 전원이 0V에서 올라오는 순간, 셀은 0도 1도 아닌 준안정점에서 출발해 _어느 한쪽으로 쓰러지는데_, 어느 쪽으로 쓰러질지는 두 인버터 트랜지스터의 **미세한 강도 불균형** 이 결정합니다. 제조 공정의 무작위 편차(특히 트랜지스터 문턱전압 `Vth` mismatch, 도핑·선폭 미세 변동)로 한쪽 인버터가 아주 조금 더 빨리/강하게 켜지면 셀은 매번 그쪽으로 쓰러져 _그 셀 고유의 0/1 선호_ 를 보입니다. 이 편차는 (1) 제조 시 무작위로 굳어져 칩마다 다르고, (2) 같은 칩에서는 전원을 켤 때마다 대체로 같은 방향이라, 수천 셀의 power-up 패턴을 모으면 칩의 _지문(fingerprint)_ 이 됩니다. 별도 키를 심지 않아도, 이미 있는 SRAM 어레이의 물성만으로 고유값이 나오므로 추가 회로가 거의 필요 없습니다. 다만 `Vth` mismatch가 작은 경계 셀들은 온도·전압·노화에 따라 가끔 반대로 쓰러져 _노이즈_ 가 생기므로, 아래의 Fuzzy Extractor로 보정해야 안정된 키가 됩니다.
...
**helper data 가 _키를 노출하지 않고_ 오류만 보정하는 원리.** Fuzzy Extractor는 ECC(예: BCH/Reed-Muller 등 블록 코드, 구체 코드는 구현마다 다름 — 확인 필요)를 써서 노이즈 낀 PUF 응답을 매번 같은 키로 안정화합니다. 핵심은 helper data 가 _키도 PUF 원응답도 아닌, 신드롬(코드워드와의 차이)만_ 담는다는 점입니다.

```
[등록(enrollment), 1회]
  R0  = PUF 원응답(노이즈 있는 비트열)
  C   = ECC 코드워드 하나(랜덤 코드워드 또는 키에서 유도)
  helper W = R0 XOR C     ← 공개 저장 (OTP/Flash, 비밀 아님)

[재생(reconstruction), 매 부팅]
  R'  = PUF 재응답(R0 과 몇 비트 다름)
  C'  = R' XOR W = (R0 XOR W) XOR (R0 XOR R') = C XOR (R0 XOR R')
        → C 에 "PUF 노이즈 비트(R0 XOR R')"만큼 오류가 얹힌 값
  ECC decode(C') → C  (노이즈 비트 수가 코드 정정능력 이내면 복원)
  키 = KDF(C)  (또는 C 자체)
```

왜 안전한가: helper data `W = R0 XOR C`는 _마스크(C)로 가려진_ 값이라, 공격자가 `W`를 그대로 읽어도 `R0`(원응답)도 `C`(코드워드)도 단독으로는 알 수 없습니다 — 즉 키 정보가 거의 새지 않습니다. 반면 그 _진짜 칩의_ PUF만이 `R0≈R'`을 재생할 수 있어, helper data와 결합해야만 `C`가 복원됩니다. 그래서 helper data는 공개 저장해도 되고("public helper"), 키는 어디에도 저장하지 않은 채 매 부팅 _재생_ 됩니다. 노이즈 비트 수가 ECC 정정능력을 넘으면 복원 실패(키 불안정)하므로, §6 디버그 체크리스트의 "PUF 응답이 부팅마다 다름 → helper-data CRC / ECC syndrome / temperature"가 바로 이 정정한계를 의심하라는 항목입니다.
...
일반 레지스터 검증과 달리 OTP 검증이 까다로운 데는 구조적인 이유가 있습니다. 먼저 OTP 필드의 물리 주소가 SoC 마다 달라서, 테스트가 특정 주소에 의존하면 포팅 시 전면 수정이 필요합니다. 또한 필드 간 의존성 — 예컨대 Secure Boot 가 꺼져 있으면 ROTPK 검증 자체가 무의미해지는 관계 — 을 모델링하지 않으면 유효하지 않은 조합으로 테스트가 통과해 버립니다. Program-once 특성도 문제입니다: 레지스터는 write 후 read-back 으로 확인하면 되지만 OTP 는 한 번 blow 하면 되돌릴 수 없으므로 시뮬레이션에서 force 로 상태를 설정해야 하고, 그 force 가 silicon 동작을 정확히 반영하는지 별도로 보장해야 합니다. 마지막으로 Boot Mode × Boot Device × Secure Boot × JTAG 를 교차하면 수백 개 조합이 발생하는 조합 폭발이 있어, 단순 열거 방식으로는 coverage 를 체계적으로 닫을 수 없습니다.
...
자세한 내용은 [Module 07 (DV 방법론)](../07_bootrom_dv_methodology/) 에서 다룹니다.
...
:::caution[실무 주의점 — ROTPK 미기록 상태에서 Secure Boot 활성화]
...
:::tip[🤔 Q1 — Hash 저장 이유 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — Lifecycle 비가역성 (Bloom: Evaluate)]
...
- 📝 [**Module 01 퀴즈**](../quiz/01_hardware_root_of_trust_quiz/)
- ➡️ [**Module 02 — Chain of Trust**](../02_chain_of_trust_boot_stages/): trust anchor 위에서 BL1 → BL2 → BL3x → OS 가 신뢰를 어떻게 _전파_ 하는가.
