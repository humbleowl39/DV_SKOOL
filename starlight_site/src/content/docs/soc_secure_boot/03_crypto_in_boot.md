---
title: "Module 03 — Crypto in Boot"
---
...
- [Module 01](../01_hardware_root_of_trust/) — ROTPK / OTP
- [Module 02](../02_chain_of_trust_boot_stages/) — verify-then-execute, cert tree
:::
---
...
(표의 **modexp** = modular exponentiation, 큰 정수의 거듭제곱을 어떤 수로 나눈 나머지를 구하는 연산 — RSA 의 핵심 계산이라 전용 HW 엔진이 크다.) 선택의 결과는 설계 맥락에 따라 갈립니다. BootROM 면적이 제한된 SoC 라면 ECDSA 가 유리하고, 기존 **PKI**(Public Key Infrastructure — 공개키와 그 소유자를 인증서로 묶어 관리하는 키·인증서 발급/검증 체계) 와의 레거시 호환성이 필요하다면 RSA 를 유지해야 합니다. PQC 전환을 미리 준비한다면 두 알고리즘을 모두 지원하는 algorithm agility 구조가 필요합니다.
...
특히 면접/실무에서 가장 자주 나오는 세 가지: (1) "RSA vs ECDSA 어느 쪽?" (2) "PQC 전환 어떻게?" (3) "ROTPK 가 침해되면?" — 이 세 질문은 모두 이 모듈의 도구 이해도를 측정합니다. 도구를 잡으면 Module 04 (boot device) 의 RPMB **HMAC**(Hash-based MAC — 비밀 키와 해시 함수로 만드는 **MAC**(Message Authentication Code, 메시지 인증 코드): 데이터가 변조되지 않았고 _키를 아는 쪽이 보냈음_ 을 동시에 보증), Module 05 (attack surface) 의 side-channel, Module 07 (DV) 의 **DPI-C**(Direct Programming Interface for C — SystemVerilog 검증 환경이 C 함수를 직접 호출하게 해 주는 표준, C 로 작성한 정답 모델을 끌어다 쓸 때 사용) golden reference 가 모두 같은 어휘로 보입니다.
...
서명은 _누가_ 만들었는지만 보장합니다. _최신_ 인지는 별도 메커니즘이 필요합니다.

서명 검증이 완벽하더라도 공격자가 이미 배포된 구버전 image 를 보관했다가 나중에 다시 Flash 에 심으면 어떻게 될까요? v1.0 에서 취약점이 발견돼 v2.0 으로 패치했는데, 공격자가 Flash 를 v1.0 으로 교체하면 BootROM 은 서명이 유효하므로 그냥 통과시킵니다. 결국 이미 수정된 취약점이 다시 살아나는 Rollback 공격이 성립합니다. 이것을 막으려면 서명이 아닌 _시간 축의 단조 증가_ 장치가 필요합니다. OTP Anti-Rollback Counter (ARC) 가 그 역할을 합니다 — OTP 비트를 blown 상태로 누적하여 현재 최소 허용 버전을 기록하고, image 의 버전이 그 값보다 낮으면 서명이 유효하더라도 거부합니다. 새 FW 를 설치할 때마다 추가 비트를 blow 하므로 이전 버전으로의 롤백은 영구히 차단됩니다.
...
**왜 OTP 비트 수가 곧 업데이트 횟수 한계인가 — thermometer(unary) 코딩.** ARC가 일반 2진 카운터가 아니라 _단조 증가만 물리적으로 가능하도록_ thermometer(온도계, unary) 코딩을 쓰기 때문입니다. OTP fuse는 한 번 blow(0→1)하면 _되돌릴 수 없는_ 단방향 소자입니다. 버전을 2진수로 저장하면 어떤 증가는 비트를 1→0으로 내려야 하는데(예: `0111`→`1000`) OTP에서는 불가능합니다. 그래서 ARC는 "버전 N = 하위 N개 비트를 blow"로 인코딩합니다.

```
version 0:  0000 0000 ...   (blown 0개)
version 1:  0000 0001       (1개 blow)
version 2:  0000 0011       (한 개 더 blow — 누적)
version 3:  0000 0111
current min version = blown 비트의 개수(= 가장 높이 채워진 위치)
```

각 업데이트는 _비트 하나를 추가로 blow_ 할 뿐이고, blow는 비가역이라 **감소가 물리적으로 불가능** 합니다 — 누구도(공격자 포함) fuse를 0으로 되돌릴 수 없으니 버전을 낮출 수 없습니다. 이것이 "서명이 유효해도 구버전 거부"를 _하드웨어 물성_ 으로 보장하는 메커니즘입니다. 동시에 비트 하나당 메이저 버전 하나이므로, **32비트 = 최대 32회** 가 직접적인 업데이트 횟수 한계가 됩니다(2진수였다면 32비트로 40억까지 셌겠지만, 단방향성을 위해 표현력을 희생). 한계에 다다르면 RPMB 등 재기록 가능 보조 카운터로 미세 버전을 관리합니다([Module 04](../04_boot_device_and_boot_mode/)).
...
### 5.3a 서명 _검증 연산_ 의 내부 — RSA / ECDSA

위에서 검증을 "`Sig^e mod n == H`" / "곡선 연산으로 (r,s) 검증"으로만 적었습니다. 그 안을 한 단계 풀어, _무엇을 정확히 비교하는지_ 와 _왜 안전한지_ 를 봅니다.

#### RSA 검증 — `sig^e mod n` 그리고 PKCS#1 v1.5 padding 복원

RSA 검증은 두 부분입니다: (1) 공개키 연산으로 padding 된 블록을 복원하고, (2) 그 padding 패턴 + 해시가 _정확히_ 맞는지 확인.

1. **공개키 연산**: `m = sig^e mod n`. 서명자가 private key `d`로 `sig = m^d mod n`을 만들었으므로, 공개 지수 `e`로 거듭제곱하면 원래 padding 블록 `m`이 복원됩니다(RSA 의 `(m^d)^e ≡ m`). 여기서 `e`는 보통 65537이라 곱셈 수가 적어 _검증이 빠릅니다_ — 이것이 RSA 검증 속도 우위의 출처입니다.
2. **복원된 블록의 구조 검사**: RSASSA-PKCS1-v1_5 padding 은 _고정된 패턴_ 입니다.

```
복원된 m (RSA-2048이면 256 byte):
  ┌────┬────┬──────────────┬────┬──────────────────────────┐
  │ 00 │ 01 │ FF FF … FF   │ 00 │  DigestInfo ‖ H (해시)    │
  └────┴────┴──────────────┴────┴──────────────────────────┘
   고정  BT   PS(0xFF 패딩,   구분  알고리즘 식별 ASN.1 +
        =1   길이 가변)       자  실제 해시 H'
```

검증기는 _이 전체 패턴_ 을 한 글자도 빠짐없이 확인해야 합니다 — 맨 앞 `00 01`, 그다음 모두 `0xFF`인 PS, 구분자 `00`, 그리고 DigestInfo(어느 해시 알고리즘인지 박힌 ASN.1) 다음에 오는 H'가 `SHA-256(cert.body)`와 일치하는지. **왜 패턴까지 다 봐야 하나?** 해시만 맞춰 보고 PS의 `0xFF` 개수나 구분자 위치를 느슨하게 검사하면, 공격자가 padding 안에 여분 바이트를 끼워 _서명 없이도_ 통과하는 위조(Bleichenbacher류 / "PKCS#1 v1.5 padding oracle")가 가능합니다. 그래서 ⑧ `rsa_pkcs1_unpad`는 _고정 패턴 + 해시_ 를 모두 강하게 확인합니다.

#### ECDSA 검증 — 곡선 위의 두 점 합

ECDSA 서명은 쌍 `(r, s)`이고, 검증은 공개점 `Q`(= `d·G`, `G`는 곡선의 기준점)와 해시 `H`로 다음을 계산합니다(확인 필요 — 표준 검증식, 깊이 자제).

```
  w  = s⁻¹ mod n            (n = 곡선 위수)
  u1 = H · w mod n
  u2 = r · w mod n
  R  = u1·G + u2·Q          (곡선 위 점 두 개의 스칼라 곱 + 합)
  검증 통과 조건:  R.x mod n == r
```

직관: 서명자가 nonce `k`로 `r = (k·G).x`를 만들고 `s = k⁻¹(H + r·d)`로 묶었기 때문에, 검증식의 `u1·G + u2·Q`가 정확히 `k·G`로 되돌아와 그 x좌표가 `r`과 같아집니다. 곡선 스칼라 곱(`u·G`)이 무거워 _검증이 RSA보다 느립니다_ — ECDSA의 속도 단점 출처. 반대로 키/서명이 작은 건 곡선 차원이 낮아서입니다.

**왜 nonce `k` 재사용이 private key 누출로 직결되는가.** `k`가 _같은_ 두 서명 `(r, s1)`, `(r, s2)`가 있으면(같은 `k` → 같은 `r`), 두 식 `s1 = k⁻¹(H1 + r·d)`, `s2 = k⁻¹(H2 + r·d)`에서:

```
  s1 - s2 = k⁻¹ (H1 - H2)   →   k = (H1 - H2) / (s1 - s2)   (mod n)
  k 를 알면:  d = (s1·k - H1) / r   (mod n)   ← private key 가 그대로 풀림
```

즉 `k` 한 번만 겹쳐도 미지수 두 개(`k`, `d`)를 두 방정식으로 풀어 private key `d`가 노출됩니다. 그래서 build server의 서명 nonce는 _매번 새 고품질 난수_ (또는 RFC 6979식 결정적 생성)여야 하고, §6 디버그 체크리스트의 "ECDSA verify intermittent FAIL → nonce 재사용/RNG 약함"이 바로 이 위험을 가리킵니다.
...
#### ML-DSA 검증이 RSA/ECDSA와 _근본적으로_ 다른 점

크기/속도 표만 보면 "그냥 더 큰 서명"처럼 보이지만, 검증 _연산의 종류_ 자체가 다릅니다(개념 수준, 확인 필요). RSA는 큰 정수의 **모듈러 거듭제곱**(modexp), ECDSA는 **타원곡선 점 스칼라 곱**입니다 — 둘 다 "하나의 큰 수에 대한 무거운 산술"이라 비슷한 BigNum/곡선 HW 엔진을 공유합니다. 반면 ML-DSA(격자 기반)는 modexp도 곡선도 쓰지 않고, _작은 계수의 다항식 벡터/행렬_ 에 대한 연산(다항식 곱셈은 보통 NTT — Number Theoretic Transform로 가속)과 계수 범위 검사로 서명을 검증합니다. 그래서 HW 관점에서 (1) 필요한 엔진이 다르고(modexp/ECC 엔진이 아니라 NTT 버터플라이 + 작은 모듈러 곱셈 + 샘플링), (2) 메모리/대역 특성이 다릅니다(키·서명이 KB 단위라 SRAM·버스 대역 압박이 큼). 즉 기존 RSA/ECC 가속기를 그대로 재사용할 수 없고 _별도 엔진/면적_ 을 잡아야 하는 것이 PQC 전환의 실리콘 비용 핵심입니다.
...
BootROM 의 보안 핸드셰이크 (특히 인터칩 키 교환) 는 HW + FW 협력으로 동작. FW 의 C 코드를 DPI-C 로 **Scoreboard**(검증 환경에서 DUT 의 실제 출력과 정답 모델이 계산한 기대값을 자동 비교하는 컴포넌트) 에 연동하면:
...
자세한 DPI-C 아키텍처는 [Module 07 (DV 방법론)](../07_bootrom_dv_methodology/) 에서 다룹니다.
...
:::caution[실무 주의점 — Anti-Rollback 카운터 미증가 후 FW 업데이트]
...
:::tip[🤔 Q1 — Image 가 아닌 Hash 서명 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — PQC 하이브리드 정당화 (Bloom: Evaluate)]
...
- 📝 [**Module 03 퀴즈**](../quiz/03_crypto_in_boot_quiz/)
- ➡️ [**Module 04 — Boot Device & Boot Mode**](../04_boot_device_and_boot_mode/): cert + image 가 _어디에서_ 어떻게 읽혀 오는가 — UFS, eMMC, USB DL, RPMB.
