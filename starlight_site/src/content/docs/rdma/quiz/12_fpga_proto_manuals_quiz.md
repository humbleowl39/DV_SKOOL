---
pagefind: false
title: "Quiz — Module 12: FPGA Prototyping & Lab Manuals"
---

[← Module 12 본문으로 돌아가기](../../12_fpga_proto_manuals/)
...
이 순서는 "목적지부터 확인하고 나서 엔진을 연결한다"는 설계 접근법을 반영한다. 사용자 애플리케이션(1)과 디바이스 드라이버(2)를 먼저 빌드해 "인터페이스 계약"을 정의한 다음, 그 계약에 맞게 FPGA 엔진(3)을 설계한다. 결과 전송(4)과 인터럽트(5)가 마지막에 오는 것은 FPGA→호스트 방향의 통로를 드라이버가 준비된 상태에서 열어야 하기 때문이다. 순서를 바꾸면 드라이버가 없는 상태에서 인터럽트가 발생해 검증이 불가능해진다.
