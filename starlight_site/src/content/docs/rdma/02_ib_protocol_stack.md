---
pagefind: false
title: "Module 02 — InfiniBand 프로토콜 스택"
---
...
IB 패킷 포맷을 설계할 때는 서로 모순되어 보이는 두 가지 요구를 동시에 만족해야 합니다.

1. **Subnet Manager (SM — IB 네트워크의 토폴로지·주소·경로를 설정·관리하는 소프트웨어) 가 토폴로지를 자유롭게 바꿀 수 있어야 한다** → 라우터/스위치는 패킷의 **DLID**(destination local ID, subnet 내 목적지 주소)**, VL**(virtual lane, 한 물리 링크를 여러 독립 버퍼로 나눈 가상 채널) 같은 라우팅 필드를 hop(패킷이 거치는 한 단계의 스위치/라우터) 마다 _자유롭게 변경_ 해야 합니다.
2. **그런데 sender 가 보낸 데이터는 receiver 가 받았을 때 _bit-exact_(비트 하나까지 동일) 로 같음을 검증할 수 있어야 한다** → 즉 end-to-end 무결성이 hardware-level 로 보장돼야 합니다.

문제: **CRC**(cyclic redundancy check — 데이터가 전송 중 손상됐는지 검출하기 위해 붙이는 검사값) **한 개로 둘 다 만족하면** — 라우터가 **LRH**(Local Route Header, 패킷 맨 앞의 subnet 내 라우팅 헤더) 의 필드 하나만 바꿔도 CRC 가 깨집니다. 그렇다고 **각 hop 마다 CRC 를 재계산** 하면 — sender 의 의도가 _도중에 침해_ 됐는지 receiver 가 확인할 수 없습니다.
...
- **Ethernet FCS** (Frame Check Sequence — 이더넷 프레임 끝에 붙는 4-byte CRC) — hop-by-hop, 매 link 마다 재계산. IB 의 VCRC 와 같은 역할.
...
같은 subnet 내 두 노드 — A (LID=10) 가 B (LID=42) 에게 256-byte SEND. 중간에 switch 1대. 여기서 **LID**(Local ID — subnet 안에서 한 포트를 가리키는 16-bit 주소; 출발지가 SLID, 목적지가 DLID), **SL**(Service Level — 패킷이 요구하는 QoS 등급), **QoS**(quality of service — 트래픽 종류별로 대역폭·우선순위를 보장하는 것) 가 핵심 어휘입니다.
...
| **FlowLabel** | 같은 flow 의 packet 은 같은 값 — 라우터가 ECMP(Equal-Cost Multi-Path — 비용이 같은 여러 경로에 트래픽을 분산하는 라우팅) path 고정 |
...
:::
(전체 OpCode 표는 [Module 06](../06_data_path/) 와 [Quick Reference](../09_quick_reference_card/) 에서 다룸)
...
**MTU**(Maximum Transmission Unit — 한 패킷에 담을 수 있는 payload 의 최대 크기) 가 정해지면, 그보다 큰 메시지는 여러 패킷으로 쪼개(fragmentation) 보냅니다.
...
**AETH**(ACK Extended Transport Header — ACK/NAK 패킷에 붙는 헤더) 의 **MSN**(Message Sequence Number — responder 가 끝낸 메시지를 세는 카운터) 은 "responder 가 성공적으로 완료한 메시지 수" 를 나타내며, 다음 규칙이 검증 핵심이다 (**SSN** = Send Sequence Number, 송신 메시지에 매기는 번호).
...
:::tip[🤔 Q1 — Switch fault injection (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — RoCEv2 매핑 추론 (Bloom: Apply)]
...
→ [Module 03 — RoCEv2 — Ethernet 위의 RDMA](../03_rocev2/): IB 의 LRH/VCRC/Flow Control 이 어떻게 Ethernet/IP/UDP 로 대체되는지, 그리고 BTH/xTH 가 어떻게 그대로 유지되는지.

[퀴즈 풀어보기 →](../quiz/02_ib_protocol_stack_quiz/)
