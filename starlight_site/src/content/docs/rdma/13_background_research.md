---
title: "Module 13 — Background & Industry Research"
---
...
:::tip[🤔 잠깐 — 새 논문이 나왔다, 어떤 의사결정이 _즉시_ 필요한가?]
SIGCOMM 2025 에서 _새 RDMA CC 알고리즘_ 이 발표됐습니다 — 기존 DCQCN 보다 2× 빠른 성능입니다. RDMA-IP 검증 책임자로서 _이번 주_ 안에 내려야 할 _3 가지 의사결정_ 은 무엇인가?
...
산업 입력을 그대로 검증 자산에 옮기면 두 함정에 빠집니다. 첫 번째는 **"논문이 그렇게 하니까 우리도"** 라는 함정입니다. 논문의 결과는 특정 switch, NIC, fabric 환경에서 나온 것입니다. 같은 알고리즘이 사내 leaf-spine 환경에서 동일한 성능을 내지 않을 수 있습니다. 논문을 그대로 따라가면 환경이 달라서 생기는 문제를 spec 이나 구현 버그로 오진하게 됩니다. 두 번째는 **"spec 만 보면 충분하다"** 는 함정입니다. 산업 동향은 향후 spec 의 초기 우선순위를 미리 알려주는 신호입니다. 무시하면 chip 출시 직전에 새 spec 이 채택되어 검증 자산을 통째로 뜯어야 하는 후행 비용이 폭증합니다.

연결 지도 형식이 이 두 함정 사이의 균형을 잡아줍니다. _각 외부 입력 → 내부 자산 → 검증 항목_ 의 3 hop 으로 의사결정 경로를 명시하므로, 어디까지 반영하고 어디서 멈출지가 항상 명확해집니다.
...
:::tip[🤔 Q1 — 논문 → 검증 자산 매핑 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — 산업 동향 추적 빈도 (Bloom: Evaluate)]
RDMA 검증 lead 입장에서 chip 출시 타이밍을 놓치지 않으려면 산업 동향 추적을 _어느 주기_ 로 해야 할까? 그리고 그 역할은 _누구_ 가 담당해야 하는가?
...
- [Quick Reference Card](../09_quick_reference_card/) 로 핵심 표 한 번 더 훑기.
- [용어집](../glossary/) 의 Appendix A/B/C 에서 사내 / UEC / 산업 용어 다시 확인.
- [퀴즈 13](../quiz/13_background_research_quiz/) 으로 이해도 점검.
