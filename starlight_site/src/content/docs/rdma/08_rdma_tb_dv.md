...
참고로 이 모듈은 UVM(Universal Verification Methodology — SystemVerilog 기반 검증 환경을 만드는 표준 방법론) 어휘를 씁니다. 핵심만 먼저 풀면: **stimulus**(DUT 에 넣는 입력 자극), **scoreboard**(DUT 출력과 정답 기대값을 비교하는 컴포넌트), **monitor**(신호를 관찰해 transaction 으로 뽑아내는 컴포넌트), **agent/driver/sequencer**(자극을 만들어 DUT 핀까지 전달하는 컴포넌트 묶음) 입니다.

먼저 stimulus 가 두 갈래입니다. RDMA NIC 의 input 은 _wire 위의 패킷_ 과 _PCIe 위의 host 메모리/MMIO_ 두 가지인데, 이 둘을 동시에 모델링해야 비로소 하나의 완전한 stimulus 가 됩니다. 다음으로 output 비교도 세 곳을 봐야 합니다. RDMA WRITE 의 결과는 _wire 패킷_ 이 제대로 나갔는지, _peer 메모리의 해당 region_ 이 올바르게 변경됐는지, 그리고 _CQE_(completion queue entry, 완료 통지 항목) 가 정확한 status 로 생성됐는지 세 곳을 모두 확인해야 합니다. 특히 scoreboard 의 핵심은 단순 transaction 비교가 아니라 _"peer memory 의 region X 가 sender 의 lbuf 와 bit-exact 인가?"_ 처럼 system-level semantics 를 검증하는 것입니다.
...
:::tip[🤔 잠깐 — 왜 _두 노드_ 모두 모델 안에?]
일반 ASIC DV 는 DUT 하나 + agent 들이 stimulus. 왜 RDMA-TB 는 _두 노드 (그래서 DUT 도 2개?)_ 가 환경 안에 있나? 단순히 한 NIC 만 DUT 로 두고 다른 쪽은 BFM(bus functional model — 실제 RTL 대신 인터페이스 동작만 흉내 내는 간이 모델) 으로 안 되나?
...
OpCode 별 handler 분리 — 한 driver 가 if/else 로 모든 op 처리하면 비대. **Strategy 패턴**(GoF — Gang of Four 디자인 패턴 중 하나; 동작을 객체로 분리해 상황에 맞게 갈아끼우는 기법) 으로 OpCode → handler 매핑.
...
→ Test 는 `vrdmatb_base_test` 를 extend, factory(UVM factory — 객체를 코드 수정 없이 다른 종류로 바꿔치기할 수 있게 해주는 생성 메커니즘) 로 sequence 교체 가능.
...
| Test 가 build phase 에서 UVM_FATAL | config_db(UVM config database — 컴포넌트 간에 설정값을 이름/경로로 주고받는 저장소) get path mismatch | `set_config_db` vs `get_config_db` 의 hierarchy |
...
:::tip[🤔 Q1 — 새 feature 배치 결정 (Bloom: Evaluate)]
...
:::
:::tip[🤔 Q2 — Scoreboard 의 비교 범위 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — Sub-IP TB ↔ IP-top sequence 호환 (Bloom: Apply)]
...
→ [Module 09 — Quick Reference Card](../09_quick_reference_card/): 자주 펼치는 표/공식/명령어 한눈에.

[퀴즈 풀어보기 →](../quiz/08_rdma_tb_dv_quiz/)
