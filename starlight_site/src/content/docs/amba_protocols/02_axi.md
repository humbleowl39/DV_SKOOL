---
title: "Module 02 — AXI (Advanced eXtensible Interface)"
---
...
:::
:::note[사전 지식]
- [Module 01 — APB & AHB](../01_apb_ahb/) (handshake / wait state / pipeline 개념)
...
이 모듈의 기본 용어를 먼저 풀어 둡니다. **AXI**(Advanced eXtensible Interface — ARM AMBA 의 고성능 bus). **master**(거래를 시작해 read/write 를 요청하는 쪽 / manager), **slave**(요청을 받아 응답하는 쪽 / subordinate). **channel**(채널 — AXI 가 주소·데이터·응답을 따로따로 나르도록 분리해 둔 독립 신호 묶음; AXI 는 이런 채널이 5개). **transaction**(주소+데이터+응답이 한 묶음으로 오가는 한 번의 거래). **handshake**(보내는 쪽 VALID, 받는 쪽 READY 가 _둘 다 1인 클럭_ 에서 전송이 일어나는 박자 맞춤 규칙 — 아래 §4.1 에서 상세). **throughput**(단위 시간당 처리량 — 대역폭). **NoC**(Network-on-Chip — 칩 안을 패킷 스위칭으로 잇는 대형 interconnect).
...
5 채널이 _완전 독립_ → 동시 진행 가능 → AHB 대비 **4-8 배 throughput**. (위 표의 채널 약어 **AW**=Write Address, **W**=Write Data, **B**=Write Response(B는 "buffered response"의 B), **AR**=Read Address, **R**=Read Data. 주소·데이터·응답이 각각 따로 흐릅니다.)

추가: **Outstanding** (응답 안 와도 다음 request 보낼 수 있음) + **Out-of-Order**(OoO — 응답이 _보낸 순서와 다른 순서_ 로 도착해도 됨, **ID**(각 transaction 에 붙이는 식별 태그 — 같은 ID 끼리는 순서 보장, 다른 ID 끼리는 재정렬 허용)로 매칭).
...
:::tip[🤔 잠깐 — VALID 가 _READY 를 기다리면_ 무슨 일이?]
...
AXI 신호는 채널 머리글자 + 역할로 읽습니다. AR 채널이면 `AR*`, R 채널이면 `R*`. 여기 나오는 것: **ARVALID/ARREADY**(AR 채널의 handshake 쌍), **ARID**(이 read 의 ID 태그), **ARADDR**(주소), **ARLEN**(burst 길이 = beat 수 − 1), **ARSIZE**(beat 하나의 바이트 폭, 2^ARSIZE), **ARBURST**(burst 타입), **RVALID/RREADY**(R 채널 handshake), **RID/RDATA/RRESP**(응답 ID·데이터·결과코드), **RLAST**(이 beat 가 burst 의 마지막임을 표시). **beat**(burst 안에서 한 클럭에 오가는 데이터 한 칸).
...
:::note[왜 ID 단위로만 순서를 보장하나 — ID = 독립 logical stream]
AXI 의 ID 는 단순한 태그가 아니라 "**서로 독립적인 transaction stream**" 을 표현하는 라벨입니다. 같은 ID 를 단 transaction 들은 master 가 "이것들은 한 흐름이고, 도착 순서가 의미를 가진다" 고 선언한 것입니다. 예를 들어 어떤 producer 가 buffer 에 데이터를 쓴 뒤(write A) 그 buffer 가 채워졌다는 flag 를 쓰면(write B), 같은 ID 라면 A 가 B 보다 먼저 반영되어야 consumer 가 flag 만 보고 데이터를 읽어도 안전합니다. 만약 같은 ID 안에서 순서가 뒤집히면 이 producer-consumer 의미가 깨집니다. 그래서 protocol 은 _같은 ID 안에서는_ 순서를 강제합니다.

반대로 다른 ID 는 master 가 처음부터 "이 둘 사이엔 순서 의존이 없다" 고 _약속한_ 흐름입니다. 그래서 interconnect 가 자유롭게 재정렬해도 어떤 의미도 깨지지 않습니다. 즉 in-order/OoO 의 경계가 ID 인 이유는, ID 가 곧 "순서가 중요한 단위" 의 정의이기 때문입니다.
:::

:::note[AXI 는 왜 강한 global ordering 을 버렸나 — interconnect 의 자유를 위해]
APB/AHB 같은 단일 버스는 모든 transaction 이 한 점을 통과하니 자연히 전역 순서(global order)가 잡힙니다. 하지만 그 강한 순서는 곧 제약입니다 — interconnect 가 어떤 transaction 도 추월·병합·재정렬할 수 없으니, 느린 한 transaction 이 뒤의 모두를 막습니다(head-of-line blocking). AXI 는 NoC 같은 복잡한 interconnect 위에서 throughput 을 뽑아야 했고, 그러려면 interconnect 가 transaction 을 _자유롭게_ 재배치할 수 있어야 했습니다. 그래서 AXI 는 "전체 transaction 의 전역 순서" 라는 강한 보장을 포기하고, _ID·채널 단위의 약한 ordering_ 만 남겼습니다. 보장을 최소한으로 줄인 만큼 interconnect 가 재정렬·병합·multi-path 라우팅으로 throughput 을 얻을 자유를 확보한 것입니다. ordering 이 "약한" 것은 결함이 아니라, 성능을 위해 _의도적으로_ 양보한 design rationale 입니다.
:::
...
멀티 프로세서 시스템에서 **atomic**(쪼개지지 않는 — 중간에 다른 접근이 끼어들 수 없는 한 동작으로 취급) **Read-Modify-Write**(값을 읽어 고쳐 다시 쓰는 3단계를, 그 사이 아무도 끼어들지 못하게 한 묶음으로 수행) 를 구현하는 메커니즘. **AxLOCK**(이 접근이 exclusive 임을 표시하는 신호), **EXOKAY**(exclusive 성공 응답 코드).
...
:::note[왜 byte lane 이 beat 마다 이동하나 — address-aligned lane placement 불변식]
narrow transfer 에서 유효 byte lane 이 beat 마다 위아래로 움직이는 것은 임의가 아니라, AXI 의 핵심 불변식 하나에서 나옵니다 — **"한 byte 는 항상 _자신의 주소 lower bit_ 가 가리키는 lane 에 놓인다."** 즉 어떤 byte 의 주소를 데이터 버스 폭(byte 수)으로 나눈 나머지가, 그 byte 가 차지하는 lane 번호입니다. 8-byte 버스라면 주소 0x00 의 byte 는 lane 0, 0x04 의 byte 는 lane 4, 0x0C 는 다시 lane 4(0x0C % 8 = 4) 에 놓이는 식입니다.

이 규칙이 있으면 master 와 slave 가 "어느 byte 가 어느 lane 에 있다" 를 별도 정보 교환 없이 _주소만 보고_ 동일하게 계산할 수 있습니다. 그래서 위 예시처럼 0x00 의 beat 는 하위 lane, 0x04 의 beat 는 상위 lane 에 데이터가 실리고, WSTRB 도 그 lane 만 1 이 됩니다. lane 이 "이동하는" 것이 아니라, 사실은 _주소가 바뀌니 그 주소가 가리키는 고정된 lane_ 이 따라 바뀌는 것입니다. slave 는 이 불변식 덕분에 WSTRB 와 주소만으로 정확한 메모리 위치를 찾아 쓸 수 있습니다.
:::
...
흔한 조합:
  4'b0000 — Device, Non-bufferable: MMIO 레지스터 (순서/사이드이펙트 보장 필수)
            (MMIO = Memory-Mapped I/O — 주변장치 레지스터를 메모리 주소처럼 read/write 하는 방식)
...
:::caution[실무 주의점 — WSTRB 와 narrow transfer]
...
:::tip[🤔 Q1 — Outstanding 깊이 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — VALID/READY deadlock 진단 (Bloom: Evaluate)]
...
- 📝 [**Module 02 퀴즈**](../quiz/02_axi_quiz/) — 5 문항으로 이해도 점검
- ➡️ [**Module 03 — AXI-Stream**](../03_axi_stream/) — 주소 없는 패킷 전송 모델
