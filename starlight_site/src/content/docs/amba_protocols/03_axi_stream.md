...
:::
:::note[사전 지식]
- [Module 02 — AXI](../02_axi/) (VALID/READY 핸드셰이크)
...
:::note[왜 register slice(skid buffer)가 필요한가 — back-pressure 의 timing 비용]
VALID/READY 핸드셰이크는 본질적으로 양방향 신호입니다 — master 는 TREADY 를 보고 다음 beat 로 넘어가고, slave 는 TVALID 를 보고 받습니다. 두 IP 를 그냥 선으로 직접 이으면, slave 의 TREADY 가 _조합 논리로_ master 의 다음-beat 결정에 곧장 들어갑니다. back-pressure(TREADY=0)가 발생하는 그 순간, slave 의 내부 buffer-full 신호 → TREADY → master 의 출력 mux 까지가 _한 cycle 안의 조합 경로_ 로 묶입니다. IP 가 멀리 떨어져 있거나 fan-out 이 크면 이 경로가 길어져 timing 을 못 닫고, 최악의 경우 TREADY 와 TVALID 가 서로의 조합 입력이 되어 (이전 모듈의 deadlock 규칙 위반) 글리치·발진 위험까지 생깁니다.

해법이 **register slice(skid buffer)** 입니다. master 와 slave 사이에 한 beat 를 담을 register 단을 끼워, TVALID/TREADY 의 조합 경로를 register 로 _끊습니다_. 이제 양쪽은 자기 옆의 register 단과만 핸드셰이크하므로 critical path 가 짧아지고, back-pressure 가 와도 slice 가 in-flight beat 를 흡수해 한 cycle 의 _여유(skid)_ 를 줍니다. 이름 그대로 "미끄러질 공간" 을 둬서, TREADY 가 늦게 내려와도 직전 beat 를 잃지 않고 잡아 둡니다. 즉 skid buffer 는 단순 성능 옵션이 아니라, _조합 결합을 끊어 timing 을 닫고 데이터 손실을 막는_ 구조적 필수 요소입니다.
:::
...
AXI-Stream 을 "항상 패킷 단위로 쓴다" 고 이해하면 실제 설계에서 맞지 않는 경우가 생깁니다. 패킷 기반 시스템(Ethernet, PCIe TLP)은 TLAST 로 경계를 표시하는 Byte Stream 이 맞지만, 오디오 DAC 같은 연속 샘플 스트리밍은 패킷 개념 자체가 없어서 TLAST 없이 흘러가는 Continuous Stream 이 자연스럽습니다. 어떤 유형인지는 수신 측 IP 의 기대와 맞아야 하므로, IP 를 연결하기 전에 양쪽이 같은 유형을 쓰는지 확인하는 것이 첫 번째 검증 항목입니다.

:::note[경계 정보를 어디에 실을 것인가 — in-band TLAST vs out-of-band 길이]
"이 packet 이 어디서 끝나는가" 를 수신 측에 알리는 방법은 근본적으로 두 가지입니다. (1) **in-band** — 데이터 흐름 _안에_ 경계 표시를 끼워 넣기 (TLAST=1). (2) **out-of-band** — 길이를 별도 채널/sideband(TUSER 의 length field, 또는 앞선 descriptor)로 미리 알려 주기. 어느 쪽을 택하느냐는 _길이를 미리 아느냐_ 에 달려 있습니다.

- **fixed-length stream** 은 매 packet 의 beat 수가 _이미 합의되어 고정_ 입니다. 수신 측이 "N beat 마다 한 단위" 라고 처음부터 알고 세면 되므로, 경계를 매번 in-band 로 알릴 필요가 없어 TLAST 를 생략할 수 있습니다 (경계 정보가 사실상 out-of-band 의 상수로 빠진 셈).
- **variable-length stream**(Ethernet frame, TLP 등)은 packet 마다 길이가 다르고, 더 중요하게는 _보내는 순간까지_ 길이가 확정 안 될 수 있습니다(cut-through 전송). 길이를 앞세워 보낼 방법이 없으니, 경계는 흐름 안에서 마지막 beat 에 도달했을 때 in-band 로 표시할 수밖에 없습니다 — 그래서 TLAST 가 _강제_ 됩니다.

즉 TLAST 의 필요 여부는 protocol 의 변덕이 아니라 "경계 정보를 in-band 로 줄 수밖에 없는가, out-of-band 로 미리 줄 수 있는가" 라는 일반 원리의 결과입니다.
:::
...
예시 (TCP offload ↔ Ethernet MAC 스트림):
...
**실제**: spec 상 TKEEP 은 "어느 byte lane 이 valid 한가" 의 표시일 뿐, _LSB-contiguous_ 의무는 없습니다 (단 대부분의 구현은 마지막 beat 에서 LSB-contiguous 패턴만 허용 — 구현별 정책 확인 필수). 가운데에 0 이 있는 sparse pattern 을 허용하는 IP 도 있음.<br>
...
:::caution[실무 주의점 — TVALID hold 중 TKEEP/TUSER 변경]
...
:::
:::tip[🤔 Q3 — Backpressure 처리 (Bloom: Evaluate)]
