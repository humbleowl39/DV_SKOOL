...
이 모듈의 기본 용어를 먼저 풀어 둡니다. **coherency**(캐시 일관성 — 같은 주소를 여러 캐시가 들고 있을 때 모두가 _같은 최신 값_ 을 보도록 맞추는 것). **cache / cache line**(캐시 — CPU 옆의 빠른 임시 저장소; cache line = 그 안에서 한 번에 다루는 데이터 한 덩어리, 보통 64 byte). **dirty / clean**(dirty = 캐시에 든 값이 DRAM 보다 최신이라 아직 메모리에 안 내려간 상태 / clean = 메모리와 같은 상태). **stale data**(낡은 값 — 누군가 더 최신값을 들고 있는데 옛 사본을 읽은 것). **write-back**(dirty 한 값을 DRAM 에 다시 써 내려보내는 것). **snoop**(스눕 — 한 캐시의 접근 때 다른 캐시들에게 "이 주소 갖고 있니?"를 물어보는 동작). **ACE**(AXI Coherency Extensions — AXI 위에 snoop 을 얹은 일관성 확장), **CHI**(Coherent Hub Interface — 그것의 packet 기반 대규모 버전). **GPGPU**(General-Purpose GPU — 범용 계산에 쓰는 GPU). **DRAM**(메인 메모리). **MESI/MOESI**(캐시 라인 상태를 Modified/Owned/Exclusive/Shared/Invalid 로 추적하는 표준 상태 머신).
...
HSA(Heterogeneous System Architecture — CPU·GPU 가 같은 메모리를 공유해 협업하는 구조) 시나리오를 트랜잭션 단계로 펼쳐 봅니다. CPU 가 주소 `A` 에 행렬 한 줄을 써서 L2(2차 캐시) 에 **dirty(Modified — 고쳐서 DRAM 보다 최신인 상태)** 로 들고 있고, GPGPU 가 같은 `A` 를 읽으려 합니다.
...
> 핵심은 ④ 와 ⑤ 의 분리입니다. GPGPU 가 받는 데이터의 _출처_ 는 DRAM 이 아니라 **CPU 캐시** 이고(느린 main memory 를 완전히 우회), DRAM 갱신은 별개의 정책 문제입니다. **scoreboard**(DUT 의 실제 출력과 "정답"으로 계산한 expected 값을 비교하는 검증 컴포넌트)가 "DRAM 값" 을 expected(기대값)로 잡으면 mismatch(불일치 오류)가 나는 이유가 바로 여기 있습니다.
...
> 용어: **IO coherency**(자기 캐시는 없지만 _남의 캐시는 snoop_ 해서 최신값을 받는 단방향 일관성), **DMA**(Direct Memory Access — CPU 없이 메모리를 직접 옮기는 블록), **NIC**(Network Interface Controller — 네트워크 송수신 블록).
...
코어가 많아지면 모든 L1/L2 에 "이 데이터 있니?" 를 broadcast(모든 캐시에 동시에 뿌리기) 하는 것 자체가 버스를 막습니다. **LLC**(Last Level Cache — CPU 와 DRAM 사이 가장 바깥쪽 공유 캐시 / SLC, System Level Cache)가 **snoop filter(directory)**(어느 캐시가 어느 라인을 갖고 있는지 적어 둔 장부) 를 들고 어느 **upstream**(LLC 보다 코어에 가까운 안쪽) 캐시가 어느 라인을 가졌는지 추적해, 요청이 오면 _그 캐시에만_ **targeted snoop**(해당 캐시만 콕 집어 보내는 snoop) 을 보냅니다. 이로써 인터커넥트 트래픽이 급감합니다.
...
- **Back-Invalidation**: LLC 가 strictly inclusive(상위 캐시 내용이 반드시 LLC 에도 들어 있어야 하는 포함 정책)면 L1/L2 의 데이터는 LLC 에도 있어야 한다. LLC 가 라인을 evict(공간을 비우려 캐시에서 내쫓음) 하면 upstream 에 back-invalidation(상위 캐시의 사본도 무효화하라는 역방향 통보) 을 보내 그 사본도 버리게 해 orphan(주인 LLC 가 사라졌는데 상위에만 남은 미아) 데이터를 막는다. (back-invalidation 은 inclusive 정책의 직접 산물 — exclusive/NINE 이면 양상이 달라진다. 위 링크 참조.)
