...
:::
:::note[사전 지식]
- [Module 01-03](../01_apb_ahb/) 학습 완료 후 이 카드를 보면 효과 극대화
:::
---
...
| CHI | AXI 계승 | 채널→packet 기반, scalable mesh NoC, 분산 directory |

> ACE/CHI 의 coherency 확장·snoop 트랜잭션은 [Module 04 — ACE/CHI Coherency](../04_ace_chi_coherency/) 참고.
...
:::caution[실무 주의점 — Burst 길이 인코딩 off-by-one]
...
:::
:::tip[🤔 Q2 — Protocol 선택 trade-off (Bloom: Evaluate)]
...
4 개 모듈 + Quick Ref 를 완료했습니다. 다음을 권장합니다:

1. **퀴즈 풀어보기** — [퀴즈 인덱스](../quiz/)
2. **글로서리 스캔** — 모르는 용어 점검: [용어집](../glossary/)
3. **실전 적용** — 본인의 검증 환경에서 VALID/READY 데드락 패턴 검사
4. **다음 토픽** — UVM 위에 AMBA 를 올린 [UVM 코스](../../uvm/), 또는 메모리 서브시스템 [MMU](../../mmu/) / [DRAM](../../dram_ddr/). ACE/CHI 가 떠받치는 시스템 신뢰성 관점은 [RAS](../../ras/) 참고
