...
수백 GB 파라미터를 가진 모델을 학습하는 서버를 생각해 봅시다. CPU 메인보드의 DIMM 슬롯은 이미 가득 찼고, 더 꽂을 자리가 없습니다. RAM을 늘리려면 서버를 통째로 교체해야 하는데, 정작 비어 있는 것은 PCIe 슬롯입니다. 한편 GPU는 호스트 메모리에 있는 데이터를 가져다 연산하려고 매번 PCIe DMA로 복사해 오는데, 이 복사 비용과 일관성 관리 비용이 누적되어 실제 연산보다 데이터 이동에 시간을 더 쓰는 상황이 벌어집니다.
...
PCIe는 본질적으로 **DMA(producer-consumer) 모델**입니다. 디바이스가 호스트 메모리의 한 영역을 통째로 읽거나 쓰는 데에는 효율적이지만, cacheline 단위로 수시로 작은 접근을 하기에는 TLP 오버헤드와 지연이 큽니다. 메모리를 마치 자기 RAM처럼 Load/Store로 접근하려면 다른 프로토콜이 필요합니다. 그것이 CXL의 출발점입니다.

이 모듈을 건너뛰면 이후 모든 내용 — Flex Bus가 왜 PCIe 물리 계층을 재사용하는지, .cache와 .mem이 왜 따로 있는지, Bias coherency가 왜 필요한지 — 이 "그냥 그렇게 정의됐다"로만 남습니다. 동기를 이해해야 설계 결정이 인과로 읽힙니다.
...
CXL.io 프로토콜 자체가 PCIe TLP/DLLP 그대로입니다. enumeration, configuration, 인터럽트, 일부 DMA는 여전히 PCIe 방식으로 처리됩니다. CXL이 새로 추가한 것은 그 위에 얹은 **.cache와 .mem** 이라는 두 coherent 프로토콜입니다.
