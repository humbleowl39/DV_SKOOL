...
:::
:::note[사전 지식]
- [Module 01](../01_can_bus_fundamentals/) ~ [Module 04](../04_attack_surface_and_defense/) 완료 — 이 모듈은 새 내용을 가르치지 않고 _압축_ 합니다.
:::
---
...
키워드 맵을 활용하는 방식은 단순 암기가 아닙니다. "필수 키워드" 는 그 주제에 대해 대화할 때 빠지면 "이 사람이 기본을 모른다" 는 인상을 주는 것이고, "차별화 키워드" 는 맥락과 함께 쓸 때만 가치가 있습니다. 예를 들어 "OSNMA" 라는 단어만 던지는 것이 아니라, "Galileo OSNMA 는 위성 신호 자체에 서명을 넣어 GPS 수신기가 위조 신호를 직접 검증할 수 있게 한다" 고 한 문장을 붙여야 차별화가 됩니다.
...
:::caution[실무 주의점 — OTA 검증 단계 누락 시 fleet 전체 위험]
...
:::tip[🤔 Q1 — 4-layer 스택 적용 (Bloom: Apply)]
...
:::
:::tip[🤔 Q2 — Tesla 사례 평가 (Bloom: Evaluate)]
