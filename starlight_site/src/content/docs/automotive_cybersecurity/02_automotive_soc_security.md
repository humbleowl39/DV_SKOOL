...
차량 OEM 이 5 보안 layer 를 모두 구현했다고 가정합니다. L1 에서는 Secure Boot 와 HSM 으로 부팅 체인과 키를 보호하고, L2 에서는 SecOC 로 CAN 메시지마다 인증을 붙이며, L3 에서는 Gateway 로 도메인을 격리하고, L4 에서는 V2X 통신에 TLS 를 적용하며, L5 에서는 OTA 서명과 서버 인증을 갖췄습니다. 완벽해 보이는 이 스택에서 **한 layer 만 빠뜨리면 어떻게 되나?**
