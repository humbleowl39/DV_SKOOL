---
title: "Module 04 — Attack Surface & Defense"
---
...
:::
:::note[사전 지식]
- [Module 01–03](../01_can_bus_fundamentals/) (CAN, SoC 보안 스택, Tesla 사례)
...
**측정 원리 — 왕복 시간(round-trip ToF)을 거리로 환산.** UWB가 재는 것은 신호의 **왕복 비행 시간** 입니다. 차량이 키에 짧은 펄스를 보내고, 키가 응답을 되쏘기까지의 _전체 왕복 시간_ 에서 키 내부의 _알려진 처리 지연_ 을 빼면 순수 전파 시간이 남고, `거리 = (전파 시간 / 2) × 빛의 속도`로 환산합니다(빛은 30 cm/ns, 분해능 65 ps → 약 10 cm). 핵심은 **릴레이 공격이 _추가 지연을 숨길 수 없다_** 는 점입니다 — 중계기가 신호를 받아 멀리 있는 진짜 키까지 전달했다 되받아 오면, 그 중계 처리 + 추가 전파에 _최소 수십 ns_ 가 더 붙습니다. UWB는 이 추가 시간을 그대로 _더 먼 거리_ 로 환산하므로, 키가 실제로는 집 안(수십 m 밖)에 있는데 차 옆 2 m로 위장하려는 릴레이는 측정 거리가 임계(예: 2 m)를 넘어 거부됩니다. 시간을 늦출 수는 있어도 _빛보다 빠르게 당길 수는 없어_ 거리를 짧게 위조하는 것이 물리적으로 불가능 — 이것이 RSSI(신호 세기, 출력만 키우면 속는다)와 달리 ToF가 릴레이에 강한 이유입니다.
...
**가명 인증서의 익명성 ↔ 추적의 균형 메커니즘.** SCMS의 어려운 요구는 두 가지가 상충한다는 것입니다: (a) 평소엔 차량을 _추적할 수 없어야_ 한다(프라이버시), (b) 그러나 부정 행위 시엔 _실 신원으로 역추적_ 할 수 있어야 한다(책임). SCMS는 이를 _권한 분리_ 와 _연결 값(linkage value)_ 으로 풉니다(확인 필요 — SCMS 일반 구조).

- **평소엔 unlinkable**: 차량은 다수의 단기 **가명 인증서** 를 받아 주기적으로 갈아 씁니다(20개씩 로테이션). 서명에 실 신원이 없고 인증서가 자주 바뀌므로, 도청자는 _어느 메시지들이 같은 차에서 왔는지_ 조차 엮을(link) 수 없습니다 — 서로 다른 가명들이 통계적으로 무관해 보이도록 발급됩니다.
- **권한 분리로 단독 추적 불가**: 가명 ↔ 실 신원의 연결 정보는 _한 기관이 다 갖지 않습니다_. 대략 두 **Linkage Authority(LA1, LA2)** 가 각자 _부분 연결 값(linkage seed의 조각)_ 만 보유하고, **Pseudonym CA** 는 그 조각들을 받아 가명을 발급합니다. 어느 한 기관도 혼자서는 "이 가명 = 이 차"를 알 수 없어, 일상적 추적이 구조적으로 막힙니다.
- **Misbehavior 시에만 역연결**: 한 차량이 부정 메시지를 내 Misbehavior Authority가 제재를 결정하면, _여러 기관이 협력_ 해 그 차의 부분 연결 값들을 합쳐 **linkage value** 를 복원하고, 그 차량이 발급받은 _모든 가명_ 을 한꺼번에 식별해 CRL로 폐기합니다. 즉 "평소엔 어느 기관도 못 엮다가, 정해진 다자 절차를 거쳐야만 실 신원/전체 가명으로 역연결"되는 구조라, 프라이버시와 책임을 동시에 만족합니다.
...
3. **왜 spoof 이 걸러지나**: 공격자는 _키가 공개되기 전에는_ 유효한 MAC을 만들 수 없고(키를 모름), 키가 공개된 뒤에는 수신기가 이미 그 시점의 메시지를 받아 둔 상태라 _과거를 위조해 끼워 넣을 수 없습니다_(시간 순서가 봉인). 즉 가짜 위성 신호는 올바른 MAC/키 체인을 동반하지 못해 인증에 실패하고, 수신기가 그 위치 해를 버립니다. 자동차 입장에선 RF GPS spoofing(Level 2)이 OSNMA 검증 단계에서 걸러져, §5.x의 "이전 위치에서 물리적으로 불가능한 이동" 같은 plausibility 방어와 함께 다층으로 작동합니다.
...
:::caution[실무 주의점 — V2X Sybil 공격 임계값 설정 오류]
...
:::
:::tip[🤔 Q2 — Defense in depth (Bloom: Analyze)]
...
:::
:::tip[🤔 Q3 — V2X Sybil 방어 (Bloom: Evaluate)]
...
→ [Module 05 — Quick Reference Card](../05_quick_reference_card/): 모듈 1–4 를 한 장으로 압축 + 면접/리뷰 직전 cheat sheet.

[퀴즈 풀어보기 →](../quiz/04_attack_surface_and_defense_quiz/)
