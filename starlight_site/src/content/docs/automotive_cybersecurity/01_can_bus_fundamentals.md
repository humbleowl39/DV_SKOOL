---
title: "Module 01 — CAN Bus Fundamentals"
---
...
2. 인포테인먼트 칩의 ARM CPU 의 _firmware_ (펌웨어 — 기기에 내장돼 하드웨어를 직접 구동하는 저수준 소프트웨어) 를 다시 flash.
3. 인포테인먼트가 CAN bus 의 _gateway_ (게이트웨이 — 서로 다른 네트워크 구간 사이에서 메시지를 중계·필터링하는 노드) 역할 → **CAN bus 에 임의 메시지 송신 가능**.
4. CAN 으로 _스티어링 / 브레이크 / 가속_ 메시지 위조 → 차량 _원격 조종_.

Chrysler 가 **140만 대 리콜** — 차량 사이버보안의 분수령.

**왜 가능했나?** **CAN**(Controller Area Network — 차량 내 여러 제어기를 한 가닥 버스로 잇는 직렬 통신 규격) bus 는 1983 년 폐쇄 네트워크 가정으로 설계되었기 때문입니다. 모든 메시지는 누가 보냈는지 표시조차 없어 임의의 **ECU**(Electronic Control Unit — 차량 부품 하나를 제어하는 소형 컴퓨터; 엔진·브레이크·도어 등마다 하나씩 있음) 가 임의의 ID 로 송신할 수 있고, 내용은 평문 그대로 버스에 흐르며, 세션 개념이 없어 과거 프레임을 그대로 재전송해도 수신 ECU 가 이를 거부할 방법이 없습니다. 이 모든 것이 _"버스에 꽂힌 모든 ECU 는 신뢰 가능"_ 이라는 가정 위에서는 안전했지만, 그 가정이 _OBD-II port_ (On-Board Diagnostics II — 차량 진단용으로 법으로 의무화된 외부 커넥터; 내부 CAN 에 직결됨) _+ 인포테인먼트 + 텔레매틱스_ (telematics — 차량의 무선 원격 통신 기능, 예: 긴급통화·원격제어) _+ V2X_ (Vehicle-to-Everything — 차량이 다른 차·인프라와 주고받는 무선 통신) 로 깨지자 모든 보안 가정이 함께 무너졌습니다.
...
:::tip[🤔 잠깐 — CAN bus 를 _완전히 교체_ 못 하나?]
...
- **안전성 인증**: ISO 26262 (차량 기능안전 국제 표준 — 전자 시스템 고장이 사고로 이어지지 않도록 요구하는 규격) 의 ASIL-D (Automotive Safety Integrity Level — 위험도 등급 A~D 중 가장 엄격한 D) 인증된 CAN MAC IP 가 _수십 년 검증됨_. Ethernet 의 같은 인증은 _신생_.
...
→ 즉 CAN 보안 결함은 _설계 결함_ 이 아니라 **설계 가정의 변화** 입니다. OBD-II (1996) → 텔레매틱스 (2000s) → V2X (2010s) 가 폐쇄 가정을 단계적으로 무너뜨렸고, 그 차이를 메우는 것이 **SecOC**(Secure Onboard Communication — CAN 메시지에 인증 태그를 붙여 위조·재전송을 막는 차량 내부 통신 보안 계층) / **HSM**(Hardware Security Module — 암호키를 외부에 노출하지 않고 내부에서만 암호 연산을 수행하는 격리된 보안 칩) / **Gateway**(게이트웨이 — 차량 내 도메인 사이 메시지를 중계·필터링하는 중앙 ECU) 입니다.

---

## 3. 작은 예 — SecOC 적용 CAN-FD 1 프레임 인증 사이클

가장 단순한 시나리오. 노드 **TX-ECU** 가 brake-status 메시지를 노드 **RX-ECU** 에게 SecOC 로 인증해서 전송합니다. **CAN-FD**(CAN with Flexible Data-rate — payload 를 최대 64바이트로 늘리고 데이터 구간 속도를 높인 CAN 확장) 64 B payload (페이로드 — 프레임에서 실제 데이터가 담기는 부분) 한 프레임. 이 절에 등장하는 **MAC**(Message Authentication Code — 비밀키로 메시지에서 계산하는 짧은 검증값; 키 없는 사람은 못 만들어 위조를 막음)·**Freshness Value/FV**(매 전송마다 증가하는 카운터 — 같은 메시지의 재전송replay 을 수신측이 거부하게 함) 는 SecOC 의 두 축입니다.
...
:::note[왜 CRC 는 악의적 변조를 못 막나 — 무결성 ≠ 인증성]
CAN의 15-bit CRC는 **키 없는 결정적 함수** 입니다 — 데이터만 알면 _누구나_ 같은 규칙으로 CRC를 재계산할 수 있습니다. 그래서 전송 중 노이즈로 비트가 _우발적으로_ 뒤집히면 CRC가 안 맞아 검출되지만(=무결성/integrity 검사는 됨), _공격자_ 가 데이터를 의도적으로 바꾼 뒤 그에 맞는 _올바른 CRC를 다시 계산해 붙이면_ 검사를 그대로 통과합니다. 즉 CRC는 "데이터가 우연히 깨졌는가"는 알아도 "정당한 발신자가 보냈는가(authenticity)"는 전혀 보장하지 못합니다.

대비하면: **해시/CRC = 키 없음 → 누구나 재계산 가능 → 무결성만**. **MAC(예: AES-CMAC) = 비밀키 필요 → 키 가진 자만 재계산 가능 → 무결성 + 인증성**. CAN에 빠진 것이 바로 이 _키_ 이고, SecOC가 MAC을 얹어 "키를 가진 ECU만 valid 태그를 만든다"로 인증성을 보강하는 이유입니다.
:::

### 4.2 Arbitration — wired-AND 와 dominant/recessive

CAN 은 _비파괴적 우선순위 중재_ 를 사용합니다. 동작 원리는 이렇습니다. CAN 버스의 물리 신호는 Dominant (논리 0) 와 Recessive (논리 1) 두 상태를 가지는데, 여러 노드가 동시에 송신하면 wired-AND 특성에 따라 Dominant 가 항상 이깁니다. 모든 노드는 자신이 보낸 비트와 버스에 실제로 나타난 값을 매 비트마다 비교하여, 자신이 Recessive(1) 를 보냈는데 버스에는 Dominant(0) 가 관측되면 "다른 노드가 더 높은 우선순위를 가지고 있다" 고 판단하고 즉시 송신을 포기합니다. 결과적으로 **낮은 ID 번호를 가진 메시지가 항상 우선순위에서 이기며**, 전체 과정이 bit 단위로 결정적이기 때문에 충돌이나 재전송 없이 중재가 완료됩니다.
...
**보안 함의**: 공격자가 ID = `0x000` 으로 송신하면 항상 우선순위 승리 → **DoS**(Denial of Service — 정상 통신을 마비시켜 서비스를 못 쓰게 만드는 공격) via 우선순위 하이재킹.
...
BusOff: "Bus-Off\n통신 완전 차단\n128 × 11 rec 관찰 후 복귀"
NOTE: "error +8 / success −1\n→ 12.5% 만 넘어도 단조 증가" { shape: rectangle; style.stroke-dash: 4 }
...
ErrorActive -> NOTE { style.stroke-dash: 4; style.opacity: 0.5 }
```
...
#### AES-CMAC 자체의 동작과 truncation 의 의미

§3에서 SecOC가 "CMAC을 계산"한다고 호출만 했는데, 그 CMAC이 _무엇인지_ 와 _왜 잘라도(truncation) 쓰는지_ 를 한 단계만 봅니다(깊이는 자제, 확인 필요 — RFC 4493).

- **CBC-MAC 기반 + 마지막 블록 서브키 처리**: CMAC은 AES를 CBC 모드로 메시지 블록들을 _연쇄_ 시켜 마지막 블록의 암호 출력을 MAC으로 쓰는 CBC-MAC 구조입니다. 다만 순수 CBC-MAC은 가변 길이 메시지에서 위조 취약점이 있어, CMAC은 메시지의 _마지막 블록_ 에 AES 키로 미리 유도한 **서브키(K1 또는 K2)** 를 XOR해 처리합니다(메시지가 블록 크기로 딱 떨어지면 K1, 패딩이 필요하면 K2). 이 마지막-블록 보정이 길이 확장류 위조를 막는 CMAC의 핵심입니다. 키 없는 사람은 같은 입력에 대한 CMAC을 만들 수 없으므로, 이것이 "발신자 인증"이 됩니다.
- **truncation 이 보안강도에 직접 영향**: AES-CMAC의 전체 출력은 16 B(128-bit)이지만, CAN-FD payload에 다 넣을 공간이 없어 SecOC는 _상위 몇 바이트만_ 잘라(예: 4 B = 32-bit) 씁니다. 잘린 MAC 길이 `n` 비트는 곧 위조 성공 확률을 결정합니다 — 공격자가 무작위로 MAC을 맞힐 확률이 약 `1/2^n`이므로, 4 B면 `2^-32`, 8 B면 `2^-64`로 _기하급수적_ 으로 강해집니다. 즉 truncation은 "payload 공간 ↔ 위조 저항(2^n)"의 직접적 trade-off이며, brute-force 위조 시도를 IDS/rate-limit로 억제한다는 전제 아래 짧은 MAC을 쓰는 것입니다.
...
TEC와 REC의 변화 규칙에서 핵심은 **비대칭성** 입니다. 한 번의 에러가 발생하면 카운터가 +8씩 뛰는 반면, 한 번의 성공은 고작 -1만 내려갑니다. 이 말은 에러가 전체 송수신의 12.5%만 넘어도 카운터가 단조 증가한다는 뜻입니다. Bus-Off attack 은 이 비대칭을 의도적으로 악용합니다.

**이 +8/−1 수치는 어디서 왔나 — CAN 표준의 error confinement 규칙.** 이 가중치는 임의값이 아니라 CAN 표준(ISO 11898-1, 원래 Bosch CAN spec)의 **error confinement** 규칙에서 정의된 값입니다. 규칙의 의도는 "_고장난(에러를 자주 내는) 노드를 버스에서 빨리 떼어내되, 어쩌다 한 번의 노이즈로는 멀쩡한 노드를 벌하지 않는다_"입니다. 그래서 에러는 빠르게 누적(+8), 성공은 천천히 회복(−1)으로 _비대칭_ 을 둬, 지속적으로 문제를 일으키는 노드만 Error-Passive → Bus-Off로 단조 격리되도록 설계됐습니다. 표에서 _에러를 처음 송출한 노드_(Active Error Flag를 먼저 낸 첫 감지자)에 +8의 무거운 가중을 주는 것도 같은 취지 — "에러 신호를 처음 만든 쪽이 문제 발생지일 가능성이 크다"고 보아 더 빨리 책임을 묻습니다(반면 단순 수신 에러 감지는 +1로 가볍게). 정상 동작에서는 이 규칙이 _안전한 자동 격리_ 지만, 공격자가 victim의 프레임만 골라 의도적 비트 충돌을 일으키면 그 victim의 TEC만 +8로 폭주시켜 _선택적으로_ Bus-Off에 빠뜨릴 수 있습니다 — 안전 메커니즘이 attack vector로 뒤집히는 지점입니다.
...
→ CAN 자체에 이 공격에 대한 프로토콜 레벨 방어 **없음**. Gateway 의 Rate Limiting (레이트 리미팅 — 단위 시간당 메시지 수를 제한해 flooding 을 억제) + **IDS**(Intrusion Detection System — 네트워크 트래픽 패턴을 감시해 비정상 메시지를 탐지하는 시스템) 의 error-frame burst 모니터링이 필수 보강.
...
OBD-II 포트는 1996년 미국 EPA가 배기가스 진단을 위해 법적으로 의무화한 16핀 커넥터입니다. 문제는 이 포트의 6번 핀(CAN-H)과 14번 핀(CAN-L)이 차량 내부 CAN 버스에 **직접 연결** 된다는 점입니다. 법적 의무이기 때문에 막을 수 없고, 연결하는 순간 게이트웨이 없이는 CAN 버스 전체에 접근이 가능해집니다. 즉, OBD-II 는 보안 설계가 아니라 진단 편의를 위해 설계된 물리 진입점이며, 차량 사이버보안의 '항상 열려 있는 문' 역할을 합니다.
...
네 가지 결함은 서로 독립적이지 않습니다. 무인증이 있으니 메시지를 위조할 수 있고, 브로드캐스트이니 위조한 메시지가 버스 전체에 전파되며, 무상태이니 과거 프레임을 녹화해서 재전송해도 막을 방법이 없고, 무암호화이니 어떤 데이터가 어떤 형식으로 흐르는지 sniff 한 뒤 역공학하기가 쉽습니다. 이 사슬이 OBD-II 라는 합법적 진입점 하나와 결합하면, 아래 다이어그램의 경로가 완성됩니다.
...
| 증상 | 1차 의심 | 어디 보나 |
|---|---|---|
| 정상 frame 인데 RX 가 거부 (SecOC) | FV mismatch (TX 가 +1, RX 도 +1, 그런데 prev 가 안 갱신됨) | RX SecOC 의 prev_FV 갱신 logic, NVM (Non-Volatile Memory — 전원이 꺼져도 값이 유지되는 메모리) 저장 시점 |
...
| 동일 ID 가 2 개 source 에서 송신 | Spoofing 또는 듀얼 redundant 설계 | DBC (CAN Database — 각 메시지 ID 의 발신자·신호 배치를 정의한 설정 파일) 의 sender 매핑, sniff 로 시간차 확인 |
...
:::caution[실무 주의점 — OBD-II 포트는 항상 열려있다]
...
:::tip[🤔 Q1 — CAN 4 결함 매핑 (Bloom: Analyze)]
...
:::
:::tip[🤔 Q2 — Bus-off attack 방어 (Bloom: Apply)]
...
→ [Module 02 — Automotive SoC Security](../02_automotive_soc_security/): CAN 의 결함 위에 HSM / SecOC / Gateway / IDS / TEE (Trusted Execution Environment — CPU 안에 격리된 보안 실행 영역) 가 어떻게 layer 로 쌓이는지.

[퀴즈 풀어보기 →](../quiz/01_can_bus_fundamentals_quiz/)
