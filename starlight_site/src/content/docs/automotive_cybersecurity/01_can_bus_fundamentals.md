...
**왜 가능했나?** CAN bus 는 1983 년 폐쇄 네트워크 가정으로 설계되었기 때문입니다. 모든 메시지는 누가 보냈는지 표시조차 없어 임의의 ECU 가 임의의 ID 로 송신할 수 있고, 내용은 평문 그대로 버스에 흐르며, 세션 개념이 없어 과거 프레임을 그대로 재전송해도 수신 ECU 가 이를 거부할 방법이 없습니다. 이 모든 것이 _"버스에 꽂힌 모든 ECU 는 신뢰 가능"_ 이라는 가정 위에서는 안전했지만, 그 가정이 _OBD-II port + 인포테인먼트 + 텔레매틱스 + V2X_ 로 깨지자 모든 보안 가정이 함께 무너졌습니다.
...
→ [Module 02 — Automotive SoC Security](../02_automotive_soc_security/): CAN 의 결함 위에 HSM / SecOC / Gateway / IDS / TEE 가 어떻게 layer 로 쌓이는지.
