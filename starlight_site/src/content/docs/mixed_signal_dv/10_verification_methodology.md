---
title: "Ch10. RNM/AMS 검증 방법론 통합 — DVCon Best Practices"
---
...
- Tristate, pull-up/down, ODT(On-Die Termination, 칩 내부 종단 저항) — wreal는 표현 불가
- Power supply의 sag(부하가 걸릴 때 전압이 순간 처지는 것)/load regulation(부하 변화에 대해 출력 전압을 일정하게 유지하는 능력) — 단순 real로 부족
...
uvm_test: "UVM Test"
uvm_env: "UVM env (mixed-signal aware)" {
  axi_agent: "AXI agent\n(digital)"
  power_agent: "Power agent\n(DMS)"
  volt_mon: "Voltage monitor\n(DMS)"
  scoreboard: "Scoreboard\n(digital + real value 비교)"
}
dut: "DUT (RTL + RNM)"
...
opt_a: "Option A: RNM (default)\n— 빠름"
opt_b: "Option B: Verilog-AMS behavior\n— 중간"
opt_c: "Option C: SPICE netlist\n— 정확"
...
|---|---|---|
| Buck converter (입력보다 낮은 전압을 효율적으로 만드는 스위칭 강압 변환기) | SPICE | EEnet UDN |
...
상황: 모바일 AP(Application Processor, 스마트폰의 메인 SoC)의 PMIC + I/O + ADC 통합 검증
...
- [ ] RNM의 σ를 SPICE Monte Carlo 결과로 backannotate(정밀 시뮬레이션에서 뽑은 실제 수치를 모델 파라미터에 거꾸로 반영)
...
RNM은 deterministic(같은 입력이면 항상 같은 출력 — 무작위 잡음이 저절로 생기지 않음). thermal(열잡음) · flicker(1/f noise — 저주파에서 큰 플리커 잡음) · supply noise는 손으로 주입해야 합니다. spec에 "noise 5 mVrms"(밀리볼트 RMS — 잡음의 실효 크기)가 있으면 모델에 `$dist_normal`로 매 sample마다 더해줘야 의미 있는 검증.
...
(**aliasing**(에일리어싱)은 샘플이 너무 드물어 빠른 신호가 느린 가짜 신호로 잘못 보이는 현상입니다.) sine wave를 step 10개로 그리면 simulator는 그 10개 점에서만 sample을 봅니다. monitor가 zero-crossing을 놓치거나 peak이 잘못 잡힙니다. **cycle 당 100 step 이상**이 일반적 출발점.
...
§2의 Pattern 4가 coverage 작성 패턴이라면, 이 절은 **전략 레벨**입니다. mixed-signal coverage는 **structural coverage**(코드 라인·분기처럼 코드 구조의 실행 여부를 보는 커버리지)**가 거의 의미 없고** 대부분의 신뢰는 spec 기반 functional coverage에서 옵니다 — analog DUT가 보통 RTL이 아니라 RNM behavioral 또는 Spice netlist이기 때문.
...
**Verification plan(V-plan**, 검증 계획서 — 무엇을 어떻게 검증할지 항목별로 정리한 문서)의 항목 하나하나가 coverage point 또는 **directed test**(특정 시나리오를 콕 집어 실행하는 지정 테스트)에 매핑되어야 합니다. 도구가 V-plan을 import해 회귀 결과와 자동 매핑하는 기능(VC Verification Planner, vManager 등)이 있어 활용 권장.
...
- **② Output space** — 출력 (code, lock_status, eoc(End-Of-Conversion, ADC 변환 완료 신호) count)의 도달도. boundary 코드, saturation 코드 명시
...
- **race condition**(둘 이상의 동작 순서가 보장되지 않아 결과가 들쭉날쭉한 경합) — `clocking` block(샘플/구동 타이밍을 명확히 규정하는 SV 구문) 누락, NBA region 의존
...
- fail 자동 분류 (logged `uvm_error` id → 카테고리) → triage(fail을 원인별로 분류·우선순위화하는 작업) 부담 ↓
...
top_test: "Top UVM Test"
uvm_env: "UVM env" {
  axi_ag: "AXI agent\n(digital)"
  power_ag: "Power agent\n(UDN-aware)"
  ddr_ag: "DDR agent\n(digital)"
  adc_ag: "ADC agent\n(wreal in/out)"
  cov_sb: "coverage + scoreboard"
}
dut: "DUT" {
  ddr5_phy: "DDR5 PHY\n· DLL: RNM\n· IO: RNM\n· SA: RNM/SPICE"
  pmic: "PMIC\n· Buck: UDN\n· LDO: UDN"
...
- 다음: [Ch11. 도구 지형](../11_tools_ecosystem/)
