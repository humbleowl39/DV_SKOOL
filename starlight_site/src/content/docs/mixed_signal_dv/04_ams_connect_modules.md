---
title: "Ch04. AMS · Verilog-AMS · Connect Module"
---
...
AMS 시뮬레이션을 처음 배울 때 가장 중요한 것은 "두 개의 시뮬레이터가 같은 시간 축 위에서 동시에 동작한다"는 그림을 머릿속에 잡는 것입니다. 한쪽에는 VCS나 Xcelium 같은 디지털 시뮬레이터가 SV testbench와 디지털 RTL을 이벤트 기반으로 처리하고, 다른 한쪽에는 FineSim, HSPICE, Spectre 같은 SPICE 시뮬레이터가 아날로그 넷리스트를 수치 적분으로 풀고 있습니다. 두 엔진은 각자 자신의 영역을 독립적으로 계산하지만, **동일한 wall-clock time**(시뮬레이션이 모형화하는 동일한 가상 시각)을 공유하면서 주기적으로 동기화 점을 맞춥니다. 여기서 **AMS**(Analog Mixed-Signal Simulation)는 디지털 시뮬레이터와 SPICE를 함께 돌리는 방식, **connect module**(두 영역의 경계에서 0/1 논리와 실제 전압을 변환하는 어댑터), **RTL**(레지스터 수준으로 기술한 디지털 설계)을 가리킵니다.
...
디지털과 아날로그가 만나는 경계에는 반드시 번역이 필요합니다. 디지털 신호는 1'b1 또는 1'b0이지만, 아날로그 세계에서 "1"은 0.9 V인가요, 0.8 V인가요, 1.1 V인가요? 디지털의 X(값을 모름/충돌)는 어떤 전압에 해당하나요? 디지털의 Z(**tri-state** — 아무도 구동하지 않아 둥둥 떠 있는 high-impedance 상태)는 무한 임피던스를 의미하는데 이것을 어떻게 표현할까요?
...
이 번역 규칙 — threshold 전압(0/1을 가르는 판정 기준 전압), rise/fall time(신호가 올라가/내려가는 데 걸리는 시간), drive strength(driver가 net을 끌어당기는 세기/출력 임피던스) — 을 정의하는 것이 **connect module**의 역할입니다. 규칙이 없으면 디지털의 "1"이 아날로그 측에 어떤 전압으로 나타나야 하는지 알 수 없고, 아날로그의 전압이 디지털 측에서 어떻게 0 또는 1로 해석되는지도 알 수 없습니다.
...
**discipline**(net이 전기적인지 논리적인지 등 신호의 종류를 규정하는 속성)이 양 끝에서 다르면 변환이 필요합니다. VAMS-2023 **LRM**(Language Reference Manual, 언어 표준 문서)은 자동 삽입을 위한 **connection rule(connectrules)** 문법을 제공합니다.
...
대응 (참고로 **NaN**은 Not-a-Number — 0/0 같은 부정 연산 결과로 생기는 "수가 아님" 값이며, 한 번 생기면 이후 계산을 모두 오염시킵니다):

- X를 mid-voltage(VDD/2 — 전원 전압의 절반)로 매핑하거나, 시뮬레이션을 fatal로 중단
...
위 예시는 입력 전압을 `gain`배로 키우는 **amplifier**(증폭기)입니다.

## 5. 실제 회로 예시 — RC 저역통과 필터

저항(R)과 캐패시터(C)로 만든 **저역통과 필터**(low-pass filter — 낮은 주파수는 통과시키고 높은 주파수는 깎아내는 회로)의 Verilog-AMS 모델입니다.
...
| BGR(Band-Gap Reference — 온도·전원에 거의 영향받지 않는 기준 전압을 만드는 회로) 출력 voltage variation | **AMS** |
...
실제 SoC는 같은 칩에 **1.8 V I/O · 0.9 V core · 0.6 V LP(Low-Power, 저전력) 도메인**이 공존합니다. 각 boundary에 맞는 connect rule을 **분리해서 binding**(설계 요소를 실제 net/타입에 연결)해야 합니다. 한 rule을 chip 전체에 적용하면 voltage swing이 잘못 매핑되어 false-pass/fail이 일어납니다.
...
- **supply_sensitive** — VDD가 변하면 V_high가 따라가도록 (PG-aware = Power/Ground-aware, 전원 상태를 인식)

> Connect module은 simulator가 **elaboration**(설계 계층을 펼쳐 인스턴스와 연결을 확정하는 컴파일 단계) 시 boundary를 분석해 **자동으로 삽입**합니다. user가 instance를 명시할 필요는 없지만, **어떤 rule이 어디에 적용되는지**를 elaboration log로 확인해야 합니다 — 잘못된 도메인의 rule이 적용되면 silently 0/1 매핑이 비대칭이 됩니다. 새 IP integration 시 회귀의 1차 fail 카테고리가 거의 이것입니다.
...
가장 유용한 사용처는 **다양한 nettype을 받아들이는 generic IP** (I/O pad — 칩 외부 핀과 잇는 패드, level shifter — 서로 다른 전압 도메인 사이에서 신호 레벨을 변환, repeater — 신호를 중계 증폭해 멀리 보냄, switch fabric — 여러 입출력을 교차 연결하는 스위치망):
...
- SVA(SystemVerilog Assertions, 조건 위반을 감시하는 선언적 검사) / covergroup의 직접 인자로 쓰면 type을 모르므로 에러 — 양 끝 port의 nettype이 resolve(확정)된 뒤에 사용
...
- 다음: [Ch05. RNM with SystemVerilog](../05_rnm_systemverilog/)
