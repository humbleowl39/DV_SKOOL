...
먼저 핵심 UVM 용어: **env**(environment — 검증 컴포넌트들을 묶는 최상위 컨테이너), **agent**(한 인터페이스를 담당하는 driver+monitor+sequencer 묶음), **transaction**(한 번의 통신/자극을 표현하는 데이터 단위), **sequencer**(transaction을 driver로 흘려보내는 컴포넌트), **driver**(transaction을 실제 신호로 구동), **monitor**(신호를 관찰해 transaction으로 복원), **scoreboard**(기대값과 관측값을 비교해 판정), **virtual sequencer**(여러 agent의 sequencer를 한곳에서 조율하는 상위 sequencer)입니다.
...
위 그림의 용어: **RAL**(Register Abstraction Layer — 레지스터를 추상화해 이름으로 읽고 쓰게 해 주는 UVM 계층), **APB/AXI**(SoC 내부 레지스터·블록을 잇는 표준 버스 프로토콜), **irq**(interrupt request — 인터럽트 요청 신호), **ref model**(reference model — spec대로 정답을 계산하는 참조 모델)입니다.
...
UVM의 `uvm_component`는 module hierarchy 바깥에 있습니다. DUT 신호와 연결하려면 **SV interface**(신호 묶음과 그 접근 규칙을 모은 SV 구문) **인스턴스**를 component가 **virtual interface**(컴포넌트가 실제 interface 인스턴스를 가리키도록 잡는 핸들)로 잡고 있어야 하는데, mixed-signal에서는 그 interface에 `nettype` 신호가 섞여 있어 선언·접근·sampling이 까다로워집니다.
...
(**modport**는 interface 안의 신호를 누가 입력/출력으로 보는지 방향을 정해 묶은 "관점"입니다.)
...
**sequence_item**(한 transaction의 내용을 담는 데이터 객체)이 "DC 1.2 V 인가" 또는 "0.5 V 진폭 1 MHz sine 100 cycle"이면, driver가 그 의미를 sub-event step으로 풉니다. **driver는 wave 생성 알고리즘을 직접 갖고 있지 않고** — interface helper 또는 별도 **BFM**(Bus Functional Model — 프로토콜 동작을 함수로 흉내 내는 모델) library에 위임합니다.
...
> `fork/join_none`으로 백그라운드에 둘 때 sub-seq의 **완료 동기화**를 잊기 쉽습니다. UVM의 `raise_objection/drop_objection`(아직 할 일이 남았음을 알려 phase 종료를 막는 메커니즘)이 모든 sub-seq에 걸려 있어야 phase가 일찍 끝나지 않습니다.
...
같은 입력에 대해 **spec이 기대하는 출력**을 계산하는 코드. scoreboard가 DUT 관측값과 이걸 비교합니다. digital에선 **ISS**(Instruction Set Simulator, 명령어를 spec대로 실행하는 참조 시뮬레이터)·golden RTL이 이 자리에 있습니다. mixed-signal에서는 spec의 analog 수식 또는 floating-point 알고리즘이 reference가 됩니다.
...
| **DPI-C** (Direct Programming Interface — SystemVerilog에서 C 함수를 직접 호출하는 표준 인터페이스) | C 함수 호출, `pure` import | 표준 C 수학 라이브러리, FFT(Fast Fourier Transform, 주파수 성분 분석)/필터 가능 | 빌드 의존성, debug 복잡 |
...
spec의 수식을 SV로 그대로 옮기면 **round-off**(반올림 오차) **· catastrophic cancellation**(거의 같은 두 큰 수를 빼면 유효 자릿수가 급격히 사라지는 현상)이 발생할 수 있습니다. 결과의 의미가 spec과 같아도 ref가 잘못된 숫자를 뱉으면 DUT가 옳아도 mismatch.
...
> analog scoreboard는 **fail의 책임 소재**가 모호할 수 있습니다 — DUT bug? RNM model bug? reference 수식 오류? scoreboard tolerance 잘못? fail 시 빠르게 좁히려면 **같은 입력으로 다른 reference에 돌렸을 때 결과**를 cross-check할 도구가 있어야 합니다 — Ch10 §8.5의 5가지 실험 표를 scoreboard fail 대응 SOP(Standard Operating Procedure, 표준 대응 절차)로 두세요.
...
- 12-bit SAR ADC(Successive-Approximation Register ADC — 비트를 하나씩 시도하며 이진 탐색으로 변환하는 방식의 ADC), Vref = 1.8 V, sample rate 1 MHz
