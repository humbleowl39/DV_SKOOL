...
### 0. 예약어를 노드 키로 쓰지 않는다 ⚠️

다음 여섯 개는 d2 키워드다. 노드 키로 쓰면 **조용히 깨진다** — `d2` 는 exit 0 을 반환하고
경고도 내지 않는다.

```
top   left   near   label   class   link      (대소문자 무관)
```

| 어디에 썼나 | 증상 |
| :-- | :-- |
| 리프 노드 `TOP: "tb_top"` | 라벨이 사라지고 **`top`** 이 표시된다 |
| 컨테이너 `TOP: "..." { A; B }` | **자식 노드가 통째로 없어진다** |

`bottom` · `right` · `center` · `style` · `shape` · `grid` 는 안전하다(실측 확인).

컨테이너 **안**의 `label: "..."` 는 정당한 속성이므로 그대로 써도 된다 —
문제는 최상위 노드 키로 쓸 때다.

대체 이름을 쓴다: `TOP` → `TBTOP`/`TESTTOP`/`FULLCHIP`, `LINK` → `TLINK`/`LNKA`.

`scripts/gen_d2.mjs` 가 이것을 검출해 **생성을 거부하고 exit 1** 한다(`--check` 에서도 동작).
...
컨테이너 라벨은 상단 **중앙**에 놓이고, 진입 화살표도 상단으로 들어온다.
제목이 길어 컨테이너 폭을 거의 채우면 **화살표가 제목을 관통**한다.

컨테이너에는 짧은 이름만 두고, 설명은 자식 노드 라벨이나 본문으로 옮긴다.

### 1-c. 컨테이너 안의 `direction` 은 무시된다

elk 레이아웃에서 컨테이너 내부에 `direction: down` 을 써도 **반영되지 않는다**.
자식 노드의 배치 방향을 바꿀 수 없으므로, 배치가 의미를 왜곡한다면
**노드 구성 자체를 바꾼다**(예: 행 3개를 나열하는 대신 "선택된 것 / 나머지" 대비 구조로).
